  ...
  return (
    <div className="card-wrap" style={{ background: '#1C1C1C', border: '1.5px solid #2E2E2E', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column', height: '100%', position: 'relative', transition: 'all 0.28s ease' }}
      ...
      <div style={{ position: 'relative', aspectRatio: '1/1', background: '#111111', overflow: 'hidden' }}>
        ...
        {product.isDigital ? (
          <div style={{ position: 'absolute', top: 10, left: 10, background: '#D4AF37', color: '#fff', fontSize: 10, fontWeight: 800, padding: '3px 8px', borderRadius: 4, display: 'flex', alignItems: 'center' }}>
            <Download size={12} />
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            {product.isDigital ? (
              <div style={{ marginBottom: '0.875rem' }}>
                <p style={{ fontSize: '0.775rem', fontWeight: 700, color: '#AAAAAA', marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{t.contactQuestion}</p>
                <div style={{ display: 'flex', borderRadius: 10, overflow: 'hidden', border: '1.5px solid #2E2E2E', marginBottom: '0.75rem' }}>
                  <button type="button" onClick={() => { setContactMethod('email'); setFd(p => ({ ...p, customerWhatsapp: '' })); }} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '0.7rem 0', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.8rem', fontWeight: 700, background: contactMethod === 'email' ? '#D4AF37' : 'transparent', color: contactMethod === 'email' ? '#1C1C1C' : '#AAAAAA' }}>
                    <Mail size={14} />{t.contactViaEmail}
                  </button>
                  <button type="button" onClick={() => { setContactMethod('whatsapp'); setFd(p => ({ ...p, customerEmail: '' })); }} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '0.7rem 0', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.8rem', fontWeight: 700, background: contactMethod === 'whatsapp' ? '#D4AF37' : 'transparent', color: contactMethod === 'whatsapp' ? '#1C1C1C' : '#AAAAAA' }}>
