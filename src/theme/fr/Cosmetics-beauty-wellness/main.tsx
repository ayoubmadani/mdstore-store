        ...
        <div className="footer-g" style={{ paddingBottom: '40px', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '13px', lineHeight: '1.8', color: 'rgba(255,255,255,0.4)', maxWidth: '260px', fontWeight: 300 }}>
              {store.hero?.subtitle?.substring(0, 80) || 'Les meilleurs Produits de soin et beauté pour tout âge et type de peau.'}
            </p>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {[
                { icon: <Phone size={13} />, val: store?.contact?.phone || 'Numéro sera ajouté prochainement' },
                { icon: <MapPin size={13} />, val: `${store?.contact?.wilaya} / ${store?.contact?.address}` },
                { icon: <span style={{ fontSize: '13px' }}>✉️</span>, val: store?.contact?.email || 'Email sera ajouté prochainement' },
              ].map((item, i) => {
                if(item)
                return ((
            ...
            <div style={{ marginTop: '1.5rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.35)', fontWeight: 500 }}>Disponibles maintenant</span>
            </div>
          </div>
      ...
      <div style={{ padding: '15px', display: 'flex', flexDirection: 'column', flex: 1 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '15px', marginTop: 'auto' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px' }}>
            ...
            <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--dim)' }}>DA</span>
          </div>
        ...
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', paddingBottom: '16px', borderBottom: '1px solid var(--line)' }}>
            <h2 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: '1.3rem', fontWeight: 600, color: 'var(--ink)', margin: 0 }}>
              {activeFilter ? cats.find((c: any) => c.id === activeFilter)?.name || 'Produits' : 'Nouveau'}
            </h2>
            <span style={{ fontSize: '13px', color: 'var(--dim)' }}>{filtered.length} Produit</span>
          </div>

          {filtered.length === 0 ? (
            <div style={{ padding: '64px 0', textAlign: 'center' }}>
              <Sparkles style={{ width: '40px', height: '40px', color: 'var(--dim)', opacity: 0.4, margin: '0 auto 12px' }} />
              <p style={{ fontSize: '1rem', color: 'var(--dim)' }}>Aucun produit actuellement</p>
            </div>
          ) : (
          ...
          <div style={{ paddingTop: '8px' }}>
            ...
            <div style={{ marginBottom: '14px' }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '10px', flexWrap: 'wrap' }}>
                ...
                <span style={{ fontSize: '14px', color: 'var(--dim)' }}>DA</span>
            ...
            {product.offers?.length > 0 && (
              <div style={{ marginBottom: '20px' }}>
                <p style={{ fontSize: '12px', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--mid)', marginBottom: '10px' }}>Forfaits</p>
                  ...
                  <label key={offer.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', border: `1.5px solid ${selectedOffer === offer.id ? 'var(--plum)' : 'var(--line-dk)'}`, cursor: 'pointer', marginBottom: '8px', borderRadius: '6px', transition: 'all 0.2s', backgroundColor: selectedOffer === offer.id ? 'var(--soft)' : 'transparent' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <div>
                        ...
                        <p style={{ fontSize: '11px', color: 'var(--dim)', margin: 0 }}>Quantité: {offer.quantity}</p>
                      </div>
                    </div>
                    <span style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--plum)' }}>{offer.price.toLocaleString()} <span style={{ fontSize: '11px', fontWeight: 400, color: 'var(--dim)' }}>DA</span></span>
                  </label>
                ))}
              </div>
            )}
      ...
      {product.store?.cart && (
        <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
          <button onClick={addToCart} disabled={isAdded} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', padding: '11px', border: `1.5px solid ${isAdded ? 'var(--plum)' : 'var(--line-dk)'}`, background: isAdded ? 'var(--soft)' : 'transparent', color: isAdded ? 'var(--plum)' : 'var(--mid)', fontFamily: "'DM Sans',sans-serif", fontWeight: 600, fontSize: '13px', cursor: 'pointer', borderRadius: '4px', transition: 'all 0.2s' }}>
            {isAdded ? <><CheckCircle2 style={{ width: '14px', height: '14px' }} className="anim-check" />Ajouté</> : <><ShoppingCart style={{ width: '14px', height: '14px' }} />Ajouter au panier</>}
          </button>
        ...
        <div style={{ animation: 'fade-up 0.3s ease' }}>
          {product.store?.cart && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              <p style={{ fontSize: '12px', fontWeight: 600, color: 'var(--mid)', margin: 0, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Informations de livraison</p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ border: '1px solid var(--line-dk)', borderRadius: '6px', marginBottom: '14px', overflow: 'hidden' }}>
              <div style={{ padding: '10px 14px', backgroundColor: 'var(--soft)', borderBottom: '1px solid var(--line)' }}>
                <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--mid)', margin: 0 }}>Résumé de la commande</p>
              </div>
              {[
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '12px 14px', backgroundColor: 'var(--soft)' }}>
                ...
                <span style={{ fontSize: '1.6rem', fontWeight: 700, color: 'var(--plum)' }}>{total().toLocaleString()} <span style={{ fontSize: '13px', fontWeight: 400, color: 'var(--dim)' }}>DA</span></span>
              </div>
            </div>
            ...
            <button type="submit" disabled={sub} className="btn-plum" style={{ width: '100%', fontSize: '15px', padding: '13px', cursor: sub ? 'not-allowed' : 'pointer', opacity: sub ? 0.7 : 1, borderRadius: '4px', marginBottom: '8px' }}>
              {sub ? <><Loader2 style={{ width: '16px', height: '16px', animation: 'spin 1s linear infinite' }} /> En cours...</> : '🛒 Confirmer la commande'}
            </button>
            <p style={{ fontSize: '11px', color: 'var(--dim)', textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px' }}>
              <Lock style={{ width: '10px', height: '10px', color: 'var(--plum)' }} /> Paiement Sécurisé et chiffré
            </p>
          </form>
        </div>
      )}
  ...
  if (!items.length) return (
    <div dir="ltr" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{ textAlign: 'center', padding: '4rem 2rem', border: '1px dashed var(--line-dk)', borderRadius: '8px', maxWidth: 400, width: '100%' }}>
        ...
        <Link href="/" className="btn-plum" style={{ display: 'inline-flex' }}>Acheter maintenant</Link>
      </div>
    </div>
  );
    ...
    <div dir="ltr" style={{ maxWidth: '1280px', margin: '0 auto', padding: '40px 24px 80px', minHeight: '100vh' }}>
      ...
      <div className="cart-g">
            ...
            <div key={i} style={{ display: 'flex', gap: '16px', padding: '20px 0', borderBottom: '1px solid var(--line)' }}>
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div>
                  ...
                  <p style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--plum)' }}>{item.finalPrice?.toLocaleString()} DA</p>
                </div>
        ...
        <div style={{ background: 'var(--soft)', border: '1px solid var(--line)', borderRadius: '8px', padding: '28px', alignSelf: 'start' }}>
          <p style={{ fontSize: '12px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--plum)', marginBottom: '20px' }}>Informations de livraison</p>
          <form onSubmit={handleSubmit}>
            <div className="form-2c">
            ...
            <div style={{ margin: '20px 0' }}>
              <p style={{ fontSize: '12px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--plum)', marginBottom: '12px' }}>Type de livraison</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                {(['home', 'office'] as const).map(t => (
                  ...
                  >
                    ...
                    <p style={{ fontWeight: 700, fontSize: '13px', color: 'var(--ink)', marginBottom: '2px' }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
                    {selW && <p style={{ fontWeight: 600, fontSize: '12px', color: 'var(--plum)' }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} DA</p>}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div style={{ marginBottom: '16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                <span style={{ fontSize: '13px', color: 'var(--dim)' }}>Sous-total</span>
                <span style={{ fontWeight: 600, fontSize: '13px', color: 'var(--ink)' }}>{cartTotal.toLocaleString()} DA</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: '12px', marginBottom: '12px', borderBottom: '1px solid var(--line)' }}>
                <span style={{ fontSize: '13px', color: 'var(--dim)' }}>Livraison</span>
                <span style={{ fontWeight: 600, fontSize: '13px', color: 'var(--ink)' }}>{getLiv() ? `${getLiv().toLocaleString()} DA` : '—'}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                <span style={{ fontWeight: 600, color: 'var(--ink)' }}>Total</span>
                <span style={{ fontSize: '1.6rem', fontWeight: 700, color: 'var(--plum)' }}>{finalTotal.toLocaleString()} <span style={{ fontSize: '13px', fontWeight: 400, color: 'var(--dim)' }}>DA</span></span>
              </div>
            </div>
            ...
            <button type="submit" disabled={submitting} className="btn-plum" style={{ width: '100%', fontSize: '15px', padding: '13px', cursor: submitting ? 'not-allowed' : 'pointer', opacity: submitting ? 0.7 : 1 }}>
              {submitting ? <><Loader2 style={{ width: '16px', height: '16px', animation: 'spin 1s linear infinite' }} /> En cours...</> : '🛒 Confirmer la commande'}
            </button>
          </form>
        </div>
      </div>
...
export function Cookies() {
  return (
    <Shell title="Cookies" sub="Légal">
      ...
      <IB title="Cookies analytiques" body="Données agrégées pour améliorer la plateforme." tag="Facultatif" />
...
export function Contact({ store }: { store?: any }) {
  ...
  const contactItems = [
    { icon: '📞', label: 'Téléphone', val: store?.contact?.phone || '+213 550 000 000' },
    { icon: '✉️', label: 'Email', val: store?.contact?.email || 'info@store.dz' },
    { icon: '📍', label: 'Adresse', val: [store?.contact?.wilaya, store?.contact?.address].filter(Boolean).join(' / ') || 'Algérie' },
  ];
      ...
      <div className="contact-g" style={{ maxWidth: '960px', margin: '0 auto', padding: '36px 24px 80px' }}>
        ...
        <div>
          <div style={{ backgroundColor: 'var(--white)', border: '1px solid var(--line)', borderRadius: '8px', padding: '24px', marginBottom: '12px' }}>
            <p style={{ fontSize: '12px', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--plum)', marginBottom: '16px' }}>Moyens de contact</p>
          ...
          <div style={{ padding: '14px', border: '1px solid var(--line)', borderRadius: '8px', background: 'var(--soft)', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--rose)', display: 'inline-block' }} />
            <span style={{ fontSize: '13px', fontWeight: 500, color: 'var(--mid)' }}>Disponibles maintenant</span>
          </div>
        </div>

        {/* Form */}
        <div style={{ backgroundColor: 'var(--white)', border: '1px solid var(--line)', borderRadius: '8px', padding: '28px' }}>
          <p style={{ fontSize: '12px', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--plum)', marginBottom: '20px' }}>Envoyer un message</p>
          {sent ? (
            <div style={{ minHeight: '200px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--line)', borderRadius: '8px', textAlign: 'center', backgroundColor: 'var(--soft)', padding: '32px' }}>
              ...
              <h3 className="serif" style={{ fontSize: '1.3rem', fontStyle: 'italic', color: 'var(--ink)', margin: '0 0 6px' }}>Message envoyé !</h3>
              <p style={{ fontSize: '13px', color: 'var(--dim)' }}>Nous vous répondrons sous 24h 💄</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                ...
                <div>
                  <p style={{ fontSize: '12px', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--dim)', marginBottom: '6px' }}>Téléphone</p>
              ...
              <div>
                <p style={{ fontSize: '12px', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--dim)', marginBottom: '6px' }}>Email</p>
                <input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="votre@email.com" required style={INP_ST()}
                  onFocus={e => (e.target.style.borderColor = 'var(--plum)')} onBlur={e => (e.target.style.borderColor = 'var(--line-dk)')} />
              </div>
              <div>
                <p style={{ fontSize: '12px', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--dim)', marginBottom: '6px' }}>Votre message</p>
