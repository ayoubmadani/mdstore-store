...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  return (
    <>
      ...
      {/* Main nav */}
      <nav dir="ltr" style={{ background: scrolled ? DARK : DARK, borderBottom: '3px solid ' + OR, position: 'sticky', top: 0, zIndex: 50, transition: 'box-shadow 0.25s', boxShadow: scrolled ? '0 4px 24px rgba(0,0,0,0.35)' : 'none' }}>
        ...
        <div className="glb-search-ov" onClick={e => { if (e.target === e.currentTarget) setShowSearch(false); }}>
          <div className="glb-search-panel">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
              <span style={{ fontSize: '0.8rem', fontWeight: 600, color: 'rgba(255,255,255,0.5)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Recherche</span>
            ...
            <form className="glb-search-form" onSubmit={handleSearch}>
              ...
              <input ref={searchInputRef} className="glb-search-input" type="text" placeholder="Rechercher des équipements de camping..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </form>
            {loading && <p style={{ textAlign: 'center', color: OR, fontSize: '0.82rem', padding: '2rem' }}>Recherche en cours...</p>}
                ...
                {listSearch.map((p: any) => (
                  <Link key={p.id} href={`/product/${p.slug || p.id}`} className="glb-search-card" onClick={() => setShowSearch(false)}>
                    ...
                    <div className="glb-search-card-info">
                      ...
                      <p style={{ fontSize: '0.76rem', fontWeight: 700, color: OR }}>{Number(p.price).toLocaleString()} DA</p>
                    </div>
                  </Link>
                ))}
              </div>
            ...
            {!loading && searchQuery.length >= 2 && listSearch.length === 0 && (
              <p style={{ textAlign: 'center', color: 'rgba(255,255,255,0.25)', fontSize: '0.875rem', padding: '3rem' }}>Aucun résultat</p>
            )}
          </div>
        </div>
      )}
...
export function Footer({ store }: any) {
  return (
    <footer dir="ltr" style={{ background: DARK, borderTop: `3px solid ${OR}`, marginTop: 80, padding: '3.5rem 1.5rem 2rem' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        <div className="footer-cols" style={{ paddingBottom: '2.5rem', marginBottom: '1.5rem', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          <div>
            <h4 style={{ fontSize: '0.65rem', fontWeight: 700, color: OR, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '1rem' }}>Pages</h4>
          ...
          <div>
            <h4 style={{ fontSize: '0.65rem', fontWeight: 700, color: OR, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '1rem' }}>Contact</h4>
            {[
        ...
        {products.length === 0 ? (
          ...
          <div className="products-grid">
            {products.map((p: any) => {
              ...
              return <Card key={p.id} product={p} displayImage={img} discount={disc} store={store} viewDetails="Explorer" />;
            })}
          </div>
        )}
        {countPage > 1 && (
      ...
      <div className="details-inner" style={{ maxWidth: 1280, margin: '0 auto' }}>
        <div className="gallery-container">
          <div style={{ position: 'relative', aspectRatio: '4/3', overflow: 'hidden', background: GL, borderRadius: 10, border: `1px solid ${BD}` }}>
            ...
            {discount > 0 && <div style={{ position: 'absolute', top: 10, right: 10, background: OR, color: '#fff', padding: '3px 10px', fontSize: 11, fontWeight: 700, borderRadius: 4 }}>{discount}% Réduction</div>}
            {allImages.length > 1 && (
              <>
        ...
        <div>
          <div className="info-container">
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: GL, borderRadius: 4, padding: '0.3rem 0.75rem', marginBottom: '0.75rem', border: `1px solid rgba(61,107,53,0.2)` }}>
              <Compass size={11} color={G} /><span style={{ fontSize: '0.62rem', fontWeight: 700, color: GD, letterSpacing: '0.08em', textTransform: 'uppercase' }}>Équipements extérieur</span>
            </div>
            ...
            <div style={{ padding: '1rem 1.25rem', background: GL, borderRadius: 10, border: `1px solid rgba(61,107,53,0.15)`, marginBottom: '1.5rem' }}>
              <p style={{ fontSize: '0.62rem', fontWeight: 700, color: SUB, marginBottom: '0.2rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Prix</p>
              ...
              <div style={{ marginBottom: '1.25rem' }}>
                {product.offers.map((o: any) => (
                  <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.875rem', border: `1.5px solid ${selectedOffer === o.id ? OR : BD}`, borderRadius: 8, cursor: 'pointer', marginBottom: '0.5rem', background: selectedOffer === o.id ? 'rgba(212,98,42,0.06)' : CARD, transition: 'all 0.18s' }}>
                    ...
                    <span className="price-mono" style={{ fontWeight: 700, color: GD }}>{o.price.toLocaleString()} DA</span>
                  </label>
                ))}
              </div>
            )}
        ...
        <div className="anim-fade-up">
          {product.store.cart && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <p style={{ fontWeight: 700, fontSize: '0.65rem', color: SUB, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Informations de livraison</p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: '0.875rem' }}>
              <p style={{ fontSize: '0.65rem', fontWeight: 700, color: SUB, marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.07em' }}>Livraison</p>
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.75rem', border: `1.5px solid ${fd.typeLivraison === t ? OR : BD}`, borderRadius: 8, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? 'rgba(212,98,42,0.07)' : CARD, transition: 'all 0.18s', fontFamily: 'inherit' }}>
                    <p style={{ fontWeight: 700, fontSize: '0.78rem', marginBottom: 3, color: fd.typeLivraison === t ? ORD : SUB }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
                    {selW && <p className="price-mono" style={{ fontSize: '1rem', fontWeight: 700, color: fd.typeLivraison === t ? ORD : SUB }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} DA</p>}
                  </button>
                ))}
              </div>
            </div>
            <div style={{ marginBottom: '0.875rem' }}>
              <p style={{ fontSize: '0.65rem', fontWeight: 700, color: SUB, marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.07em' }}>Quantité</p>
            ...
            <div style={{ background: GL, borderRadius: 10, border: '1px solid rgba(61,107,53,0.15)', padding: '1rem 1.125rem', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.75rem', fontWeight: 700, color: GD }}>{total().toLocaleString()} <span style={{ fontSize: '0.75rem' }}>DA</span></span>
              </div>
            </div>
  ...
  if (!items.length) return (
    <div dir="ltr" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: BG }}>
      <div style={{ textAlign: 'center', padding: '4rem 2rem', border: `1px solid ${BD}`, borderRadius: 12, maxWidth: 400, width: '100%', background: CARD }}>
        ...
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: OR, color: '#fff', padding: '0.75rem 1.875rem', fontWeight: 700, fontSize: '0.875rem', borderRadius: 8 }}>Acheter maintenant</Link>
      </div>
    </div>
  );
      ...
      <div className="cart-inner">
        <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BD}`, alignSelf: 'start', overflow: 'hidden' }}>
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '0.875rem', padding: '0.875rem 1rem', borderBottom: `1px solid ${BD}` }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p className="price-mono" style={{ fontSize: '1.1rem', fontWeight: 700, color: GD }}>{item.finalPrice?.toLocaleString()} DA</p>
                <p style={{ fontSize: '0.65rem', color: SUB, marginTop: '0.15rem' }}>Quantité: {item.quantity}</p>
              </div>
          ...
          <div style={{ padding: '1rem', background: GL, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontWeight: 700, color: SUB, fontSize: '0.82rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Sous-total</span>
            <span className="price-mono" style={{ fontSize: '1.25rem', fontWeight: 700, color: GD }}>{cartTotal.toLocaleString()} DA</span>
          </div>
        </div>
        <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BD}`, padding: '1.75rem', alignSelf: 'start' }}>
          <h3 style={{ fontWeight: 700, fontSize: '0.82rem', color: INK, marginBottom: '1.5rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Informations de livraison</h3>
          <form onSubmit={handleSubmit}>
            <div className="form-row-2">
            ...
            <div style={{ margin: '1rem 0' }}>
              <p style={{ fontSize: '0.65rem', fontWeight: 700, color: SUB, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '0.5rem' }}>Type de livraison</p>
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.75rem', border: `1.5px solid ${fd.typeLivraison === t ? OR : BD}`, borderRadius: 8, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? 'rgba(212,98,42,0.07)' : CARD, fontFamily: 'inherit', transition: 'all 0.18s' }}>
                    <p style={{ fontWeight: 700, fontSize: '0.78rem', color: fd.typeLivraison === t ? ORD : SUB, marginBottom: 3 }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
                    {selW && <p style={{ fontWeight: 700, fontSize: '0.95rem', color: fd.typeLivraison === t ? ORD : SUB }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} DA</p>}
                  </button>
                ))}
              </div>
            </div>
            <div style={{ background: GL, borderRadius: 10, border: '1px solid rgba(61,107,53,0.15)', padding: '1rem', margin: '1rem 0' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.875rem', fontWeight: 700, color: GD }}>{finalTotal.toLocaleString()} <span style={{ fontSize: '0.75rem' }}>DA</span></span>
              </div>
            </div>
      ...
      <div className="contact-inner" style={{ maxWidth: 1100, margin: '0 auto', padding: '3rem 1.5rem 6rem' }}>
        <div>
          <div style={{ background: CARD, borderRadius: 10, border: `1px solid ${BD}`, padding: '1.25rem', marginBottom: '1rem' }}>
            {[
              { icon: <Phone size={13} />, label: 'Téléphone', val: store?.contact?.phone || 'Non disponible' },
              { icon: <MapPin size={13} />, label: 'Adresse', val: [store?.contact?.wilaya, store?.contact?.address].filter(Boolean).join(' / ') || 'Algérie' },
            ].map((r, i) => (
          ...
          <div style={{ background: 'rgba(212,98,42,0.07)', borderRadius: 8, border: '1px solid rgba(212,98,42,0.2)', padding: '0.75rem 1rem', display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
            ...
            <span style={{ fontSize: '0.78rem', fontWeight: 700, color: ORD }}>Réponse sous une heure</span>
          </div>
        </div>
        <div style={{ background: CARD, borderRadius: 10, border: `1px solid ${BD}`, padding: '2rem' }}>
          {sent ? (
            <div style={{ textAlign: 'center', padding: '3.5rem 1rem' }}>
              ...
              <button onClick={() => setSent(false)} style={{ padding: '0.7rem 1.875rem', border: `1px solid ${OR}`, borderRadius: 8, background: 'rgba(212,98,42,0.07)', color: ORD, fontWeight: 700, cursor: 'pointer', fontSize: '0.875rem', fontFamily: 'inherit' }}>Envoyer un autre message</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
