...
/*
  ...
  ██╔══██╗██║   ██║██║     ██║  ██║    ██║   ██║██╔══██╗██╔══██╗██╔══██║██║╚████║
  ██████╦╝╚██████╔╝███████╗██████╔╝    ╚██████╔╝██║  ██║██████╔╝██║  ██║██║ ╚███║
  ═══════ THEME: BOLD / COLOR: #8B5CF6 + #1D1D1D + #F8F8F6
*/
const THEME_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Ticker ── */
  /* fond changé en violet */
  .ticker-wrap { overflow: hidden; background: #8B5CF6; color: #fff; height: 36px; display: flex; align-items: center; }
...
const S = {
  ...
  inputErr: { borderColor: '#8B5CF6' } as React.CSSProperties,
  btnPrimary: {
    width: '100%', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8,
    background: '#8B5CF6', color: '#fff', fontWeight: 700, fontSize: '0.95rem',
...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  const DropResults = () => (
    <div style={{ paddingTop: 25, position: 'absolute', top: 'calc(100% + 6px)', right: 0, left: 0, background: '#fff', border: '1.5px solid #E0E0E0', borderRadius: 10, boxShadow: '0 12px 40px rgba(0,0,0,0.12)', zIndex: 60, overflow: 'auto' }} className="anim-slide-down">
      ...
      {loading ? <div style={{ padding: '1.5rem', textAlign: 'center', color: '#8B5CF6', fontSize: '0.85rem', fontWeight: 600 }}>Recherche en cours...</div>
        : listSearch.length > 0 ? (
            ...
            {listSearch.map((p: any) => (
              <Link href={`/product/${p.id}`} key={p.id} onClick={() => setSearchQuery('')}
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '0.75rem', color: '#8B5CF6', fontWeight: 700 }}>{p.price} DA</div>
                </div>
              </Link>
            ))}

            {/* Bouton Voir plus - statique (défile avec le scroll) */}
            <button
          ...
          <div style={{ padding: '0.625rem 1.25rem', background: '#fff', borderTop: '1px solid #E8E8E8', position: 'relative' }} className="anim-slide-down">
            <form onSubmit={handleSearch} style={{ position: 'relative' }}>
              <input autoFocus type="text" placeholder="Rechercher un produit..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                style={{ width: '100%', padding: '0.75rem 2.75rem 0.75rem 1rem', border: '1.5px solid #8B5CF6', borderRadius: 8, background: '#F8F8F6', fontSize: '0.9rem', outline: 'none' }} />
              <Search size={15} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', color: '#8B5CF6' }} />
            </form>
            {searchQuery.length >= 2 && <DropResults />}
          </div>
        )}
          ...
          <div style={{ padding: '0.375rem 1.25rem 0.875rem' }}>
            {[{ h: '/', l: 'Accueil' }, { h: '/contact', l: 'Contactez-nous' }].map(i => (
              <Link key={i.h} href={i.h} onClick={() => setOpen(false)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 0', borderBottom: '1px solid #F0F0F0', fontSize: '0.9rem', fontWeight: 600, color: '#111' }}>
                {i.l} <ArrowRight size={14} style={{ color: '#8B5CF6' }} />
              </Link>
            ))}
          ...
          <div>
            ...
            {[
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', marginBottom: '0.75rem', color: '#666', fontSize: '0.875rem' }}>
                <span style={{ color: '#8B5CF6' }}>{r.icon}</span>{r.val}
              </div>
            ))}
...
export function Card({ product, displayImage, discount, store, viewDetails }: any) {
  ...
  return (
    <div className="card-wrap" style={{ background: '#fff', border: '1.5px solid #EBEBEB', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column', height: '100%', position: 'relative', transition: 'all 0.28s ease' }}
      onMouseEnter={e => { const el = e.currentTarget as HTMLDivElement; el.style.borderColor = '#8B5CF6'; el.style.boxShadow = '0 8px 32px rgba(230,57,70,0.1)'; el.style.transform = 'translateY(-4px)'; }}
      ...
      <div style={{ position: 'relative', aspectRatio: '1/1', background: '#F4F4F2', overflow: 'hidden' }}>
        ...
        {discount > 0 && (
          <div style={{ position: 'absolute', top: 10, right: 10, background: '#8B5CF6', color: '#fff', fontSize: 11, fontWeight: 800, padding: '2px 9px', borderRadius: 4 }}>
            -{discount}%
          </div>
        )}
      </div>
  ...
  return (
    <div dir="ltr">
      ...
      <section style={{ position: 'relative', background: '#111', overflow: 'hidden' }}>
        ...
        {/* Geometric accent */}
        <div style={{ position: 'absolute', top: 0, left: 0, width: '40%', height: '100%', background: 'linear-gradient(135deg, #8B5CF6 0%, transparent 60%)', opacity: 0.12, pointerEvents: 'none' }} />
        ...
        <div className="hero-inner" style={{ maxWidth: 1280, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          ...
          <div>
            ...
            <h1 style={{ fontSize: 'clamp(2.25rem,6vw,4.5rem)', fontWeight: 800, color: '#fff', lineHeight: 1.1, letterSpacing: '-0.03em', marginBottom: '1.25rem' }}
              dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(store.hero?.title || 'Acheter<br/><span style="color:#8B5CF6">En confiance</span> et obtenez<br/>le meilleur') }} />
        ...
        <div className="trust-grid" style={{ maxWidth: 1280, margin: '0 auto' }}>
          {[
            ...
            <div key={i} style={{ padding: '1.125rem 1rem', textAlign: 'center', borderLeft: i < 3 ? '1px solid #EBEBEB' : 'none' }}>
              <div style={{ color: '#8B5CF6', marginBottom: '0.375rem', display: 'flex', justifyContent: 'center' }}>{item.icon}</div>
      ...
      {cats.length > 0 && (
        <section style={{ padding: '4rem 1.5rem', maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.75rem' }}>
            <div style={{ width: 4, height: 28, background: '#8B5CF6', borderRadius: 2 }} />
            <h2 style={{ fontSize: 'clamp(1.375rem,3.5vw,2rem)', fontWeight: 800, color: '#111' }}>Catégories</h2>
          </div>
          <div className="cats-grid">
            <Link href="?" style={{ display:'inline-flex', alignItems:'center', padding:'0.5rem 1.25rem', borderRadius:999, border:`1.5px solid ${!activeCategory ? '#8B5CF6' : '#ccc'}`, background: !activeCategory ? '#8B5CF6' : 'transparent', color: !activeCategory ? '#fff' : 'inherit', fontSize:'0.82rem', fontWeight:600, cursor:'pointer' }}>
                Tout
              </Link>
              {cats.map((cat: any) => {
              ...
              <Link key={cat.id} href={`?category=${cat.id}`} style={{
                padding: '0.75rem 0.875rem', border: `1.5px solid ${isActive ? '#8B5CF6' : '#EBEBEB'}`, borderRadius: 8,
                textAlign: 'center', fontSize: '0.875rem', fontWeight: 600, color: isActive ? '#fff' : '#333',
                background: isActive ? '#8B5CF6' : '#fff', transition: 'all 0.18s'
              }}
                ...
                onMouseLeave={e => { const el = e.currentTarget; el.style.borderColor = isActive ? '#8B5CF6' : '#EBEBEB'; el.style.color = isActive ? '#fff' : '#333'; el.style.background = isActive ? '#8B5CF6' : '#fff'; }}>
                {cat.name}
              </Link>
              );
            })}
        ...
        {countPage > 1 && (
          <div className="pagination" dir="ltr">
            ...
            {Array.from({ length: countPage }).map((_, i) => {
              ...
              return (
                <Link key={pn} href={{ query: { page: pn } }} scroll={false} style={{ width: 36, height: 36, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.875rem', fontWeight: 700, border: `1.5px solid ${isA ? '#8B5CF6' : '#EBEBEB'}`, background: isA ? '#8B5CF6' : '#fff', color: isA ? '#fff' : '#111' }}>
                  {pn}
                </Link>
              );
            })}
...
export function Details({ product, discount, allImages, allAttrs, finalPrice, selectedVariants, setSelectedOffer, selectedOffer, handleVariantSelection, domain }: any) {
  ...
  // Mise à jour de l'état écran pour contrôler le sticky
  useEffect(() => {
        ...
        <div className="info-column">
          <div className="info-container" style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E8E8E8' }}>
            ...
            <div style={{ background: '#F5F3FF', border: '1.5px solid #DDD6FE', borderRadius: 10, padding: '1rem 1.25rem', marginBottom: '1.5rem' }}>
              <p style={{ fontSize: '0.75rem', color: '#8B5CF6', fontWeight: 700, marginBottom: '0.25rem', textTransform: 'uppercase' }}>Prix total</p>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.375rem' }}>
                <span className="price-mono" style={{ fontSize: '2.25rem', fontWeight: 800, color: '#111' }}>{finalPrice.toLocaleString()}</span>
                <span style={{ fontWeight: 700, color: '#8B5CF6' }}>DA</span>
              </div>
            </div>
              ...
              <div style={{ marginBottom: '1.25rem' }}>
                {product.offers.map((o: any) => (
                  <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.875rem 1rem', border: `1.5px solid ${selectedOffer === o.id ? '#8B5CF6' : '#E8E8E8'}`, borderRadius: 10, cursor: 'pointer', marginBottom: '0.5rem', background: selectedOffer === o.id ? 'rgba(139, 92, 246, 0.04)' : 'transparent', transition: 'all 0.18s' }}>
                    ...
                    <span className="price-mono" style={{ fontWeight: 800, color: '#8B5CF6', fontSize: '1.1rem' }}>{o.price.toLocaleString()} DA</span>
                  </label>
                ))}
              </div>
            )}
...
const FR = ({ error, label, children }: { error?: string; label?: string; children: React.ReactNode }) => (
  <div>
    ...
    {children}
    {error && <p style={{ fontSize: '0.75rem', color: '#8B5CF6', marginTop: '0.3rem', display: 'flex', alignItems: 'center', gap: 4 }}><AlertCircle size={11} />{error}</p>}
  </div>
);
        ...
        <div className="cart-add-btns" style={{ marginBottom: '1.25rem' }}>
          ...
          <button
            ...
            style={{
              flex: 1,
              ...S.btnPrimary,
              background: '#8B5CF6', // couleur violette principale
              width: 'auto',
              borderRadius: 10
            }}
            onMouseEnter={e => ((e.currentTarget as HTMLButtonElement).style.background = '#7C3AED')} // teinte plus foncée au survol
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: '0.875rem' }}>
              ...
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.875rem', border: `1.5px solid ${fd.typeLivraison === t ? '#8B5CF6' : '#E8E8E8'}`, borderRadius: 10, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? 'rgba(230,57,70,0.05)' : '#fff', transition: 'all 0.18s', fontFamily: 'inherit' }}>
                    <p style={{ fontWeight: 700, fontSize: '0.85rem', marginBottom: 4, color: fd.typeLivraison === t ? '#8B5CF6' : '#555' }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
            ...
            <div style={{ background: '#F8F8F6', border: '1.5px solid #EBEBEB', borderRadius: 10, padding: '1rem 1.125rem', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '0.375rem' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.625rem', fontWeight: 800, color: '#8B5CF6' }}>{total().toLocaleString()} <span style={{ fontSize: '0.8rem', fontFamily: 'inherit', fontWeight: 700, color: '#8B5CF6' }}>DA</span></span>
              </div>
            </div>
  ...
  if (!items.length) return (
    <div dir="ltr" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: '#F8F8F6' }}>
      <div style={{ textAlign: 'center', padding: '4rem 2rem', border: '2px dashed #E8E8E8', borderRadius: 16, maxWidth: 440, width: '100%', background: '#fff' }}>
        ...
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#8B5CF6', color: '#fff', padding: '0.8rem 2rem', borderRadius: 10, fontWeight: 700, fontSize: '0.875rem' }}>Acheter maintenant</Link>
      </div>
    </div>
  );
          ...
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '1rem', padding: '1rem', borderBottom: '1px solid #F0F0F0' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p className="price-mono" style={{ fontSize: '1.125rem', fontWeight: 800, color: '#8B5CF6' }}>{item.finalPrice?.toLocaleString()} DA</p>
          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  ...
                  >
                    ...
                    <p style={{ fontWeight: 700, fontSize: '0.78rem', color: fd.typeLivraison === t ? '#8B5CF6' : '#888' }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
            ...
            <div style={{ background: '#F8F8F6', border: '1.5px solid #EBEBEB', borderRadius: 10, padding: '1rem 1.125rem', margin: '1rem 0' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.75rem', fontWeight: 800, color: '#8B5CF6' }}>{finalTotal.toLocaleString()} <span style={{ fontSize: '0.8rem', fontWeight: 700, color: '#8B5CF6' }}>DA</span></span>
              </div>
            </div>
...
const Shell = ({ children, title }: { children: React.ReactNode; title: string }) => (
  <div dir="ltr" style={{ minHeight: '100vh', background: '#F8F8F6' }}>
    <div style={{ background: '#111', paddingTop: 96, paddingBottom: 48, paddingLeft: 24, paddingRight: 24, textAlign: 'center' }}>
      <div style={{ display: 'inline-block', background: 'rgb(136 57 230 / 15%)', border: '1px solid rgb(139 57 230 / 30%)', padding: '0.375rem 1rem', borderRadius: 6, marginBottom: '1.25rem' }}>
        <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#8B5CF6', letterSpacing: '0.1em', textTransform: 'uppercase' }}>MdStore</span>
      </div>
...
const InfoBlock = ({ title, body }: { title: string; body: string }) => (
  <div style={{ padding: '1.375rem 0', borderBottom: '1px solid #EBEBEB', display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
    <div style={{ width: 4, height: 20, background: '#8B5CF6', borderRadius: 2, flexShrink: 0, marginTop: 4 }} />
    <div>
      ...
      <div className="contact-inner" style={{ maxWidth: 1100, margin: '0 auto', padding: '4rem 1.5rem 6rem' }}>
        ...
        <div>
          <div style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E8E8E8', padding: '1.75rem', marginBottom: '1rem' }}>
            <p style={{ fontSize: '0.7rem', fontWeight: 800, color: '#8B5CF6', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '1.25rem' }}>Informations de contact</p>
            {[
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.875rem', marginBottom: '1.125rem' }}>
                <div style={{ width: 44, height: 44, borderRadius: 10, background: 'rgba(230,57,70,0.08)', border: '1px solid rgb(136 57 230 / 15%)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#8B5CF6', flexShrink: 0 }}>{r.icon}</div>
                <div>
        ...
        <div style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E8E8E8', padding: '2rem' }}>
          {sent ? (
            <div style={{ textAlign: 'center', padding: '4rem 1rem' }}>
              ...
              <button onClick={() => setSent(false)} style={{ padding: '0.75rem 1.75rem', borderRadius: 10, border: '1.5px solid #8B5CF6', background: 'transparent', color: '#8B5CF6', fontWeight: 700, cursor: 'pointer', fontSize: '0.875rem', fontFamily: 'inherit' }}>Envoyer un autre message</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
              ...
              <button
                ...
                style={{
                  ...S.btnPrimary,
                  background: '#8B5CF6', // couleur violette principale (Violet 500)
                  color: '#fff',
                ...
                onMouseEnter={e => !loading && ((e.currentTarget as HTMLButtonElement).style.background = '#7C3AED')} // violet légèrement plus foncé au survol
                onMouseLeave={e => ((e.currentTarget as HTMLButtonElement).style.background = '#8B5CF6')}
              >
                {loading ? (
                  <>
