      ...
      <nav className="gg-nav">
        <div className="gg-nav-inner">
          ...
          <nav className="gg-nav-links">
            <Link href="/" className="gg-nav-link">Accueil</Link>
            <Link href="/products" className="gg-nav-link">Produits</Link>
            <Link href="/contact" className="gg-nav-link">Contact</Link>
          </nav>
                ...
                {results.map((p: any) => (
                  <Link key={p.id} href={`/product/${p.slug || p.id}`} className="gg-srch-item" onClick={closeSearch} style={{ textDecoration: 'none', color: 'inherit' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <p style={{ fontSize: '12px', color: 'var(--pink)', margin: 0, fontFamily: "'Cormorant Garamond',serif", fontWeight: 600 }}>{(typeof p.price === 'string' ? parseFloat(p.price) : p.price)?.toLocaleString()} DA</p>
                    </div>
                  </Link>
                ))}
                <button
  ...
  return (
    <footer className="gg-footer">
      <div className="gg-footer-grid">
        ...
        {/* Links */}
        <div>
          <p className="gg-footer-title">Pages</p>
          {[['/', 'Accueil'], ['/products', 'Produits'], ['/cart', 'Panier'], ['/contact', 'Contactez-nous']].map(([href, label]) => (
        ...
        {/* Legal */}
        <div>
          <p className="gg-footer-title">Légal</p>
          {[['/privacy', 'Politique de confidentialité'], ['/terms', 'Conditions de service'], ['/cookies', 'Cookies']].map(([href, label]) => (
            <Link key={href} href={href} className="gg-footer-link">{label}</Link>
          ))}
        </div>
        ...
        {/* Contact */}
        <div>
          <p className="gg-footer-title">Contact</p>
          {store?.contact?.phone && (
      ...
      <div className="gg-footer-bottom" style={{ maxWidth: 1360, margin: '0 auto', padding: '20px 24px' }}>
        <p className="gg-footer-copy">© {new Date().getFullYear()} {store?.name || 'Glamour & Glow'} · Tous droits réservés</p>
      ...
      <div className="gg-det-grid">
        ...
        <div style={{ paddingTop: 8 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 14 }}>
            ...
            <span style={{ fontSize: 14, color: 'var(--dim)' }}>DA</span>
  ...
  if (!items.length) return (
    <div dir="ltr" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', background: 'var(--soft)' }}>
      <div style={{ textAlign: 'center', padding: '4rem 2rem', border: '1px dashed var(--line-dk)', borderRadius: 12, maxWidth: 400, width: '100%', background: 'var(--white)' }}>
        ...
        <Link href="/" className="gg-btn gg-btn-pink">Acheter maintenant</Link>
      </div>
    </div>
  );
        ...
        <div style={{ background: 'var(--white)', border: '1px solid var(--line)', borderRadius: 12, padding: 28, alignSelf: 'start' }}>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
              ...
              <FR label="Téléphone" error={errors.phone}><input type="tel" value={fd.customerPhone} onChange={e => setFd({ ...fd, customerPhone: e.target.value })} style={INP(!!errors.phone)} /></FR>
            </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))}
                    ...
                    <p style={{ fontWeight: 700, fontSize: 13, color: 'var(--ink)', margin: '0 0 2px' }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
                    {selW && <p style={{ fontWeight: 600, fontSize: 12, color: 'var(--pink)', margin: 0 }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} DA</p>}
                  </button>
                ))}
              </div>
            </div>
...
export function Cookies() {
  return (
    <Shell title="Cookies" sub="Légal">
      <InfoBlock title="Cookies essentiels" body="Essentiels pour les sessions et Panier et le paiement. Non désactivables." tag="requis" />
      <InfoBlock title="Cookies de préférences" body="Sauvegarde vos préférences pour une meilleure expérience." tag="Facultatif" />
...
export function Contact({ store }: { store?: any }) {
  ...
  const contactItems = [
    { icon: <Phone size={14} />, label: 'Téléphone', val: store?.contact?.phone || '+213 550 000 000' },
    { icon: <Mail size={14} />, label: 'Email', val: store?.contact?.email || 'info@store.dz' },
