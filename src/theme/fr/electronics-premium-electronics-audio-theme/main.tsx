        ...
        <div className="glb-search-ov" onClick={e => { if (e.target === e.currentTarget) setShowSearch(false); }}>
          <div className="glb-search-panel">
            ...
            <form className="glb-search-form" onSubmit={handleSearch}>
              ...
              <input ref={searchInputRef} className="glb-search-input" type="text" placeholder="Rechercher un appareil audio..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            ...
            {loading && <p style={{ textAlign: 'center', color: A, fontSize: '0.85rem', fontWeight: 600, padding: '2rem' }}>Recherche en cours...</p>}
                ...
                {listSearch.map((p: any) => (
                  <Link key={p.id} href={`/product/${p.slug || p.id}`} className="glb-search-card" onClick={() => setShowSearch(false)}>
                    ...
                    <div className="glb-search-card-info">
                      ...
                      <p className="price-mono" style={{ fontSize: '0.8rem', fontWeight: 700, color: A }}>{Number(p.price).toLocaleString()} DA</p>
                    </div>
                  </Link>
                ))}
              </div>
            ...
            {!loading && searchQuery.length >= 2 && listSearch.length === 0 && (
              <p style={{ textAlign: 'center', color: '#B0AAA4', fontSize: '0.9rem', padding: '3rem' }}>Aucun résultat</p>
            )}
          </div>
        </div>
      )}
...
export function Footer({ store }: any) {
  return (
    <footer dir="ltr" style={{ background: '#141418', color: 'rgba(255,255,255,0.5)', marginTop: 80, padding: '4rem 1.5rem 1.5rem', borderTop: `1px solid ${BD}` }}>
        ...
        <div className="footer-inner">
          <div>
            ...
            <p style={{ fontSize: '0.875rem', lineHeight: 1.8, maxWidth: 300 }}>
              {store?.hero?.subtitle?.substring(0, 100) || 'Systèmes audio et home cinéma haut de gamme, conçus pour les amateurs de son pur.'}
            </p>
          ...
          <div>
            <h4 className="eyebrow" style={{ fontSize: '0.68rem', fontWeight: 700, color: A, textTransform: 'uppercase' as const, marginBottom: '1.25rem' }}>Pages</h4>
          ...
          <div>
            <h4 className="eyebrow" style={{ fontSize: '0.68rem', fontWeight: 700, color: A, textTransform: 'uppercase' as const, marginBottom: '1.25rem' }}>Contact</h4>
            {[
...
export function Home({ store, page }: any) {
  ...
  const marqueeWords = ['Son pur', 'Qualité exceptionnelle', 'Expérience premium', 'Home cinéma', 'Haute fidélité'];
    ...
    <div dir="ltr">
      ...
      <section style={{ position: 'relative', overflow: 'hidden', background: BG }}>
        ...
        <div style={{ position: 'relative', textAlign: 'center', maxWidth: 760, margin: '0 auto', padding: 'clamp(4.5rem,9vw,7rem) 1.5rem 3rem' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, border: `1px solid ${BD}`, padding: '0.35rem 1rem', marginBottom: '1.5rem' }}>
            ...
            <span className="eyebrow" style={{ fontSize: '0.68rem', fontWeight: 600, color: A, textTransform: 'uppercase' as const }}>L'élite du son premium</span>
          </div>
          <h1 className="font-display" style={{ fontSize: 'clamp(2.1rem,6vw,4rem)', fontWeight: 600, color: TXT, lineHeight: 1.25, marginBottom: '1.25rem' }}
            dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(store.hero?.title || 'Une expérience sonore<br/><span style="color:#7C6B5E">exceptionnelle</span>') }} />
          <p style={{ fontSize: '1rem', color: SUB, lineHeight: 1.8, marginBottom: '2.25rem', maxWidth: 520, margin: '0 auto 2.25rem' }}>
            {store.hero?.subtitle || 'Systèmes audio et home cinéma haut de gamme, conçus pour les amateurs de son pur.'}
          </p>
      ...
      <div style={{ background: CARD, borderBottom: `1px solid ${BD}` }}>
        <div className="trust-grid" style={{ maxWidth: 1280, margin: '0 auto' }}>
          {[
            { icon: <ShieldCheck size={18} />, t: 'Garantie étendue', d: 'Protection qualité' },
            { icon: <Volume2 size={18} />, t: 'Son pur', d: 'Technologie avancée' },
            { icon: <Truck size={18} />, t: 'Livraison premium', d: '58 wilayas algériennes' },
            { icon: <Headphones size={18} />, t: 'Support spécialisé', d: 'Experts audio' },
          ].map((item, i) => (
      ...
      {cats.length > 0 && (
        <section style={{ padding: '2.5rem 1.5rem', maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.25rem' }}>
            ...
            <p className="eyebrow" style={{ fontSize: '0.75rem', fontWeight: 600, color: SUB, textTransform: 'uppercase' as const }}>Collections</p>
          </div>
      ...
      <section id="products" style={{ padding: '0.5rem 1.5rem 5rem', maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
          <h2 className="font-display" style={{ fontSize: 'clamp(1.4rem,3vw,1.9rem)', fontWeight: 600, color: TXT }}>La collection premium</h2>
        </div>
        {products.length === 0 ? (
      ...
      <div className="details-inner" style={{ maxWidth: 1280, margin: '0 auto' }}>
        <div className="gallery-container">
          <div style={{ position: 'relative', aspectRatio: '1/1', overflow: 'hidden', background: '#FAFAF8' }}>
            ...
            {discount > 0 && (
              <div style={{ position: 'absolute', top: 12, right: 12, background: A, color: '#FFFFFF', padding: '3px 10px', fontSize: 12, fontWeight: 800 }}>{discount}% de réduction</div>
            )}
            {allImages.length > 1 && (
              <>
        ...
        <div>
          <div className="info-container">
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, border: `1px solid ${BD}`, padding: '0.25rem 0.75rem', marginBottom: '0.875rem' }}>
              ...
              <span className="eyebrow" style={{ fontSize: '0.65rem', fontWeight: 700, color: A, textTransform: 'uppercase' as const }}>Édition de luxe certifiée</span>
            </div>
            ...
            <div style={{ borderTop: `1px solid ${BD}`, borderBottom: `1px solid ${BD}`, padding: '1.125rem 0', marginBottom: '1.5rem' }}>
              <p className="eyebrow" style={{ fontSize: '0.65rem', color: A, fontWeight: 700, marginBottom: '0.3rem', textTransform: 'uppercase' as const }}>Prix</p>
              ...
              <div style={{ marginBottom: '1.25rem' }}>
                {product.offers.map((o: any) => (
                  <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 1rem', border: `1px solid ${selectedOffer === o.id ? A : BD}`, cursor: 'pointer', marginBottom: '0.5rem', background: selectedOffer === o.id ? AL : 'transparent', transition: 'all 0.3s' }}>
                    ...
                    <span className="price-mono" style={{ fontWeight: 800, color: A, fontSize: '1.05rem' }}>{o.price.toLocaleString()} DA</span>
                  </label>
                ))}
              </div>
            )}
        ...
        <div className="anim-slide-fade">
          {product.store.cart && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <p className="eyebrow" style={{ fontWeight: 600, fontSize: '0.72rem', color: SUB, textTransform: 'uppercase' as const }}>Informations de livraison</p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: '0.875rem' }}>
              <p className="eyebrow" style={{ fontSize: '0.72rem', fontWeight: 600, color: SUB, marginBottom: '0.5rem', textTransform: 'uppercase' as const }}>Livraison</p>
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.75rem', border: `1px solid ${fd.typeLivraison === t ? A : BD}`, borderRadius: 0, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? AL : 'transparent', transition: 'all 0.3s', fontFamily: 'inherit' }}>
                    <p style={{ fontWeight: 600, fontSize: '0.82rem', marginBottom: 4, color: fd.typeLivraison === t ? A : SUB }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
                    {selW && <p className="price-mono" style={{ fontSize: '1rem', fontWeight: 800, color: fd.typeLivraison === t ? A : SUB }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} DA</p>}
                  </button>
                ))}
              </div>
            </div>
            <div style={{ marginBottom: '0.875rem' }}>
              <p className="eyebrow" style={{ fontSize: '0.72rem', fontWeight: 600, color: SUB, marginBottom: '0.5rem', textTransform: 'uppercase' as const }}>Quantité</p>
            ...
            <div style={{ border: `1px solid ${BD}`, padding: '0.875rem 1rem', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '0.25rem' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.6rem', fontWeight: 700, color: A }}>{total().toLocaleString()} <span style={{ fontSize: '0.8rem', fontWeight: 700 }}>DA</span></span>
              </div>
            </div>
  ...
  if (success) return (
    <div dir="ltr" style={{ minHeight: '70vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: BG }}>
      <div style={{ textAlign: 'center', background: CARD, padding: '3.5rem 2rem', border: `1px solid ${BD}`, borderTop: `3px solid ${A}`, maxWidth: 460, width: '100%' }}>
        ...
        <h2 className="font-display" style={{ fontSize: '1.5rem', fontWeight: 600, color: TXT, marginBottom: '0.625rem' }}>Commande reçue !</h2>
        <p style={{ color: SUB, lineHeight: 1.7, marginBottom: '2rem' }}>Nous vous contacterons bientôt pour confirmer la commande.</p>
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: A, color: '#FFFFFF', padding: '0.8rem 2rem', fontWeight: 700, fontSize: '0.9rem' }}>Retour à la boutique</Link>
      </div>
    </div>
  );
  ...
  if (!items.length) return (
    <div dir="ltr" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: BG }}>
      <div style={{ textAlign: 'center', padding: '4rem 2rem', border: `1px dashed ${BD}`, maxWidth: 440, width: '100%', background: CARD }}>
        ...
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: A, color: '#FFFFFF', padding: '0.75rem 1.75rem', fontWeight: 700, fontSize: '0.875rem' }}>Acheter maintenant</Link>
      </div>
    </div>
  );
  ...
  return (
    <div dir="ltr" style={{ padding: '2.5rem 1.5rem', maxWidth: 1280, margin: '0 auto', minHeight: '100vh', background: BG }}>
      <div style={{ marginBottom: '2rem' }}>
        <h1 className="font-display" style={{ fontSize: 'clamp(1.6rem,5vw,2.4rem)', fontWeight: 600, color: TXT }}>Panier</h1>
      </div>
      <div className="cart-inner">
        <div style={{ background: CARD, border: `1px solid ${BD}`, overflow: 'hidden', alignSelf: 'start' }}>
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '0.875rem', padding: '0.875rem', borderBottom: `1px solid ${BD}` }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p className="price-mono" style={{ fontSize: '1.05rem', fontWeight: 700, color: A }}>{item.finalPrice?.toLocaleString()} DA</p>
          ...
          <div style={{ padding: '0.875rem', borderTop: `1px solid ${BD}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span className="price-mono" style={{ fontSize: '1.25rem', fontWeight: 700, color: A }}>{cartTotal.toLocaleString()} DA</span>
          </div>
        </div>
        <div style={{ background: CARD, border: `1px solid ${BD}`, overflow: 'hidden', padding: '1.5rem', alignSelf: 'start' }}>
          <h3 className="font-display" style={{ fontWeight: 600, fontSize: '1rem', color: TXT, marginBottom: '1.25rem' }}>Informations de livraison</h3>
          <form onSubmit={handleSubmit}>
            <div className="form-row-2">
            ...
            <div style={{ margin: '1rem 0' }}>
              <p className="eyebrow" style={{ fontSize: '0.72rem', fontWeight: 600, color: SUB, textTransform: 'uppercase' as const, marginBottom: '0.5rem' }}>Type de livraison</p>
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.75rem', border: `1px solid ${fd.typeLivraison === t ? A : BD}`, borderRadius: 0, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? AL : 'transparent', fontFamily: 'inherit', transition: 'all 0.3s' }}>
                    <p style={{ fontWeight: 600, fontSize: '0.78rem', color: fd.typeLivraison === t ? A : SUB }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
                    {selW && <p className="price-mono" style={{ fontWeight: 800, fontSize: '0.9rem', color: fd.typeLivraison === t ? A : SUB, marginTop: 2 }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} DA</p>}
                  </button>
                ))}
              </div>
            </div>
            <div style={{ border: `1px solid ${BD}`, padding: '0.875rem 1rem', margin: '1rem 0' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.9rem', fontWeight: 700, color: A }}>{finalTotal.toLocaleString()} <span style={{ fontSize: '0.8rem' }}>DA</span></span>
              </div>
            </div>
    ...
    <div dir="ltr" style={{ background: BG, minHeight: '100vh' }}>
      <div style={{ background: CARD, paddingTop: 96, paddingBottom: 48, paddingLeft: 24, paddingRight: 24, borderBottom: `1px solid ${BD}` }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <h1 className="font-display" style={{ fontSize: 'clamp(2rem,5vw,3rem)', fontWeight: 600, color: TXT, marginBottom: '0.5rem' }}>Contactez-nous</h1>
          <p style={{ color: SUB, fontSize: '0.9375rem' }}>Notre équipe est disponible pour répondre à vos questions</p>
        </div>
      </div>
      <div className="contact-inner" style={{ maxWidth: 1100, margin: '0 auto', padding: '4rem 1.5rem 6rem' }}>
        <div>
          ...
          <div style={{ border: `1px solid ${BD}`, padding: '0.875rem 1rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <span style={{ fontSize: '0.8rem', fontWeight: 600, color: A }}>Réponse sous une heure</span>
          </div>
        </div>
        <div style={{ background: CARD, border: `1px solid ${BD}`, padding: '2rem' }}>
          {sent ? (
            <div style={{ textAlign: 'center', padding: '4rem 1rem' }}>
              <CheckCircle2 size={40} style={{ color: '#22C55E', display: 'block', margin: '0 auto 1.25rem' }} />
              <h2 className="font-display" style={{ fontSize: '1.5rem', fontWeight: 600, color: TXT, marginBottom: '0.5rem' }}>Envoyé !</h2>
              <p style={{ color: SUB, lineHeight: 1.7, marginBottom: '2rem' }}>Nous vous répondrons dans les plus brefs délais.</p>
              <button onClick={() => setSent(false)} style={{ padding: '0.75rem 1.75rem', borderRadius: 0, border: `1px solid ${A}`, background: AL, color: A, fontWeight: 700, cursor: 'pointer', fontSize: '0.875rem', fontFamily: 'inherit' }}>Envoyer un autre message</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
              <div className="form-row-2" style={{ marginBottom: '0.875rem' }}>
                <div>
                  <label className="eyebrow" style={{ display: 'block', fontSize: '0.72rem', fontWeight: 600, color: SUB, marginBottom: '0.3rem', textTransform: 'uppercase' as const }}>Nom</label>
                  <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required style={S.input} />
                </div>
                <div>
                  <label className="eyebrow" style={{ display: 'block', fontSize: '0.72rem', fontWeight: 600, color: SUB, marginBottom: '0.3rem', textTransform: 'uppercase' as const }}>Téléphone</label>
                  <input type="tel" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} required style={S.input} />
                </div>
              </div>
              <div style={{ marginBottom: '0.875rem' }}>
                <label className="eyebrow" style={{ display: 'block', fontSize: '0.72rem', fontWeight: 600, color: SUB, marginBottom: '0.3rem', textTransform: 'uppercase' as const }}>E-mail</label>
                <input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} required style={S.input} />
              </div>
              <div style={{ marginBottom: '1.5rem' }}>
                <label className="eyebrow" style={{ display: 'block', fontSize: '0.72rem', fontWeight: 600, color: SUB, marginBottom: '0.3rem', textTransform: 'uppercase' as const }}>Message</label>
