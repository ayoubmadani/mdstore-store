...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  return (
    <>
      ...
      <nav dir="ltr" style={{ position: 'sticky', top: 0, zIndex: 50, background: '#fff', borderBottom: `1px solid ${scrolled ? BD : '#F3F4F6'}`, boxShadow: scrolled ? '0 1px 6px rgba(0,0,0,0.05)' : 'none', transition: 'all 0.2s' }}>
        ...
        <div className="glb-search-ov" onClick={e => { if (e.target === e.currentTarget) setShowSearch(false); }}>
          <div className="glb-search-panel">
            ...
            <form className="glb-search-form" onSubmit={handleSearch}>
              ...
              <input ref={searchInputRef} className="glb-search-input" type="text" placeholder="Saisir le nom du produit..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </form>
            {loading && <p style={{ textAlign: 'center', color: ED, fontSize: '0.82rem', padding: '2rem' }}>Recherche en cours...</p>}
                ...
                {listSearch.map((p: any) => (
                  <Link key={p.id} href={`/product/${p.slug || p.id}`} className="glb-search-card" onClick={() => setShowSearch(false)}>
                    ...
                    <div className="glb-search-card-info">
                      ...
                      <p style={{ fontSize: '0.78rem', fontWeight: 700, color: ED }}>{Number(p.price).toLocaleString()} DA</p>
                    </div>
                  </Link>
                ))}
              </div>
            ...
            {!loading && searchQuery.length >= 2 && listSearch.length === 0 && (
              <p style={{ textAlign: 'center', color: '#D1D5DB', fontSize: '0.875rem', padding: '3rem' }}>Aucun résultat</p>
            )}
          </div>
        </div>
      )}
        ...
        <div className='footer-inner'>
          <div>
            ...
            <p style={{ fontSize: '0.875rem', lineHeight: 1.8, maxWidth: 280 }}>
              {store?.hero?.subtitle?.substring(0, 90) || 'Matériaux de construction et fixations et outils quotidiens pour les ateliers et les entrepreneurs.'}
            </p>
            <p style={{ marginTop: '2rem', fontSize: '0.7rem', color: 'rgba(255,255,255,0.2)' }}>© {new Date().getFullYear()} {store?.name}</p>
          </div>
          <div>
            <h4 style={{ fontSize: '0.72rem', fontWeight: 700, color: E, textTransform: 'uppercase' as const, letterSpacing: '0.1em', marginBottom: '1rem' }}>Pages</h4>
          ...
          <div>
            <h4 style={{ fontSize: '0.72rem', fontWeight: 700, color: E, textTransform: 'uppercase' as const, letterSpacing: '0.1em', marginBottom: '1rem' }}>Contact</h4>
            {[
    ...
    <div className="util-card" style={{ background: CARD, display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ padding: '0.75rem 0.875rem 0.625rem', display: 'flex', flexDirection: 'column', flex: 1, borderTop: `1px solid ${BD}` }}>
        ...
        <div style={{ marginBottom: '0.625rem' }}>
          ...
          <span style={{ fontSize: '0.7rem', color: SUB, marginRight: 4 }}>{store.currency || 'DA'}</span>
    ...
    <div dir="ltr">
        ...
        <div style={{ position: 'relative', maxWidth: 1280, margin: '0 auto', width: '100%', padding: 'clamp(3rem,6vw,5rem) 1.5rem' }}>
          <div style={{ maxWidth: 580 }}>
            ...
            <div style={{ display: 'flex', gap: '2rem', marginTop: '2.5rem', paddingTop: '2rem', borderTop: `1px solid ${BD}` }}>
              {[{ n: '+500', l: 'Produit' }, { n: '58', l: 'Wilaya' }, { n: '100%', l: 'Garanti' }].map((s, i) => (
                <div key={i}>
      ...
      <div style={{ background: EL, borderBottom: `1px solid rgba(16,185,129,0.15)` }}>
        <div className="trust-strip" style={{ maxWidth: 1280, margin: '0 auto', padding: '0 1.5rem' }}>
          {[
            { icon: <Truck size={13} />, t: 'Livraison pour 58 wilayas' },
            { icon: <Shield size={13} />, t: 'Paiement À la livraison' },
            { icon: <Layers size={13} />, t: 'Stock renouvelé quotidiennement' },
            { icon: <Wrench size={13} />, t: 'Outils Certifiés' },
          ].map((item, i) => (
      ...
      {cats.length > 0 && (
        <section style={{ padding: '2.5rem 1.5rem', maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', marginBottom: '1.25rem' }}>
            ...
            <h2 style={{ fontSize: '0.875rem', fontWeight: 700, color: INK }}>Parcourir par Catégorie</h2>
          </div>
          <div className="cats-section-grid">
            <Link href="?" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '1rem 0.75rem', border: `1px solid ${!activeCategory ? E : BD}`, borderRadius: 4, background: !activeCategory ? EL : CARD, textAlign: 'center', transition: 'all 0.18s' }}
              ...
              <span style={{ fontSize: '0.78rem', fontWeight: 600, color: INK, lineHeight: 1.3 }}>Tout</span>
            </Link>
      ...
      <section id="products" style={{ padding: cats.length ? '0 0 5rem' : '2rem 0 5rem', maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 1.5rem', marginBottom: '1px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '1rem 0' }}>
            <span style={{ width: 3, height: 16, background: E, borderRadius: 1, display: 'inline-block' }} />
            <h2 style={{ fontSize: '0.875rem', fontWeight: 700, color: INK }}>Tous les Produits</h2>
          </div>
          {store.count > 0 && <span style={{ fontSize: '0.72rem', color: SUB }}>{store.count} Produit</span>}
        </div>
      ...
      <div className="details-inner" style={{ maxWidth: 1280, margin: '0 auto' }}>
        <div className="gallery-container">
          <div style={{ position: 'relative', aspectRatio: '1/1', overflow: 'hidden', background: '#F3F4F6', border: `1px solid ${BD}` }}>
            ...
            {discount > 0 && <div style={{ position: 'absolute', top: 10, right: 10, background: E, color: '#fff', padding: '2px 8px', fontSize: 11, fontWeight: 800 }}>{discount}% Réduction</div>}
            {allImages.length > 1 && (
              <>
        ...
        <div>
          <div className="info-container">
            <p style={{ fontSize: '0.7rem', fontWeight: 700, color: E, textTransform: 'uppercase' as const, letterSpacing: '0.08em', marginBottom: '0.5rem' }}>Matériaux de Construction essentiels</p>
            ...
            <div style={{ padding: '1rem', background: EL, border: `1px solid rgba(16,185,129,0.2)`, marginBottom: '1.5rem' }}>
              <p style={{ fontSize: '0.65rem', fontWeight: 700, color: SUB, marginBottom: '0.2rem', textTransform: 'uppercase' as const, letterSpacing: '0.08em' }}>Prix</p>
              ...
              <div style={{ marginBottom: '1.25rem' }}>
                {product.offers.map((o: any) => (
                  <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.7rem 0.875rem', border: `1px solid ${selectedOffer === o.id ? E : BD}`, cursor: 'pointer', marginBottom: '0.375rem', background: selectedOffer === o.id ? EL : CARD, transition: 'all 0.15s' }}>
                    ...
                    <span className="price-mono" style={{ fontWeight: 800, color: ED }}>{o.price.toLocaleString()} DA</span>
                  </label>
                ))}
              </div>
            )}
        ...
        <div className="anim-fade-up">
          {product.store.cart && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <p style={{ fontWeight: 700, fontSize: '0.7rem', color: SUB, textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Informations de livraison</p>
              <button onClick={() => setIsOrderNow(false)} style={{ fontSize: '0.78rem', color: '#9CA3AF', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>Annuler</button>
            </div>
          )}
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: '0.875rem' }}>
              <p style={{ fontSize: '0.7rem', fontWeight: 700, color: SUB, marginBottom: '0.5rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Livraison</p>
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.7rem', border: `1px solid ${fd.typeLivraison === t ? E : BD}`, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? EL : CARD, transition: 'all 0.15s', fontFamily: 'inherit' }}>
                    <p style={{ fontWeight: 600, fontSize: '0.8rem', marginBottom: 3, color: fd.typeLivraison === t ? ED : SUB }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
                    {selW && <p className="price-mono" style={{ fontSize: '0.95rem', fontWeight: 800, color: fd.typeLivraison === t ? ED : '#9CA3AF' }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} DA</p>}
                  </button>
                ))}
              </div>
            </div>
            <div style={{ marginBottom: '0.875rem' }}>
              <p style={{ fontSize: '0.7rem', fontWeight: 700, color: SUB, marginBottom: '0.5rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Quantité</p>
            ...
            <div style={{ background: EL, border: `1px solid rgba(16,185,129,0.2)`, padding: '0.875rem 1rem', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.625rem', fontWeight: 800, color: ED }}>{total().toLocaleString()} <span style={{ fontSize: '0.75rem' }}>DA</span></span>
              </div>
            </div>
  ...
  if (!items.length) return (
    <div dir="ltr" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: BG }}>
      <div style={{ textAlign: 'center', padding: '4rem 2rem', border: `1px solid ${BD}`, maxWidth: 400, width: '100%', background: CARD }}>
        ...
        <p style={{ color: '#9CA3AF', fontSize: '0.9rem', marginBottom: '2rem' }}>Panier vide</p>
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: E, color: '#fff', padding: '0.7rem 1.75rem', fontWeight: 700, fontSize: '0.875rem', borderRadius: 4 }}>Acheter maintenant</Link>
      </div>
    </div>
  );
  ...
  return (
    <div dir="ltr" style={{ padding: '2rem 1.5rem', maxWidth: 1280, margin: '0 auto', minHeight: '100vh', background: BG }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '2rem' }}>
        ...
        <h1 style={{ fontSize: 'clamp(1.375rem,5vw,2rem)', fontWeight: 800, color: INK }}>Panier</h1>
      </div>
      <div className="cart-inner">
        <div style={{ background: CARD, border: `1px solid ${BD}`, alignSelf: 'start', borderTop: `2px solid ${E}` }}>
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '0.75rem', padding: '0.875rem', borderBottom: `1px solid ${BD}` }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p className="price-mono" style={{ fontSize: '1rem', fontWeight: 800, color: ED }}>{item.finalPrice?.toLocaleString()} DA</p>
                <p style={{ fontSize: '0.7rem', color: SUB, marginTop: '0.15rem' }}>Quantité: {item.quantity}</p>
              </div>
          ...
          <div style={{ padding: '0.875rem', background: EL, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span className="price-mono" style={{ fontSize: '1.125rem', fontWeight: 800, color: ED }}>{cartTotal.toLocaleString()} DA</span>
          </div>
        </div>
        <div style={{ background: CARD, border: `1px solid ${BD}`, padding: '1.5rem', alignSelf: 'start', borderTop: `2px solid ${E}` }}>
          <h3 style={{ fontWeight: 700, fontSize: '0.875rem', color: INK, marginBottom: '1.25rem' }}>Informations de livraison</h3>
          <form onSubmit={handleSubmit}>
            <div className="form-row-2">
            ...
            <div style={{ margin: '1rem 0' }}>
              <p style={{ fontSize: '0.7rem', fontWeight: 700, color: SUB, textTransform: 'uppercase' as const, marginBottom: '0.5rem', letterSpacing: '0.06em' }}>Type de livraison</p>
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.7rem', border: `1px solid ${fd.typeLivraison === t ? E : BD}`, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? EL : CARD, fontFamily: 'inherit', transition: 'all 0.15s' }}>
                    <p style={{ fontWeight: 600, fontSize: '0.78rem', color: fd.typeLivraison === t ? ED : SUB, marginBottom: 3 }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
                    {selW && <p style={{ fontWeight: 800, fontSize: '0.9rem', color: fd.typeLivraison === t ? ED : '#9CA3AF' }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} DA</p>}
                  </button>
                ))}
              </div>
            </div>
            <div style={{ background: EL, border: `1px solid rgba(16,185,129,0.2)`, padding: '0.875rem 1rem', margin: '1rem 0' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.75rem', fontWeight: 800, color: ED }}>{finalTotal.toLocaleString()} <span style={{ fontSize: '0.75rem' }}>DA</span></span>
              </div>
            </div>
    ...
    <div dir="ltr" style={{ background: BG, minHeight: '100vh' }}>
      <div style={{ background: INK, paddingTop: 88, paddingBottom: 40, paddingLeft: 24, paddingRight: 24, borderTop: `3px solid ${E}` }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <p style={{ fontSize: '0.65rem', fontWeight: 700, color: E, textTransform: 'uppercase' as const, letterSpacing: '0.12em', marginBottom: '0.625rem' }}>Everyday Hardware</p>
          <h1 style={{ fontSize: 'clamp(2rem,5vw,3rem)', fontWeight: 800, color: '#fff' }}>Contactez-nous</h1>
        </div>
      </div>
      <div className="contact-inner" style={{ maxWidth: 1100, margin: '0 auto', padding: '3rem 1.5rem 6rem' }}>
        <div>
          ...
          <div style={{ background: EL, border: `1px solid rgba(16,185,129,0.2)`, padding: '0.75rem 1rem', display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
            ...
            <span style={{ fontSize: '0.78rem', fontWeight: 600, color: ED }}>Réponse sous une heure</span>
          </div>
        </div>
        <div style={{ background: CARD, border: `1px solid ${BD}`, padding: '1.75rem', borderTop: `2px solid ${E}` }}>
          {sent ? (
            <div style={{ textAlign: 'center', padding: '3.5rem 1rem' }}>
              ...
              <h2 style={{ fontSize: '1.375rem', fontWeight: 800, color: INK, marginBottom: '0.5rem' }}>Envoyé !</h2>
              <p style={{ color: SUB, lineHeight: 1.7, marginBottom: '2rem', fontSize: '0.9rem' }}>Nous vous répondrons dans les plus brefs délais.</p>
              <button onClick={() => setSent(false)} style={{ padding: '0.7rem 1.75rem', border: `1px solid ${E}`, background: EL, color: ED, fontWeight: 700, cursor: 'pointer', fontSize: '0.875rem', fontFamily: 'inherit' }}>Envoyer un autre message</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
              <div className="form-row-2" style={{ marginBottom: '0.875rem' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.7rem', fontWeight: 700, color: SUB, marginBottom: '0.3rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Nom</label>
                  <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required style={S.input} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '0.7rem', fontWeight: 700, color: SUB, marginBottom: '0.3rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Téléphone</label>
              ...
              <div style={{ marginBottom: '1.25rem' }}>
                <label style={{ display: 'block', fontSize: '0.7rem', fontWeight: 700, color: SUB, marginBottom: '0.3rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Message</label>
