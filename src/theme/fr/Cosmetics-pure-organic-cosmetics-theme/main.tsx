          ...
          <div className="glb-search-panel">
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'2rem' }}>
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                ...
                <span style={{ fontSize:'0.65rem', fontWeight:600, color: SUB, letterSpacing:'0.12em', textTransform:'uppercase' }}>Recherche</span>
              </div>
            ...
            <form className="glb-search-form" onSubmit={handleSearch}>
              ...
              <input ref={searchInputRef} className="glb-search-input" type="text" placeholder="Rechercher un produit..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </form>
                ...
                {listSearch.map((p:any) => (
                  <Link key={p.id} href={`/product/${p.slug||p.id}`} className="glb-search-card" onClick={() => setShowSearch(false)}>
                    ...
                    <div className="glb-search-card-info">
                      ...
                      <p style={{ fontSize:'0.74rem', fontWeight:700, color: RO }}>{Number(p.price).toLocaleString()} DA</p>
                    </div>
                  </Link>
                ))}
              </div>
    ...
    <footer dir="ltr" style={{ background: INK, color:'rgba(255,255,255,0.38)', marginTop:80 }}>
      <div style={{ maxWidth:1320, margin:'0 auto', padding:'4.5rem 2rem 2rem' }}>
        <div className="footer-cols" style={{ paddingBottom:'3rem', marginBottom:'2rem', borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div>
            <h4 style={{ fontSize:'0.58rem', fontWeight:700, color: RO, textTransform:'uppercase', letterSpacing:'0.16em', marginBottom:'1.5rem' }}>Pages</h4>
          ...
          <div>
            <h4 style={{ fontSize:'0.58rem', fontWeight:700, color: RO, textTransform:'uppercase', letterSpacing:'0.16em', marginBottom:'1.5rem' }}>Contact</h4>
            {[
    ...
    <div className="co-card">
      ...
      <div style={{ padding:'0.875rem 1rem 0.75rem', flex:1 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          ...
          <span style={{ fontSize:'0.62rem', color: SUB }}>{store.currency||'DA'}</span>
  ...
  return (
    <div dir="ltr">
          ...
          <div style={{ maxWidth:1320, margin:'0 auto', width:'100%', padding:'clamp(3rem,8vw,6rem) clamp(1.25rem,4vw,3rem)', display:'flex', justifyContent:'flex-start' }}>
            <div className='anim-fade-up' style={{ maxWidth:540, width:'100%' }}>
              ...
              <div style={{ display:'inline-flex', alignItems:'center', gap:10, marginBottom:'1.75rem', background:'rgba(255,255,255,0.1)', backdropFilter:'blur(8px)', border:'1px solid rgba(255,255,255,0.2)', borderRadius:50, padding:'6px 16px' }}>
                ...
                <span style={{ fontSize:'0.6rem', fontWeight:600, color:'rgba(255,255,255,0.9)', letterSpacing:'0.2em', textTransform:'uppercase' }}>Pur · Naturel · Bio</span>
              </div>
              ...
              <p style={{ fontSize:'clamp(0.9rem,2vw,1.05rem)', color:'rgba(255,255,255,0.75)', lineHeight:1.9, marginBottom:'2.5rem', fontWeight:300, maxWidth:380 }}>
                {store.hero?.subtitle || 'Cosmétiques Bio purs fabriqués avec les meilleurs ingrédients Naturels pour une peau radieuse.'}
              </p>
      ...
      <section className="trust-strip" style={{ borderBottom:`1px solid ${BD}` }}>
        {[
          { icon:<Leaf size={16}/>,      t:'Ingrédients Bio',    s:'100% naturel Certifiée' },
          { icon:<Droplets size={16}/>,  t:'Huiles pures',       s:'Sans additifs chimiques' },
          { icon:<Heart size={16}/>,     t:'Cruelty-Free',    s:'Non testé sur les animaux' },
          { icon:<Sparkles size={16}/>,  t:'100% végétalien',     s:'Vegan Certified' },
        ].map((item,i) => (
      ...
      {cats.length > 0 && (
        <section style={{ padding:'2.75rem 1.5rem', maxWidth:1320, margin:'0 auto' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'1rem', marginBottom:'1.25rem' }}>
            <h2 style={{ fontSize:'1rem', fontWeight:600, color: INK, letterSpacing:'-0.01em', whiteSpace:'nowrap' as const }}>Parcourir les Catégories</h2>
      ...
      <section style={{ background: INK, padding:'5rem 1.5rem', marginTop:'-2rem' }}>
        <div style={{ maxWidth:1320, margin:'0 auto' }}>
          ...
          <div className="features-grid">
            {[
              { icon:<Leaf size={20}/>,     t:'100% bio',     d:'Ingrédients certifiés biologiques provenant des meilleures sources naturelles du monde.' },
              { icon:<Droplets size={20}/>, t:'Sans produits chimiques', d:'Totalement exempt de parabènes, silicones et colorants synthétiques.' },
              { icon:<Heart size={20}/>,    t:'Sûr pour la peau',    d:'Testé et approuvé par des experts en beauté et dermatologie pour tous les types de peau.' },
              { icon:<Sparkles size={20}/>, t:'Vegan',         d:'100% éthique — sans ingrédients animaux ni tests sur animaux.' },
            ].map((f,i) => (
      ...
      <div className="details-inner" style={{ maxWidth:1320, margin:'0 auto' }}>
        ...
        <div className="gallery-container">
          <div style={{ position:'relative', aspectRatio:'3/4', overflow:'hidden', borderRadius:20, background: CR, border:`1px solid ${BD}` }}>
            ...
            {discount > 0 && <div style={{ position:'absolute', top:14, right:14, background:`linear-gradient(135deg,${RO},${ROD})`, color:'#fff', padding:'5px 13px', fontSize:11, fontWeight:700, borderRadius:20 }}>{discount}% Réduction</div>}
            {allImages.length > 1 && (
              <>
                  ...
                  <label key={o.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'0.9rem 1rem', border:`1.5px solid ${selectedOffer===o.id ? RO : BD}`, borderRadius:12, cursor:'pointer', marginBottom:'0.5rem', background: selectedOffer===o.id ? ROL : CARD, transition:'all 0.18s' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:'0.625rem' }}>
                      ...
                      <div>
                        ...
                        <p style={{ fontSize:'0.68rem', color: SUB }}>Quantité: {o.quantity}</p>
                      </div>
                    </div>
                    <span className="price-mono" style={{ fontWeight:700, color: INK }}>{o.price.toLocaleString()} DA</span>
                  </label>
                ))}
              </div>
            )}
        ...
        <div className="anim-fade-up">
          {product.store.cart && (
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1rem' }}>
              <p style={{ fontWeight:700, fontSize:'0.62rem', color: SUB, textTransform:'uppercase', letterSpacing:'0.1em' }}>Informations de livraison</p>
              <button onClick={() => setON(false)} style={{ fontSize:'0.78rem', color: SUB, background:'none', border:'none', cursor:'pointer', fontFamily:'inherit' }}>Annuler ✕</button>
            </div>
          )}
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom:'0.875rem' }}>
              <p style={{ fontSize:'0.62rem', fontWeight:700, color: SUB, marginBottom:'0.5rem', textTransform:'uppercase', letterSpacing:'0.1em' }}>Livraison</p>
              <div className="delivery-grid">
                {(['home','office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding:'0.8rem', border:`1.5px solid ${fd.typeLivraison===t ? RO : BD}`, borderRadius:12, textAlign:'center', cursor:'pointer', background: fd.typeLivraison===t ? ROL : CARD, transition:'all 0.18s', fontFamily:'inherit' }}>
                    <p style={{ fontWeight:600, fontSize:'0.78rem', marginBottom:3, color: fd.typeLivraison===t ? ROD : SUB }}>{t==='home'?'À domicile':'Au bureau'}</p>
                    {selW && <p className="price-mono" style={{ fontSize:'1rem', fontWeight:700, color: fd.typeLivraison===t ? ROD : SUB }}>{(t==='home'?selW.livraisonHome:selW.livraisonOfice).toLocaleString()} DA</p>}
                  </button>
                ))}
              </div>
            </div>
            <div style={{ marginBottom:'0.875rem' }}>
              <p style={{ fontSize:'0.62rem', fontWeight:700, color: SUB, marginBottom:'0.5rem', textTransform:'uppercase', letterSpacing:'0.1em' }}>Quantité</p>
            ...
            <div style={{ background: CR, borderRadius:14, border:`1px solid ${BD}`, padding:'1.125rem 1.25rem', marginBottom:'1rem' }}>
              {[
                { l:'Produit', v: product.name.slice(0,30)+(product.name.length>30?'...':'') },
          ...
          {items.map((item,i) => (
            <div key={i} style={{ display:'flex', gap:'1rem', padding:'1.125rem', borderBottom:`1px solid ${BD}` }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <p className="price-mono" style={{ fontSize:'1.1rem', fontWeight:700, color: INK }}>{item.finalPrice?.toLocaleString()} DA</p>
              </div>
        ...
        <div style={{ background: CARD, borderRadius:18, border:`1px solid ${BD}`, padding:'2rem' }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ margin:'0.875rem 0' }}>
              <p style={{ fontSize:'0.62rem', fontWeight:700, color: SUB, textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:'0.5rem' }}>Type de livraison</p>
              <div className="delivery-grid">
                {(['home','office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding:'0.85rem', border:`1.5px solid ${fd.typeLivraison===t ? RO : BD}`, borderRadius:12, textAlign:'center', cursor:'pointer', background: fd.typeLivraison===t ? ROL : CARD, fontFamily:'inherit', transition:'all 0.18s' }}>
                    <p style={{ fontWeight:600, fontSize:'0.78rem', color: fd.typeLivraison===t ? ROD : SUB, marginBottom:3 }}>{t==='home'?'À domicile':'Au bureau'}</p>
                    {selW && <p style={{ fontWeight:700, fontSize:'0.95rem', color: fd.typeLivraison===t ? ROD : SUB }}>{(t==='home'?selW.livraisonHome:selW.livraisonOfice).toLocaleString()} DA</p>}
                  </button>
                ))}
              </div>
            </div>
      ...
      <div className="contact-inner" style={{ maxWidth:1100, margin:'0 auto', padding:'3rem 1.5rem 6rem' }}>
        <div>
          <div style={{ background: CARD, borderRadius:18, border:`1px solid ${BD}`, padding:'1.5rem', marginBottom:'1rem' }}>
            {[
              { icon:<Phone size={13}/>, l:'Téléphone', val: store?.contact?.phone||'Non disponible' },
              { icon:<MapPin size={13}/>, l:'Adresse', val: [store?.contact?.wilaya, store?.contact?.address].filter(Boolean).join(' / ')||'Algérie' },
            ].map((r,i) => (
          ...
          <div style={{ background: ROL, border:`1px solid ${ROB}`, borderRadius:12, padding:'0.875rem 1rem', display:'flex', alignItems:'center', gap:'0.625rem' }}>
            <Flower2 size={12} color={RO}/>
            <span style={{ fontSize:'0.75rem', fontWeight:500, color: ROD }}>Réponse sous une heure une</span>
          </div>
        </div>
