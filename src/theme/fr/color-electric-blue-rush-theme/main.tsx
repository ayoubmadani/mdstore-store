...
/*
  ...
  ██╔══██╗██║   ██║██║     ██║  ██║    ██║   ██║██╔══██╗██╔══██╗██╔══██║██║╚████║
  ██████╦╝╚██████╔╝███████╗██████╔╝    ╚██████╔╝██║  ██║██████╔╝██║  ██║██║ ╚███║
  ═══════ THEME: BOLD / COLOR: #1D4ED8 + #1D1D1D + #F8F8F6
*/
...
const THEME_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Ticker ── */
  /* تم تغيير خلفية الشريط المتحرك إلى الأزرق */
  ...
  /* ── Card stripe ── */
  /* تم تغيير لون الخط العلوي للبطاقة إلى الأزرق */
  .card-stripe::before {
    content: '';
    position: absolute;
    inset: 0;
...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  // 1. تعريف الTennisيقات (Styles) داخل الـ Component
  ...
  // 2. مكون Résultats de recherche مع خاصية التمرير (Overflow Max Height)
  const DropResults = () => (
    <div style={{
      paddingTop: 25,
      ...
      maxHeight: '300px', overflowY: 'auto' // Contrôle في الطول الزائد
    }} className="anim-slide-down">
      <button onClick={() => setSearchQuery('')} className='fixed top-3 left-3 cursor-pointer hover:text-red-400'>
        <X size={14} />
      </button>
      {loading ? <div style={{ padding: '1rem', textAlign: 'center', color: '#1D4ED8', fontSize: '0.85rem' }}>Recherche en cours...</div>
        : listSearch.length > 0 ? (
          <>
            {listSearch.map((p: any) => (
              <Link href={`/product/${p.id}`} key={p.id} onClick={() => setSearchQuery('')}
                ...
                <div>
                  ...
                  <div style={{ fontSize: '0.75rem', color: '#1D4ED8', fontWeight: 700 }}>{p.price} DA</div>
                </div>
              </Link>
            ))}
            <button onClick={handleSearch} style={{ width: '100%', padding: '0.75rem', background: '#F8F8F6', border: 'none', fontWeight: 700, cursor: 'pointer', fontSize: '0.8rem', color: '#444' }}>Voir tout</button>
          </>
      ...
      <nav dir="rtl" style={{
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 1.25rem', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem' }}>
          ...
          <div className="nav-desktop">
            ...
            {/* 3. استخدام le styleات المعرفة مسبقاً */}
            {store?.cart !== false && (
          ...
          <div style={{ padding: '0.625rem 1.25rem', background: '#fff', borderTop: '1px solid #E8E8E8', position: 'relative' }} className="anim-slide-down">
            <form onSubmit={handleSearch} style={{ position: 'relative' }}>
              <input autoFocus type="text" placeholder="Rechercher un produit..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                style={{ width: '100%', padding: '0.75rem 1rem 0.75rem 2.75rem', border: '1.5px solid #1D4ED8', borderRadius: 8, background: '#F8F8F6', fontSize: '0.9rem', outline: 'none' }} />
              <Search size={15} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#1D4ED8' }} />
            </form>
            {searchQuery.length >= 2 && <DropResults />}
          </div>
        )}
          ...
          <div style={{ padding: '0.375rem 1.25rem 0.875rem' }}>
            {[{ h: '/', l: 'Accueil' }, { h: '/contact', l: 'Contactez-nous' }].map(i => (
              <Link key={i.h} href={i.h} onClick={() => setOpen(false)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 0', borderBottom: '1px solid #F0F0F0', fontSize: '0.9rem', fontWeight: 600, color: '#111' }}>
                {i.l} <ArrowLeft size={14} style={{ color: '#1D4ED8' }} />
              </Link>
            ))}
...
export function Card({ product, displayImage, discount, store, viewDetails }: any) {
  ...
  return (
    <div className="card-wrap" style={{ background: '#fff', border: '1.5px solid #EBEBEB', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column', height: '100%', position: 'relative', transition: 'all 0.28s ease' }}
      onMouseEnter={e => { const el = e.currentTarget as HTMLDivElement; el.style.borderColor = '#1D4ED8'; el.style.boxShadow = '0 8px 32px rgba(230,57,70,0.1)'; el.style.transform = 'translateY(-4px)'; }}
  ...
  return (
    <div dir="rtl">
      ...
      <section style={{ position: 'relative', background: '#111', overflow: 'hidden' }}>
        ...
        {/* Geometric accent - تم تحديث التدرج للأزرق */}
        <div style={{ position: 'absolute', top: 0, left: 0, width: '40%', height: '100%', background: 'linear-gradient(135deg, #3B82F6 0%, transparent 60%)', opacity: 0.12, pointerEvents: 'none' }} />

        {/* Circle accent - تم تحديث لون الدائرة الزخرفية */}
        <div style={{ position: 'absolute', bottom: -60, right: -60, width: 300, height: 300, border: '60px solid rgba(59, 130, 246, 0.07)', borderRadius: '50%', pointerEvents: 'none' }} />

        <div className="hero-inner" style={{ maxWidth: 1280, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          {/* Text */}
          <div>
            {/* Badge - تم تحديث الخلفية والحدود للأزرق */}
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(59, 130, 246, 0.15)', border: '1px solid rgba(59, 130, 246, 0.3)', borderRadius: 6, padding: '0.375rem 0.875rem', marginBottom: '1.5rem' }}>
              <Zap size={13} color="#3B82F6" fill="#3B82F6" />
              <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#3B82F6', letterSpacing: '0.08em', textTransform: 'uppercase' }}>{store.name}</span>
            </div>
            ...
            <h1 style={{ fontSize: 'clamp(2.25rem,6vw,4.5rem)', fontWeight: 800, color: '#fff', lineHeight: 1.1, letterSpacing: '-0.03em', marginBottom: '1.25rem' }}
              dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(store.hero?.title || 'Acheter<br/><span style="color:#3B82F6">En confiance</span> et obtenez<br/>le meilleur') }} />
            ...
            <div className="hero-actions">
              <a href="#products" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#3B82F6', color: '#fff', fontWeight: 700, fontSize: '0.9rem', padding: '0.875rem 1.75rem', borderRadius: 10, transition: 'all 0.2s', boxShadow: '0 4px 20px rgba(59, 130, 246, 0.35)' }}
        ...
        <div className="trust-grid" style={{ maxWidth: 1280, margin: '0 auto' }}>
          {[
            ...
            <div key={i} style={{ padding: '1.125rem 1rem', textAlign: 'center', borderLeft: i < 3 ? '1px solid #EBEBEB' : 'none' }}>
              <div style={{ color: '#1D4ED8', marginBottom: '0.375rem', display: 'flex', justifyContent: 'center' }}>{item.icon}</div>
          ...
          <div className="cats-grid">
            <Link href="?" style={{ display:'inline-flex', alignItems:'center', padding:'0.5rem 1.25rem', borderRadius:999, border:`1.5px solid ${!activeCategory ? '#1D4ED8' : '#ccc'}`, background: !activeCategory ? '#1D4ED8' : 'transparent', color: !activeCategory ? '#fff' : 'inherit', fontSize:'0.82rem', fontWeight:600, cursor:'pointer' }}>
              ...
              <Link key={cat.id} href={`?category=${cat.id}`} style={{
                ...
                onMouseEnter={e => { const el = e.currentTarget; el.style.borderColor = '#1D4ED8'; el.style.color = '#1D4ED8'; el.style.background = 'rgba(230,57,70,0.04)'; }}
...
export function Details({ product, discount, allImages, allAttrs, finalPrice, selectedVariants, setSelectedOffer, selectedOffer, handleVariantSelection, domain }: any) {
  ...
  // Contrôle في وضعية الـ Sticky بناءً على حجم الشاشة بشكل Sécurisé
  useEffect(() => {
      ...
      <div className="details-inner" style={{ maxWidth: 1280, margin: '0 auto' }}>
            ...
            <div className="thumb-row" style={{
              ...
              {allImages.map((img: string, idx: number) => (
                <button key={idx} onClick={() => setSel(idx)} style={{
                  ...
                  borderRadius: 8,
                  overflow: 'hidden',
                  border: `2px solid ${sel === idx ? '#1D4ED8' : '#E8E8E8'}`,
        ...
        <div>
          <div style={{
            ...
            {/* صندوق Prix (Blue Theme) */}
            <div style={{ background: '#EFF6FF', border: '1.5px solid #BFDBFE', borderRadius: 10, padding: '1rem 1.25rem', marginBottom: '1.5rem' }}>
              <p style={{ fontSize: '0.75rem', color: '#1D4ED8', fontWeight: 700, marginBottom: '0.25rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Prix actuel</p>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.375rem' }}>
                <span className="price-mono" style={{ fontSize: '2.25rem', fontWeight: 800, color: '#111' }}>{finalPrice.toLocaleString()}</span>
                <span style={{ fontWeight: 700, color: '#1D4ED8', fontSize: '1.1rem' }}>DA</span>
              </div>
            </div>
            ...
            {product.offers?.length > 0 && (
              <div style={{ marginBottom: '1.25rem' }}>
                {product.offers.map((o: any) => (
                  <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.875rem 1rem', border: `1.5px solid ${selectedOffer === o.id ? '#1D4ED8' : '#E8E8E8'}`, borderRadius: 10, cursor: 'pointer', marginBottom: '0.5rem', background: selectedOffer === o.id ? 'rgba(29, 78, 216, 0.03)' : 'transparent', transition: 'all 0.18s' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                      ...
                      <div>
                        ...
                        <p style={{ fontSize: '0.72rem', color: '#888' }}>Quantité: {o.quantity}</p>
                      </div>
                    </div>
                    <span className="price-mono" style={{ fontWeight: 800, color: '#1D4ED8', fontSize: '1.1rem' }}>{o.price.toLocaleString()} DA</span>
                  </label>
                ))}
              </div>
            )}
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {attr.variants.map((v: any) => {
                    ...
                    return (
                      ...
                      >
                        {/* إظهار النص uniquement في الحالات غير المرئية (color/image) */}
      ...
      {product.store.cart && (
        <div className="cart-add-btns" style={{ marginBottom: '1.25rem' }}>
          ...
          <button
            ...
            onMouseEnter={e => ((e.currentTarget as HTMLButtonElement).style.background = '#2563EB')} // درجة أزرق أغمق عند التمرير
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: '0.875rem' }}>
              ...
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.875rem', border: `1.5px solid ${fd.typeLivraison === t ? '#1D4ED8' : '#E8E8E8'}`, borderRadius: 10, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? 'rgba(230,57,70,0.05)' : '#fff', transition: 'all 0.18s', fontFamily: 'inherit' }}>
                    <p style={{ fontWeight: 700, fontSize: '0.85rem', marginBottom: 4, color: fd.typeLivraison === t ? '#1D4ED8' : '#555' }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
            ...
            <div style={{ background: '#F8F8F6', border: '1.5px solid #EBEBEB', borderRadius: 10, padding: '1rem 1.125rem', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '0.375rem' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.625rem', fontWeight: 800, color: '#1D4ED8' }}>{total().toLocaleString()} <span style={{ fontSize: '0.8rem', fontFamily: 'inherit', fontWeight: 700, color: '#1D4ED8' }}>DA</span></span>
              </div>
            </div>
  ...
  if (success) return (
    <div dir="rtl" style={{ minHeight: '70vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: '#F8F8F6' }}>
      <div style={{ textAlign: 'center', background: '#fff', padding: '3.5rem 2rem', borderRadius: 16, border: '1.5px solid #E8E8E8', maxWidth: 460, width: '100%' }}>
        ...
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#1D4ED8', color: '#fff', padding: '0.8rem 2rem', borderRadius: 10, fontWeight: 700, fontSize: '0.9rem' }}>Retour à la boutique</Link>
      </div>
    </div>
  );
  ...
  if (!items.length) return (
    <div dir="rtl" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: '#F8F8F6' }}>
      <div style={{ textAlign: 'center', padding: '4rem 2rem', border: '2px dashed #E8E8E8', borderRadius: 16, maxWidth: 440, width: '100%', background: '#fff' }}>
        ...
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#1D4ED8', color: '#fff', padding: '0.8rem 2rem', borderRadius: 10, fontWeight: 700, fontSize: '0.875rem' }}>Acheter maintenant</Link>
      </div>
    </div>
  );
          ...
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '1rem', padding: '1rem', borderBottom: '1px solid #F0F0F0' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p className="price-mono" style={{ fontSize: '1.125rem', fontWeight: 800, color: '#1D4ED8' }}>{item.finalPrice?.toLocaleString()} DA</p>
          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  ...
                  >
                    ...
                    <p style={{ fontWeight: 700, fontSize: '0.78rem', color: fd.typeLivraison === t ? '#1D4ED8' : '#888' }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
            ...
            <div style={{ background: '#F8F8F6', border: '1.5px solid #EBEBEB', borderRadius: 10, padding: '1rem 1.125rem', margin: '1rem 0' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.75rem', fontWeight: 800, color: '#1D4ED8' }}>{finalTotal.toLocaleString()} <span style={{ fontSize: '0.8rem', fontWeight: 700, color: '#1D4ED8' }}>DA</span></span>
              </div>
            </div>
...
const Shell = ({ children, title }: { children: React.ReactNode; title: string }) => (
  <div dir="rtl" style={{ minHeight: '100vh', background: '#F8F8F6' }}>
    <div style={{ background: '#111', paddingTop: 96, paddingBottom: 48, paddingLeft: 24, paddingRight: 24, textAlign: 'center' }}>
      <div style={{ display: 'inline-block', background: 'rgb(85 57 230 / 15%)', border: '1px solid rgba(230,57,70,0.3)', padding: '0.375rem 1rem', borderRadius: 6, marginBottom: '1.25rem' }}>
      ...
      <div className="contact-inner" style={{ maxWidth: 1100, margin: '0 auto', padding: '4rem 1.5rem 6rem' }}>
        ...
        <div>
          <div style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E8E8E8', padding: '1.75rem', marginBottom: '1rem' }}>
            <p style={{ fontSize: '0.7rem', fontWeight: 800, color: '#1D4ED8', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '1.25rem' }}>Informations de contact</p>
            {[
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.875rem', marginBottom: '1.125rem' }}>
                <div style={{ width: 44, height: 44, borderRadius: 10, background: 'rgba(230,57,70,0.08)', border: '1px solid rgb(85 57 230 / 15%)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#1D4ED8', flexShrink: 0 }}>{r.icon}</div>
                <div>
        ...
        <div style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E8E8E8', padding: '2rem' }}>
          {sent ? (
            <div style={{ textAlign: 'center', padding: '4rem 1rem' }}>
              ...
              <button onClick={() => setSent(false)} style={{ padding: '0.75rem 1.75rem', borderRadius: 10, border: '1.5px solid #1D4ED8', background: 'transparent', color: '#1D4ED8', fontWeight: 700, cursor: 'pointer', fontSize: '0.875rem', fontFamily: 'inherit' }}>Envoyer un autre message</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
              ...
              <button
                ...
                style={{
                  ...S.btnPrimary,
                  background: '#1D4ED8', // اللون الأزرق الأساسي
                  color: '#fff',
                ...
                onMouseEnter={e => !loading && ((e.currentTarget as HTMLButtonElement).style.background = '#1E40AF')} // أزرق داكن عند التحويم
                onMouseLeave={e => ((e.currentTarget as HTMLButtonElement).style.background = '#1D4ED8')}
              >
                {loading ? (
                  <>
