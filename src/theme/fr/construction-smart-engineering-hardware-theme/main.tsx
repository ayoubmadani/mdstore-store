                ...
                {listSearch.map((p: any) => (
                  <Link key={p.id} href={`/product/${p.slug || p.id}`} className="glb-search-card" onClick={() => setShowSearch(false)}>
                    ...
                    <div className="glb-search-card-info">
                      ...
                      <p style={{ fontSize: '0.8rem', fontWeight: 700, color: AD }}>{Number(p.price).toLocaleString()} DA</p>
                    </div>
                  </Link>
                ))}
              </div>
    ...
    <footer dir="rtl" style={{ background: SL, color: 'rgba(255,255,255,0.5)', marginTop: 80, padding: '4rem 1.5rem 1.5rem' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        <div className="footer-inner">
          ...
          <div>
            <h4 style={{ fontSize: '0.7rem', fontWeight: 700, color: A, textTransform: 'uppercase' as const, letterSpacing: '0.1em', marginBottom: '1.25rem' }}>Pages</h4>
          ...
          <div>
            <h4 style={{ fontSize: '0.7rem', fontWeight: 700, color: A, textTransform: 'uppercase' as const, letterSpacing: '0.1em', marginBottom: '1.25rem' }}>Contact</h4>
            {[
      ...
      <div style={{ padding: '0.875rem 1rem 1rem', display: 'flex', flexDirection: 'column', flex: 1 }}>
        ...
        <div style={{ marginTop: 'auto' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.375rem', marginBottom: '0.75rem' }}>
            ...
            <span style={{ fontSize: '0.72rem', color: '#94A3B8' }}>{store.currency || 'DA'}</span>
    ...
    <div dir="rtl">
      ...
      <section style={{ position: 'relative', overflow: 'hidden', background: SL, minHeight: 'clamp(460px, 60vh, 680px)', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
        ...
        <div style={{ position: 'relative', maxWidth: 1280, margin: '0 auto', width: '100%', padding: 'clamp(5rem,10vw,8rem) 1.5rem clamp(3rem,6vw,5rem)' }}>
          ...
          <div style={{ display: 'flex', gap: '2rem', marginTop: '3rem', flexWrap: 'wrap' }}>
            {[
              { icon: <Settings size={14} />, val: '+500', lbl: 'Produit' },
              { icon: <Truck size={14} />, val: '58', lbl: 'Wilaya' },
              { icon: <HardHat size={14} />, val: '100%', lbl: 'Certifié' },
            ].map((s, i) => (
      ...
      <div style={{ background: CARD, borderBottom: `1px solid ${BD}` }}>
        <div className="trust-grid" style={{ maxWidth: 1280, margin: '0 auto' }}>
          {[
            { icon: <HardHat size={18} />, t: 'Équipements Certifiée', d: 'Qualité ISO standard' },
            { icon: <Ruler size={18} />, t: 'Précision dans la mesure', d: 'Outils interlockés' },
            { icon: <Truck size={18} />, t: 'Livraison Sécurisé', d: '58 wilayas algériennes' },
            { icon: <Shield size={18} />, t: 'Paiement À la livraison', d: 'Sans risques' },
          ].map((item, i) => (
      ...
      {cats.length > 0 && (
        <section style={{ padding: '2.5rem 1.5rem', maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
            ...
            <p style={{ fontSize: '0.75rem', fontWeight: 600, color: '#64748B' }}>Catégories</p>
          </div>
      ...
      <section id="products" style={{ padding: '0.5rem 1.5rem 5rem', maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
          ...
          <h2 style={{ fontSize: 'clamp(1.25rem,3vw,1.75rem)', fontWeight: 700, color: SL }}>Produits</h2>
        </div>
        {products.length === 0 ? (
      ...
      <div className="details-inner" style={{ maxWidth: 1280, margin: '0 auto' }}>
        <div className="gallery-container">
          <div style={{ position: 'relative', aspectRatio: '1/1', borderRadius: 10, overflow: 'hidden', background: '#F8FAFC', border: `1px solid ${BD}` }}>
            ...
            {discount > 0 && (
              <div style={{ position: 'absolute', top: 12, right: 12, background: A, color: SL, padding: '3px 10px', borderRadius: 6, fontSize: 12, fontWeight: 800 }}>{discount}% Réduction</div>
            )}
            {allImages.length > 1 && (
              <>
        ...
        <div>
          <div className="info-container">
            ...
            <div style={{ background: AB, border: `1px solid rgba(245,158,11,0.25)`, borderRadius: 8, padding: '0.875rem 1.125rem', marginBottom: '1.5rem' }}>
              <p style={{ fontSize: '0.65rem', color: AD, fontWeight: 700, marginBottom: '0.2rem', textTransform: 'uppercase' as const, letterSpacing: '0.08em' }}>Prix</p>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.4rem' }}>
                <span className="price-mono" style={{ fontSize: '2.5rem', fontWeight: 800, color: AD }}>{finalPrice.toLocaleString()}</span>
                <span style={{ fontSize: '1rem', fontWeight: 700, color: '#94A3B8' }}>DA</span>
              </div>
            </div>
                  ...
                  <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 1rem', border: `1px solid ${selectedOffer === o.id ? A : BD}`, borderRadius: 8, cursor: 'pointer', marginBottom: '0.5rem', background: selectedOffer === o.id ? AB : CARD, transition: 'all 0.18s' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                      ...
                      <div>
                        ...
                        <p style={{ fontSize: '0.72rem', color: '#94A3B8' }}>Quantité: {o.quantity}</p>
                      </div>
                    </div>
                    <span className="price-mono" style={{ fontWeight: 800, color: AD, fontSize: '1.05rem' }}>{o.price.toLocaleString()} DA</span>
                  </label>
                ))}
              </div>
            )}
        ...
        <div className="anim-slide-fade">
          {product.store.cart && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <p style={{ fontWeight: 600, fontSize: '0.72rem', color: '#64748B', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Informations de livraison</p>
              <button onClick={() => setIsOrderNow(false)} style={{ fontSize: '0.8rem', color: '#94A3B8', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>Annuler</button>
            </div>
          )}
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: '0.875rem' }}>
              <p style={{ fontSize: '0.72rem', fontWeight: 600, color: '#64748B', marginBottom: '0.5rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Livraison</p>
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.75rem', border: `1px solid ${fd.typeLivraison === t ? A : BD}`, borderRadius: 8, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? AB : CARD, transition: 'all 0.18s', fontFamily: 'inherit' }}>
                    <p style={{ fontWeight: 600, fontSize: '0.82rem', marginBottom: 4, color: fd.typeLivraison === t ? AD : '#64748B' }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
                    {selW && <p className="price-mono" style={{ fontSize: '1rem', fontWeight: 800, color: fd.typeLivraison === t ? AD : '#94A3B8' }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} DA</p>}
                  </button>
                ))}
              </div>
            </div>
            <div style={{ marginBottom: '0.875rem' }}>
              <p style={{ fontSize: '0.72rem', fontWeight: 600, color: '#64748B', marginBottom: '0.5rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Quantité</p>
            ...
            <div style={{ background: AB, border: `1px solid rgba(245,158,11,0.25)`, borderRadius: 8, padding: '0.875rem 1rem', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '0.25rem' }}>
                <span style={{ fontWeight: 700, fontSize: '0.875rem', color: SL }}>Total</span>
                <span className="price-mono" style={{ fontSize: '1.75rem', fontWeight: 800, color: AD }}>{total().toLocaleString()} <span style={{ fontSize: '0.8rem', fontWeight: 700 }}>DA</span></span>
              </div>
            </div>
  ...
  if (!items.length) return (
    <div dir="rtl" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: BG }}>
      <div style={{ textAlign: 'center', padding: '4rem 2rem', border: `1px dashed ${BD}`, borderRadius: 12, maxWidth: 440, width: '100%', background: CARD }}>
        ...
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: A, color: SL, padding: '0.75rem 1.75rem', borderRadius: 8, fontWeight: 700, fontSize: '0.875rem' }}>Acheter maintenant</Link>
      </div>
    </div>
  );
      ...
      <div className="cart-inner">
        <div style={{ background: CARD, borderRadius: 10, border: `1px solid ${BD}`, overflow: 'hidden', alignSelf: 'start' }}>
          ...
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '0.875rem', padding: '0.875rem', borderBottom: `1px solid ${BD}` }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p className="price-mono" style={{ fontSize: '1.05rem', fontWeight: 800, color: AD }}>{item.finalPrice?.toLocaleString()} DA</p>
                <p style={{ fontSize: '0.72rem', color: '#94A3B8', marginTop: '0.2rem' }}>Quantité: {item.quantity}</p>
              </div>
          ...
          <div style={{ padding: '0.875rem', background: AB, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontWeight: 600, color: '#64748B', fontSize: '0.875rem' }}>Sous-total</span>
            <span className="price-mono" style={{ fontSize: '1.25rem', fontWeight: 800, color: AD }}>{cartTotal.toLocaleString()} DA</span>
          </div>
        </div>
        <div style={{ background: CARD, borderRadius: 10, border: `1px solid ${BD}`, overflow: 'hidden', padding: '1.5rem', alignSelf: 'start' }}>
          ...
          <h3 style={{ fontWeight: 700, fontSize: '0.875rem', color: SL, marginBottom: '1.25rem' }}>Informations de livraison</h3>
          <form onSubmit={handleSubmit}>
            <div className="form-row-2">
            ...
            <div style={{ margin: '1rem 0' }}>
              <p style={{ fontSize: '0.72rem', fontWeight: 600, color: '#64748B', textTransform: 'uppercase' as const, marginBottom: '0.5rem', letterSpacing: '0.06em' }}>Type de livraison</p>
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.75rem', border: `1px solid ${fd.typeLivraison === t ? A : BD}`, borderRadius: 8, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? AB : CARD, fontFamily: 'inherit', transition: 'all 0.2s' }}>
                    ...
                    <p style={{ fontWeight: 600, fontSize: '0.78rem', color: fd.typeLivraison === t ? AD : '#64748B' }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
                    {selW && <p style={{ fontWeight: 800, fontSize: '0.9rem', color: fd.typeLivraison === t ? AD : '#94A3B8', marginTop: 2 }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} DA</p>}
                  </button>
                ))}
              </div>
            </div>
            <div style={{ background: AB, border: `1px solid rgba(245,158,11,0.25)`, borderRadius: 8, padding: '0.875rem 1rem', margin: '1rem 0' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                <span style={{ fontWeight: 700, color: SL }}>Total</span>
                <span className="price-mono" style={{ fontSize: '2rem', fontWeight: 800, color: AD }}>{finalTotal.toLocaleString()} <span style={{ fontSize: '0.8rem' }}>DA</span></span>
              </div>
            </div>
    ...
    <div dir="rtl" style={{ background: BG, minHeight: '100vh' }}>
      <div style={{ background: SL, paddingTop: 96, paddingBottom: 48, paddingLeft: 24, paddingRight: 24 }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: '0.9375rem' }}>Notre équipe technique est disponible pour répondre à vos questions</p>
        </div>
      </div>
        ...
        <div>
          <div style={{ background: CARD, borderRadius: 10, border: `1px solid ${BD}`, padding: '1.5rem', marginBottom: '1rem' }}>
            {[
              ...
              { icon: <MapPin size={15} />, label: 'Adresse', val: [store?.contact?.wilaya, store?.contact?.address].filter(Boolean).join(' / ') || 'Algérie' },
            ].map((r, i) => (
          ...
          <div style={{ background: AB, borderRadius: 8, border: `1px solid rgba(245,158,11,0.25)`, padding: '0.875rem 1rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <span style={{ fontSize: '0.8rem', fontWeight: 600, color: AD }}>Réponse sous une heure</span>
          </div>
        </div>
        <div style={{ background: CARD, borderRadius: 10, border: `1px solid ${BD}`, padding: '2rem', overflow: 'hidden', position: 'relative' as const }}>
          ...
          {sent ? (
            <div style={{ textAlign: 'center', padding: '4rem 1rem' }}>
              <CheckCircle2 size={40} style={{ color: '#22C55E', display: 'block', margin: '0 auto 1.25rem' }} />
              <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: SL, marginBottom: '0.5rem' }}>Envoyé !</h2>
              <p style={{ color: '#64748B', lineHeight: 1.7, marginBottom: '2rem' }}>Nous vous répondrons dans les plus brefs délais.</p>
              <button onClick={() => setSent(false)} style={{ padding: '0.75rem 1.75rem', borderRadius: 8, border: `1px solid ${A}`, background: AB, color: AD, fontWeight: 700, cursor: 'pointer', fontSize: '0.875rem', fontFamily: 'inherit' }}>Envoyer un autre message</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
              <div className="form-row-2" style={{ marginBottom: '0.875rem' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.72rem', fontWeight: 600, color: '#64748B', marginBottom: '0.3rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Nom</label>
                  <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required style={S.input} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '0.72rem', fontWeight: 600, color: '#64748B', marginBottom: '0.3rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Téléphone</label>
                  <input type="tel" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} required style={S.input} />
                </div>
              </div>
              <div style={{ marginBottom: '0.875rem' }}>
                <label style={{ display: 'block', fontSize: '0.72rem', fontWeight: 600, color: '#64748B', marginBottom: '0.3rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Email</label>
                <input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} required style={S.input} />
              </div>
              <div style={{ marginBottom: '1.5rem' }}>
                <label style={{ display: 'block', fontSize: '0.72rem', fontWeight: 600, color: '#64748B', marginBottom: '0.3rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Message</label>
