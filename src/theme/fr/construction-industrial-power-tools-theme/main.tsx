    ...
    <>
      ...
      <div className="top-bar" dir="ltr">
        ...
        <div style={{ display: 'flex', gap: '1rem' }}>
          {[{ h: '/Privacy', l: 'Confidentialité' }, { h: '/Terms', l: 'Conditions' }].map(lnk => (
      ...
      {/* Main nav */}
      <nav dir="ltr" style={{ position: 'sticky', top: 0, zIndex: 50, background: B, borderBottom: `4px solid ${R}`, boxShadow: scrolled ? '0 2px 12px rgba(27,87,126,0.3)' : 'none', transition: 'box-shadow 0.2s' }}>
          ...
          <div style={{ padding: '0.25rem 1.5rem 0.875rem' }}>
            {[{ h: '/', l: 'Accueil' }, { h: '/contact', l: 'Contactez-nous' }].map(i => (
              <Link key={i.h} href={i.h} onClick={() => setOpen(false)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.7rem 0', borderBottom: `1px solid rgba(255,255,255,0.08)`, fontSize: '0.875rem', fontWeight: 600, color: 'rgba(255,255,255,0.8)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                {i.l} <ArrowRight size={13} style={{ color: R }} />
              </Link>
            ))}
        ...
        <div className="glb-search-ov" onClick={e => { if (e.target === e.currentTarget) setShowSearch(false); }}>
          <div className="glb-search-panel">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
              <span style={{ fontSize: '0.8rem', fontWeight: 700, color: SUB, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Rechercher des produits</span>
            ...
            {loading && <p style={{ textAlign: 'center', color: B, fontSize: '0.82rem', padding: '2rem' }}>Recherche en cours...</p>}
                ...
                {listSearch.map((p: any) => (
                  <Link key={p.id} href={`/product/${p.slug || p.id}`} className="glb-search-card" onClick={() => setShowSearch(false)}>
                    ...
                    <div className="glb-search-card-info">
                      ...
                      <p style={{ fontSize: '0.78rem', fontWeight: 700, color: R }}>{Number(p.price).toLocaleString()} DA</p>
                    </div>
                  </Link>
                ))}
              </div>
            ...
            {!loading && searchQuery.length >= 2 && listSearch.length === 0 && (
              <p style={{ textAlign: 'center', color: '#B0BEC5', fontSize: '0.875rem', padding: '3rem' }}>Aucun résultat</p>
            )}
          </div>
        </div>
      )}
    ...
    <footer dir="ltr" style={{ background: INK, borderTop: `4px solid ${R}`, color: 'rgba(255,255,255,0.4)', marginTop: 80, padding: '3.5rem 1.5rem 1.25rem' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        <div className='footer-inner'>
          ...
          <div>
            <h4 style={{ fontSize: '0.68rem', fontWeight: 700, color: R, textTransform: 'uppercase' as const, letterSpacing: '0.1em', marginBottom: '1rem' }}>Liens</h4>
          ...
          <div>
            <h4 style={{ fontSize: '0.68rem', fontWeight: 700, color: R, textTransform: 'uppercase' as const, letterSpacing: '0.1em', marginBottom: '1rem' }}>Contact</h4>
            {[
  ...
  return (
    ...
    <div style={{ padding: '0.625rem 0.875rem 0.875rem', background: CARD, display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      ...
      <Link href={`/product/${product.slug || product.id}`} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, background: R, color: '#fff', fontWeight: 700, fontSize: '0.78rem', padding: '0.55rem', borderRadius: 2, textTransform: 'uppercase', letterSpacing: '0.06em', transition: 'background 0.18s' }}
        onMouseEnter={e => ((e.currentTarget as HTMLAnchorElement).style.background = RD)}
        onMouseLeave={e => ((e.currentTarget as HTMLAnchorElement).style.background = R)}>
        Commander maintenant <ArrowRight size={12} />
      </Link>
    </div>
  </div>
  );
    ...
    <div dir="ltr">
      ...
      <section style={{ position: 'relative', overflow: 'hidden', background: '#0A1929', minHeight: 'clamp(420px, 56vh, 640px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ position: 'relative', maxWidth: 860, margin: '0 auto', width: '100%', padding: 'clamp(5rem,10vw,8rem) 2rem 4rem', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '1rem', color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, marginBottom: '2.5rem', maxWidth: 480, margin: '0 auto 2.5rem' }}>
            {store.hero?.subtitle || 'Outils électriques et équipements industriels professionnels pour les entrepreneurs et professionnels. Qualité garantie.'}
          </p>
      ...
      <div style={{ border: `1px solid ${BD}`, borderLeft: 'none', borderRight: 'none', background: CARD }}>
        <div className="trust-grid" style={{ maxWidth: 1280, margin: '0 auto' }}>
          {[
            { icon: <Truck size={18} />, t: 'Livraison rapide', s: 'pour 58 wilayas' },
            { icon: <Shield size={18} />, t: 'Paiement Sécurisé', s: 'À la livraison' },
            { icon: <Settings size={18} />, t: 'Équipements Certifiée', s: 'Industriellement' },
            { icon: <Zap size={18} />, t: 'Haute performance', s: 'Garanti' },
          ].map((item, i) => (
      ...
      {cats.length > 0 && (
        <section style={{ padding: '2.5rem 1.5rem', maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.25rem' }}>
            ...
            <h2 style={{ fontSize: '0.875rem', fontWeight: 700, color: INK, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Catégories</h2>
          </div>
      ...
      <section id="products" style={{ padding: cats.length ? '0 1.5rem 5rem' : '2rem 1.5rem 5rem', maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem', paddingBottom: '1rem', borderBottom: `1px solid ${BD}` }}>
          <div style={{ width: 4, height: 18, background: R }} />
          <h2 style={{ fontSize: '0.875rem', fontWeight: 700, color: INK, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Tous les Produits</h2>
          {store.count > 0 && <span style={{ marginRight: 'auto', fontSize: '0.72rem', color: SUB }}>{store.count} Produit</span>}
        </div>
        {products.length === 0 ? (
          <div style={{ padding: '5rem', textAlign: 'center', border: `1px dashed ${BD}` }}>
            ...
            <p style={{ color: '#90A4AE', fontSize: '0.875rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Aucun produit</p>
          </div>
        ) : (
  ...
  return (
    <div dir="ltr" style={{ background: BG, paddingBottom: '4rem' }}>
      <div style={{ background: B, borderBottom: `3px solid ${R}`, padding: '0.875rem 1.5rem', marginBottom: '0' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          <Link href="/" style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Accueil</Link>
      ...
      <div className="details-inner" style={{ maxWidth: 1280, margin: '0 auto' }}>
        <div className="gallery-container">
          <div style={{ position: 'relative', aspectRatio: '1/1', overflow: 'hidden', background: '#ECEFF1', border: `1px solid ${BD}` }}>
            ...
            {discount > 0 && <div style={{ position: 'absolute', top: 0, right: 0, background: R, color: '#fff', padding: '4px 10px', fontSize: 11, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{discount}% Réduction</div>}
            {allImages.length > 1 && (
              <>
              ...
              <div style={{ marginBottom: '1.25rem' }}>
                {product.offers.map((o: any) => (
                  <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem', border: `1px solid ${selectedOffer === o.id ? B : BD}`, cursor: 'pointer', marginBottom: '0.375rem', background: selectedOffer === o.id ? BL : CARD, borderLeft: selectedOffer === o.id ? `3px solid ${B}` : `3px solid transparent`, transition: 'all 0.15s' }}>
                    ...
                    <span className="price-mono" style={{ fontWeight: 800, color: R }}>{o.price.toLocaleString()} DA</span>
                  </label>
                ))}
              </div>
            )}
        ...
        <div className="anim-fade-up">
          {product.store.cart && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <p style={{ fontWeight: 700, fontSize: '0.65rem', color: SUB, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Informations de livraison</p>
              <button onClick={() => setIsOrderNow(false)} style={{ fontSize: '0.78rem', color: '#90A4AE', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>Annuler</button>
            </div>
          )}
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: '0.875rem' }}>
              <p style={{ fontSize: '0.65rem', fontWeight: 700, color: SUB, marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Type de livraison</p>
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.7rem', border: `1px solid ${fd.typeLivraison === t ? B : BD}`, borderTop: `2px solid ${fd.typeLivraison === t ? R : BD}`, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? BL : CARD, transition: 'all 0.15s', fontFamily: 'inherit' }}>
                    <p style={{ fontWeight: 700, fontSize: '0.75rem', marginBottom: 3, color: fd.typeLivraison === t ? INK : SUB, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
                    {selW && <p className="price-mono" style={{ fontSize: '1rem', fontWeight: 800, color: fd.typeLivraison === t ? R : '#90A4AE' }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} DA</p>}
                  </button>
                ))}
              </div>
            </div>
            <div style={{ marginBottom: '0.875rem' }}>
              <p style={{ fontSize: '0.65rem', fontWeight: 700, color: SUB, marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Quantité</p>
  ...
  if (!items.length) return (
    <div dir="ltr" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: BG }}>
      <div style={{ textAlign: 'center', padding: '4rem 2rem', border: `1px solid ${BD}`, maxWidth: 400, width: '100%', background: CARD }}>
        ...
        <p style={{ color: '#90A4AE', fontSize: '0.875rem', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '2rem' }}>Panier vide</p>
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: R, color: '#fff', padding: '0.7rem 1.75rem', fontWeight: 700, fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Acheter maintenant</Link>
      </div>
    </div>
  );
      ...
      <div className="cart-inner">
        <div style={{ background: CARD, border: `1px solid ${BD}`, borderTop: `3px solid ${R}`, alignSelf: 'start' }}>
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '0.875rem', padding: '0.875rem', borderBottom: `1px solid ${BD}` }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p className="price-mono" style={{ fontSize: '1.125rem', fontWeight: 800, color: R }}>{item.finalPrice?.toLocaleString()} DA</p>
                <p style={{ fontSize: '0.68rem', color: SUB, marginTop: '0.1rem', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Quantité: {item.quantity}</p>
              </div>
          ...
          <div style={{ padding: '0.875rem', background: BL, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontWeight: 700, color: SUB, fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Sous-total</span>
            <span className="price-mono" style={{ fontSize: '1.25rem', fontWeight: 800, color: R }}>{cartTotal.toLocaleString()} DA</span>
          </div>
        </div>
        <div style={{ background: CARD, border: `1px solid ${BD}`, borderTop: `3px solid ${B}`, padding: '1.5rem', alignSelf: 'start' }}>
          <h3 style={{ fontWeight: 700, fontSize: '0.8rem', color: INK, marginBottom: '1.25rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Informations de livraison</h3>
          <form onSubmit={handleSubmit}>
            <div className="form-row-2">
            ...
            <div style={{ margin: '1rem 0' }}>
              <p style={{ fontSize: '0.65rem', fontWeight: 700, color: SUB, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.5rem' }}>Type de livraison</p>
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.7rem', border: `1px solid ${fd.typeLivraison === t ? B : BD}`, borderTop: `2px solid ${fd.typeLivraison === t ? R : BD}`, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? BL : CARD, fontFamily: 'inherit', transition: 'all 0.15s' }}>
                    <p style={{ fontWeight: 700, fontSize: '0.72rem', textTransform: 'uppercase', color: fd.typeLivraison === t ? INK : SUB, marginBottom: 3 }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
                    {selW && <p style={{ fontWeight: 800, fontSize: '0.95rem', color: fd.typeLivraison === t ? R : '#90A4AE' }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} DA</p>}
                  </button>
                ))}
              </div>
            </div>
