...
import {
  ...
  CheckCircle2, ToggleRight, Shield, ArrowRight,
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'var(--ink)', display: 'flex', flexDirection: 'column', animation: 'menu-in 0.45s cubic-bezier(0.22,1,0.36,1) forwards' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 32px' }}>
        {[{ h: '/', l: 'Accueil' }, { h: '/contact', l: 'Contactez-nous' }, { h: '/Privacy', l: 'Confidentialité' }, { h: '/Terms', l: 'Conditions' }].map(lnk => (
      ...
      <div style={{ padding: '16px 32px', borderTop: '1px solid rgba(242,239,232,0.1)', display: 'flex', justifyContent: 'space-between' }}>
        <span className="sm" style={{ fontSize: '8px', letterSpacing: '0.2em', color: 'rgba(242,239,232,0.25)' }}>ZINE DROP · {new Date().getFullYear()} · Algérie</span>
...
export function Navbar({ store, domain, onMenuOpen }: { store: any; domain: string; onMenuOpen?: () => void }) {
  ...
  const Drop = () => (
    <div style={{
      ...
      border: '2px solid var(--ink)', // bordure épaisse et visible
      zIndex: 200,
      borderTop: '4px solid var(--punch)', // accent supérieur en couleur Punch
      boxShadow: '6px 6px 0 var(--ink)', // ombre nette et solide
      overflow: 'hidden'
    }} className="anim-slide-down">

      {/* Bouton fermer en haut - aligné avec le design */}
      <div style={{
        display: 'flex',
      ...
      {loading ? (
        ...
        <div style={{ maxHeight: '350px', overflowY: 'auto' }}>
          ...
          {/* Bouton Voir plus - style Neo-Brutalism */}
          <button
            onClick={() => doSearch()}
            style={{
              width: '100%',
      ...
      <div className="footer-cols" style={{ position: 'relative', zIndex: 2, padding: '40px 24px', borderBottom: '1px solid rgba(242,239,232,0.1)' }}>
        ...
        <div>
          <p style={{ fontSize: '13px', lineHeight: '1.8', letterSpacing: '0.05em', color: 'rgba(242,239,232,0.45)', maxWidth: '300px', marginBottom: '20px' }}>
            {store?.hero?.subtitle?.substring(0, 90) || 'Boutique de style authentique. Chaque pièce raconte une histoire. Livraison rapide dans toutes les wilayas.'}
          </p>
          <p style={{ fontSize: '10px', letterSpacing: '0.08em', color: 'rgba(242,239,232,0.2)' }}>© {yr} {store?.name}. Tous droits réservés.</p>
        </div>
        ...
        {/* Section 3 — Contact */}
        <div>
          <p style={{ fontSize: '8px', letterSpacing: '0.22em', color: 'var(--punch)', marginBottom: '14px', textTransform: 'uppercase' }}>// Contact</p>
          {[
          ...
          <div style={{ marginTop: '14px', padding: '12px 14px', border: '1px solid var(--punch)', background: 'rgba(255,45,0,0.08)' }}>
            <p className="ub" style={{ fontSize: '12px', fontWeight: 900, color: 'var(--punch)', marginBottom: 2 }}>Disponibles maintenant ↗</p>
  ...
  return (
    <Link href={`/product/${product.slug || product.id}`} style={{ display: 'flex', flexDirection: 'column', textDecoration: 'none', background: 'var(--paper)', height: '100%' }}
      ...
      <div style={{ position: 'relative', width: '100%', aspectRatio: '3/4', overflow: 'hidden', background: 'var(--paper-dk)', flexShrink: 0 }}>
        ...
        {discount > 0 && (
          <div className="sm" style={{ position: 'absolute', top: 10, left: 10, background: 'var(--punch)', color: 'var(--paper)', fontWeight: 700, fontSize: '10px', padding: '3px 8px', letterSpacing: '0.06em' }}>
            -{discount}%
          </div>
        )}
        {/* Quick view on hover */}
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'var(--ink)', color: 'var(--paper)', textAlign: 'center', padding: '10px', fontSize: '10px', letterSpacing: '0.2em', fontWeight: 700, fontFamily: 'inherit', transform: hov ? 'translateY(0)' : 'translateY(100%)', transition: 'transform 0.28s cubic-bezier(0.22,1,0.36,1)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
          <span className="sm">{viewDetails || 'Voir le produit'}</span>
      ...
      <div style={{ padding: '12px 12px 14px', borderTop: '1px solid rgba(10,9,6,0.07)', flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 'auto' }}>
          <span className="ub" style={{ fontWeight: 900, fontSize: '1rem', color: hov ? 'var(--punch)' : 'var(--ink)', letterSpacing: '-0.02em', transition: 'color 0.2s' }}>
            ...
            <span style={{ fontWeight: 400, fontSize: '0.6em', marginLeft: 3, opacity: 0.6 }}>{store?.currency || 'DA'}</span>
          </span>
          {orig > price && (
    ...
    <div dir="ltr" style={{ backgroundColor: 'var(--paper)' }}>
      ...
      <section className="noise-ov" style={{ position: 'relative', minHeight: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: 'var(--ink)', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '8vw 6vw 0 7vw', position: 'relative', zIndex: 4 }}>
          ...
          <div className="rise" style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '28px' }}>
            ...
            <span className="sm" style={{ fontSize: '8px', letterSpacing: '0.28em', color: 'var(--punch)' }}>Première édition · Vêtements de rue · {new Date().getFullYear()}</span>
          </div>
          ...
          <h1 className="ub rise rise-1" style={{ fontWeight: 900, fontSize: 'clamp(4rem,15vw,14rem)', letterSpacing: '-0.04em', lineHeight: 0.86, color: 'var(--paper)', textTransform: 'uppercase', marginBottom: 0 }}>
            {store.hero?.title
              ...
              : <><span>Portez</span><br /><span style={{ color: 'var(--punch)' }}>Votre identité</span></>
            }
          </h1>
            ...
            <div className='rise rise-3' style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              ...
              {cats.length > 0 && (
                <a href="#cats" className="sm" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', fontSize: '9px', letterSpacing: '0.18em', color: 'rgba(242,239,232,0.35)', border: '1px solid rgba(242,239,232,0.14)', padding: '14px 22px', transition: 'all 0.22s' }}
                  ...
                  Sections <ArrowRight style={{ width: '12px', height: '12px' }} />
                </a>
              )}
            </div>
          </div>
        ...
        <div className="stats-grid" style={{ position: 'relative', zIndex: 4, borderTop: '1px solid rgba(242,239,232,0.1)', marginTop: '40px' }}>
          {[{ n: '100%', l: 'authentique' }, { n: `${products.length || '∞'}`, l: 'article' }, { n: '48H', l: 'Livraison' }, { n: 'DZ', l: 'local' }].map((s, i) => (
      ...
      {cats.length > 0 && (
        <div id="cats" style={{ borderBottom: '1px solid var(--ink)', display: 'flex', alignItems: 'stretch', overflowX: 'auto' }}>
          <div style={{ flexShrink: 0, padding: '10px 18px', borderRight: '1px solid var(--ink)', display: 'flex', alignItems: 'center' }}>
            <span className="sm" style={{ fontSize: '8px', letterSpacing: '0.2em', color: 'var(--ash)', whiteSpace: 'nowrap' }}>Parcourir par le style</span>
          </div>
      ...
      <section id="drops">
        <div className="sec-bar">
          <span className="ub" style={{ fontWeight: 900, fontSize: 'clamp(0.7rem,1.4vw,0.95rem)', color: 'var(--paper)', letterSpacing: '0.04em', textTransform: 'uppercase' }}>La collection complète</span>
          <span className="sm" style={{ fontSize: '8px', letterSpacing: '0.2em', color: 'rgba(242,239,232,0.4)' }}>{products.length} article</span>
        </div>
        ...
        {products.length === 0 ? (
          <div style={{ minHeight: '420px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--ink)', margin: '1px' }}>
            ...
            <p className="sm" style={{ fontSize: '9px', letterSpacing: '0.2em', color: 'var(--mist)', marginTop: '16px' }}>Bient't...</p>
          </div>
        ) : (
          ...
          <div style={{ position: 'relative', width: '100%', height: '100%' }}>
            ...
            <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'linear-gradient(to top,rgba(10,9,6,0.88) 0%,transparent 50%)', padding: '24px 20px 20px', pointerEvents: 'none' }}>
              ...
              <div className="ub" style={{ fontWeight: 900, fontSize: 'clamp(2rem,5vw,4rem)', color: 'var(--paper)', letterSpacing: '-0.03em', lineHeight: 1 }}>
                ...
                <span className="sm" style={{ fontWeight: 400, fontSize: '1rem', marginLeft: '8px', opacity: 0.65 }}>DA</span>
              </div>
            </div>
            {allImages.length > 1 && (
            ...
            {!inStock && !autoGen && (
              <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(242,239,232,0.88)', backdropFilter: 'blur(4px)' }}>
                <span className="ub" style={{ fontWeight: 900, fontSize: '3rem', color: 'var(--punch)', transform: 'rotate(-5deg)', display: 'inline-block' }}>SOLD OUT</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="details-info">
          ...
          <div style={{ marginBottom: '22px', paddingBottom: '22px', borderBottom: '1px solid var(--paper-dk)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', flexWrap: 'wrap', gap: '10px' }}>
              ...
              <span className="sm" style={{ fontSize: '1rem', color: 'var(--ash)' }}>DA</span>
                ...
                <label key={o.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 14px', border: `1px solid ${selectedOffer === o.id ? 'var(--ink)' : 'var(--paper-dk)'}`, backgroundColor: selectedOffer === o.id ? 'var(--ink)' : 'transparent', cursor: 'pointer', marginBottom: '5px', transition: 'all 0.18s' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div>
                      ...
                      <p className="sm" style={{ fontSize: '8px', color: selectedOffer === o.id ? 'rgba(242,239,232,0.5)' : 'var(--ash)', letterSpacing: '0.12em' }}>Quantité: {o.quantity}</p>
                    </div>
                  </div>
                  <span className="ub" style={{ fontWeight: 900, fontSize: '1.1rem', color: selectedOffer === o.id ? 'var(--punch)' : 'var(--ink)', letterSpacing: '-0.02em' }}>{o.price.toLocaleString()}<span className="sm" style={{ fontWeight: 400, fontSize: '9px', marginLeft: '3px' }}>DA</span></span>
                </label>
              ))}
            </div>
          )}
      ...
      {product.store?.cart && (
        <div className="cart-add-btns" style={{ marginBottom: '14px' }}>
          <button onClick={addToCart} disabled={isAdded} className="sm" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '12px', cursor: isAdded ? 'default' : 'pointer', fontFamily: 'inherit', fontWeight: 700, fontSize: '9px', letterSpacing: '0.18em', textTransform: 'uppercase', border: `1px solid ${isAdded ? '#22c55e' : 'var(--ink)'}`, background: isAdded ? 'rgba(34,197,94,0.08)' : 'transparent', color: isAdded ? '#22c55e' : 'var(--ink)', transition: 'all 0.25s' }}>
            {isAdded ? <><CheckCircle2 size={12} className="anim-check" />Ajouté au panier</> : <><ShoppingCart size={12} />Ajouter au panier</>}
          </button>
          ...
          {product.store?.cart && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              ...
              <button onClick={() => setIsOrderNow(false)} className="sm" style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 10px', border: '1px solid var(--ink)', background: 'transparent', color: 'var(--ash)', fontSize: '8px', letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'inherit' }}>
                <X style={{ width: '9px', height: '9px' }} /> Annuler
              </button>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit}>
            <div className="form-2c">
              <FR error={errors.customerName} label="NAME">
                <input type="text" value={fd.customerName} onChange={e => setFd({ ...fd, customerName: e.target.value })} placeholder="Nom complet" style={IS(!!errors.customerName)} onFocus={onF} onBlur={e => onB(e, !!errors.customerName)} />
              </FR>
              ...
              <div className="dlv-2c">
                {(['home', 'office'] as const).map((type, i) => (
                  <button key={type} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: type }))}
                    ...
                    <p className="sm" style={{ fontSize: '8px', letterSpacing: '0.16em', color: fd.typeLivraison === type ? 'var(--paper)' : 'var(--ash)', marginBottom: '3px' }}>{type === 'home' ? 'Pour la maison' : 'Au bureau'}</p>
                    {selW && <p className="ub" style={{ fontSize: '1rem', fontWeight: 900, color: fd.typeLivraison === type ? 'var(--punch)' : 'var(--mist)', letterSpacing: '-0.02em', lineHeight: 1 }}>
                      {(type === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()}<span className="sm" style={{ fontWeight: 400, fontSize: '8px', marginLeft: '3px' }}>DA</span>
                    </p>}
                  </button>
                ))}
              </div>
            ...
            <div style={{ border: '1px solid var(--ink)', marginBottom: '12px' }}>
              ...
              {[{ l: 'ITEM', v: product.name.slice(0, 20) + (product.name.length > 20 ? '...' : '') }, { l: 'UNIT', v: `${fp.toLocaleString()} DA` }, { l: '× QTY', v: fd.quantity }, { l: 'SHIP', v: selW ? `${getLiv().toLocaleString()} DA` : 'TBD' }].map(row => (
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '10px 13px', borderTop: '2px solid var(--ink)', background: 'var(--paper-dk)' }}>
                ...
                <span className="ub" style={{ fontWeight: 900, fontSize: '1.7rem', color: 'var(--punch)', letterSpacing: '-0.03em', lineHeight: 1 }}>
                  {total().toLocaleString()}<span className="sm" style={{ fontWeight: 400, fontSize: '10px', marginLeft: '4px' }}>DA</span>
                </span>
              </div>
            </div>
            ...
            <p className="sm" style={{ fontSize: '8px', letterSpacing: '0.16em', color: 'var(--mist)', textAlign: 'center', marginTop: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px' }}>
              <Shield style={{ width: '9px', height: '9px', color: 'var(--ink)' }} /> Sécurisé · Chiffré
            </p>
          </form>
        </div>
      )}
  ...
  if (success) return (
    <div dir="ltr" style={{ minHeight: '70vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', background: 'var(--paper)', fontFamily: "'Space Mono',monospace" }}>
      <div style={{ textAlign: 'center', padding: '4rem 2.5rem', border: '2px solid var(--ink)', borderTop: '4px solid var(--punch)', maxWidth: 460, width: '100%' }}>
        ...
        <p style={{ fontSize: '10px', letterSpacing: '0.1em', color: 'var(--ash)', marginBottom: '2rem', lineHeight: 1.8 }}>Merci. Nous vous contacterons bient't.</p>
        <Link href="/" className="btn-z" style={{ display: 'inline-flex', clipPath: 'none', padding: '13px 28px' }}>Retour à la boutique</Link>
      </div>
    </div>
  );
  ...
  if (!items.length) return (
    <div dir="ltr" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', background: 'var(--paper)', fontFamily: "'Space Mono',monospace" }}>
      <div style={{ textAlign: 'center', padding: '4rem 2rem', border: '1px solid var(--ink)', maxWidth: 400, width: '100%' }}>
        ...
        <Link href="/" className="btn-z" style={{ display: 'inline-flex', clipPath: 'none', padding: '13px 28px' }}>Acheter maintenant</Link>
      </div>
    </div>
  );
        ...
        <div className='cart-layout'>
          ...
          <div style={{ border: '1px solid var(--ink)', borderTop: '3px solid var(--punch)', alignSelf: 'start' }}>
              ...
              <div key={i} style={{ display: 'flex', gap: '1rem', padding: '14px', borderBottom: '1px solid var(--paper-dk)' }}>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  <div>
                    ...
                    <p className="ub" style={{ fontSize: '1.1rem', fontWeight: 900, color: 'var(--punch)', letterSpacing: '-0.02em' }}>{item.finalPrice?.toLocaleString()} DA</p>
                  </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '14px', background: 'var(--paper-dk)' }}>
              ...
              <span className="ub" style={{ fontSize: '1.375rem', fontWeight: 900, letterSpacing: '-0.02em' }}>{cartTotal.toLocaleString()} DA</span>
            </div>
          </div>
          ...
          <div style={{ border: '1px solid var(--ink)', borderTop: '3px solid var(--punch)', padding: '22px', alignSelf: 'start' }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {/* Summary */}
              <div style={{ border: '1px solid var(--ink)', marginBottom: '12px' }}>
                {[{ l: 'SUBTOTAL', v: `${cartTotal.toLocaleString()} DA` }, { l: 'SHIP', v: getLiv() ? `${getLiv().toLocaleString()} DA` : 'TBD' }].map(row => (
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '10px 13px', borderTop: '2px solid var(--ink)', background: 'var(--paper-dk)' }}>
                  <span className="sm" style={{ fontSize: '8px', letterSpacing: '0.2em', color: 'var(--ash)' }}>TOTAL</span>
                  <span className="ub" style={{ fontWeight: 900, fontSize: '1.7rem', color: 'var(--punch)', letterSpacing: '-0.03em', lineHeight: 1 }}>
                    {finalTotal.toLocaleString()}<span className="sm" style={{ fontWeight: 400, fontSize: '10px', marginLeft: '4px' }}>DA</span>
                  </span>
                </div>
              </div>
...
export function Privacy() {
  return (
    <PageShell title="Confidentialité" code="DOC-001">
      <IB title="Les données que nous collectons" body="Nom et numéro de Téléphone et adresse de Livraison uniquement — le minimum nécessaire pour traiter votre commande." />
      <IB title="Comment nous les utilisons" body="Exclusivement pour traiter et livrer votre commande. Aucun marketing, aucune vente de données." />
      <IB title="Sécurité" body="Chiffrement haute sécurité. Vos données sont protégées en tout temps." />
      <IB title="Partage des données" body="Nous ne vendons jamais vos données. Partagées avec les partenaires de livraison." tag="Garanti" />
    </PageShell>
  );
}
...
export function Terms() {
  return (
    <PageShell title="Conditions" code="DOC-002">
      <IB title="Commandes" body="Aucuns frais cachés. Le prix affiché est le prix final." />
      <IB title="Produits authentiques" body="Produits authentiques uniquement. Les contrefaçons sont interdites." tag="Strict" />
...
export function Cookies() {
  return (
    <PageShell title="Cookies" code="DOC-003">
      <IB title="Essentiels" body="Requis pour les sessions, le panier et finaliser la commande. Non désactivables." tag="toujours activé" tagColor="var(--ink)" />
      <IB title="Préférences" body="Sauvegarde vos préférences pour une meilleure expérience." tag="Facultatif" />
      <IB title="Analytiques" body="Données agrégées pour améliorer la plateforme. Aucune donnée personnelle." tag="Facultatif" />
      <div style={{ marginTop: '20px', padding: '14px', border: '1px solid var(--ink)', display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
        ...
        <div>
          ...
          <p className="sm" style={{ fontSize: '10px', lineHeight: '1.8', color: 'var(--ash)' }}>Vous pouvez gérer ou supprimer les cookies depuis les paramètres de votre navigateur.</p>
        </div>
      </div>
    </PageShell>
  );
  ...
  return (
    <div dir="ltr" style={{ backgroundColor: 'var(--paper)', fontFamily: "'Space Mono',monospace", minHeight: '100vh' }}>
      <div style={{ backgroundColor: 'var(--punch)', padding: '80px 6vw 40px', position: 'relative', overflow: 'hidden' }}>
        ...
        <h1 className="ub" style={{ fontWeight: 900, fontSize: 'clamp(3rem,9vw,9rem)', color: 'var(--paper)', letterSpacing: '-0.04em', lineHeight: 0.88, position: 'relative', zIndex: 2 }}>Contactez-nous.</h1>
        <p className="sm" style={{ fontSize: '9px', letterSpacing: '0.2em', color: 'rgba(242,239,232,0.55)', marginTop: '14px', position: 'relative', zIndex: 2 }}>Réponse sous 24h</p>
      </div>

      <div className="contact-grid" style={{ maxWidth: '860px', margin: '0 auto', padding: '44px 24px 80px' }}>
        {/* Info */}
        <div>
          <p className="sm" style={{ fontSize: '8px', letterSpacing: '0.22em', color: 'var(--ash)', marginBottom: '14px' }}>/ Informations de contact</p>
          {[
            { emoji: '📞', label: 'Téléphone', val: store?.contact?.phone },
            { emoji: '📍', label: 'Adresse', val: [store?.contact?.wilaya, store?.contact?.address].filter(Boolean).join(' / ') },
            { emoji: '✉️', label: 'Email', val: store?.contact?.email },
          ...
          <div style={{ marginTop: '20px', padding: '18px', background: 'var(--ink)', position: 'relative', overflow: 'hidden' }}>
            ...
            <p className="sm" style={{ fontSize: '8px', letterSpacing: '0.18em', color: 'rgba(242,239,232,0.35)', marginTop: '8px', position: 'relative', zIndex: 2 }}>DROP CULTURE · Algérie</p>
          </div>
        </div>

        {/* Form */}
        <div>
          <p className="sm" style={{ fontSize: '8px', letterSpacing: '0.22em', color: 'var(--ash)', marginBottom: '14px' }}>/ Envoyer un message</p>
          {sent ? (
            <div style={{ minHeight: '260px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--ink)', textAlign: 'center' }}>
              ...
              <p className="sm" style={{ fontSize: '8px', letterSpacing: '0.2em', color: 'var(--ash)' }}>Réponse sous 24h</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <div className="form-2c">
                <div>
                  <p className="sm" style={{ fontSize: '8px', letterSpacing: '0.22em', color: 'var(--ash)', marginBottom: '5px' }}>Nom</p>
                  <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required style={{ ...IS() }} onFocus={onF} onBlur={e => onB(e)} />
                </div>
                <div>
                  <p className="sm" style={{ fontSize: '8px', letterSpacing: '0.22em', color: 'var(--ash)', marginBottom: '5px' }}>Téléphone</p>
              ...
              <div>
                <p className="sm" style={{ fontSize: '8px', letterSpacing: '0.22em', color: 'var(--ash)', marginBottom: '5px' }}>Email</p>
              ...
              <div>
                <p className="sm" style={{ fontSize: '8px', letterSpacing: '0.22em', color: 'var(--ash)', marginBottom: '5px' }}>Votre message</p>
                <textarea value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} placeholder="Comment pouvons-nous vous aider ?" rows={4} required style={{ ...IS(), resize: 'none' }} onFocus={onF} onBlur={e => onB(e)} />
              </div>
              <button type="submit" disabled={loading} className="btn-z" style={{ justifyContent: 'center', width: '100%', clipPath: 'none', opacity: loading ? 0.7 : 1, cursor: loading ? 'not-allowed' : 'pointer' }}>
                {loading ? <><Loader2 style={{ width: '13px', height: '13px', animation: 'spin 1s linear infinite' }} /> En cours...</> : <>Envoyer le message <ArrowUpRight style={{ width: '13px', height: '13px' }} /></>}
              </button>
            </form>
          )}
        </div>
