...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  const DropResults = () => (
    <div style={{ paddingTop: 25, position: 'absolute', top: 'calc(100% + 6px)', right: 0, left: 0, background: '#fff', border: '1.5px solid #E0E0E0', borderRadius: 10, boxShadow: '0 12px 40px rgba(0,0,0,0.12)', zIndex: 60, maxHeight: '300px', overflowY: 'auto' }} className="anim-slide-down">
      ...
      {loading ? <div style={{ padding: '1.5rem', textAlign: 'center', color: '#E63946', fontSize: '0.85rem', fontWeight: 600 }}>Recherche en cours...</div>
        : listSearch.length > 0 ? (
            ...
            {listSearch.map((p: any) => (
              <Link href={`/product/${p.id}`} key={p.id} onClick={() => setSearchQuery('')}
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '0.75rem', color: '#E63946', fontWeight: 700 }}>{p.price} DA</div>
                </div>
              </Link>
            ))}
            <button
            ...
            {store?.design?.logoUrl && store.design.logoUrl !== '/default-logo.png' && !imgError ? (
              ...
              <div style={{ display: 'flex', alignItems: 'center' }}>
                <div style={{
                  ...
                  whiteSpace: 'nowrap' // garantir que le nom ne se coupe pas
                }}>
          ...
          <div style={{ padding: '0.625rem 1.25rem', background: '#fff', borderTop: '1px solid #E8E8E8', position: 'relative' }} className="anim-slide-down">
            <form onSubmit={handleSearch} style={{ position: 'relative' }}>
              <input autoFocus type="text" placeholder="Rechercher un produit..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                style={{ width: '100%', padding: '0.75rem 2.75rem 0.75rem 1rem', border: '1.5px solid #E63946', borderRadius: 8, background: '#F8F8F6', fontSize: '0.9rem', outline: 'none' }} />
              <Search size={15} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', color: '#E63946' }} />
            </form>
            {searchQuery.length >= 2 && <DropResults />}
          </div>
        )}
        ...
        <div className="hero-inner" style={{ maxWidth: 1280, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          ...
          <div>
            ...
            <h1 style={{ fontSize: 'clamp(2.25rem,6vw,4.5rem)', fontWeight: 800, color: '#fff', lineHeight: 1.1, letterSpacing: '-0.03em', marginBottom: '1.25rem' }}
              dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(store.hero?.title || 'Acheter<br/><span style="color:#E63946">En confiance</span> et obtenez<br/>le meilleur') }} />
        ...
        <div className="trust-grid" style={{ maxWidth: 1280, margin: '0 auto' }}>
          {[
            ...
            <div key={i} style={{ padding: '1.125rem 1rem', textAlign: 'center', borderLeft: i < 3 ? '1px solid #EBEBEB' : 'none' }}>
              <div style={{ color: '#E63946', marginBottom: '0.375rem', display: 'flex', justifyContent: 'center' }}>{item.icon}</div>
      ...
      <div className="details-inner" style={{ maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div>
          <div className="info-container">
            ...
            <div style={{
              background: '#FFF5F5', // fond rouge très pâle pour le confort visuel
              border: '1.5px solid #E63946', // bordure rouge explicite
              borderRadius: 12,
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
                <span className="price-mono" style={{
                  fontSize: '2.5rem',
                  fontWeight: 800,
                  color: '#111' // prix en noir pour contraste fort avec le rouge
                }}>
            ...
            {/* Offres - couleur rouge harmonisée */}
              ...
              <div style={{ marginBottom: '1.25rem' }}>
                {product.offers.map((o: any) => (
                  <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.875rem 1rem', border: `1.5px solid ${selectedOffer === o.id ? '#E63946' : '#E8E8E8'}`, borderRadius: 10, cursor: 'pointer', marginBottom: '0.5rem', background: selectedOffer === o.id ? 'rgba(230, 57, 70, 0.04)' : 'transparent', transition: 'all 0.18s' }}>
                    ...
                    <span className="price-mono" style={{ fontWeight: 800, color: '#E63946', fontSize: '1.1rem' }}>{o.price.toLocaleString()} DA</span>
                  </label>
                ))}
              </div>
            )}

            {/* Attributs - couleur rouge harmonisée */}
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: '0.875rem' }}>
              ...
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.875rem', border: `1.5px solid ${fd.typeLivraison === t ? '#E63946' : '#E8E8E8'}`, borderRadius: 10, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? 'rgba(230,57,70,0.05)' : '#fff', transition: 'all 0.18s', fontFamily: 'inherit' }}>
                    <p style={{ fontWeight: 700, fontSize: '0.85rem', marginBottom: 4, color: fd.typeLivraison === t ? '#E63946' : '#555' }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
            ...
            <div style={{ background: '#F8F8F6', border: '1.5px solid #EBEBEB', borderRadius: 10, padding: '1rem 1.125rem', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '0.375rem' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.625rem', fontWeight: 800, color: '#E63946' }}>{total().toLocaleString()} <span style={{ fontSize: '0.8rem', fontFamily: 'inherit', fontWeight: 700, color: '#E63946' }}>DA</span></span>
              </div>
            </div>
  ...
  if (success) return (
    <div dir="ltr" style={{ minHeight: '70vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: '#F8F8F6' }}>
      <div style={{ textAlign: 'center', background: '#fff', padding: '3.5rem 2rem', borderRadius: 16, border: '1.5px solid #E8E8E8', maxWidth: 460, width: '100%' }}>
        ...
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#E63946', color: '#fff', padding: '0.8rem 2rem', borderRadius: 10, fontWeight: 700, fontSize: '0.9rem' }}>Retour à la boutique</Link>
      </div>
    </div>
  );
  ...
  if (!items.length) return (
    <div dir="ltr" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: '#F8F8F6' }}>
      <div style={{ textAlign: 'center', padding: '4rem 2rem', border: '2px dashed #E8E8E8', borderRadius: 16, maxWidth: 440, width: '100%', background: '#fff' }}>
        ...
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#E63946', color: '#fff', padding: '0.8rem 2rem', borderRadius: 10, fontWeight: 700, fontSize: '0.875rem' }}>Acheter maintenant</Link>
      </div>
    </div>
  );
          ...
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '1rem', padding: '1rem', borderBottom: '1px solid #F0F0F0' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p className="price-mono" style={{ fontSize: '1.125rem', fontWeight: 800, color: '#E63946' }}>{item.finalPrice?.toLocaleString()} DA</p>
          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  ...
                  >
                    ...
                    <p style={{ fontWeight: 700, fontSize: '0.78rem', color: fd.typeLivraison === t ? '#E63946' : '#888' }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
            ...
            <div style={{ background: '#F8F8F6', border: '1.5px solid #EBEBEB', borderRadius: 10, padding: '1rem 1.125rem', margin: '1rem 0' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.75rem', fontWeight: 800, color: '#E63946' }}>{finalTotal.toLocaleString()} <span style={{ fontSize: '0.8rem', fontWeight: 700, color: '#E63946' }}>DA</span></span>
              </div>
            </div>
      ...
      <div className="contact-inner" style={{ maxWidth: 1100, margin: '0 auto', padding: '4rem 1.5rem 6rem' }}>
        ...
        <div>
          <div style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E8E8E8', padding: '1.75rem', marginBottom: '1rem' }}>
            <p style={{ fontSize: '0.7rem', fontWeight: 800, color: '#E63946', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '1.25rem' }}>Informations de contact</p>
            {[
        ...
        <div style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E8E8E8', padding: '2rem' }}>
          {sent ? (
            <div style={{ textAlign: 'center', padding: '4rem 1rem' }}>
              ...
              <button onClick={() => setSent(false)} style={{ padding: '0.75rem 1.75rem', borderRadius: 10, border: '1.5px solid #E63946', background: 'transparent', color: '#E63946', fontWeight: 700, cursor: 'pointer', fontSize: '0.875rem', fontFamily: 'inherit' }}>Envoyer un autre message</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
