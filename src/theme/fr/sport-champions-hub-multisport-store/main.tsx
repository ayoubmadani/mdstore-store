...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, left: 0, background: '#fff', border: '1px solid var(--g200)', borderRadius: 'var(--radius-s)', boxShadow: 'var(--shadow-m)', zIndex: 200, overflowY: 'auto', maxHeight: 300 }}>
    ...
    {loading 
      ? <div style={{ padding: '1.25rem', textAlign: 'center', color: 'var(--accent)', fontSize: '0.875rem', fontWeight: 600 }}>Recherche en cours...</div>
      : ls.length > 0 
        ? (
          <div>
            {ls.map((p: any) => (
              ...
              >
                ...
                <div>
                  ...
                  <div style={{ fontSize: '0.75rem', color: 'var(--accent)', fontWeight: 700 }}>{p.price} DA</div>
                </div>
              </Link>
            ))}
            <button
...
export function Footer({ store }: any) {
  return (
    <footer dir="ltr" style={{ background: 'var(--primary)', color: 'var(--g500)', padding: '5rem 1.5rem 0' }}>
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
          ...
          <div>
            ...
            <div style={{ marginTop: '1.5rem', padding: '1rem 1.25rem', background: 'rgba(233,69,96,0.1)', border: '1px solid rgba(233,69,96,0.2)', borderRadius: 'var(--radius-s)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: '0.375rem' }}>
                ...
                <span className="oswald" style={{ fontSize: '0.95rem', color: '#fff', fontWeight: 600 }}>Disponibles maintenant</span>
              </div>
              <p style={{ fontSize: '0.75rem', color: 'var(--g500)' }}>Réponse en quelques heures</p>
            </div>
          </div>
        ...
        <div style={{ padding: '1.5rem 0', borderTop: '1px solid rgba(255,255,255,0.06)', textAlign: 'center', marginTop: '0' }}>
          <p style={{ fontSize: '0.82rem', color: 'var(--g600)' }}>© {new Date().getFullYear()} {store?.name}. Tous droits réservés. Conçu pour les champions.</p>
        </div>
      </div>
    </footer>
  );
  ...
  return (
    <div className="prod-card" onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}>
      ...
      <div className="card-img-inner" style={{ position: 'relative', height: 240, overflow: 'hidden', background: 'var(--g100)' }}>
        ...
        <button className="quick-view-btn" style={{ display: 'none' }}>Aperçu rapide</button>
      </div>
      ...
      <div style={{ padding: '18px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.375rem' }}>
            ...
            <span style={{ fontSize: '0.72rem', color: 'var(--g500)', fontWeight: 600 }}>{store?.currency || 'DA'}</span>
        ...
        <div className="hero-layout" style={{ maxWidth: 1400, margin: '0 auto', position: 'relative', zIndex: 1, width: '100%' }}>
          ...
          <div>
            ...
            <h1 className="oswald anim-fade-up d1" style={{ fontSize: 'clamp(2.5rem,6vw,4.5rem)', fontWeight: 700, color: '#fff', lineHeight: 1.1, marginBottom: '1.375rem', textTransform: 'uppercase', letterSpacing: '1px' }}
              dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(store.hero?.title || 'Élevez votre <span style="color:var(--accent);display:block">niveau sportif</span>') }}>
            </h1>
      ...
      <div style={{ background: 'var(--white)', borderBottom: '1px solid var(--g200)', borderTop: '1px solid var(--g200)' }}>
        <div style={{ maxWidth: 1400, margin: '0 auto' }}>
          <div className="trust-grid">
            {[
              { icon: <Truck size={20} />, t: 'Livraison rapide', d: '48h pour toutes les wilayas' },
              { icon: <RotateCcw size={20} />, t: 'Retour facile', d: 'Politique 30 jours' },
              { icon: <Shield size={20} />, t: 'Produits authentique', d: 'Garantie 100%' },
              { icon: <Trophy size={20} />, t: 'Haute qualité', d: 'Équipements professionnelle' },
            ].map((item, i) => (
        ...
        <Link href="?" className="cat-card">
          ...
          <Link key={cat.id} href={`?category=${cat.id}`} className="cat-card">
            ...
            <div style={{ padding: '14px 16px', textAlign: 'center' }}>
              <p style={{ fontSize: '0.78rem', color: 'var(--g500)' }}>Produit sportif</p>
              <span style={{ display: 'inline-block', marginTop: 6, fontSize: '0.75rem', color: 'var(--accent)', fontWeight: 600 }}>Acheter maintenant ←</span>
            </div>
          </Link>
        ))}
      </div>
      ...
      <section id="products" style={{ padding: '1rem 1.5rem 6rem', maxWidth: 1400, margin: '0 auto' }}>
        ...
        {products.length === 0 ? (
          <div style={{ padding: '5rem', textAlign: 'center', border: '1px dashed var(--g300)', borderRadius: 'var(--radius)', background: '#fff' }}>
            ...
            <p className="oswald" style={{ fontSize: '1.25rem', color: 'var(--g500)' }}>Aucun produit actuellement</p>
          </div>
        ) : (
...
export function Details({ product, discount, allImages, allAttrs, finalPrice, inStock, autoGen, selectedVariants, setSelectedOffer, selectedOffer, handleVariantSelection, domain }: any) {
  ...
  return (
    <div dir="ltr" style={{ background: 'var(--g50)', minHeight: '100vh', paddingBottom: '5rem' }}>
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '2.5rem 1.5rem' }}>
        <div className="details-layout">
          ...
          <div style={{ background: '#fff', borderRadius: 'var(--radius)', border: '1px solid var(--g200)', padding: '2rem' }}>
            <p style={{ fontSize: '0.75rem', color: 'var(--accent)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 8 }}>Sport</p>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.625rem', marginBottom: '0.5rem' }}>
              ...
              <span style={{ fontWeight: 500, color: 'var(--g500)', fontSize: '1rem' }}>DA</span>
            </div>
            <p style={{ fontSize: '0.75rem', color: 'var(--g500)', marginBottom: '1.375rem' }}>Livraison calculée à la commande</p>
            ...
            {product.offers?.length > 0 && (
              <div style={{ marginBottom: '1.5rem' }}>
                <p style={{ fontSize: '0.78rem', fontWeight: 600, color: 'var(--g700)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.75rem' }}>Forfaits Disponibles</p>
                  ...
                  <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.875rem 1rem', border: `1.5px solid ${selectedOffer === o.id ? 'var(--accent)' : 'var(--g200)'}`, borderRadius: 'var(--radius-s)', cursor: 'pointer', marginBottom: '0.5rem', background: selectedOffer === o.id ? 'rgba(233,69,96,0.04)' : 'transparent', transition: 'all 0.2s' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                      ...
                      <div>
                        ...
                        <p style={{ fontSize: '0.72rem', color: 'var(--g500)' }}>Quantité: {o.quantity}</p>
                      </div>
                    </div>
                    <span className="oswald" style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--accent)' }}>{o.price.toLocaleString()} DA</span>
                  </label>
                ))}
              </div>
            )}
            ...
            {product.desc && (
              <div style={{ marginTop: '2rem', paddingTop: '1.75rem', borderTop: '1px solid var(--g200)' }}>
                <p className="oswald" style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--g900)', marginBottom: '0.875rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Description du produit</p>
      ...
      {product.store?.cart && (
        <div className="cart-add-btns" style={{ marginBottom: '1.25rem' }}>
          <button onClick={addToCart} disabled={isAdded} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '0.875rem', borderRadius: 50, cursor: isAdded ? 'default' : 'pointer', fontFamily: "'Inter',sans-serif", fontWeight: 600, fontSize: '0.9rem', border: `1.5px solid ${isAdded ? 'var(--success)' : 'var(--g300)'}`, background: isAdded ? 'rgba(46,204,113,0.1)' : '#fff', color: isAdded ? 'var(--success)' : 'var(--g600)', transition: 'all 0.25s' }}>
            {isAdded ? <><CheckCircle2 size={15} className="anim-check" />Ajouté au panier</> : <><ShoppingCart size={15} />Ajouter au panier</>}
          </button>
        ...
        <div className="anim-fade-up">
          {product.store?.cart && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <p style={{ fontSize: '0.78rem', fontWeight: 600, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Informations de livraison</p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: '0.875rem' }}>
              <p style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--g600)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.5rem' }}>Quantité</p>
            ...
            {/* Delivery type */}
            <div style={{ marginBottom: '0.875rem' }}>
              <p style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--g600)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.5rem' }}>Livraison</p>
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.875rem', border: `1.5px solid ${fd.typeLivraison === t ? 'var(--accent)' : 'var(--g300)'}`, borderRadius: 'var(--radius-s)', textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? 'rgba(233,69,96,0.05)' : '#fff', fontFamily: 'inherit', transition: 'all 0.2s' }}>
                    <p style={{ fontWeight: 600, fontSize: '0.82rem', color: fd.typeLivraison === t ? 'var(--accent)' : 'var(--g600)', marginBottom: 4 }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
                    {selW && <p className="oswald" style={{ fontSize: '1rem', fontWeight: 700, color: fd.typeLivraison === t ? 'var(--accent)' : 'var(--g500)' }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} <span style={{ fontSize: '0.7rem', fontFamily: "'Inter',sans-serif", fontWeight: 400 }}>DA</span></p>}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div style={{ border: '1px solid var(--g200)', borderRadius: 'var(--radius-s)', marginBottom: '1.25rem', overflow: 'hidden' }}>
              <div style={{ padding: '9px 14px', background: 'var(--g50)', borderBottom: '1px solid var(--g200)' }}>
                <p style={{ fontSize: '0.78rem', fontWeight: 600, color: 'var(--g600)', margin: 0 }}>Résumé de la commande</p>
              </div>
              {[{ l: 'Produit', v: product.name.slice(0, 24) + (product.name.length > 24 ? '...' : '') }, { l: 'Prix', v: `${fp.toLocaleString()} DA` }, { l: 'Quantité', v: `× ${fd.quantity}` }, { l: 'Livraison', v: selW ? `${getLiv().toLocaleString()} DA` : '—' }].map(row => (
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '11px 14px', background: 'var(--g50)' }}>
                <span style={{ fontSize: '0.82rem', color: 'var(--g600)' }}>Total</span>
                <span className="oswald" style={{ fontSize: '1.625rem', fontWeight: 700, color: 'var(--accent)' }}>{total().toLocaleString()} <span style={{ fontSize: '0.82rem', fontFamily: "'Inter',sans-serif", fontWeight: 400, color: 'var(--g500)' }}>DA</span></span>
              </div>
            </div>
            ...
            <button type="submit" disabled={sub} className="btn-pri" style={{ width: '100%', padding: '0.925rem', borderRadius: 50, opacity: sub ? 0.7 : 1, cursor: sub ? 'not-allowed' : 'pointer', marginBottom: 8 }}>
              {sub ? <><Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> En cours...</> : '🛒 Confirmer la commande'}
            </button>
            <p style={{ textAlign: 'center', fontSize: '0.72rem', color: 'var(--g400)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
              <Lock size={11} style={{ color: 'var(--accent)' }} /> Paiement Sécurisé et chiffré
            </p>
          </form>
        </div>
      )}
...
export function Cart({ domain, store }: { domain: string; store: any }) {
  ...
  if (success) return (
    <div dir="ltr" style={{ minHeight: '70vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', background: 'var(--g50)' }}>
  ...
  if (!items.length) return (
    <div dir="ltr" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', background: 'var(--g50)' }}>
      <div style={{ textAlign: 'center', padding: '4rem 2rem', border: '1px solid var(--g200)', borderRadius: 'var(--radius)', maxWidth: 400, width: '100%', background: '#fff' }}>
        ...
        <p className="oswald" style={{ fontSize: '1.375rem', fontWeight: 700, color: 'var(--g500)', marginBottom: '0.5rem', textTransform: 'uppercase' }}>Panier vide</p>
        <p style={{ fontSize: '0.875rem', color: 'var(--g400)', marginBottom: '1.75rem' }}>Ajoutez quelques Équipements et commencez votre aventure !</p>
        <Link href="/" className="btn-pri" style={{ display: 'inline-flex', borderRadius: 50 }}>Acheter maintenant</Link>
      </div>
    </div>
  );

  return (
    <div dir="ltr" style={{ minHeight: '100vh', background: 'var(--g50)', padding: '2.5rem 1.5rem 5rem' }}>
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: '2.5rem', paddingBottom: '1.25rem', borderBottom: '1px solid var(--g200)' }}>
          <h1 className="oswald" style={{ fontSize: 'clamp(1.75rem,4vw,2.5rem)', fontWeight: 700, color: 'var(--g900)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Panier</h1>
          <span style={{ fontSize: '0.875rem', color: 'var(--g500)' }}>{items.length} Produit</span>
        </div>
        ...
        <div className="cart-layout">
          ...
          <div style={{ background: '#fff', borderRadius: 'var(--radius)', border: '1px solid var(--g200)', overflow: 'hidden', alignSelf: 'start' }}>
              ...
              <div key={i} style={{ display: 'flex', gap: '1rem', padding: '1.125rem', borderBottom: '1px solid var(--g100)' }}>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  <div>
                    ...
                    <p className="oswald" style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--accent)' }}>{item.finalPrice?.toLocaleString()} DA</p>
                  </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '1rem 1.125rem', background: 'var(--g50)', borderTop: '1px solid var(--g200)' }}>
              <span style={{ fontWeight: 600, fontSize: '0.875rem', color: 'var(--g600)' }}>Sous-total</span>
              <span className="oswald" style={{ fontSize: '1.375rem', fontWeight: 700, color: 'var(--g900)' }}>{cartTotal.toLocaleString()} DA</span>
            </div>
          </div>

          {/* Checkout */}
          <div style={{ background: '#fff', borderRadius: 'var(--radius)', border: '1px solid var(--g200)', padding: '1.75rem', alignSelf: 'start' }}>
            <h3 className="oswald" style={{ fontWeight: 700, fontSize: '1.1rem', color: 'var(--g900)', marginBottom: '1.5rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Informations de livraison</h3>
            <form onSubmit={handleSubmit}>
              <div className="form-row-2" style={{ marginBottom: '0.875rem' }}>
                <FR error={errors.name} label="Nom"><input type="text" value={fd.customerName} onChange={e => setFd({ ...fd, customerName: e.target.value })} style={INP(!!errors.name)}
                  onFocus={e => { e.target.style.borderColor = 'var(--accent)'; }} onBlur={e => { e.target.style.borderColor = errors.name ? 'var(--danger)' : 'var(--g300)'; }} /></FR>
                <FR error={errors.phone} label="Téléphone"><input type="tel" value={fd.customerPhone} onChange={e => setFd({ ...fd, customerPhone: e.target.value })} style={INP(!!errors.phone)}
              ...
              <div style={{ marginBottom: '1.25rem' }}>
                <p style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--g500)', textTransform: 'uppercase', marginBottom: '0.625rem', letterSpacing: '0.04em' }}>Type de livraison</p>
                <div className="delivery-grid">
                  {(['home', 'office'] as const).map(t => (
                    <button
                      key={t}
                    ...
                    >
                      ...
                      <p style={{ fontWeight: 600, fontSize: '0.78rem', color: fd.typeLivraison === t ? 'var(--g800)' : 'var(--g500)' }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
                      {selW && <p className="oswald" style={{ fontSize: '0.9rem', fontWeight: 700, color: 'var(--accent)', marginTop: '3px' }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} DA</p>}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div style={{ background: 'var(--g50)', border: '1px solid var(--g200)', borderRadius: 'var(--radius-s)', padding: '1rem', marginBottom: '1.125rem' }}>
                {[{ l: 'Sous-total', v: cartTotal.toLocaleString() }, { l: 'Livraison', v: getLiv() ? getLiv().toLocaleString() : '—' }].map(r => (
                  <div key={r.l} style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: '0.5rem', marginBottom: '0.5rem', borderBottom: '1px solid var(--g200)', fontSize: '0.875rem' }}>
                    ...
                    <span style={{ fontWeight: 600, color: 'var(--g800)' }}>{r.v} DA</span>
                  </div>
                ))}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', paddingTop: '0.375rem' }}>
                  <span style={{ fontWeight: 700, color: 'var(--g800)' }}>Total</span>
                  <span className="oswald" style={{ fontSize: '1.625rem', fontWeight: 700, color: 'var(--accent)' }}>{finalTotal.toLocaleString()} <span style={{ fontSize: '0.82rem', fontFamily: "'Inter',sans-serif", fontWeight: 400, color: 'var(--g500)' }}>DA</span></span>
                </div>
              </div>
              ...
              <button type="submit" disabled={submitting} className="btn-pri" style={{ width: '100%', padding: '0.925rem', borderRadius: 50, opacity: submitting ? 0.7 : 1, cursor: submitting ? 'not-allowed' : 'pointer' }}>
                {submitting ? <><Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> En cours...</> : '🛒 Confirmer la commande'}
              </button>
            </form>
          </div>
        </div>
...
const Shell = ({ title, sub, children }: { title: string; sub?: string; children: React.ReactNode }) => (
  <div dir="ltr" style={{ background: 'var(--g50)', minHeight: '100vh' }}>
...
export function Cookies() {
  return (
    <Shell title="Cookies" sub="Légal">
      ...
      <IB title="Cookies analytiques" body="Données agrégées pour améliorer votre expérience Shopping." tag="Facultatif" />
    </Shell>
  );
}
    ...
    <div dir="ltr" style={{ background: 'var(--g50)', minHeight: '100vh' }}>
      <div style={{ background: 'var(--primary)', padding: '5rem 1.5rem 3.5rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          <p style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '0.14em', marginBottom: '0.75rem' }}>Contact</p>
          <h1 className="oswald" style={{ fontSize: 'clamp(2rem,5vw,3.5rem)', fontWeight: 700, color: '#fff', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Ravi de vous aider</h1>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '3rem 1.5rem 5rem' }}>
        <div className="contact-layout">
          {/* Info */}
          <div>
            {[{ e: '📞', l: 'Téléphone', v: store?.contact?.phone || '—' }, { e: '📍', l: 'Adresse', v: [store?.contact?.wilaya, store?.contact?.address].filter(Boolean).join(' / ') || '—' }, { e: '📧', l: 'Email', v: store?.contact?.email || '—' }].map((r, i) => (
            ...
            <div style={{ padding: '1rem', border: '1px solid rgba(233,69,96,0.2)', borderRadius: 'var(--radius-s)', background: 'rgba(233,69,96,0.04)', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span className="oswald" style={{ fontWeight: 600, color: 'var(--g900)', fontSize: '0.95rem' }}>Disponibles maintenant</span>
            </div>
          </div>
            ...
            {sent ? (
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div className="form-row-2">
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: 'var(--g600)', marginBottom: '0.4rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Téléphone</label>
                    <input type="tel" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} required style={INP()}
                      onFocus={e => { e.target.style.borderColor = 'var(--accent)'; }} onBlur={e => { e.target.style.borderColor = 'var(--g300)'; }} />
                  </div>
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: 'var(--g600)', marginBottom: '0.4rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Email</label>
                  <input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} required style={INP()}
                    onFocus={e => { e.target.style.borderColor = 'var(--accent)'; }} onBlur={e => { e.target.style.borderColor = 'var(--g300)'; }} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: 'var(--g600)', marginBottom: '0.4rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Votre message</label>
