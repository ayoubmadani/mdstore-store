...
/* ══════════════════════════════════════════════════════════════
   Parfum de la Nature — Natural Perfume Boutique
   ─────────────────────────────────────────────────────────────
   ...
   ✦ "Ajouter au panier" button directly on product cards  
   ✦ Rounded soft cards with shadow
   ✦ Fabriqué en Algérie footer badge
══════════════════════════════════════════════════════════════ */
const CSS = `
                        ...
                        <form onSubmit={doSearch}>
                            <div style={{ position: 'relative', maxWidth: '600px', margin: '0 auto' }}>
                                ...
                                <input
                                    autoFocus
                                    type="text"
                                    placeholder="Rechercher un parfum ou un produit..."
                                    value={sq}
                        ...
                        {sq.length >= 2 && (
                            <div style={{
                                ...
                                {loading ? (
                                    ...
                                    <div style={{ padding: '1.5rem', textAlign: 'center', fontSize: '13px', color: 'var(--mist)' }}>Aucun résultat correspondant</div>
                                )}
                            </div>
                        )}
                    </div>
...
/* ══════════════════════════════════════════════════════════════
   FOOTER — sage green, 3 sections + Fabriqué en Algérie
══════════════════════════════════════════════════════════════ */
                ...
                <div className="footer-g">
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '16px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(255,255,255,0.15)', borderRadius: '10px', padding: '10px 14px' }}>
                            ...
                            <div>
                                <p style={{ fontSize: '11px', fontWeight: 700, color: 'rgba(255,255,255,0.9)', margin: 0, lineHeight: 1.2 }}>Fabriqué en</p>
                                <p style={{ fontSize: '14px', fontWeight: 700, color: 'white', margin: 0, lineHeight: 1.2 }}>Algérie</p>
                            </div>
                        </div>
                        {/* Social icons */}
                ...
                <div style={{ position: 'relative', zIndex: 2, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', minHeight: '260px', padding: '32px 24px' }}>
                    <div style={{ textAlign: 'right', maxWidth: '280px' }}>
                        <h1 className="fu" style={{ fontSize: 'clamp(1.6rem,5vw,2.4rem)', fontWeight: 700, color: store.hero?.imageUrl ? 'white' : 'var(--char)', lineHeight: 1.2, marginBottom: '10px' }}>
                            {store.hero?.title || 'La magie de la nature en\nchaque goutte'}
                        </h1>
                        ...
                        <div style={{ position: 'relative', borderRadius: '12px', overflow: 'hidden', background: 'var(--cream)', aspectRatio: '1/1' }}>
                            ...
                            {!inStock && !autoGen && (
                                <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.8)', backdropFilter: 'blur(4px)', borderRadius: '12px' }}>
                                    <span style={{ fontSize: '14px', fontWeight: 600, color: 'var(--mist)' }}>Stock épuisé</span>
                                </div>
                            )}
                        </div>
                        {allImages.length > 1 && (
                    ...
                    <div>
                        ...
                        {product.offers?.length > 0 && (
                            <div style={{ marginBottom: '16px' }}>
                                <p style={{ fontSize: '12px', fontWeight: 600, color: 'var(--mid)', marginBottom: '8px' }}>Forfaits Disponibles</p>
                        ...
                        {product.desc && (
                            <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: '1px solid var(--line)' }}>
                                <p style={{ fontSize: '12px', fontWeight: 600, color: 'var(--mid)', marginBottom: '10px' }}>Description du produit</p>
        ...
        {product.store?.cart && (
                <div className="cart-btns" style={{ marginBottom: '12px' }}>
                    <button onClick={addToCart} disabled={isAdded}
                        ...
                        {isAdded ? <><CheckCircle2 size={14} className="anim-check" />Ajouté</> : <><ShoppingBag size={14} />Panier</>}
                    </button>
            ...
            {(isOrderNow || !product.store?.cart) && (
                <div style={{ background: 'var(--ivory)', border: '1.5px solid var(--line-2)', borderRadius: '12px', overflow: 'hidden' }}>
                    <div style={{ padding: '12px 16px', background: 'var(--sage-lt)', borderBottom: '1px solid var(--line)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <p style={{ fontSize: '13px', fontWeight: 700, color: 'var(--sage)', margin: 0 }}>Données de la commande</p>
                        {product.store?.cart && (
                    ...
                    <form onSubmit={handleSubmit} style={{ padding: '16px' }}>
                            ...
                            <FR error={errors.customerCommune} label="Commune">
                                <div style={{ position: 'relative' }}>
                                    ...
                                    <select value={fd.customerCommune} disabled={!fd.customerWelaya || loadingC} onChange={e => setFd({ ...fd, customerCommune: e.target.value })}
                                        ...
                                        <option value="">{loadingC ? '...' : fd.customerWelaya ? 'Choisir Commune' : 'Choisir Wilaya en premier'}</option>
                            ...
                            <div className="dlv-2c">
                                {(['home', 'office'] as const).map(type => (
                                    <button key={type} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: type }))}
                                        ...
                                        <p style={{ fontSize: '12px', fontWeight: 600, color: fd.typeLivraison === type ? 'var(--sage)' : 'var(--mid)', margin: '0 0 3px' }}>{type === 'home' ? '🏠 Maison' : '🏢 Bureau'}</p>
                        ...
                        <div style={{ background: 'var(--sage-lt)', borderRadius: '10px', padding: '14px 16px', marginBottom: '14px' }}>
                            {[{ l: 'Produit', v: product.name.slice(0, 20) + (product.name.length > 20 ? '...' : '') }, { l: 'Prix', v: `${fp.toLocaleString()} DA` }, { l: 'Quantité', v: `× ${fd.quantity}` }, { l: 'Livraison', v: selW ? `${getLiv().toLocaleString()} DA` : '—' }].map(row => (
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '10px' }}>
                                ...
                                <span style={{ fontSize: '1.4rem', fontWeight: 700, color: 'var(--char)' }}>{total().toLocaleString()} <span style={{ fontSize: '12px', fontWeight: 500 }}>DA</span></span>
                            </div>
                        </div>
                        ...
                        <button type="submit" disabled={sub} className="btn-sage" style={{ width: '100%', padding: '13px', fontSize: '14px', borderRadius: '8px', cursor: sub ? 'not-allowed' : 'pointer', opacity: sub ? 0.55 : 1, boxShadow: '0 4px 14px rgba(107,143,107,0.3)' }}>
                            {sub ? <><Loader2 style={{ width: '15px', height: '15px', animation: 'spin 1s linear infinite' }} /> En cours...</> : <><ShoppingCart style={{ width: '15px', height: '15px' }} />Confirmer la commande</>}
                        </button>
                        <p style={{ fontSize: '11px', textAlign: 'center', color: 'var(--mist)', marginTop: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
                            <Shield style={{ width: '11px', height: '11px', color: 'var(--sage)' }} /> Transaction Sécurisée et chiffrées
                        </p>
                    </form>
                </div>
            )}
    ...
    if (!items.length) return (
        <div dir="ltr" style={{ minHeight: '55vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', background: 'var(--white)', fontFamily: "'Cairo',sans-serif" }}>
            <div style={{ textAlign: 'center', padding: '3rem 2rem', maxWidth: 320, width: '100%' }}>
                ...
                <p style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--mid)', marginBottom: 8 }}>Panier vide</p>
                <p style={{ fontSize: '12px', color: 'var(--mist)', marginBottom: 20 }}>Ajoutez vos parfums préférés</p>
                <Link href="/" className="btn-sage" style={{ display: 'inline-flex', padding: '11px 28px', fontSize: '13px', borderRadius: '8px' }}>Acheter maintenant</Link>
            </div>
        </div>
    );
        ...
        <div dir="ltr" style={{ minHeight: '100vh', background: 'var(--ivory)', padding: '20px 16px 60px', fontFamily: "'Cairo',sans-serif" }}>
            <div style={{ maxWidth: 860, margin: '0 auto' }}>
                <h1 style={{ fontSize: '1.3rem', fontWeight: 700, color: 'var(--char)', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: '12px', fontWeight: 500, color: 'var(--mist)', marginRight: 'auto' }}>{items.length} Produits</span>
                </h1>
                ...
                <div className="cart-g">
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            ...
                            <div key={i} style={{ display: 'flex', gap: '12px', padding: '14px', borderRadius: '12px', background: 'var(--white)', border: '1px solid var(--line)', transition: 'box-shadow 0.25s' }}
                                ...
                                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                                    <div>
                                        ...
                                        <p style={{ fontSize: '13px', fontWeight: 700, color: 'var(--sage)' }}>{item.finalPrice?.toLocaleString()} DA</p>
                                    </div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', borderRadius: '10px', background: 'var(--sage-lt)' }}>
                            <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--mid)' }}>Sous-total</span>
                            <span style={{ fontSize: '15px', fontWeight: 700, color: 'var(--char)' }}>{cartTotal.toLocaleString()} DA</span>
                        </div>
                    </div>
                    ...
                    <div style={{ background: 'var(--white)', borderRadius: '12px', border: '1px solid var(--line)', overflow: 'hidden' }}>
                        <div style={{ padding: '12px 16px', background: 'var(--sage-lt)', borderBottom: '1px solid var(--line)' }}>
                            <p style={{ fontSize: '13px', fontWeight: 700, color: 'var(--sage)', margin: 0 }}>Finaliser la commande</p>
                        </div>
                        <form onSubmit={handleSubmit} style={{ padding: '16px' }}>
                            <div className="form-2c">
                                <FR error={errors.n} label="Nom">
                                ...
                                <FR error={errors.p} label="Téléphone">
                            ...
                            <div style={{ background: 'var(--sage-lt)', borderRadius: '10px', padding: '12px 14px', marginBottom: '14px' }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '8px' }}>
                                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--mid)' }}>Total</span>
                                    <span style={{ fontSize: '1.3rem', fontWeight: 700, color: 'var(--char)' }}>{finalTotal.toLocaleString()} DA</span>
                                </div>
                            </div>
                            <button type="submit" disabled={submitting} className="btn-sage" style={{ width: '100%', padding: '13px', fontSize: '14px', borderRadius: '8px', cursor: submitting ? 'not-allowed' : 'pointer', opacity: submitting ? 0.55 : 1, boxShadow: '0 4px 14px rgba(107,143,107,0.3)' }}>
                                {submitting ? <><Loader2 style={{ width: '15px', height: '15px', animation: 'spin 1s linear infinite' }} />En cours...</> : <><ShoppingCart style={{ width: '15px', height: '15px' }} />Confirmer la commande</>}
                            </button>
                        </form>
                    </div>
                </div>
...
export function Privacy() {
    return (
        <Shell title="Politique de confidentialité" emoji="🔒">
            <IB title="Les données que nous collectons" body="Votre nom et numéro de Téléphone et adresse de Livraison uniquement — nous ne collectons pas plus que nécessaire." />
            <IB title="Comment nous utilisons vos données" body="Exclusivement pour traiter et livrer votre commande. Aucun usage commercial." />
            <IB title="Sécurité et chiffrement" body="Toutes vos données sont protégées par chiffrement SSL." />
            <IB title="Partage des données" body="Nous ne vendons pas vos données. Partagées avec le partenaire de livraison." tag="Garanti" />
        </Shell>
    );
}
...
export function Terms() {
    return (
        <Shell title="Conditions de service" emoji="📋">
            <IB title="Commandes" body="Aucuns frais cachés. Le prix affiché est le prix final." />
            <IB title="Qualité Produits" body="Tous nos Parfums Naturels 100% de qualité garantie." tag="Garanti" />
            <IB title="Retours" body="Retour accepté sous 7 jours en cas de défaut." />
            <IB title="Loi" body="Ces Conditions sont soumises aux lois de la République Algérienne." />
        </Shell>
    );
}
...
export function Cookies() {
    return (
        <Shell title="Cookies" emoji="🍪">
            <IB title="Cookies essentiels" body="Essentiels pour faire fonctionner Boutique et Panier." tag="requis" />
            <IB title="Cookies de préférences" body="Sauvegarde vos préférences pour une meilleure expérience." tag="Facultatif" />
            <IB title="Contrôle" body="Vous pouvez gérer les cookies depuis les paramètres de votre navigateur." />
        </Shell>
    );
}
    ...
    return (
        <div dir="ltr" style={{ background: 'var(--white)', minHeight: '100vh', fontFamily: "'Cairo',sans-serif", padding: '24px 20px 60px' }}>
            ...
            <div style={{ marginTop: '24px', background: 'var(--ivory)', borderRadius: '12px', border: '1px solid var(--line)', padding: '20px' }}>
                <p style={{ fontSize: '13px', fontWeight: 700, color: 'var(--char)', marginBottom: '16px' }}>Envoyer un message</p>
                {sent ? (
                    ...
                    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        <div className="form-2c">
                            <div>
                                <p style={{ fontSize: '11px', fontWeight: 600, color: 'var(--mid)', marginBottom: 5 }}>Nom</p>
                                <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required style={INP()} onFocus={onF} onBlur={onB} />
                            </div>
                            <div>
                                <p style={{ fontSize: '11px', fontWeight: 600, color: 'var(--mid)', marginBottom: 5 }}>Téléphone</p>
                        ...
                        <button type="submit" disabled={loading} className="btn-sage" style={{ width: '100%', padding: '12px', fontSize: '13px', borderRadius: '8px', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.55 : 1 }}>
                            {loading ? <><Loader2 style={{ width: '14px', height: '14px', animation: 'spin 1s linear infinite' }} />En cours...</> : 'Envoyer le message 🌿'}
                        </button>
                    </form>
                )}
            </div>
