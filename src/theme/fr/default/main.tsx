...
export function Navbar({ store }: { store: Store }) {
  ...
  const links = [
    { href: '/',        label: isRTL ? 'Accueil'       : 'Home'    },
    { href: '/contact', label: isRTL ? 'Contactez-nous'      : 'Contact' },
  ];
    ...
    <footer style={{ borderTop: '1px solid #e5e5e5', background: '#f8f8f8', padding: '2.5rem 0 1.5rem' }}>
      <div style={{ ...S.container, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '2rem' }}>
        <div>
          ...
          <p style={{ fontSize: '0.8rem', color: '#666', lineHeight: 1.6 }}>{isRTL ? 'Boutique en ligne' : 'Online Store'}</p>
        </div>
        <div>
          <p style={{ fontWeight: 700, marginBottom: '0.75rem', fontSize: '0.85rem', color: '#444' }}>{isRTL ? 'Liens' : 'Links'}</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
            {[
              { href: '/',        label: isRTL ? 'Accueil'         : 'Home'    },
              { href: '/contact', label: isRTL ? 'Contactez-nous'        : 'Contact' },
              { href: '/Privacy', label: isRTL ? 'Politique de confidentialité'   : 'Privacy' },
              { href: '/Terms',   label: isRTL ? 'Conditions et clauses'  : 'Terms'   },
            ].map(l => (
        ...
        {(store?.contact?.phone || store?.contact?.email || store?.contact?.wilaya || store?.contact?.address) && (
          <div>
            <p style={{ fontWeight: 700, marginBottom: '0.75rem', fontSize: '0.85rem', color: '#444' }}>{isRTL ? 'Contactez-nous' : 'Contact'}</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
              {store?.contact?.phone && (
    ...
    <Link href={`/product/${product.slug || product.id}`} style={{ textDecoration: 'none', color: 'inherit', display: 'flex', flexDirection: 'column', border: '1px solid #e5e5e5', borderRadius: 4, overflow: 'hidden', background: '#fff', transition: 'border-color 0.15s' }}>
      ...
      <div style={{ padding: '0.75rem', flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontWeight: 700, fontSize: '0.95rem' }}>{price.toLocaleString()} <span style={{ fontWeight: 400, fontSize: '0.8rem', color: '#666' }}>{store?.currency || 'DA'}</span></span>
...
export const Home = ({ store }: any) => {
  const isRTL = (store?.language || 'ar') === 'ar';
  const t = {
    viewDetails: isRTL ? 'Voir les détails' : 'View Details',
      ...
      <div style={{ borderBottom: '1px solid #e5e5e5', padding: '0.6rem 1rem', background: '#f8f8f8' }}>
        <div style={S.container}>
          <nav style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.78rem', color: '#888' }}>
            <Link href="/" style={{ color: '#555', textDecoration: 'none' }}>Accueil</Link>
            <ChevronLeft size={12} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2rem', alignItems: 'start' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '1.6rem', fontWeight: 700, marginBottom: '1rem' }}>
              {finalPrice.toLocaleString()} <span style={{ fontSize: '0.9rem', fontWeight: 400, color: '#666' }}>DA</span>
            </div>
            <div style={{ display: 'inline-block', background: (inStock || autoGen) ? '#111' : '#e5e5e5', color: (inStock || autoGen) ? '#fff' : '#666', fontSize: '0.75rem', padding: '3px 10px', borderRadius: 3, marginBottom: '1.5rem', fontWeight: 600 }}>
              {(inStock || autoGen) ? 'Disponible' : 'Non disponible'}
            </div>

            {/* Offers */}
            {product.offers?.length > 0 && (
              <div style={{ marginBottom: '1.25rem' }}>
                <p style={{ fontSize: '0.82rem', fontWeight: 600, color: '#444', marginBottom: '0.5rem' }}>Offres</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
            ...
            {product.desc && (
              <div style={{ marginTop: '2rem', paddingTop: '1.5rem', borderTop: '1px solid #e5e5e5' }}>
                <p style={{ fontSize: '0.82rem', fontWeight: 600, color: '#444', marginBottom: '0.75rem' }}>Description du produit</p>
...
export function ProductForm({ product, userId, domain, selectedOffer, setSelectedOffer, selectedVariants, platform, priceLoss = 0 }: ProductFormProps) {
  ...
  const validate = () => {
    ...
    if (!fd.customerName.trim() || fd.customerName.length < 3) e.customerName = 'Nom requis (3 caractères minimum)';
    if (!/^(0|\+213)[5-7][0-9]{8}$/.test(fd.customerPhone.replace(/\s/g, ''))) e.customerPhone = 'Numéro invalide (ex: 0550123456)';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    } catch { showError('Erreur lors de la connexion au serveur'); } finally { setSubmitting(false); }
  };
    ...
    <div style={{ border: '1px solid #e5e5e5', borderRadius: 4, overflow: 'hidden', marginTop: '1.5rem' }}>
      ...
      <form onSubmit={handleSubmit} style={{ padding: '1rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: 0 }}>
          ...
          <FR label="Commune" error={errors.customerCommune}>
            <select value={fd.customerCommune} disabled={!fd.customerWelaya || loadingCommunes} onChange={e => setFd({ ...fd, customerCommune: e.target.value })} style={{ ...S.inputBase, appearance: 'none' as any, opacity: (!fd.customerWelaya || loadingCommunes) ? 0.5 : 1 }}>
              <option value="">{loadingCommunes ? 'En cours...' : 'Choisir Commune'}</option>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
            {(['home', 'office'] as const).map(t => (
              <button key={t} type="button" onClick={() => setFd({ ...fd, typeLivraison: t })}
                ...
                <span style={{ fontSize: '0.75rem' }}>{t === 'home' ? 'Maison' : 'Bureau'}</span>
                {selW && <span style={{ fontSize: '0.72rem', opacity: 0.7 }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice)} DA</span>}
              </button>
            ))}
          </div>
        </FR>
        ...
        <div style={{ background: '#f8f8f8', padding: '0.75rem', borderRadius: 4, marginBottom: '1rem', fontSize: '0.82rem', color: '#555' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
            <span>Prix unitaire</span><span style={{ fontWeight: 600, color: '#111' }}>{fp.toLocaleString()} DA</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
            <span>Quantité</span><span>× {fd.quantity}</span>
          </div>
          {selW && (
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
              <span>Livraison</span><span>{getLiv()} DA</span>
            </div>
          )}
          <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 6, borderTop: '1px solid #e5e5e5', fontWeight: 700, fontSize: '0.95rem', color: '#111' }}>
            <span>Total</span><span>{getTotal().toLocaleString()} DA</span>
          </div>
        </div>

        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <button type="submit" disabled={submitting} style={{ ...S.btnBlack, flex: 1, justifyContent: 'center', opacity: submitting ? 0.7 : 1 }}>
            {submitting ? <><Loader2 size={14} /> Envoi en cours...</> : <><ShoppingCart size={14} /> Confirmer la commande</>}
          </button>
          {product.store.cart && (
        ...
        <p style={{ fontSize: '0.72rem', color: '#aaa', textAlign: 'center', marginTop: '0.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
          <Shield size={11} /> Vos données Sécurisées et chiffrées
        </p>
      </form>
    </div>
  );
...
export function Cart({ domain, store }: { domain: string; store: any }) {
  ...
  if (!items.length) return (
    <div dir={isRTL ? 'rtl' : 'ltr'} style={{ ...S.body, textAlign: 'center', padding: '5rem 1rem', minHeight: '50vh' }}>
      ...
      <p style={{ color: '#aaa', marginBottom: '1.5rem' }}>{isRTL ? 'Panier vide' : 'Cart is empty'}</p>
      <Link href="/" style={{ ...S.btnBlack, textDecoration: 'none', display: 'inline-flex' }}>{isRTL ? 'Continuer les achats' : 'Continue Shopping'}</Link>
    </div>
  );
    ...
    <div dir={isRTL ? 'rtl' : 'ltr'} style={{ ...S.body, padding: '2rem 0', minHeight: '60vh' }}>
      <div style={{ ...S.container, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2rem', alignItems: 'start' }}>
        ...
        <div style={{ border: '1px solid #e5e5e5', borderRadius: 4, padding: '1.25rem', background: '#f8f8f8' }}>
          <h3 style={{ fontSize: '0.95rem', fontWeight: 700, marginBottom: '1rem' }}>{isRTL ? 'Résumé du Panier' : 'Summary'}</h3>
          ...
          <p style={{ fontSize: '0.75rem', color: '#aaa', marginBottom: '1rem' }}>* Les frais de Livraison sont calculés à la commande</p>
          <button style={{ ...S.btnBlack, width: '100%', justifyContent: 'center' }}>{isRTL ? 'Finaliser la commande' : 'Checkout'}</button>
        </div>
      </div>
    </div>
  );
...
export function Terms() {
  return (
    <SimplePage title="Conditions et clauses">
      <p>En utilisant notre site, vous acceptez de vous conformer aux conditions énoncées.</p>
      <br />
      <p>Nous nous réservons le droit de modifier les prix et la disponibilité sans préavis.</p>
    </SimplePage>
  );
}
...
export function Contact({ store }: { store: any }) {
  const isRTL = (store?.language || 'ar') === 'ar';
  return (
    <SimplePage title={isRTL ? 'Contactez-nous' : 'Contact Us'}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
        ...
        {!store?.contact?.phone && !store?.contact?.email && (
          <p style={{ color: '#aaa' }}>{isRTL ? 'Aucune information de contact' : 'No contact info available'}</p>
        )}
      </div>
    </SimplePage>
  );
