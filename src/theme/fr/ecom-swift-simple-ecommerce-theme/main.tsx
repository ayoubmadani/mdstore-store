...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  const SearchResults = () => (
    <div style={{
      ...
      {loading ? (
        <div style={{ padding: '1.5rem', textAlign: 'center', color: '#888' }}>Recherche en cours...</div>
      ) : listSearch.length > 0 ? (
        <>
        ...
        <div style={{ padding: '1.5rem', textAlign: 'center', color: '#888' }}>Aucun résultat</div>
      )}
    </div>
  );
  ...
  return (
    <>
      ...
      <header dir="ltr" style={{ borderBottom: '1px solid var(--border-color)', background: '#fff', position: 'sticky', top: 0, zIndex: 100 }}>
      <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '75px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
          ...
          <nav className="lg-flex" style={{ gap: '1.5rem', fontSize: '0.85rem', fontWeight: 600 }}>
            <Link href="/" style={{ color: '#000' }}>Accueil</Link>
            <Link href="/contact" style={{ color: '#000' }}>Contactez-nous</Link>
          </nav>
        </div>
...
export function Footer({ store }: any) {
  return (
    <footer dir="ltr" style={{ borderTop: '1px solid var(--border-color)', padding: '4rem 0 2rem', background: 'var(--bg-alt)' }}>
      <div className="container grid-3">
        <div>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '1rem' }}>{store?.name}</h3>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', lineHeight: 1.6 }}>Acheter En confiance et simplicité. Expérience fluide et Livraison rapide.</p>
        </div>
        <div>
          <h4 style={{ fontWeight: 700, marginBottom: '1rem' }}>Liens</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.9rem', color: 'var(--text-muted)' }}>
            <Link href="/">Accueil</Link>
            <Link href="/contact">Contactez-nous</Link>
            <Link href="/Privacy">Politique de confidentialité</Link>
            <Link href="/Terms">Conditions et clauses</Link>
          </div>
        </div>
        <div>
      ...
      <div style={{ border: `1px solid ${store.design.secondaryColor}`, borderRadius: '8px', overflow: 'hidden', background: 'var(--bg-main)', transition: 'border-color 0.2s', height: '100%', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '1rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <span style={{ color: store.design.primaryColor, fontWeight: 700, fontSize: '1.1rem' }}>{price.toLocaleString()} {store.currency || 'DA'}</span>
    ...
    <div dir="ltr">
      ...
      {cats.length > 0 && (
        ...
        <section className="container" style={{ padding: '4rem 1.5rem 0' }}>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 700, marginBottom: '2rem' }}>Catégories</h2>
      ...
      <section id="products" className="container" style={{ padding: '4rem 1.5rem' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 700, marginBottom: '2rem' }}>Derniers Produits</h2>

        {products.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '4rem', background: 'var(--bg-alt)', borderRadius: '8px' }}>
            <p style={{ color: 'var(--text-muted)' }}>Aucun produit actuellement</p>
          </div>
        ) : (
      ...
      <section style={{ borderBottom: '1px solid var(--border-color)' }}>
        <div className="container grid-3" style={{ padding: '3rem 1.5rem' }}>
          {[
            { icon: <Truck size={20} />, t: 'Livraison rapide', d: 'dans toutes les wilayas' },
            { icon: <ShieldCheck size={20} />, t: 'Qualité Garantie', d: 'Produits authentiques' },
            { icon: <CheckCircle2 size={20} />, t: 'Achats Sécurisés', d: 'Paiement À la livraison' },
          ].map((item, idx) => (
      ...
      <div className='grid-2' style={{ gap: '3rem', alignItems: 'start' }}>
        ...
        <div>
          <div style={{ aspectRatio: '1/1', background: 'var(--bg-alt)', borderRadius: '8px', overflow: 'hidden', position: 'relative' }}>
            ...
            {discount > 0 && <span style={{ position: 'absolute', top: 12, right: 12, background: 'var(--primary)', color: '#fff', fontSize: '12px', padding: '4px 10px', borderRadius: '4px' }}>Réduction {discount}%</span>}
          </div>
          {allImages.length > 1 && (
        ...
        <div>
          ...
          <div style={{ fontSize: '2rem', fontWeight: 700, marginBottom: '1.5rem' }}>{finalPrice.toLocaleString()} <span style={{ fontSize: '1rem', color: 'var(--text-muted)' }}>DA</span></div>

          {/* Offers */}
          {product.offers?.length > 0 && (
            <div style={{ marginBottom: '1.5rem' }}>
              <p style={{ fontWeight: 600, marginBottom: '0.5rem' }}>Offres :</p>
              {product.offers.map((o: any) => (
                <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '1rem', border: `1px solid ${selectedOffer === o.id ? 'var(--primary)' : 'var(--border-color)'}`, borderRadius: '6px', marginBottom: '0.5rem', cursor: 'pointer', background: selectedOffer === o.id ? 'var(--bg-alt)' : 'transparent' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    ...
                    <span>{o.name} <span style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>(Quantité: {o.quantity})</span></span>
                  </div>
                  <span style={{ fontWeight: 600 }}>{o.price.toLocaleString()} DA</span>
                </label>
              ))}
            </div>
          )}
          ...
          {product.desc && (
            <div style={{ marginTop: '3rem' }}>
              <h3 style={{ fontSize: '1.1rem', fontWeight: 600, marginBottom: '1rem' }}>Description du produit</h3>
    ...
    <div>
      <form onSubmit={handleSubmit}>
        <div style={{ background: 'var(--bg-alt)', padding: '1.5rem', borderRadius: '8px', marginBottom: '1.5rem' }}>
          <h3 style={{ fontSize: '1.1rem', fontWeight: 600, marginBottom: '1rem' }}>Commande rapide</h3>
...
export function Cart({ domain, store }: { domain: string; store: any }) {
  ...
  if (!items.length) return (
    <div className="container" style={{ textAlign: 'center', padding: '6rem 1.5rem', minHeight: '60vh' }}>
      ...
      <h2 style={{ marginBottom: '1rem' }}>Panier vide</h2>
  ...
  return (
    <div className="container" style={{ padding: '4rem 1.5rem', minHeight: '70vh' }}>
      <h1 style={{ fontSize: '2rem', fontWeight: 700, marginBottom: '2rem' }}>Panier</h1>
      <div className="grid-2" style={{ gap: '3rem', alignItems: 'start' }}>
        <div>
          {items.map((item, i) => (
        ...
        <div style={{ background: 'var(--bg-alt)', padding: '2rem', borderRadius: '8px' }}>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 600, marginBottom: '1.5rem' }}>Résumé du Panier</h3>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1.25rem', fontWeight: 700, marginBottom: '1.5rem' }}>
            <span>Total:</span>
            <span>{total.toLocaleString()} DA</span>
          </div>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: '1rem' }}>* Frais Livraison seront calculés ultérieurement.</p>
...
export function Privacy() {
  return (
    <div className="container" style={{ padding: '4rem 1.5rem', maxWidth: '800px' }}>
      <h1 style={{ fontSize: '2rem', fontWeight: 700, marginBottom: '1rem' }}>Politique de confidentialité</h1>
      <SimpleDivider />
...
export function Terms() {
  return (
    <div className="container" style={{ padding: '4rem 1.5rem', maxWidth: '800px' }}>
      <h1 style={{ fontSize: '2rem', fontWeight: 700, marginBottom: '1rem' }}>Conditions et clauses</h1>
      <SimpleDivider />
      <div style={{ lineHeight: 1.8, color: 'var(--text-muted)' }}>
