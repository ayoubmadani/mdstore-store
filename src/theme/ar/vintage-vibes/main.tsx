...
const THEME_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Buttons & Inputs ── */
  .btn-primary {
    background: var(--primary); color: #fff; padding: 0.8rem 2rem; border-radius: 2px;
    border: 2px solid var(--primary); font-weight: 700; cursor: pointer; transition: background 0.2s;
  ...
  .btn-outline {
    background: transparent; color: var(--text-main); padding: 0.8rem 1.5rem; border-radius: 2px;
    border: 2px solid var(--border-color); font-weight: 500; cursor: pointer; transition: all 0.2s;
    display: inline-flex; align-items: center; justify-content: center; gap: 0.5rem;
  }
  .btn-outline:hover { border-color: var(--primary); color: var(--primary); }

  .input-base {
    width: 100%; padding: 0.8rem 1rem; border-radius: 2px; border: 1px solid var(--border-color);
...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  const SearchResults = () => (
    <div style={{
      ...
      maxHeight: '350px',
      overflowY: 'auto',
      boxShadow: '4px 4px 0px rgba(139,69,19,0.15)',
      marginTop: '5px',
      borderRadius: '2px'
    }}>
      {loading ? (
  ...
  return (
    <header dir="rtl" style={{ borderBottom: '2px solid var(--border-color)', background: 'var(--bg-main)', position: 'sticky', top: 0, zIndex: 100 }}>
      {/* Top decorative strip */}
      <div style={{ height: '4px', background: `repeating-linear-gradient(90deg, var(--primary) 0px, var(--primary) 8px, var(--bg-alt) 8px, var(--bg-alt) 16px)` }} />
      <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '75px' }}>

        <div style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
          <Link href="/" style={{ fontSize: '1.4rem', fontWeight: 900, color: 'var(--text-main)', letterSpacing: '-0.5px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.2rem' }}>
          <div style={{ position: 'relative', width: '250px' }} className="lg-block">
            <form onSubmit={handleSearchSubmit}>
              <input
                ...
                style={{ width: '100%', padding: '0.6rem 1rem 0.6rem 2.5rem', border: '1px solid var(--border-color)', background: 'var(--bg-alt)', borderRadius: '2px', outline: 'none', fontSize: '0.8rem', color: 'var(--text-main)' }}
              />
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
            <Link href="/cart" style={{ position: 'relative', color: 'var(--text-main)' }}>
              <ShoppingBag size={22} />
              {itemsCartCount > 0 && (
                <span style={{ position: 'absolute', top: -5, right: -8, background: 'var(--primary)', color: '#fff', fontSize: 9, width: 17, height: 17, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%' }}>
                  {itemsCartCount}
                </span>
              )}
            </Link>
      ...
      {open && (
        <div className="anim-fade-in" style={{ position: 'absolute', top: '79px', left: 0, right: 0, background: 'var(--bg-main)', borderBottom: '2px solid var(--border-color)', padding: '1.2rem', zIndex: 110 }}>
          <div style={{ position: 'relative', marginBottom: '1.5rem' }}>
            <form onSubmit={handleSearchSubmit}>
              <input
                ...
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                style={{ width: '100%', padding: '0.8rem 1rem', border: '1px solid var(--border-color)', borderRadius: '2px', background: 'var(--bg-alt)', outline: 'none', color: 'var(--text-main)' }}
              />
            </form>
            {searchQuery.length >= 2 && <SearchResults />}
          </div>
...
export function Footer({ store }: any) {
  return (
    <footer dir="rtl" style={{ borderTop: '2px solid var(--border-color)', padding: '4rem 0 2rem', background: '#3C2415' }}>
      <div className="container grid-3">
        <div>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '1rem', color: '#FDF6E3' }}>{store?.name}</h3>
          <p style={{ color: '#8B7355', fontSize: '0.9rem', lineHeight: 1.6 }}>أصالة الماضي وجماله في كل قطعة.</p>
        </div>
        <div>
          <h4 style={{ fontWeight: 700, marginBottom: '1rem', color: '#FDF6E3' }}>روابط</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.9rem', color: '#8B7355' }}>
        ...
        <div>
          <h4 style={{ fontWeight: 700, marginBottom: '1rem', color: '#FDF6E3' }}>تواصل</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.9rem', color: '#8B7355' }}>
      ...
      <div className="container" style={{ textAlign: 'center', marginTop: '3rem', paddingTop: '1.5rem', borderTop: '1px solid #5A3820', color: '#8B7355', fontSize: '0.8rem' }}>
...
export function Card({ product, displayImage, discount, store, viewDetails }: any) {
  ...
  return (
    <Link href={`/product/${product.slug || product.id}`} style={{ display: 'block' }} className="anim-fade-in">
      <div style={{ border: '1px solid var(--border-color)', borderRadius: '4px', overflow: 'hidden', background: 'var(--bg-main)', transition: 'box-shadow 0.2s', height: '100%', display: 'flex', flexDirection: 'column', boxShadow: '2px 2px 0 rgba(139,69,19,0.1)' }}>
        <div style={{ aspectRatio: '1/1', background: 'var(--bg-alt)', position: 'relative' }}>
          {displayImage ? <img src={displayImage} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'sepia(10%)' }} /> : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><ShoppingBag color="var(--border-color)" /></div>}
          {discount > 0 && <span style={{ position: 'absolute', top: 10, right: 10, background: 'var(--primary)', color: '#fff', fontSize: '10px', padding: '4px 8px', borderRadius: '2px', fontWeight: 700 }}>-{discount}%</span>}
        </div>
    ...
    <div dir="rtl">
      ...
      <section style={{
        background: store.hero.imageUrl
          ? `linear-gradient(rgba(60, 36, 21, 0.6), rgba(60, 36, 21, 0.5)), url(${store.hero.imageUrl})`
          : '#F5EACC',
        ...
        {!store.hero.imageUrl && (
          <>
            <div style={{ position: 'absolute', top: 16, right: 16, width: 40, height: 40, borderTop: '2px solid var(--border-color)', borderRight: '2px solid var(--border-color)' }} />
            <div style={{ position: 'absolute', top: 16, left: 16, width: 40, height: 40, borderTop: '2px solid var(--border-color)', borderLeft: '2px solid var(--border-color)' }} />
            <div style={{ position: 'absolute', bottom: 16, right: 16, width: 40, height: 40, borderBottom: '2px solid var(--border-color)', borderRight: '2px solid var(--border-color)' }} />
            <div style={{ position: 'absolute', bottom: 16, left: 16, width: 40, height: 40, borderBottom: '2px solid var(--border-color)', borderLeft: '2px solid var(--border-color)' }} />
          </>
        )}
        <div className="container anim-fade-in" style={{ width: '100%', position: 'relative', zIndex: 1 }}>
          <p style={{ fontSize: '0.8rem', letterSpacing: '0.3em', color: store.hero.imageUrl ? '#D4C09A' : 'var(--text-muted)', marginBottom: '1rem', textTransform: 'uppercase' }}>
            ✦ مجموعة كلاسيكية ✦
          </p>
          <h1 style={{
            fontSize: 'clamp(2rem, 5vw, 3.5rem)',
            color: store.hero.imageUrl ? '#FDF6E3' : '#3C2415',
            fontWeight: 700,
            marginBottom: '1.2rem',
            letterSpacing: '-0.5px',
          }}>
            {store.hero?.title?.replace(/<[^>]+>/g, '') || 'روح الماضي الجميل'}
          </h1>

          <p style={{
            fontSize: '1.1rem',
            color: store.hero.imageUrl ? '#F5EACC' : 'var(--text-muted)',
            maxWidth: '560px',
            margin: '0 auto 2.5rem',
            lineHeight: '1.8',
          }}>
            {store.hero?.subtitle || 'قطع فريدة تحمل عبق الزمن وأناقة العصور الخالية.'}
          </p>
      ...
      <section id="products" className="container" style={{ padding: '4rem 1.5rem' }}>
        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 700 }}>المقتنيات الكلاسيكية</h2>
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem', marginTop: '0.5rem' }}>
            <span style={{ height: '1px', width: '40px', background: 'var(--border-color)', display: 'block' }} />
            ...
            {Array.from({ length: countPage }).map((_, i) => {
              ...
              return (
                <Link key={pn} href={{ query: { page: pn } }} scroll={false} style={{
                  ...
                  borderRadius: '2px', border: `1px solid ${isActive ? 'var(--primary)' : 'var(--border-color)'}`,
      ...
      <section style={{ borderTop: '1px solid var(--border-color)', borderBottom: '1px solid var(--border-color)', background: 'var(--bg-alt)' }}>
        <div className="container grid-3" style={{ padding: '3rem 1.5rem' }}>
          {[
            ...
            { icon: <ShieldCheck size={20} />, t: 'جودة كلاسيكية', d: 'منتجات أصيلة' },
            { icon: <CheckCircle2 size={20} />, t: 'تسوق آمن', d: 'دفع عند الاستلام' },
          ].map((item, idx) => (
            <div key={idx} style={{ textAlign: 'center' }}>
              <div style={{ display: 'inline-flex', padding: '1rem', background: 'var(--bg-main)', borderRadius: '2px', marginBottom: '1rem', color: 'var(--primary)', border: '1px solid var(--border-color)' }}>{item.icon}</div>
  ...
  return (
    <div dir="rtl" className="container" style={{ padding: '3rem 1.5rem' }}>
      <div className="grid-2" style={{ gap: '3rem', alignItems: 'start' }}>
        ...
        {/* Gallery */}
        <div>
          <div style={{ aspectRatio: '1/1', background: 'var(--bg-alt)', borderRadius: '4px', overflow: 'hidden', position: 'relative', border: '1px solid var(--border-color)' }}>
            {allImages[sel] ? <img src={allImages[sel]} style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'sepia(8%)' }} alt="" /> : null}
            {discount > 0 && <span style={{ position: 'absolute', top: 12, right: 12, background: 'var(--primary)', color: '#fff', fontSize: '12px', padding: '4px 10px', borderRadius: '2px' }}>خصم {discount}%</span>}
          </div>
          {allImages.length > 1 && (
            <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem', overflowX: 'auto' }}>
              {allImages.map((img: string, idx: number) => (
                <button key={idx} onClick={() => setSel(idx)} style={{ width: 60, height: 60, flexShrink: 0, border: `2px solid ${sel === idx ? 'var(--primary)' : 'var(--border-color)'}`, borderRadius: '2px', padding: 0, cursor: 'pointer', overflow: 'hidden' }}>
        ...
        <div>
          ...
          <div style={{ fontSize: '2rem', fontWeight: 700, marginBottom: '1.5rem', color: 'var(--primary)' }}>{finalPrice.toLocaleString()} <span style={{ fontSize: '1rem', color: 'var(--text-muted)' }}>دج</span></div>
          ...
          {product.offers?.length > 0 && (
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              {product.offers.map((o: any) => (
                <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '1rem', border: `2px solid ${selectedOffer === o.id ? 'var(--primary)' : 'var(--border-color)'}`, borderRadius: '2px', marginBottom: '0.5rem', cursor: 'pointer', background: selectedOffer === o.id ? 'var(--bg-alt)' : 'transparent' }}>
    ...
    <div>
      <form onSubmit={handleSubmit}>
        <div style={{ background: 'var(--bg-alt)', padding: '1.5rem', borderRadius: '4px', marginBottom: '1.5rem', border: '1px solid var(--border-color)' }}>
          ...
          <FR label="طريقة التوصيل">
            <div className="grid-2">
              {(['home', 'office'] as const).map(t => (
                <button key={t} type="button" onClick={() => setFd({ ...fd, typeLivraison: t })} className="btn-outline" style={{ borderColor: fd.typeLivraison === t ? 'var(--primary)' : 'var(--border-color)', background: fd.typeLivraison === t ? 'var(--bg-main)' : 'transparent', borderRadius: '2px' }}>
          ...
          <FR label="الكمية">
            <div style={{ display: 'flex', alignItems: 'center', border: '1px solid var(--border-color)', borderRadius: '2px', width: 'fit-content', background: 'var(--bg-main)' }}>
  ...
  return (
    <div className="container" style={{ padding: '4rem 1.5rem', minHeight: '70vh' }}>
      ...
      <div className="grid-2" style={{ gap: '3rem', alignItems: 'start' }}>
        <div>
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '1rem', padding: '1rem', border: '1px solid var(--border-color)', borderRadius: '4px', marginBottom: '1rem' }}>
              <img src={item.product?.productImage} alt="" style={{ width: 80, height: 80, objectFit: 'cover', borderRadius: '2px', filter: 'sepia(8%)' }} />
              <div style={{ flex: 1 }}>
    ...
    <div className="container" style={{ padding: '5rem 1.5rem', maxWidth: '450px', margin: '0 auto' }} dir="rtl">
      {sent ? (
        <div style={{ textAlign: 'center', padding: '2rem 0' }} className="anim-fade-in">
          <div style={{ display: 'inline-flex', padding: '1rem', background: 'var(--bg-alt)', borderRadius: '2px', marginBottom: '1.5rem', border: '1px solid var(--border-color)' }}>
            <CheckCircle2 size={32} color="var(--primary)" />
          </div>
          <h2 style={{ fontSize: '1.25rem', fontWeight: 900, marginBottom: '0.5rem' }}>تم الإرسال</h2>
        ...
        <div>
          <div style={{ marginBottom: '2.5rem' }}>
            <h1 style={{ fontSize: '1.5rem', fontWeight: 900, marginBottom: '0.5rem' }}>تواصل معنا</h1>
