...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  const Drop = () => (
    <div style={{position:'absolute',top:'calc(100% + 6px)',right:0,left:0,background:'#fff',border:'1px solid var(--line-dk)',borderRadius:'6px',boxShadow:'0 12px 40px rgba(107,45,139,0.12)',zIndex:200,overflow:'hidden'}}>
      {loading ? <div style={{padding:'1.25rem',textAlign:'center',color:'var(--plum)',fontSize:'13px'}}>جاري البحث...</div>
      : ls.length > 0 ? ls.map((p:any) => (
        <Link href={`/product/${p.id}`} key={p.id} onClick={()=>setSq('')}
          ...
          <img src={p.productImage||p.imagesProduct?.[0]?.imageUrl} style={{width:38,height:38,borderRadius:4,objectFit:'cover',flexShrink:0}} alt=""/>
          <div>
            <div style={{fontSize:'13px',fontWeight:600,color:'var(--ink)'}}>{p.name}</div>
            <div style={{fontSize:'12px',color:'var(--plum)',fontWeight:700}}>{p.price} دج</div>
          </div>
        </Link>
      )) : sq.length >= 2 && <div style={{padding:'1.25rem',textAlign:'center',color:'var(--dim)',fontSize:'13px'}}>لا توجد نتائج</div>}
    </div>
  );
  ...
  return (
    <header dir="rtl" style={{fontFamily:"'DM Sans',sans-serif",position:'sticky',top:0,zIndex:100}}>
      ...
      <div style={{backgroundColor:'var(--white)',borderBottom:'1px solid var(--line)',boxShadow:scrolled?'0 2px 12px rgba(107,45,139,0.08)':'none',transition:'box-shadow 0.3s'}}>
        <div className="nav-top" style={{maxWidth:'1280px',margin:'0 auto',padding:'0 24px',height:'70px'}}>

          {/* Logo */}
          <Link href="/" style={{display:'inline-flex',flexDirection:'column',alignItems:'center',flexShrink:0}}>
            {(store.design.logoUrl && store.design.logoUrl !== '/default-logo.png')
              ...
              : <>
                  <span className="serif" style={{fontSize:'1.5rem',fontWeight:700,fontStyle:'italic',color:'var(--plum)',lineHeight:1,letterSpacing:'0.02em'}}>{store.name}</span>
                  <span style={{fontSize:'9px',letterSpacing:'0.22em',textTransform:'uppercase',color:'var(--dim)',marginTop:'2px'}}>Beauty Supply</span>
                </>
            }
          </Link>

          {/* Desktop: links + search + cart */}
          <div style={{display:'flex',alignItems:'center',gap:'4px'}}>
            ...
            {/* Desktop search */}
            <div className="nav-links" style={{position:'relative',marginLeft:'8px'}}>
              <form onSubmit={doSearch} style={{position:'relative'}}>
                <input type="text" placeholder="بحث..." value={sq} onChange={e=>setSq(e.target.value)}
                  style={{...INP_ST(),width:'200px',padding:'7px 32px 7px 12px',borderRadius:20}}
            ...
            {/* Cart icon */}
            <Link href="/cart" style={{position:'relative',width:38,height:38,display:'flex',alignItems:'center',justifyContent:'center',border:'1.5px solid var(--line-dk)',borderRadius:'4px',color:'var(--plum)',marginLeft:'4px',transition:'all 0.2s'}}
              onMouseEnter={e=>{const el=e.currentTarget as HTMLAnchorElement;el.style.background='var(--soft)';el.style.borderColor='var(--plum)';}}
              onMouseLeave={e=>{const el=e.currentTarget as HTMLAnchorElement;el.style.background='transparent';el.style.borderColor='var(--line-dk)';}}>
              <ShoppingBag size={17}/>
              {count > 0 && <span className="cart-badge">{count}</span>}
            </Link>

            {/* Mobile toggle */}
            <button className="nav-mob" onClick={()=>setOpen(p=>!p)}
              style={{background:'none',border:'1.5px solid var(--line-dk)',cursor:'pointer',color:'var(--plum)',padding:'7px',borderRadius:'4px',alignItems:'center',justifyContent:'center',marginRight:'8px'}}>
      ...
      {/* Mobile search + menu */}
      <div style={{maxHeight:open?'300px':'0',overflow:'hidden',transition:'max-height 0.3s ease',backgroundColor:'var(--white)',borderBottom:open?'1px solid var(--line)':'none'}}>
        <div style={{padding:'12px 24px 16px',display:'flex',flexDirection:'column',gap:'12px'}}>
          {/* Search */}
          <form onSubmit={doSearch} style={{position:'relative'}}>
            <input type="text" placeholder="ابحث عن منتج..." value={sq} onChange={e=>setSq(e.target.value)} style={{...INP_ST(),padding:'10px 36px 10px 12px'}}/>
            <Search size={15} style={{position:'absolute',left:12,top:'50%',transform:'translateY(-50%)',color:'var(--dim)',pointerEvents:'none'}}/>
          </form>
          {sq.length >= 2 && <Drop/>}
          {/* Nav links */}
          {[...navLinks, {href:'/cart',label:'السلة'}].map(l=>(
            <Link key={l.href} href={l.href} onClick={()=>setOpen(false)}
              style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 0',fontSize:'15px',fontWeight:500,color:'var(--mid)',borderBottom:'1px solid var(--line)'}}>
      ...
      <div style={{borderBottom:'1px solid var(--line)',padding:'10px 24px',fontSize:'13px',color:'var(--dim)'}}>
        <div style={{maxWidth:'1280px',margin:'0 auto',display:'flex',alignItems:'center',gap:'6px'}}>
          <Link href="/" style={{textDecoration:'none',color:'var(--mid)',transition:'color 0.2s'}}
            ...
            onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.color='var(--mid)';}}>الرئيسية</Link>
          <span>›</span>
          ...
          {sent ? (
            ...
            <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:'14px'}}>
              ...
              <button type="submit" disabled={loading} className="btn-plum" style={{justifyContent:'center',width:'100%',fontSize:'14px',padding:'12px',opacity:loading?0.7:1,cursor:loading?'not-allowed':'pointer'}}>
                {loading?<><Loader2 style={{width:'15px',height:'15px',animation:'spin 1s linear infinite'}}/> جاري...</>:<>إرسال الرسالة <ArrowRight style={{width:'14px',height:'14px'}}/></>}
              </button>
            </form>
          )}
        </div>
