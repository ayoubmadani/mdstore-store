...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  const SearchResults = () => (
    <div style={{
      ...
      {loading ? (
        <div style={{ padding: '1.5rem', textAlign: 'center', color: '#8A80A0' }}>جاري البحث...</div>
      ) : listSearch.length > 0 ? (
        <>
        {listSearch.map((p: any) => (
          ...
          >
            ...
            <div>
              ...
              <div style={{ fontSize: '0.8rem', color: '#00D4FF' }}>{p.price} دج</div>
            </div>
          </Link>
        ))}
        ...
        <div style={{ padding: '1.5rem', textAlign: 'center', color: '#8A80A0' }}>لا توجد نتائج</div>
      )}
    </div>
  );
      ...
      <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '75px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
          ...
          <nav className="lg-flex" style={{ gap: '1.5rem', fontSize: '0.85rem', fontWeight: 600 }}>
            <Link href="/" style={{ color: '#E8E0F0' }}>الرئيسية</Link>
            <Link href="/contact" style={{ color: '#E8E0F0' }}>تواصل معنا</Link>
          </nav>
        </div>
    ...
    <footer dir="rtl" style={{ borderTop: '1px solid #252040', padding: '4rem 0 2rem', background: '#07071A' }}>
      <div className="container grid-3">
        <div>
          ...
          <p style={{ color: '#8A80A0', fontSize: '0.9rem', lineHeight: 1.6 }}>متجرك الأول لمعدات الألعاب والإلكترونيات. تسوق واهزم المنافسين.</p>
        </div>
        <div>
          <h4 style={{ fontWeight: 700, marginBottom: '1rem', color: '#E8E0F0' }}>روابط</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.9rem' }}>
            <Link href="/" style={{ color: '#8A80A0' }}>الرئيسية</Link>
            <Link href="/contact" style={{ color: '#8A80A0' }}>تواصل معنا</Link>
            <Link href="/Privacy" style={{ color: '#8A80A0' }}>سياسة الخصوصية</Link>
            <Link href="/Terms" style={{ color: '#8A80A0' }}>الشروط والأحكام</Link>
          </div>
        </div>
        <div>
          <h4 style={{ fontWeight: 700, marginBottom: '1rem', color: '#E8E0F0' }}>تواصل</h4>
      ...
      <div className="epic-card" style={{ border: `1px solid #252040`, borderRadius: '8px', overflow: 'hidden', background: '#15152A', transition: 'border-color 0.3s', height: '100%', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '1rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <span style={{ background: 'linear-gradient(90deg, #7B2FBE, #00D4FF)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', fontWeight: 700, fontSize: '1.1rem' }}>{price.toLocaleString()} {store.currency || 'دج'}</span>
  ...
  return (
    <div dir="rtl">
      ...
      <section style={{
        ...
        <div className="container anim-fade-in" style={{ width: '100%' }}>
          ...
          <p style={{
            ...
            {store.hero?.subtitle || 'معدات الألعاب الاحترافية لتجربة إي-سبورتس لا مثيل لها.'}
          </p>
      ...
      <section id="products" className="container" style={{ padding: '4rem 1.5rem' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 700, marginBottom: '2rem', color: '#E8E0F0' }}>أحدث المنتجات</h2>

        {products.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '4rem', background: '#15152A', borderRadius: '8px' }}>
            <p style={{ color: '#8A80A0' }}>لا توجد منتجات حالياً</p>
          </div>
        ) : (
      ...
      <section style={{ borderBottom: '1px solid #252040' }}>
        <div className="container grid-3" style={{ padding: '3rem 1.5rem' }}>
          {[
            { icon: <Truck size={20} color="#00D4FF" />, t: 'توصيل سريع', d: 'لجميع الولايات' },
            { icon: <ShieldCheck size={20} color="#00D4FF" />, t: 'جودة مضمونة', d: 'منتجات أصلية' },
            { icon: <CheckCircle2 size={20} color="#00D4FF" />, t: 'دفع آمن', d: 'دفع عند الاستلام' },
          ].map((item, idx) => (
      ...
      <div className="grid-2" style={{ gap: '3rem', alignItems: 'start' }}>
        ...
        <div>
          <div style={{ aspectRatio: '1/1', background: '#15152A', borderRadius: '8px', overflow: 'hidden', position: 'relative' }}>
            ...
            {discount > 0 && <span style={{ position: 'absolute', top: 12, right: 12, background: '#7B2FBE', color: '#fff', fontSize: '12px', padding: '4px 10px', borderRadius: '4px' }}>خصم {discount}%</span>}
          </div>
          {allImages.length > 1 && (
        ...
        <div>
          ...
          <div style={{ fontSize: '2rem', fontWeight: 700, marginBottom: '1.5rem', background: 'linear-gradient(90deg, #7B2FBE, #00D4FF)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>{finalPrice.toLocaleString()} <span style={{ fontSize: '1rem', color: '#8A80A0', WebkitTextFillColor: '#8A80A0' }}>دج</span></div>

          {/* Offers */}
          {product.offers?.length > 0 && (
            <div style={{ marginBottom: '1.5rem' }}>
              <p style={{ fontWeight: 600, marginBottom: '0.5rem', color: '#E8E0F0' }}>العروض:</p>
              {product.offers.map((o: any) => (
                <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '1rem', border: `1px solid ${selectedOffer === o.id ? '#7B2FBE' : '#252040'}`, borderRadius: '6px', marginBottom: '0.5rem', cursor: 'pointer', background: selectedOffer === o.id ? '#15152A' : 'transparent' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    ...
                    <span style={{ color: '#E8E0F0' }}>{o.name} <span style={{ color: '#8A80A0', fontSize: '0.85rem' }}>(الكمية: {o.quantity})</span></span>
                  </div>
                  <span style={{ fontWeight: 600, color: '#00D4FF' }}>{o.price.toLocaleString()} دج</span>
                </label>
              ))}
            </div>
          )}
          ...
          {product.desc && (
            <div style={{ marginTop: '3rem' }}>
              <h3 style={{ fontSize: '1.1rem', fontWeight: 600, marginBottom: '1rem', color: '#E8E0F0' }}>وصف المنتج</h3>
    ...
    <div>
      <form onSubmit={handleSubmit}>
        <div style={{ background: '#15152A', padding: '1.5rem', borderRadius: '8px', marginBottom: '1.5rem', border: '1px solid #252040' }}>
          <h3 style={{ fontSize: '1.1rem', fontWeight: 600, marginBottom: '1rem', color: '#E8E0F0' }}>الطلب السريع</h3>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', fontSize: '1.25rem', fontWeight: 700, color: '#E8E0F0' }}>
          <span>المجموع:</span>
          <span style={{ color: '#00D4FF' }}>{total().toLocaleString()} دج</span>
        </div>
...
export function Cart({ domain, store }: { domain: string; store: any }) {
  ...
  if (!items.length) return (
    <div className="container" style={{ textAlign: 'center', padding: '6rem 1.5rem', minHeight: '60vh' }}>
      ...
      <h2 style={{ marginBottom: '1rem', color: '#E8E0F0' }}>السلة فارغة</h2>
  ...
  return (
    <div className="container" style={{ padding: '4rem 1.5rem', minHeight: '70vh' }}>
      <h1 style={{ fontSize: '2rem', fontWeight: 700, marginBottom: '2rem', color: '#E8E0F0' }}>سلة المشتريات</h1>
      <div className="grid-2" style={{ gap: '3rem', alignItems: 'start' }}>
        <div>
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '1rem', padding: '1rem', border: '1px solid #252040', borderRadius: '8px', marginBottom: '1rem', background: '#15152A' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontWeight: 700, color: '#00D4FF' }}>{item.finalPrice} دج <span style={{ fontSize: '0.8rem', color: '#8A80A0', fontWeight: 400 }}>x {item.quantity}</span></div>
              </div>
        ...
        <div style={{ background: '#15152A', padding: '2rem', borderRadius: '8px', border: '1px solid #252040' }}>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 600, marginBottom: '1.5rem', color: '#E8E0F0' }}>ملخص السلة</h3>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1.25rem', fontWeight: 700, marginBottom: '1.5rem', color: '#E8E0F0' }}>
            <span>الإجمالي:</span>
            <span style={{ color: '#00D4FF' }}>{total.toLocaleString()} دج</span>
          </div>
          <p style={{ color: '#8A80A0', fontSize: '0.85rem', marginBottom: '1rem' }}>* تكاليف التوصيل سيتم احتسابها لاحقاً.</p>
...
export function Privacy() {
  return (
    <div className="container" style={{ padding: '4rem 1.5rem', maxWidth: '800px' }}>
      <h1 style={{ fontSize: '2rem', fontWeight: 700, marginBottom: '1rem', color: '#E8E0F0' }}>سياسة الخصوصية</h1>
      <SimpleDivider />
      <div style={{ lineHeight: 1.8, color: '#8A80A0' }}>
        <h3 style={{ color: '#E8E0F0', marginTop: '2rem', marginBottom: '0.5rem' }}>1. جمع المعلومات</h3>
        <p>نقوم بجمع المعلومات الشخصية الضرورية فقط لإتمام طلبك وتوصيله (الاسم، العنوان، رقم الهاتف).</p>

        <h3 style={{ color: '#E8E0F0', marginTop: '2rem', marginBottom: '0.5rem' }}>2. الاستخدام</h3>
...
export function Terms() {
  return (
    <div className="container" style={{ padding: '4rem 1.5rem', maxWidth: '800px' }}>
      <h1 style={{ fontSize: '2rem', fontWeight: 700, marginBottom: '1rem', color: '#E8E0F0' }}>الشروط والأحكام</h1>
      <SimpleDivider />
      <div style={{ lineHeight: 1.8, color: '#8A80A0' }}>
    ...
    <div className="container" style={{ padding: '5rem 1.5rem', maxWidth: '450px', margin: '0 auto' }} dir="rtl">
      {sent ? (
        <div style={{ textAlign: 'center', padding: '2rem 0' }} className="anim-fade-in">
          ...
          <h2 style={{ fontSize: '1.25rem', fontWeight: 900, marginBottom: '0.5rem', color: '#E8E0F0' }}>تم الإرسال</h2>
          <p style={{ color: '#8A80A0', fontSize: '0.9rem', marginBottom: '2rem' }}>شكراً لتواصلك معنا. سنقوم بالرد عليك في أقرب وقت.</p>
          <button
        ...
        <div>
          <div style={{ marginBottom: '2.5rem' }}>
            <h1 style={{ fontSize: '1.5rem', fontWeight: 900, marginBottom: '0.5rem', color: '#E8E0F0', letterSpacing: '-0.5px' }}>تواصل معنا</h1>
            <p style={{ color: '#8A80A0', fontSize: '0.85rem' }}>املأ النموذج وسنرد عليك قريباً.</p>
          </div>
