        ...
        <div className="glb-search-ov" onClick={e => { if (e.target === e.currentTarget) setShowSearch(false); }}>
          <div className="glb-search-panel">
            ...
            <form className="glb-search-form" onSubmit={handleSearch}>
              ...
              <input ref={searchInputRef} className="glb-search-input" type="text" placeholder="ابحث عن جهاز أو منتج تقني..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            ...
            {loading && <p style={{ textAlign: 'center', color: A2, fontSize: '0.85rem', fontWeight: 600, padding: '2rem' }}>جاري البحث...</p>}
                ...
                {listSearch.map((p: any) => (
                  <Link key={p.id} href={`/product/${p.slug || p.id}`} className="glb-search-card" onClick={() => setShowSearch(false)}>
                    ...
                    <div className="glb-search-card-info">
                      ...
                      <p className="price-mono" style={{ fontSize: '0.8rem', fontWeight: 700, color: A2 }}>{Number(p.price).toLocaleString()} دج</p>
                    </div>
                  </Link>
                ))}
              </div>
            ...
            {!loading && searchQuery.length >= 2 && listSearch.length === 0 && (
              <p style={{ textAlign: 'center', color: '#3D4458', fontSize: '0.9rem', padding: '3rem' }}>لا توجد نتائج</p>
            )}
          </div>
        </div>
      )}
...
export function Footer({ store }: any) {
  return (
    <footer dir="rtl" style={{ background: '#070A12', color: 'rgba(241,244,250,0.5)', marginTop: 80, padding: '4rem 1.5rem 1.5rem', borderTop: `1px solid ${BD}` }}>
        ...
        <div className="footer-inner">
          <div>
            ...
            <p style={{ fontSize: '0.875rem', lineHeight: 1.8, maxWidth: 300 }}>
              {store?.hero?.subtitle?.substring(0, 100) || 'أحدث الأجهزة الإلكترونية والتقنيات الذكية بمعايير جودة واختبار صارمة.'}
            </p>
          ...
          <div>
            <h4 className="eyebrow" style={{ fontSize: '0.68rem', fontWeight: 700, color: A2, textTransform: 'uppercase' as const, letterSpacing: '0.1em', marginBottom: '1.25rem' }}>الصفحات</h4>
          ...
          <div>
            <h4 className="eyebrow" style={{ fontSize: '0.68rem', fontWeight: 700, color: A2, textTransform: 'uppercase' as const, letterSpacing: '0.1em', marginBottom: '1.25rem' }}>تواصل</h4>
            {[
    ...
    <div dir="rtl">
        ...
        <div style={{ position: 'relative', maxWidth: 1280, margin: '0 auto', width: '100%', padding: 'clamp(5rem,10vw,8rem) 1.5rem clamp(3rem,6vw,5rem)' }}>
          <div style={{ maxWidth: 640 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: AL, border: `1px solid rgba(110,91,255,0.4)`, padding: '0.35rem 0.875rem', marginBottom: '1.25rem' }}>
              ...
              <span className="eyebrow" style={{ fontSize: '0.7rem', fontWeight: 600, color: A2 }}>ابتكار تقني</span>
            </div>
            <h1 style={{ fontSize: 'clamp(2rem,5.5vw,3.75rem)', fontWeight: 800, color: TXT, lineHeight: 1.15, letterSpacing: '-0.02em', marginBottom: '1rem' }}
              dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(store.hero?.title || 'تقنية المستقبل<br/>بين <span style="color:#6E5BFF">يديك</span>') }} />
            <p style={{ fontSize: '1rem', color: SUB, lineHeight: 1.75, marginBottom: '2.25rem', maxWidth: 480 }}>
              {store.hero?.subtitle || 'أحدث الأجهزة الإلكترونية والتقنيات الذكية المختارة بعناية لتواكب عصر الابتكار.'}
            </p>
            <div className="hero-actions">
              <a href="#products" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: A, color: '#fff', fontWeight: 700, fontSize: '0.9rem', padding: '0.875rem 1.875rem', borderRadius: 0, transition: 'all 0.2s' }}
                onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.background = AD; }}
                onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.background = A; }}>
                اكتشف المنتجات <ArrowLeft size={16} />
              </a>
              {store?.cart !== false && (
            ...
            <div style={{ display: 'flex', gap: '2rem', marginTop: '3rem', flexWrap: 'wrap' }}>
              {[
                { icon: <Cpu size={14} />, val: '+300', lbl: 'منتج تقني' },
                { icon: <Truck size={14} />, val: '58', lbl: 'ولاية' },
                { icon: <Shield size={14} />, val: '2', lbl: 'سنة ضمان' },
              ].map((s, i) => (
      ...
      <div style={{ background: CARD, borderBottom: `1px solid ${BD}`, borderTop: `1px solid ${BD}` }}>
        <div className="trust-grid" style={{ maxWidth: 1280, margin: '0 auto' }}>
          {[
            { icon: <ScanLine size={18} />, t: 'فحص جودة دقيق', d: 'اختبار كل جهاز' },
            { icon: <Truck size={18} />, t: 'توصيل سريع', d: '58 ولاية جزائرية' },
            { icon: <Shield size={18} />, t: 'ضمان الوكيل', d: 'حماية موثوقة' },
            { icon: <Headphones size={18} />, t: 'دعم تقني', d: 'فريق مختص' },
          ].map((item, i) => (
      ...
      {cats.length > 0 && (
        <section style={{ padding: '2.5rem 1.5rem', maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
            ...
            <p className="eyebrow" style={{ fontSize: '0.75rem', fontWeight: 600, color: SUB, letterSpacing: '0.04em' }}>التصنيفات</p>
          </div>
      ...
      <section id="products" style={{ padding: '0.5rem 1.5rem 5rem', maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
          ...
          <h2 style={{ fontSize: 'clamp(1.25rem,3vw,1.75rem)', fontWeight: 700, color: TXT }}>المنتجات</h2>
        </div>
        {products.length === 0 ? (
      ...
      <div className="details-inner" style={{ maxWidth: 1280, margin: '0 auto' }}>
        <div className="gallery-container">
          <div style={{ position: 'relative', aspectRatio: '1/1', overflow: 'hidden', background: '#0E1424', border: `1px solid ${BD}` }}>
            ...
            {discount > 0 && (
              <div style={{ position: 'absolute', top: 12, right: 12, background: A, color: '#fff', padding: '3px 10px', fontSize: 12, fontWeight: 800 }}>{discount}% خصم</div>
            )}
            {allImages.length > 1 && (
              <>
        ...
        <div>
          <div className="info-container">
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: AL, border: `1px solid rgba(110,91,255,0.3)`, padding: '0.25rem 0.625rem', marginBottom: '0.875rem' }}>
              ...
              <span className="eyebrow" style={{ fontSize: '0.65rem', fontWeight: 700, color: A2 }}>جهاز تقني معتمد</span>
            </div>
            ...
            <div style={{ background: AL, border: `1px solid rgba(110,91,255,0.25)`, padding: '0.875rem 1.125rem', marginBottom: '1.5rem' }}>
              <p className="eyebrow" style={{ fontSize: '0.65rem', color: A2, fontWeight: 700, marginBottom: '0.2rem', textTransform: 'uppercase' as const, letterSpacing: '0.08em' }}>السعر</p>
              ...
              <div style={{ marginBottom: '1.25rem' }}>
                {product.offers.map((o: any) => (
                  <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 1rem', border: `1px solid ${selectedOffer === o.id ? A : BD}`, cursor: 'pointer', marginBottom: '0.5rem', background: selectedOffer === o.id ? AL : 'transparent', transition: 'all 0.18s' }}>
                    ...
                    <span className="price-mono" style={{ fontWeight: 800, color: A2, fontSize: '1.05rem' }}>{o.price.toLocaleString()} دج</span>
                  </label>
                ))}
              </div>
            )}
        ...
        <div className="anim-slide-fade">
          {product.store.cart && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <p className="eyebrow" style={{ fontWeight: 600, fontSize: '0.72rem', color: SUB, textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>بيانات التوصيل</p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: '0.875rem' }}>
              <p className="eyebrow" style={{ fontSize: '0.72rem', fontWeight: 600, color: SUB, marginBottom: '0.5rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>التوصيل</p>
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.75rem', border: `1px solid ${fd.typeLivraison === t ? A : BD}`, borderRadius: 0, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? AL : 'transparent', transition: 'all 0.18s', fontFamily: 'inherit' }}>
                    <p style={{ fontWeight: 600, fontSize: '0.82rem', marginBottom: 4, color: fd.typeLivraison === t ? A2 : SUB }}>{t === 'home' ? 'للبيت' : 'للمكتب'}</p>
                    {selW && <p className="price-mono" style={{ fontSize: '1rem', fontWeight: 800, color: fd.typeLivraison === t ? A2 : SUB }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} دج</p>}
                  </button>
                ))}
              </div>
            </div>
            <div style={{ marginBottom: '0.875rem' }}>
              <p className="eyebrow" style={{ fontSize: '0.72rem', fontWeight: 600, color: SUB, marginBottom: '0.5rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>الكمية</p>
            ...
            <div style={{ background: AL, border: `1px solid rgba(110,91,255,0.25)`, padding: '0.875rem 1rem', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '0.25rem' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.75rem', fontWeight: 800, color: A2 }}>{total().toLocaleString()} <span style={{ fontSize: '0.8rem', fontWeight: 700 }}>دج</span></span>
              </div>
            </div>
  ...
  if (success) return (
    <div dir="rtl" style={{ minHeight: '70vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: BG }}>
      <div style={{ textAlign: 'center', background: CARD, padding: '3.5rem 2rem', border: `1px solid ${BD}`, borderTop: `3px solid ${A}`, maxWidth: 460, width: '100%' }}>
        ...
        <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: TXT, marginBottom: '0.625rem' }}>تم استلام طلبك!</h2>
        <p style={{ color: SUB, lineHeight: 1.7, marginBottom: '2rem' }}>سنتواصل معك قريباً لتأكيد الطلب.</p>
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: A, color: '#fff', padding: '0.8rem 2rem', fontWeight: 700, fontSize: '0.9rem' }}>العودة للمتجر</Link>
      </div>
    </div>
  );
  ...
  if (!items.length) return (
    <div dir="rtl" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: BG }}>
      <div style={{ textAlign: 'center', padding: '4rem 2rem', border: `1px dashed ${BD}`, maxWidth: 440, width: '100%', background: CARD }}>
        ...
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: A, color: '#fff', padding: '0.75rem 1.75rem', fontWeight: 700, fontSize: '0.875rem' }}>تسوق الآن</Link>
      </div>
    </div>
  );
  ...
  return (
    <div dir="rtl" style={{ padding: '2.5rem 1.5rem', maxWidth: 1280, margin: '0 auto', minHeight: '100vh', background: BG }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '2rem' }}>
        ...
        <h1 style={{ fontSize: 'clamp(1.5rem,5vw,2.25rem)', fontWeight: 700, color: TXT }}>السلة</h1>
      </div>
      <div className="cart-inner">
        <div style={{ background: CARD, border: `1px solid ${BD}`, overflow: 'hidden', alignSelf: 'start' }}>
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '0.875rem', padding: '0.875rem', borderBottom: `1px solid ${BD}` }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p className="price-mono" style={{ fontSize: '1.05rem', fontWeight: 800, color: A2 }}>{item.finalPrice?.toLocaleString()} دج</p>
          ...
          <div style={{ padding: '0.875rem', background: AL, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span className="price-mono" style={{ fontSize: '1.25rem', fontWeight: 800, color: A2 }}>{cartTotal.toLocaleString()} دج</span>
          </div>
        </div>
        <div style={{ background: CARD, border: `1px solid ${BD}`, overflow: 'hidden', padding: '1.5rem', alignSelf: 'start' }}>
          <h3 style={{ fontWeight: 700, fontSize: '0.875rem', color: TXT, marginBottom: '1.25rem' }}>معلومات التوصيل</h3>
          <form onSubmit={handleSubmit}>
            <div className="form-row-2">
            ...
            <div style={{ margin: '1rem 0' }}>
              <p className="eyebrow" style={{ fontSize: '0.72rem', fontWeight: 600, color: SUB, textTransform: 'uppercase' as const, marginBottom: '0.5rem', letterSpacing: '0.06em' }}>نوع التوصيل</p>
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.75rem', border: `1px solid ${fd.typeLivraison === t ? A : BD}`, borderRadius: 0, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? AL : 'transparent', fontFamily: 'inherit', transition: 'all 0.2s' }}>
                    <p style={{ fontWeight: 600, fontSize: '0.78rem', color: fd.typeLivraison === t ? A2 : SUB }}>{t === 'home' ? 'للبيت' : 'للمكتب'}</p>
                    {selW && <p className="price-mono" style={{ fontWeight: 800, fontSize: '0.9rem', color: fd.typeLivraison === t ? A2 : SUB, marginTop: 2 }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} دج</p>}
                  </button>
                ))}
              </div>
            </div>
            <div style={{ background: AL, border: `1px solid rgba(110,91,255,0.25)`, padding: '0.875rem 1rem', margin: '1rem 0' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                ...
                <span className="price-mono" style={{ fontSize: '2rem', fontWeight: 800, color: A2 }}>{finalTotal.toLocaleString()} <span style={{ fontSize: '0.8rem' }}>دج</span></span>
              </div>
            </div>
    ...
    <div dir="rtl" style={{ background: BG, minHeight: '100vh' }}>
      <div style={{ background: CARD, paddingTop: 96, paddingBottom: 48, paddingLeft: 24, paddingRight: 24, borderBottom: `1px solid ${BD}` }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <h1 style={{ fontSize: 'clamp(2rem,5vw,3rem)', fontWeight: 700, color: TXT, marginBottom: '0.5rem' }}>تواصل معنا</h1>
          <p style={{ color: SUB, fontSize: '0.9375rem' }}>فريقنا التقني متاح للإجابة على استفساراتكم</p>
        </div>
      </div>
      <div className="contact-inner" style={{ maxWidth: 1100, margin: '0 auto', padding: '4rem 1.5rem 6rem' }}>
        <div>
          ...
          <div style={{ background: AL, border: `1px solid rgba(110,91,255,0.25)`, padding: '0.875rem 1rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <span style={{ fontSize: '0.8rem', fontWeight: 600, color: A2 }}>نرد في غضون ساعة</span>
          </div>
        </div>
        <div style={{ background: CARD, border: `1px solid ${BD}`, padding: '2rem', overflow: 'hidden', position: 'relative' as const }}>
          ...
          {sent ? (
            <div style={{ textAlign: 'center', padding: '4rem 1rem' }}>
              <CheckCircle2 size={40} style={{ color: '#22C55E', display: 'block', margin: '0 auto 1.25rem' }} />
              <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: TXT, marginBottom: '0.5rem' }}>تم الإرسال!</h2>
              <p style={{ color: SUB, lineHeight: 1.7, marginBottom: '2rem' }}>سنرد عليك في أقرب وقت ممكن.</p>
              <button onClick={() => setSent(false)} style={{ padding: '0.75rem 1.75rem', borderRadius: 0, border: `1px solid ${A}`, background: AL, color: A2, fontWeight: 700, cursor: 'pointer', fontSize: '0.875rem', fontFamily: 'inherit' }}>إرسال رسالة أخرى</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
              <div className="form-row-2" style={{ marginBottom: '0.875rem' }}>
                <div>
                  <label className="eyebrow" style={{ display: 'block', fontSize: '0.72rem', fontWeight: 600, color: SUB, marginBottom: '0.3rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>الاسم</label>
                  <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required style={S.input} />
                </div>
                <div>
                  <label className="eyebrow" style={{ display: 'block', fontSize: '0.72rem', fontWeight: 600, color: SUB, marginBottom: '0.3rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>الهاتف</label>
                  <input type="tel" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} required style={S.input} />
                </div>
              </div>
              <div style={{ marginBottom: '0.875rem' }}>
                <label className="eyebrow" style={{ display: 'block', fontSize: '0.72rem', fontWeight: 600, color: SUB, marginBottom: '0.3rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>البريد الإلكتروني</label>
                <input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} required style={S.input} />
              </div>
              <div style={{ marginBottom: '1.5rem' }}>
                <label className="eyebrow" style={{ display: 'block', fontSize: '0.72rem', fontWeight: 600, color: SUB, marginBottom: '0.3rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>الرسالة</label>
