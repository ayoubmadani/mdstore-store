...
function FallingPaws() {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none" style={{ zIndex:0 }}>
      {paws.map(p => (
        <div key={p.id} style={{ position:'absolute', left:p.left, top:'-20px', opacity:0,
          ...
          <Link href={`/`} className="flex items-center gap-3 group">
            ...
            <div>
              ...
              <span className="block text-[9px] font-bold tracking-[0.22em] uppercase" style={{ color:'var(--terra)', marginTop:'-2px' }}>
                {isRTL?'🐾 متجر الحيوانات الأليفة':"🐾 Pet Lover's Paradise"}
              </span>
            </div>
          </Link>
    ...
    <footer className="relative paw-pattern" style={{ backgroundColor:'var(--cocoa)', overflow:'hidden', fontFamily:"'Nunito',sans-serif" }}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12 py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 pb-12" style={{ borderBottom:'1px solid rgba(255,255,255,0.08)' }}>
          <div className="space-y-5">
            ...
            <p className="text-sm font-medium leading-relaxed" style={{ color:'rgba(253,246,238,0.55)' }}>
              {isRTL?'كل ما يحتاجه حيوانك الأليف بين يديك. منتجات مختارة بحب 🐾':'Because every pet deserves the best. Curated with love for your furry family.'}
            </p>
          ...
          <div>
            <div className="p-6 rounded-2xl relative overflow-hidden" style={{ background:'rgba(224,122,95,0.12)', border:'1px solid rgba(224,122,95,0.2)' }}>
              ...
              <p className="font-bold leading-snug mb-2" style={{ fontFamily:"'Baloo 2',cursive", fontSize:'1rem', color:'var(--cream)' }}>
                {isRTL?'"عندما تحب حيوانك الأليف، يحبك العالم كله."':'"A dog is the only thing on earth that loves you more than it loves itself."'}
              </p>
        ...
        <div className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs font-semibold" style={{ color:'rgba(253,246,238,0.3)' }}>© {new Date().getFullYear()} {store.name} · {isRTL?'جميع الحقوق محفوظة':'All rights reserved'} 🐾</p>
...
export function Home({ store }: any) {
  ...
  const t = {
    badge:      isRTL?'🐾 عالم الحيوانات الأليفة'        :'🐾 Trusted by Pet Parents',
    heroTitle:  isRTL?'كل شيء\nيحبه حيوانك!'             :'Everything\nYour Pet Loves!',
    heroSub:    isRTL?'طعام، مستلزمات، عناية وأكثر — منتجات مختارة بحب لحيوانك الأليف السعيد.':'Premium food, grooming, toys & accessories — handpicked with love for your happy companions.',
    heroBtn:    isRTL?'🛒 تسوق المنتجات'                 :'🛒 Shop Products',
    heroBtn2:   isRTL?'🎁 العروض الخاصة'                 :'🎁 Special Deals',
    categories: isRTL?'تسوق حسب الفئة'                   :'Shop by Category',
    all:        isRTL?'الكل'                               :'All Pets',
    products:   isRTL?'منتجاتنا المفضلة'                 :'Our Favourite Picks',
    noProducts: isRTL?'لا توجد منتجات بعد 🐾'             :'No products yet 🐾',
    viewDetails:isRTL?'اعرف المزيد'                        :'View Details',
  };
  const pillars = [
    { emoji:'🥩', title:isRTL?'غذاء طبيعي':'Natural Food',  sub:isRTL?'مكونات عالية الجودة':'High-quality ingredients' },
    { emoji:'🏥', title:isRTL?'رعاية صحية':'Health Care',   sub:isRTL?'منتجات موصى بها':'Vet-recommended products'   },
    { emoji:'🎾', title:isRTL?'ألعاب وترفيه':'Fun & Play',  sub:isRTL?'لعبهم المفضلة':'Keep them entertained'        },
    { emoji:'✂️', title:isRTL?'عناية ونظافة':'Grooming',    sub:isRTL?'اجعله يلمع':'Make them shine'                },
  ];
  return (
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12 w-full">
          <div className="max-w-2xl">
            ...
            <div className="slide-up slide-up-d4 flex flex-wrap gap-6 mt-12 pt-8" style={{ borderTop:'2px dashed var(--border)' }}>
              {[{e:'🛡️',t:isRTL?'آمن':'Vet Approved'},{e:'🚀',t:isRTL?'سريع':'Fast Delivery'},{e:'❤️',t:isRTL?'موثوق':'Made with Love'}].map((b,i)=>(
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="text-center mb-10">
            <div className="flex items-center justify-center gap-3 mb-2">
              ...
              <span className="text-[11px] font-bold tracking-[0.22em] uppercase" style={{ color:'var(--terra)' }}>{isRTL?'تسوق حسب':'Browse by'}</span>
              <PawPrint size={18} color="var(--terra-lt)"/>
            </div>
          ...
          {store.categories?.length>0?(
            ...
            <div className="py-12 text-center rounded-3xl paw-pattern" style={{ border:'2px dashed var(--border-dk)' }}>
              ...
              <p className="font-semibold" style={{ color:'var(--text-soft)' }}>{isRTL?'لا توجد تصنيفات بعد':'No categories yet'}</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section id="products" className="py-16" style={{ backgroundColor:'var(--warm-white)' }}>
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="text-center mb-12">
            ...
            <p className="mt-2 font-semibold text-sm" style={{ color:'var(--text-soft)' }}>{store.products?.length||0} {isRTL?'منتج مميز':'handpicked items'} ✨</p>
          </div>
      ...
      <section className="relative overflow-hidden py-24" style={{ background:'linear-gradient(135deg, var(--cocoa) 0%, var(--cocoa-lt) 60%, #5C3820 100%)' }}>
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-6 text-center">
          ...
          <h2 className="font-extrabold leading-tight text-white"
            ...
            {isRTL?'لأن حيوانك الأليف\nيستحق الأفضل!':'Because Your Pet\nDeserves the Best!'}
          </h2>
          ...
          <p className="text-sm font-semibold" style={{ color:'rgba(253,246,238,0.6)' }}>
            {isRTL?'منتجات مختارة بعناية لصحة وسعادة رفيقك':"Carefully chosen for your companion's health & happiness"}
          </p>
          <a href="#products" className="btn-paw inline-flex items-center gap-3 mt-8 px-10 py-4 rounded-full text-base font-bold text-white"
            ...
            <PawPrint size={16} color="rgba(255,255,255,0.85)"/> {isRTL?'تسوق الآن!':'Shop Now!'}
          </a>
        </div>
      </section>
    </div>
      ...
      <header className="py-4 sticky top-0 z-40"
        ...
        <div className="max-w-6xl mx-auto px-6 flex items-center justify-between">
          <nav className="flex items-center gap-2 text-xs font-semibold" style={{ color:'var(--text-soft)' }}>
            <span className="cursor-pointer hover:text-orange-600 transition-colors">🏠 {isRTL?'الرئيسية':'Home'}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-14">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[{e:'🛡️',l:isRTL?'آمن':'Vet Safe'},{e:'🚀',l:isRTL?'توصيل سريع':'Fast Ship'},{e:'❤️',l:isRTL?'موثوق':'Trusted'}].map((b,i)=>(
          ...
          <div className="space-y-6">
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[11px] font-bold tracking-[0.22em] uppercase" style={{ color:accent }}>{isRTL?'المنتج':'Product'}</span>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-semibold" style={{ color:'var(--text-soft)' }}>4.8 (128 {isRTL?'تقييم':'reviews'}) 🌟</span>
              </div>
            </div>
            ...
            <div className="p-5 rounded-2xl relative overflow-hidden" style={{ background:`linear-gradient(135deg, ${accent}08, ${accent}04)`, border:`2px solid ${accent}25` }}>
              ...
              <p className="text-[10px] font-bold tracking-[0.2em] uppercase mb-2 pl-4" style={{ color:'var(--text-soft)' }}>{isRTL?'💰 السعر':'💰 Price'}</p>
              <div className="flex items-baseline gap-3 pl-4">
                ...
                <span className="text-sm font-semibold" style={{ color:'var(--text-mid)' }}>دج</span>
                {product.priceOriginal&&parseFloat(product.priceOriginal)>finalPrice&&(
                  <div>
                    <span className="text-sm line-through font-semibold block" style={{ color:'var(--text-ghost)' }}>{parseFloat(product.priceOriginal).toLocaleString()} دج</span>
                    <span className="text-xs font-bold" style={{ color:'var(--terra)' }}>🎉 Save {(parseFloat(product.priceOriginal)-finalPrice).toLocaleString()} دج</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold"
              ...
              {autoGen?(isRTL?'مخزون غير محدود':'Unlimited Stock'):inStock?(isRTL?'متوفر':'In Stock'):(isRTL?'نفد':'Out of Stock')}
            </div>
            ...
            {product.offers?.length>0&&(
              <div>
                <BoneDivider/>
                <p className="text-[11px] font-bold tracking-[0.2em] uppercase mb-3" style={{ color:accent }}>🎁 {isRTL?'اختر الباقة':'Select Package'}</p>
                <div className="space-y-2">
                  {product.offers.map((offer:any)=>(
                    <label key={offer.id} className="flex items-center justify-between p-4 rounded-2xl cursor-pointer transition-all btn-paw"
                      ...
                      <span className="font-extrabold" style={{ fontFamily:"'Baloo 2',cursive", fontSize:'1.3rem', color:accent }}>{offer.price.toLocaleString()} <span className="text-xs">دج</span></span>
                    </label>
                  ))}
                </div>
              </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {(['home','office'] as const).map(type=>(
              <button key={type} type="button" onClick={()=>setFormData(p=>({...p,typeLivraison:type}))} className="btn-paw flex flex-col items-center gap-2 py-5 rounded-2xl transition-all"
                ...
                {selectedWilayaData&&<p className="text-xs font-semibold" style={{ color:'var(--text-mid)' }}>{(type==='home'?selectedWilayaData.livraisonHome:selectedWilayaData.livraisonOfice).toLocaleString()} دج</p>}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl relative overflow-hidden paw-pattern" style={{ border:'2px solid var(--border)', backgroundColor:'white' }}>
          ...
          <div className="space-y-2.5">
            {[{l:'Product',v:product.name},{l:'Unit Price',v:`${finalPrice.toLocaleString()} دج`},{l:'Quantity',v:`× ${formData.quantity}`},{l:'Shipping',v:selectedWilayaData?`${getPriceLivraison().toLocaleString()} دج`:'TBD'}].map(row=>(
...
function InfoCard({ icon, title, desc, status }: { icon:React.ReactNode; title:string; desc:string; status?:string }) {
  const isActive = status==='Always Active'||status==='دائماً نشطة';
  return (
