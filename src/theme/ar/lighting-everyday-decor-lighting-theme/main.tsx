...
/* ─── EVERYDAY DECOR & LIGHTING THEME CSS ─── */
const THEME_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @keyframes glowPulse { 0%, 100% { box-shadow: 0 0 8px rgba(232,150,10,0.3); } 50% { box-shadow: 0 0 18px rgba(232,150,10,0.6); } }
  ...
  /* ── Buttons & Inputs ── */
  .btn-primary {
    background: var(--primary); color: #fff; padding: 0.8rem 1.5rem; border-radius: 8px;
    border: none; font-weight: 600; cursor: pointer; transition: background 0.2s, box-shadow 0.2s;
    display: inline-flex; align-items: center; justify-content: center; gap: 0.5rem;
  }
  .btn-primary:hover { background: var(--primary-hover); box-shadow: 0 4px 12px rgba(232,150,10,0.35); }
  .btn-primary:disabled { opacity: 0.7; cursor: not-allowed; }

  .btn-outline {
    background: transparent; color: var(--text-main); padding: 0.8rem 1.5rem; border-radius: 8px;
  ...
  .input-base:focus { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(232,150,10,0.1); }
...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  const SearchResults = () => (
    <div style={{
      ...
      maxHeight: '350px',
      overflowY: 'auto',
      boxShadow: '0 10px 25px rgba(232,150,10,0.12)',
      marginTop: '5px',
      borderRadius: '8px'
    }}>
      {loading ? (
        ...
        listSearch.map((p: any) => (
          <Link
            ...
            style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '0.8rem', borderBottom: '1px solid var(--bg-alt)', transition: 'background 0.2s' }}
          >
            <img src={p.productImage || p.imagesProduct?.[0]?.imageUrl} style={{ width: 40, height: 40, objectFit: 'cover', borderRadius: '6px' }} alt="" />
            <div>
  ...
  return (
    <header dir="rtl" style={{ borderBottom: '2px solid var(--border-color)', background: '#FFFBF0', position: 'sticky', top: 0, zIndex: 100 }}>
      <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '75px' }}>

        <div style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
          <Link href="/" style={{ fontSize: '1.3rem', fontWeight: 900, color: 'var(--primary)' }}>
          ...
          <div style={{ position: 'relative', width: '250px' }} className="lg-block">
            <form onSubmit={handleSearchSubmit}>
              <input
                ...
                style={{ width: '100%', padding: '0.6rem 1rem 0.6rem 2.5rem', border: '1px solid var(--border-color)', background: 'var(--bg-alt)', borderRadius: '50px', outline: 'none', fontSize: '0.8rem' }}
              />
      ...
      {/* قائمة الجوال */}
      {open && (
        <div className="anim-fade-in" style={{ position: 'absolute', top: '75px', left: 0, right: 0, background: '#FFFBF0', borderBottom: '2px solid var(--border-color)', padding: '1.2rem', zIndex: 110 }}>
          <div style={{ position: 'relative', marginBottom: '1.5rem' }}>
            <form onSubmit={handleSearchSubmit}>
              <input
                ...
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                style={{ width: '100%', padding: '0.8rem 1rem', border: '1px solid var(--border-color)', borderRadius: '8px', background: 'var(--bg-alt)', outline: 'none' }}
              />
            </form>
            {searchQuery.length >= 2 && <SearchResults />}
          </div>
...
export function Footer({ store }: any) {
  return (
    <footer dir="rtl" style={{ padding: '4rem 0 2rem', background: '#1C1408', color: '#FEF3C7' }}>
      <div className="container grid-3">
        <div>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '1rem', color: '#E8960A' }}>{store?.name}</h3>
          <p style={{ color: '#8A7040', fontSize: '0.9rem', lineHeight: 1.6 }}>إضاءة دافئة وديكور مميز لكل زاوية في منزلك.</p>
        </div>
        <div>
          <h4 style={{ fontWeight: 700, marginBottom: '1rem', color: '#F0D880' }}>روابط</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.9rem' }}>
            <Link href="/" style={{ color: '#E8960A' }}>الرئيسية</Link>
            <Link href="/contact" style={{ color: '#E8960A' }}>تواصل معنا</Link>
            <Link href="/Privacy" style={{ color: '#E8960A' }}>سياسة الخصوصية</Link>
            <Link href="/Terms" style={{ color: '#E8960A' }}>الشروط والأحكام</Link>
          </div>
        </div>
        <div>
          <h4 style={{ fontWeight: 700, marginBottom: '1rem', color: '#F0D880' }}>تواصل</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.9rem', color: '#8A7040' }}>
      ...
      <div className="container" style={{ textAlign: 'center', marginTop: '3rem', paddingTop: '1.5rem', borderTop: '1px solid #2C2010', color: '#8A7040', fontSize: '0.8rem' }}>
...
export function Card({ product, displayImage, discount, store, viewDetails }: any) {
  ...
  return (
    <Link href={`/product/${product.slug || product.id}`} style={{ display: 'block' }} className="anim-fade-in">
      <div style={{ border: `1px solid var(--border-color)`, borderRadius: '8px', overflow: 'hidden', background: '#FFFBF0', transition: 'box-shadow 0.2s, border-color 0.2s', height: '100%', display: 'flex', flexDirection: 'column', boxShadow: '0 2px 8px rgba(232,150,10,0.06)' }}
        onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.boxShadow = '0 4px 16px rgba(232,150,10,0.22)'; (e.currentTarget as HTMLDivElement).style.borderColor = 'var(--primary)'; }}
        onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.boxShadow = '0 2px 8px rgba(232,150,10,0.06)'; (e.currentTarget as HTMLDivElement).style.borderColor = 'var(--border-color)'; }}
      >
    ...
    <div dir="rtl">
      ...
      <section style={{
        background: store.hero.imageUrl
          ? `linear-gradient(rgba(28, 20, 8, 0.5), rgba(28, 20, 8, 0.4)), url(${store.hero.imageUrl})`
          : 'linear-gradient(135deg, #FEF3C7 0%, #FFFBF0 60%)',
        ...
        <div className="container anim-fade-in" style={{ width: '100%' }}>
          <h1 style={{
            ...
            color: store.hero.imageUrl ? '#FFFBF0' : '#1C1408',
            fontWeight: 700,
            marginBottom: '1rem',
            letterSpacing: '-0.5px',
            textShadow: store.hero.imageUrl ? '0 2px 8px rgba(0,0,0,0.4)' : 'none'
          }}>
            {store.hero?.title?.replace(/<[^>]+>/g, '') || 'أضف دفءاً لمنزلك بإضاءتنا'}
          </h1>

          <p style={{
            fontSize: '1.15rem',
            color: store.hero.imageUrl ? '#FEF3C7' : '#8A7040',
            maxWidth: '580px',
            margin: '0 auto 2rem',
            lineHeight: '1.7'
          }}>
            {store.hero?.subtitle || 'تشكيلة واسعة من مصابيح الطاولة والإضاءة الليلية والديكور اليومي بألوان دافئة تملأ منزلك بالحياة.'}
          </p>
      ...
      {/* ── FEATURES ── */}
      <section style={{ background: '#FEF3C7', borderBottom: '2px solid var(--border-color)' }}>
        <div className="container grid-3" style={{ padding: '3rem 1.5rem' }}>
          {[
            { icon: <Lightbulb size={22} color="var(--primary)" />, t: 'إضاءة دافئة', d: 'تصاميم تضيء كل زاوية' },
            { icon: <ShieldCheck size={22} color="var(--primary)" />, t: 'جودة مضمونة', d: 'منتجات معتمدة وآمنة' },
            { icon: <Truck size={22} color="var(--primary)" />, t: 'توصيل سريع', d: 'لجميع ولايات الجزائر' },
          ].map((item, idx) => (
            <div key={idx} style={{ textAlign: 'center' }}>
              <div style={{ display: 'inline-flex', padding: '1rem', background: '#FFFBF0', borderRadius: '50%', marginBottom: '1rem', boxShadow: '0 2px 10px rgba(232,150,10,0.15)' }}>{item.icon}</div>
            ...
            {Array.from({ length: countPage }).map((_, i) => {
              ...
              return (
                <Link key={pn} href={{ query: { page: pn } }} scroll={false} style={{
                  ...
                  borderRadius: '8px', border: `1px solid ${isActive ? 'var(--primary)' : 'var(--border-color)'}`,
          ...
          {allImages.length > 1 && (
            <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem', overflowX: 'auto' }}>
              {allImages.map((img: string, idx: number) => (
                <button key={idx} onClick={() => setSel(idx)} style={{ width: 60, height: 60, flexShrink: 0, border: `1px solid ${sel === idx ? 'var(--primary)' : 'var(--border-color)'}`, borderRadius: '6px', padding: 0, cursor: 'pointer', overflow: 'hidden' }}>
          ...
          {product.offers?.length > 0 && (
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              {product.offers.map((o: any) => (
                <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '1rem', border: `1px solid ${selectedOffer === o.id ? 'var(--primary)' : 'var(--border-color)'}`, borderRadius: '8px', marginBottom: '0.5rem', cursor: 'pointer', background: selectedOffer === o.id ? 'var(--bg-alt)' : 'transparent' }}>
  ...
  return (
    <div>
      <form onSubmit={handleSubmit}>
        <div style={{ background: 'var(--bg-alt)', padding: '1.5rem', borderRadius: '8px', marginBottom: '1.5rem', border: '1px solid var(--border-color)' }}>
  ...
  return (
    <div className="container" style={{ padding: '4rem 1.5rem', minHeight: '70vh' }}>
      ...
      <div className="grid-2" style={{ gap: '3rem', alignItems: 'start' }}>
        <div>
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '1rem', padding: '1rem', border: '1px solid var(--border-color)', borderRadius: '8px', marginBottom: '1rem', background: 'var(--bg-main)' }}>
              <img src={item.product?.productImage} alt="" style={{ width: 80, height: 80, objectFit: 'cover', borderRadius: '6px' }} />
              <div style={{ flex: 1 }}>
        ...
        <div style={{ background: 'var(--bg-alt)', padding: '2rem', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 600, marginBottom: '1.5rem' }}>ملخص السلة</h3>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1.25rem', fontWeight: 700, marginBottom: '1.5rem', color: 'var(--primary)' }}>
    ...
    <div className="container" style={{ padding: '5rem 1.5rem', maxWidth: '450px', margin: '0 auto' }} dir="rtl">
      {sent ? (
        <div style={{ textAlign: 'center', padding: '2rem 0' }} className="anim-fade-in">
          <div style={{ display: 'inline-flex', padding: '1rem', background: 'var(--bg-alt)', borderRadius: '50%', marginBottom: '1.5rem' }}>
