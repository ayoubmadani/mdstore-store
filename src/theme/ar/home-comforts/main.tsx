...
import {
  ...
  Send, Home as HomeIcon, Sofa, Package
} from 'lucide-react';
...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  const SearchResults = () => (
    <div style={{
      ...
      maxHeight: '350px',
      overflowY: 'auto',
      boxShadow: '0 10px 25px rgba(158,139,125,0.12)',
      marginTop: '5px',
      borderRadius: '10px'
    }}>
      {loading ? (
  ...
  return (
    <header dir="rtl" style={{ borderBottom: '1px solid var(--border-color)', background: '#FAFAF8', position: 'sticky', top: 0, zIndex: 100 }}>
      ...
      {/* قائمة الجوال */}
      {open && (
        <div className="anim-fade-in" style={{ position: 'absolute', top: '75px', left: 0, right: 0, background: '#FAFAF8', borderBottom: '1px solid var(--border-color)', padding: '1.2rem', zIndex: 110 }}>
...
export function Footer({ store }: any) {
  return (
    <footer dir="rtl" style={{ padding: '4rem 0 2rem', background: '#2A2520', color: '#F2EDE8' }}>
      <div className="container grid-3">
        <div>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '1rem', color: '#FAFAF8' }}>{store?.name}</h3>
          <p style={{ color: '#A89D98', fontSize: '0.9rem', lineHeight: 1.6 }}>راحة منزلك تبدأ هنا. أثاث وديكور بجودة عالية وأسعار مناسبة.</p>
        </div>
        <div>
          <h4 style={{ fontWeight: 700, marginBottom: '1rem', color: '#FAFAF8' }}>روابط</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.9rem' }}>
            <Link href="/" style={{ color: '#9E8B7D' }}>الرئيسية</Link>
            <Link href="/contact" style={{ color: '#9E8B7D' }}>تواصل معنا</Link>
            <Link href="/Privacy" style={{ color: '#9E8B7D' }}>سياسة الخصوصية</Link>
            <Link href="/Terms" style={{ color: '#9E8B7D' }}>الشروط والأحكام</Link>
          </div>
        </div>
        <div>
          <h4 style={{ fontWeight: 700, marginBottom: '1rem', color: '#FAFAF8' }}>تواصل</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.9rem', color: '#A89D98' }}>
      ...
      <div className="container" style={{ textAlign: 'center', marginTop: '3rem', paddingTop: '1.5rem', borderTop: '1px solid #3A3530', color: '#7A706A', fontSize: '0.8rem' }}>
...
export function Card({ product, displayImage, discount, store, viewDetails }: any) {
  ...
  return (
    <Link href={`/product/${product.slug || product.id}`} style={{ display: 'block' }} className="anim-fade-in">
      <div style={{ border: `1px solid var(--border-color)`, borderRadius: '10px', overflow: 'hidden', background: 'var(--bg-main)', transition: 'box-shadow 0.2s, border-color 0.2s', height: '100%', display: 'flex', flexDirection: 'column', boxShadow: '0 2px 8px rgba(158,139,125,0.08)' }}
        onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.boxShadow = '0 6px 20px rgba(158,139,125,0.18)'; (e.currentTarget as HTMLDivElement).style.borderColor = 'var(--primary)'; }}
        onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.boxShadow = '0 2px 8px rgba(158,139,125,0.08)'; (e.currentTarget as HTMLDivElement).style.borderColor = 'var(--border-color)'; }}
      >
        <div style={{ aspectRatio: '1/1', background: 'var(--bg-alt)', position: 'relative' }}>
          ...
          {discount > 0 && <span style={{ position: 'absolute', top: 10, right: 10, background: 'var(--primary)', color: '#fff', fontSize: '10px', padding: '4px 8px', borderRadius: '6px' }}>-{discount}%</span>}
        </div>
    ...
    <div dir="rtl">
      ...
      <section style={{
        background: store.hero.imageUrl
          ? `linear-gradient(rgba(42, 37, 32, 0.45), rgba(42, 37, 32, 0.45)), url(${store.hero.imageUrl})`
          : '#F2EDE8',
        ...
        <div className="container anim-fade-in" style={{ width: '100%' }}>
          <h1 style={{
            ...
            color: store.hero.imageUrl ? '#FAFAF8' : '#2A2520',
            fontWeight: 700,
            marginBottom: '1rem',
            letterSpacing: '-0.5px',
            textShadow: store.hero.imageUrl ? '0 2px 8px rgba(0,0,0,0.3)' : 'none'
          }}>
            {store.hero?.title?.replace(/<[^>]+>/g, '') || 'أثاث يجعل منزلك أكثر راحة'}
          </h1>

          <p style={{
            fontSize: '1.15rem',
            color: store.hero.imageUrl ? '#F2EDE8' : '#7A706A',
            maxWidth: '580px',
            margin: '0 auto 2rem',
            lineHeight: '1.7'
          }}>
            {store.hero?.subtitle || 'اكتشف تشكيلتنا الفريدة من الأثاث والديكور المنزلي المصمم لراحتك وأناقتك.'}
          </p>
      ...
      {/* ── FEATURES ── */}
      <section style={{ background: '#F2EDE8', borderBottom: '1px solid var(--border-color)' }}>
        <div className="container grid-3" style={{ padding: '3rem 1.5rem' }}>
          {[
            { icon: <Sofa size={22} color="var(--primary)" />, t: 'راحة المنزل', d: 'منتجات مختارة بعناية لراحتك' },
            { icon: <ShieldCheck size={22} color="var(--primary)" />, t: 'جودة عالية', d: 'مواد متينة وتصاميم أصيلة' },
            { icon: <Truck size={22} color="var(--primary)" />, t: 'توصيل سريع', d: 'لجميع ولايات الجزائر' },
          ].map((item, idx) => (
            <div key={idx} style={{ textAlign: 'center' }}>
              <div style={{ display: 'inline-flex', padding: '1rem', background: '#FAFAF8', borderRadius: '50%', marginBottom: '1rem', boxShadow: '0 2px 8px rgba(158,139,125,0.1)' }}>{item.icon}</div>
      ...
      <section id="products" className="container" style={{ padding: '4rem 1.5rem' }}>
        ...
        {products.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '4rem', background: 'var(--bg-alt)', borderRadius: '10px' }}>
  ...
  return (
    <div dir="rtl" className="container" style={{ padding: '3rem 1.5rem' }}>
      <div className="grid-2" style={{ gap: '3rem', alignItems: 'start' }}>
        ...
        {/* Gallery */}
        <div>
          <div style={{ aspectRatio: '1/1', background: 'var(--bg-alt)', borderRadius: '10px', overflow: 'hidden', position: 'relative' }}>
            {allImages[sel] ? <img src={allImages[sel]} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" /> : null}
            {discount > 0 && <span style={{ position: 'absolute', top: 12, right: 12, background: 'var(--primary)', color: '#fff', fontSize: '12px', padding: '4px 10px', borderRadius: '6px' }}>خصم {discount}%</span>}
          </div>
          {allImages.length > 1 && (
            <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem', overflowX: 'auto' }}>
              {allImages.map((img: string, idx: number) => (
                <button key={idx} onClick={() => setSel(idx)} style={{ width: 60, height: 60, flexShrink: 0, border: `1px solid ${sel === idx ? 'var(--primary)' : 'var(--border-color)'}`, borderRadius: '8px', padding: 0, cursor: 'pointer', overflow: 'hidden' }}>
  ...
  return (
    <div>
      <form onSubmit={handleSubmit}>
        <div style={{ background: 'var(--bg-alt)', padding: '1.5rem', borderRadius: '10px', marginBottom: '1.5rem' }}>
          ...
          <FR label="الكمية">
            <div style={{ display: 'flex', alignItems: 'center', border: '1px solid var(--border-color)', borderRadius: '8px', width: 'fit-content', background: 'var(--bg-main)' }}>
  ...
  return (
    <div className="container" style={{ padding: '4rem 1.5rem', minHeight: '70vh' }}>
      ...
      <div className="grid-2" style={{ gap: '3rem', alignItems: 'start' }}>
        <div>
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '1rem', padding: '1rem', border: '1px solid var(--border-color)', borderRadius: '10px', marginBottom: '1rem' }}>
        ...
        <div style={{ background: 'var(--bg-alt)', padding: '2rem', borderRadius: '10px' }}>
