...
const THEME_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .hc-card { background: ${CARD}; border-radius: 12px; overflow: hidden; border: 1px solid ${BD}; transition: all 0.25s; display: flex; flex-direction: column; }
  .hc-card:hover { transform: translateY(-4px); box-shadow: 0 12px 36px rgba(196,154,108,0.15); border-color: ${W}; }
  .hc-card:hover .hc-name { color: ${W}; }
...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  return (
    <>
      <nav dir="rtl" style={{ position: 'sticky', top: 0, zIndex: 50, background: scrolled ? 'rgba(250,247,244,0.96)' : CARD, backdropFilter: 'blur(12px)', borderBottom: `1px solid ${scrolled ? BD : WB}`, boxShadow: scrolled ? '0 2px 18px rgba(196,154,108,0.1)' : 'none', transition: 'all 0.25s' }}>
        ...
        <div style={{ overflow: 'hidden', maxHeight: open ? 200 : 0, transition: 'max-height 0.25s ease', background: CARD, borderTop: open ? `1px solid ${BD}` : 'none' }}>
          <div style={{ padding: '0.5rem 1.5rem 1rem' }}>
            ...
            {store?.cart !== false && (
              <Link href="/cart" onClick={() => setOpen(false)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 0', fontSize: '0.9rem', fontWeight: 500, color: INK }}>
                السلة <ArrowLeft size={14} style={{ color: W }} />
              </Link>
            )}
          </div>
        </div>
...
export function Card({ product, displayImage, discount, store, viewDetails }: any) {
  ...
  return (
    <div className="hc-card">
      <div style={{ position: 'relative', aspectRatio: '4/3', background: WL, overflow: 'hidden' }}>
        {displayImage
          ...
          : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Sofa size={36} color={WB} /></div>
        }
        {discount > 0 && (
          <div style={{ position: 'absolute', top: 10, right: 10, background: W, color: '#fff', fontSize: 10, fontWeight: 700, padding: '3px 9px', borderRadius: 20 }}>-{discount}%</div>
        )}
      </div>
      <div style={{ padding: '1rem 1.125rem 1.125rem', display: 'flex', flexDirection: 'column', flex: 1 }}>
        <p style={{ fontSize: '0.62rem', fontWeight: 600, color: W, marginBottom: '0.3rem', textTransform: 'uppercase' as const, letterSpacing: '0.07em' }}>{store?.name}</p>
        <h3 className="hc-name" style={{ fontSize: '0.9rem', fontWeight: 600, color: INK, lineHeight: 1.4, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', transition: 'color 0.2s', flex: 1, marginBottom: '0.75rem' }}>{product.name}</h3>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <span className="price-mono" style={{ fontSize: '1.25rem', fontWeight: 700, color: WD }}>{price.toLocaleString()}</span>
            <span style={{ fontSize: '0.68rem', color: SUB, marginRight: 3 }}>{store.currency || 'دج'}</span>
            {orig > price && <div style={{ fontSize: '0.68rem', color: WB, textDecoration: 'line-through' }}>{orig.toLocaleString()}</div>}
          </div>
          <Link href={`/product/${product.slug || product.id}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: WL, color: WD, fontSize: '0.78rem', fontWeight: 600, padding: '0.4rem 0.875rem', borderRadius: 20, border: `1px solid ${WB}`, transition: 'all 0.18s' }}
            onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.background = W; (e.currentTarget as HTMLAnchorElement).style.color = '#fff'; (e.currentTarget as HTMLAnchorElement).style.borderColor = W; }}
            onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.background = WL; (e.currentTarget as HTMLAnchorElement).style.color = WD; (e.currentTarget as HTMLAnchorElement).style.borderColor = WB; }}>
...
export function Home({ store, page }: any) {
  ...
  return (
    <div dir="rtl">
      {/* HERO */}
      <section style={{ position: 'relative', overflow: 'hidden', background: BG, borderBottom: `1px solid ${BD}` }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: 'clamp(3rem,6vw,5rem) 1.5rem' }}>
          <div className="hero-inner">
            <div>
              ...
              <h1 style={{ fontSize: 'clamp(2.25rem,5vw,4rem)', fontWeight: 700, color: INK, lineHeight: 1.15, letterSpacing: '-0.02em', marginBottom: '1rem' }}
                dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(store.hero?.title || 'منزلك يستحق<br/><span style="color:#C49A6C">الأجمل</span>') }} />
              <p style={{ fontSize: '1rem', color: SUB, lineHeight: 1.8, marginBottom: '2rem', maxWidth: 420 }}>
                {store.hero?.subtitle || 'أثاث وقطع ديكور بأجواء دافئة ومريحة تحوّل بيتك إلى مكان تحبّ العودة إليه.'}
              </p>
              <div className="hero-actions">
                <a href="#products" style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: W, color: '#fff', fontWeight: 700, fontSize: '0.9rem', padding: '0.875rem 1.875rem', borderRadius: 10, transition: 'background 0.18s' }}
                  onMouseEnter={e => ((e.currentTarget as HTMLAnchorElement).style.background = WD)}
                  onMouseLeave={e => ((e.currentTarget as HTMLAnchorElement).style.background = W)}>
                  تصفح المنتجات <ArrowLeft size={15} />
                </a>
                {store?.cart !== false && (
                  <Link href="/cart" style={{ display: 'inline-flex', alignItems: 'center', gap: 7, color: SUB, fontWeight: 500, fontSize: '0.9rem', padding: '0.875rem 1.5rem', borderRadius: 10, border: `1px solid ${BD}`, background: CARD, transition: 'all 0.18s' }}
                    onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.borderColor = W; (e.currentTarget as HTMLAnchorElement).style.color = WD; }}
              ...
              <div style={{ display: 'flex', gap: '2rem', marginTop: '2.5rem', paddingTop: '2rem', borderTop: `1px solid ${BD}` }}>
                {[{ n: '+200', l: 'قطعة أثاث' }, { n: '58', l: 'ولاية' }, { n: '100%', l: 'جودة' }].map((s, i) => (
                  <div key={i}>
                    <p style={{ fontSize: '1.5rem', fontWeight: 700, color: W, lineHeight: 1 }}>{s.n}</p>
            ...
            {store.hero?.imageUrl && (
              <div style={{ position: 'relative', borderRadius: 20, overflow: 'hidden', aspectRatio: '4/3', boxShadow: '0 24px 64px rgba(196,154,108,0.2)' }}>
                <img src={store.hero.imageUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, rgba(196,154,108,0.12) 0%, transparent 60%)' }} />
              </div>
            )}
          </div>
        </div>
        ...
        <div className="trust-row">
          {[
            ...
            <div key={i} style={{ padding: '1.25rem', background: CARD, display: 'flex', alignItems: 'center', gap: '0.875rem', borderLeft: i > 0 ? `1px solid ${BD}` : 'none', borderTop: i >= 2 ? `1px solid ${BD}` : 'none' }}>
              ...
              <div>
                <p style={{ fontSize: '0.82rem', fontWeight: 600, color: INK }}>{item.t}</p>
      ...
      {/* CATEGORIES */}
      {cats.length > 0 && (
        <section style={{ padding: '0 1.5rem 2.5rem', maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.25rem' }}>
            <div style={{ width: 28, height: 28, background: WL, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Sofa size={13} color={W} />
            </div>
            <h2 style={{ fontSize: '0.9rem', fontWeight: 700, color: INK }}>تصفح الفئات</h2>
          </div>
          <div className="cats-grid">
            {cats.map((cat: any) => (
              <Link key={cat.id} href={`?category=${cat.id}`} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', padding: '1.125rem 0.75rem', border: `1px solid ${BD}`, borderRadius: 14, background: CARD, textAlign: 'center', transition: 'all 0.2s' }}
                onMouseEnter={e => { const el = e.currentTarget; el.style.borderColor = W; el.style.background = WL; el.style.transform = 'translateY(-2px)'; }}
                onMouseLeave={e => { const el = e.currentTarget; el.style.borderColor = BD; el.style.background = CARD; el.style.transform = ''; }}>
                <div style={{ width: 40, height: 40, background: WL, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Sofa size={17} color={W} />
                </div>
      ...
      <section id="products" style={{ padding: '0 1.5rem 5rem', maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <div style={{ width: 4, height: 20, background: W, borderRadius: 2 }} />
            <h2 style={{ fontSize: '0.9rem', fontWeight: 700, color: INK }}>جميع المنتجات</h2>
          </div>
          {store.count > 0 && <span style={{ fontSize: '0.72rem', color: SUB, background: WL, border: `1px solid ${WB}`, padding: '0.25rem 0.75rem', borderRadius: 20 }}>{store.count} منتج</span>}
        </div>
        {products.length === 0 ? (
