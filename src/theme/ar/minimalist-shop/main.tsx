...
const THEME_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Buttons & Inputs ── */
  .btn-primary {
    background: var(--primary); color: #fff; padding: 0.8rem 1.5rem; border-radius: 0px;
  ...
  .btn-outline {
    background: transparent; color: var(--text-main); padding: 0.8rem 1.5rem; border-radius: 0px;
  ...
  .input-base {
    width: 100%; padding: 0.8rem 1rem; border-radius: 0px; border: 1px solid var(--border-color);
...
export function Navbar({ store, domain }: { store: any; domain: string }) {
      ...
      {loading ? (
        ...
        listSearch.map((p: any) => (
          <Link
            ...
            style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '0.8rem', borderBottom: '1px solid #eee', transition: 'background 0.2s' }}
          >
            <img src={p.productImage || p.imagesProduct?.[0]?.imageUrl} style={{ width: 40, height: 40, objectFit: 'cover', borderRadius: '0px' }} alt="" />
            <div>
              <div className='line-clamp-1' style={{ fontSize: '0.85rem', fontWeight: 600, color: '#111' }}>{p.name}</div>
  ...
  return (
    <header dir="rtl" style={{ borderBottom: '1px solid #111111', background: '#FFFFFF', position: 'sticky', top: 0, zIndex: 100 }}>
      <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '75px' }}>

        <div style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
          <Link href="/" style={{ fontSize: '1.3rem', fontWeight: 900, color: '#111111', letterSpacing: '-1px' }}>
          ...
          <nav className="lg-flex" style={{ gap: '1.5rem', fontSize: '0.85rem', fontWeight: 600 }}>
            <Link href="/" style={{ color: '#111111' }}>الرئيسية</Link>
            <Link href="/contact" style={{ color: '#111111' }}>تواصل معنا</Link>
          </nav>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.2rem' }}>
          <div style={{ position: 'relative', width: '250px' }} className="lg-block">
            <form onSubmit={handleSearchSubmit}>
              <input
                ...
                style={{ width: '100%', padding: '0.6rem 1rem 0.6rem 2.5rem', border: '1px solid #EEEEEE', background: '#F5F5F5', borderRadius: '0px', outline: 'none', fontSize: '0.8rem' }}
              />
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
            <Link href="/cart" style={{ position: 'relative', color: '#111111' }}>
              <ShoppingBag size={22} />
              {itemsCartCount > 0 && (
                <span style={{ position: 'absolute', top: -5, right: -8, background: '#111111', color: '#fff', fontSize: 9, width: 17, height: 17, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%' }}>
                  {itemsCartCount}
                </span>
              )}
            </Link>
      ...
      {open && (
        <div className="anim-fade-in" style={{ position: 'absolute', top: '75px', left: 0, right: 0, background: '#fff', borderBottom: '1px solid #111', padding: '1.2rem', zIndex: 110 }}>
          <div style={{ position: 'relative', marginBottom: '1.5rem' }}>
            <form onSubmit={handleSearchSubmit}>
              <input
                ...
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                style={{ width: '100%', padding: '0.8rem 1rem', border: '1px solid #eee', borderRadius: '0px', background: '#F5F5F5', outline: 'none' }}
              />
            </form>
            {searchQuery.length >= 2 && <SearchResults />}
          </div>
...
export function Footer({ store }: any) {
  return (
    <footer dir="rtl" style={{ padding: '4rem 0 2rem', background: '#111111' }}>
      <div className="container grid-3">
        <div>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '1rem', color: '#FFFFFF' }}>{store?.name}</h3>
          <p style={{ color: '#999999', fontSize: '0.9rem', lineHeight: 1.6 }}>تسوق بثقة وبساطة. تجربة سلسة وتوصيل سريع.</p>
        </div>
        <div>
          <h4 style={{ fontWeight: 700, marginBottom: '1rem', color: '#FFFFFF' }}>روابط</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.9rem', color: '#999999' }}>
        ...
        <div>
          <h4 style={{ fontWeight: 700, marginBottom: '1rem', color: '#FFFFFF' }}>تواصل</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.9rem', color: '#999999' }}>
      ...
      <div className="container" style={{ textAlign: 'center', marginTop: '3rem', paddingTop: '1.5rem', borderTop: '1px solid #333333', color: '#666666', fontSize: '0.8rem' }}>
...
export function Card({ product, displayImage, discount, store, viewDetails }: any) {
  ...
  return (
    <Link href={`/product/${product.slug || product.id}`} style={{ display: 'block' }} className="anim-fade-in">
      <div style={{ border: '1px solid #111111', borderRadius: '0px', overflow: 'hidden', background: '#FFFFFF', transition: 'box-shadow 0.2s', height: '100%', display: 'flex', flexDirection: 'column', boxShadow: 'none' }}>
        <div style={{ aspectRatio: '1/1', background: '#F5F5F5', position: 'relative' }}>
          {displayImage ? <img src={displayImage} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><ShoppingBag color="#EEEEEE" /></div>}
          {discount > 0 && <span style={{ position: 'absolute', top: 10, right: 10, background: '#111111', color: '#fff', fontSize: '10px', padding: '4px 8px', borderRadius: '0px' }}>-{discount}%</span>}
        </div>
        <div style={{ padding: '1rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <span style={{ color: '#111111', fontWeight: 700, fontSize: '1.1rem' }}>{price.toLocaleString()} {store.currency || 'دج'}</span>
            {orig > price && <span style={{ fontSize: '0.8rem', color: '#666666', textDecoration: 'line-through' }}>{orig.toLocaleString()}</span>}
          </div>
        </div>
      </div>
    </Link>
    ...
    <div dir="rtl">
      ...
      <section style={{
        background: store.hero?.imageUrl
          ? `linear-gradient(rgba(0,0,0,0.45), rgba(0,0,0,0.45)), url(${store.hero.imageUrl})`
          : '#FFFFFF',
        ...
        <div className="container anim-fade-in" style={{ width: '100%' }}>
          <h1 style={{
            ...
            color: store.hero?.imageUrl ? '#ffffff' : '#111111',
            fontWeight: 900,
          ...
          <p style={{
            fontSize: '1.1rem',
            color: store.hero?.imageUrl ? '#f0f0f0' : '#666666',
            maxWidth: '550px',
            margin: '0 auto 2.5rem',
            lineHeight: '1.7',
            fontWeight: 400
          }}>
            {store.hero?.subtitle || 'أفضل المنتجات، أبسط تجربة تسوق.'}
          </p>
      ...
      {cats.length > 0 && (
        <section className="container" style={{ padding: '4rem 1.5rem 0' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', overflowX: 'auto', paddingBottom: '1rem' }}>
            {cats.map((cat: any) => (
              <Link key={cat.id} href={`?category=${cat.id}`} className="btn-outline" style={{ whiteSpace: 'nowrap', borderRadius: '0px' }}>
                {cat.name}
              </Link>
            ))}
          </div>
      ...
      <section id="products" className="container" style={{ padding: '4rem 1.5rem' }}>
        ...
        {products.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '4rem', background: '#F5F5F5', borderRadius: '0px' }}>
            <p style={{ color: '#666666' }}>لا توجد منتجات حالياً</p>
          </div>
        ) : (
            ...
            {Array.from({ length: countPage }).map((_, i) => {
              ...
              return (
                <Link key={pn} href={{ query: { page: pn } }} scroll={false} style={{
                  ...
                  borderRadius: '0px', border: `1px solid ${isActive ? '#111111' : '#EEEEEE'}`,
                  background: isActive ? '#111111' : '#FFFFFF', color: isActive ? '#fff' : '#111111'
                }}>{pn}</Link>
              );
            })}
          </div>
      ...
      {/* ── FEATURES ── */}
      <section style={{ borderBottom: '1px solid #EEEEEE', borderTop: '1px solid #EEEEEE' }}>
        <div className="container grid-3" style={{ padding: '3rem 1.5rem' }}>
          {[
            ...
            <div key={idx} style={{ textAlign: 'center' }}>
              <div style={{ display: 'inline-flex', padding: '1rem', background: '#F5F5F5', borderRadius: '0px', marginBottom: '1rem' }}>{item.icon}</div>
              <h3 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '0.25rem' }}>{item.t}</h3>
              <p style={{ fontSize: '0.85rem', color: '#666666' }}>{item.d}</p>
            </div>
          ))}
        </div>
      </section>
  ...
  return (
    <div dir="rtl" className="container" style={{ padding: '3rem 1.5rem' }}>
      <div className="grid-2" style={{ gap: '3rem', alignItems: 'start' }}>
        ...
        {/* Gallery */}
        <div>
          <div style={{ aspectRatio: '1/1', background: '#F5F5F5', borderRadius: '0px', overflow: 'hidden', position: 'relative' }}>
            {allImages[sel] ? <img src={allImages[sel]} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" /> : null}
            {discount > 0 && <span style={{ position: 'absolute', top: 12, right: 12, background: '#111111', color: '#fff', fontSize: '12px', padding: '4px 10px', borderRadius: '0px' }}>خصم {discount}%</span>}
          </div>
          {allImages.length > 1 && (
            <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem', overflowX: 'auto' }}>
              {allImages.map((img: string, idx: number) => (
                <button key={idx} onClick={() => setSel(idx)} style={{ width: 60, height: 60, flexShrink: 0, border: `1px solid ${sel === idx ? '#111111' : '#EEEEEE'}`, borderRadius: '0px', padding: 0, cursor: 'pointer', overflow: 'hidden' }}>
        ...
        <div>
          ...
          <div style={{ fontSize: '2rem', fontWeight: 700, marginBottom: '1.5rem' }}>{finalPrice.toLocaleString()} <span style={{ fontSize: '1rem', color: '#666666' }}>دج</span></div>

          {product.offers?.length > 0 && (
            <div style={{ marginBottom: '1.5rem' }}>
              <p style={{ fontWeight: 600, marginBottom: '0.5rem' }}>العروض:</p>
              {product.offers.map((o: any) => (
                <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '1rem', border: `1px solid ${selectedOffer === o.id ? '#111111' : '#EEEEEE'}`, borderRadius: '0px', marginBottom: '0.5rem', cursor: 'pointer', background: selectedOffer === o.id ? '#F5F5F5' : 'transparent' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <input type="radio" checked={selectedOffer === o.id} onChange={() => setSelectedOffer(o.id)} />
                    <span>{o.name} <span style={{ color: '#666666', fontSize: '0.85rem' }}>(الكمية: {o.quantity})</span></span>
                  </div>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                {attr.variants.map((v: any) => (
                  <button key={v.id} onClick={() => handleVariantSelection(attr.name, v.value)} className="btn-outline" style={{
                    borderColor: selectedVariants[attr.name] === v.value ? '#111111' : '#EEEEEE',
                    background: selectedVariants[attr.name] === v.value ? '#111111' : 'transparent',
                    color: selectedVariants[attr.name] === v.value ? '#fff' : '#111111',
                    borderRadius: '0px', padding: '0.5rem 1rem'
                  }}>
                    {v.name}
                  </button>
                ))}
          ...
          {product.desc && (
            <div style={{ marginTop: '3rem' }}>
              ...
              <div style={{ lineHeight: 1.8, color: '#666666', fontSize: '0.95rem' }} dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(product.desc) }} />
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div>
      <form onSubmit={handleSubmit}>
        <div style={{ background: '#F5F5F5', padding: '1.5rem', borderRadius: '0px', marginBottom: '1.5rem' }}>
          ...
          <FR label="طريقة التوصيل">
            <div className="grid-2">
              {(['home', 'office'] as const).map(t => (
                <button key={t} type="button" onClick={() => setFd({ ...fd, typeLivraison: t })} className="btn-outline" style={{ borderColor: fd.typeLivraison === t ? '#111111' : '#EEEEEE', background: fd.typeLivraison === t ? '#FFFFFF' : 'transparent' }}>
          ...
          <FR label="الكمية">
            <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #EEEEEE', borderRadius: '0px', width: 'fit-content', background: '#FFFFFF' }}>
...
export function Cart({ domain, store }: { domain: string; store: any }) {
  ...
  if (!items.length) return (
    <div className="container" style={{ textAlign: 'center', padding: '6rem 1.5rem', minHeight: '60vh' }}>
      <ShoppingCart size={48} color="#EEEEEE" style={{ margin: '0 auto 1rem' }} />
  ...
  return (
    <div className="container" style={{ padding: '4rem 1.5rem', minHeight: '70vh' }}>
      ...
      <div className="grid-2" style={{ gap: '3rem', alignItems: 'start' }}>
        <div>
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '1rem', padding: '1rem', border: '1px solid #EEEEEE', borderRadius: '0px', marginBottom: '1rem' }}>
              <img src={item.product?.productImage} alt="" style={{ width: 80, height: 80, objectFit: 'cover', borderRadius: '0px' }} />
              <div style={{ flex: 1 }}>
                <h4 style={{ fontWeight: 500, marginBottom: '0.5rem' }}>{item.product?.name}</h4>
                <div style={{ fontWeight: 700 }}>{item.finalPrice} دج <span style={{ fontSize: '0.8rem', color: '#666666', fontWeight: 400 }}>x {item.quantity}</span></div>
              </div>
              <button onClick={() => remove(i)} style={{ background: 'none', border: 'none', color: '#dc2626', cursor: 'pointer' }}><Trash2 size={18} /></button>
            </div>
          ))}
        </div>
        <div style={{ background: '#F5F5F5', padding: '2rem', borderRadius: '0px' }}>
          ...
          <p style={{ color: '#666666', fontSize: '0.85rem', marginBottom: '1rem' }}>* تكاليف التوصيل سيتم احتسابها لاحقاً.</p>
...
export function Privacy() {
  return (
    <div className="container" style={{ padding: '4rem 1.5rem', maxWidth: '800px' }}>
      ...
      <SimpleDivider />
      <div style={{ lineHeight: 1.8, color: '#666666' }}>
        <h3 style={{ color: '#111111', marginTop: '2rem', marginBottom: '0.5rem' }}>1. جمع المعلومات</h3>
        <p>نقوم بجمع المعلومات الشخصية الضرورية فقط لإتمام طلبك وتوصيله (الاسم، العنوان، رقم الهاتف).</p>

        <h3 style={{ color: '#111111', marginTop: '2rem', marginBottom: '0.5rem' }}>2. الاستخدام</h3>
    ...
    <div className="container" style={{ padding: '5rem 1.5rem', maxWidth: '450px', margin: '0 auto' }} dir="rtl">
      {sent ? (
        <div style={{ textAlign: 'center', padding: '2rem 0' }} className="anim-fade-in">
          <div style={{ display: 'inline-flex', padding: '1rem', background: '#F5F5F5', borderRadius: '0px', marginBottom: '1.5rem' }}>
            <CheckCircle2 size={32} color="#111111" />
          </div>
          <h2 style={{ fontSize: '1.25rem', fontWeight: 900, marginBottom: '0.5rem' }}>تم الإرسال</h2>
          <p style={{ color: '#666666', fontSize: '0.9rem', marginBottom: '2rem' }}>شكراً لتواصلك معنا. سنقوم بالرد عليك في أقرب وقت.</p>
          <button
            onClick={() => { setSent(false); setForm({ name: '', email: '', phone: '', message: '' }); }}
            style={{ background: 'none', border: 'none', borderBottom: '1px solid #111111', padding: '0 0 4px 0', fontSize: '0.8rem', fontWeight: 700, cursor: 'pointer' }}
          >
            إرسال رسالة أخرى
          </button>
        </div>
        ...
        <div>
          <div style={{ marginBottom: '2.5rem' }}>
            <h1 style={{ fontSize: '1.5rem', fontWeight: 900, marginBottom: '0.5rem', letterSpacing: '-0.5px' }}>تواصل معنا</h1>
            <p style={{ color: '#666666', fontSize: '0.85rem' }}>املأ النموذج وسنرد عليك قريباً.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
            ...
            <button type="submit" disabled={loading} style={{ background: '#111111', color: '#fff', padding: '1rem', border: 'none', borderRadius: '0px', fontSize: '0.8rem', fontWeight: 700, letterSpacing: '0.1em', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', marginTop: '1rem', transition: 'opacity 0.2s' }}>
