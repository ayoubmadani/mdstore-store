...
export function Navbar({ store, domain, onMenuOpen }: { store:any; domain:string; onMenuOpen?:()=>void }) {
  ...
  const Drop = () => (
    <div style={{position:'absolute',top:'calc(100% + 4px)',right:0,left:0,background:'var(--paper)',border:'1px solid var(--ink)',zIndex:200,borderTop:'2px solid var(--punch)',boxShadow:'4px 4px 0 var(--ink)'}}>
      {loading ? <div style={{padding:'1rem',textAlign:'center',fontSize:'8px',letterSpacing:'0.2em',color:'var(--ash)'}}>جاري البحث...</div>
      : ls.length > 0 ? ls.map((p:any)=>(
        <Link href={`/product/${p.id}`} key={p.id} onClick={()=>setSq('')}
          style={{display:'flex',alignItems:'center',gap:'0.75rem',padding:'10px 14px',borderBottom:'1px solid var(--paper-dk)'}}>
          <img src={p.productImage||p.imagesProduct?.[0]?.imageUrl} style={{width:40,height:40,objectFit:'cover',flexShrink:0,border:'1px solid var(--paper-dk)'}} alt=""/>
          <div>
            <div className="ub" style={{fontSize:'10px',fontWeight:700,color:'var(--ink)',letterSpacing:'0.02em'}}>{p.name.toUpperCase().slice(0,24)}</div>
            <div className="ub" style={{fontSize:'13px',fontWeight:900,color:'var(--punch)'}}>{p.price} دج</div>
          </div>
        </Link>
      )) : sq.length>=2 && <div style={{padding:'1rem',textAlign:'center',fontSize:'8px',letterSpacing:'0.2em',color:'var(--mist)'}}>لا توجد نتائج</div>}
    </div>
  );
            ...
            <div className="rise rise-3" style={{display:'flex',gap:'10px',flexWrap:'wrap'}}>
              ...
              {cats.length>0 && (
                <a href="#cats" className="sm" style={{display:'inline-flex',alignItems:'center',gap:'8px',fontSize:'9px',letterSpacing:'0.18em',color:'rgba(242,239,232,0.35)',border:'1px solid rgba(242,239,232,0.14)',padding:'14px 22px',transition:'all 0.22s'}}
                  ...
                  الأقسام <ArrowRight style={{width:'12px',height:'12px'}}/>
                </a>
              )}
            </div>
          </div>
    ...
    <div dir="rtl" style={{backgroundColor:'var(--paper)',fontFamily:"'Space Mono',monospace"}}>
      ...
      <div style={{borderBottom:'1px solid var(--ink)',padding:'10px 22px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <div style={{display:'flex',alignItems:'center',gap:'8px',fontSize:'8px',letterSpacing:'0.18em',color:'var(--ash)'}}>
          <Link href="/" style={{color:'var(--ash)',transition:'color 0.2s'}} onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.color='var(--punch)';}} onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.color='var(--ash)';}}>الرئيسية</Link>
          <span style={{color:'var(--punch)'}}>/</span>
          <span className="ub" style={{fontWeight:700,fontSize:'8px',color:'var(--ink)'}}>{product.name.toUpperCase().slice(0,28)}</span>
        </div>
