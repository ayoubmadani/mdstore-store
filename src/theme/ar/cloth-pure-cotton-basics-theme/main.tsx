...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  const SearchResults = () => (
    <div style={{
      ...
      maxHeight: '350px',
      overflowY: 'auto',
      boxShadow: '0 8px 24px rgba(74,64,53,0.1)',
      marginTop: '5px',
      borderRadius: '12px'
    }}>
      {loading ? (
  ...
  return (
    <header dir="rtl" style={{ borderBottom: '1px solid var(--border-color)', background: 'var(--bg-main)', position: 'sticky', top: 0, zIndex: 100 }}>
      <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '75px' }}>

        <div style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
          <Link href="/" style={{ fontSize: '1.3rem', fontWeight: 900, color: 'var(--text-main)' }}>
          ...
          <div style={{ position: 'relative', width: '250px' }} className="lg-block">
            <form onSubmit={handleSearchSubmit}>
              <input
                ...
                style={{ width: '100%', padding: '0.6rem 1rem 0.6rem 2.5rem', border: '1px solid var(--border-color)', background: 'var(--bg-alt)', borderRadius: '50px', outline: 'none', fontSize: '0.8rem', color: 'var(--text-main)' }}
              />
      ...
      {open && (
        <div className="anim-fade-in" style={{ position: 'absolute', top: '75px', left: 0, right: 0, background: 'var(--bg-main)', borderBottom: '1px solid var(--border-color)', padding: '1.2rem', zIndex: 110 }}>
          <div style={{ position: 'relative', marginBottom: '1.5rem' }}>
            <form onSubmit={handleSearchSubmit}>
              <input
                ...
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                style={{ width: '100%', padding: '0.8rem 1rem', border: '1px solid var(--border-color)', borderRadius: '50px', background: 'var(--bg-alt)', outline: 'none', color: 'var(--text-main)' }}
              />
            </form>
            {searchQuery.length >= 2 && <SearchResults />}
          </div>
...
export function Footer({ store }: any) {
  return (
    <footer dir="rtl" style={{ borderTop: '1px solid var(--border-color)', padding: '4rem 0 2rem', background: 'var(--text-main)' }}>
      <div className="container grid-3">
        <div>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '1rem', color: 'var(--bg-main)' }}>{store?.name}</h3>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', lineHeight: 1.6 }}>ملابس قطنية خالصة، راحة يومية لا تنتهي.</p>
        </div>
        <div>
          <h4 style={{ fontWeight: 700, marginBottom: '1rem', color: 'var(--bg-main)' }}>روابط</h4>
        ...
        <div>
          <h4 style={{ fontWeight: 700, marginBottom: '1rem', color: 'var(--bg-main)' }}>تواصل</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.9rem', color: 'var(--text-muted)' }}>
      ...
      <div className="container" style={{ textAlign: 'center', marginTop: '3rem', paddingTop: '1.5rem', borderTop: '1px solid #3D3530', color: 'var(--text-muted)', fontSize: '0.8rem' }}>
...
export function Card({ product, displayImage, discount, store, viewDetails }: any) {
  ...
  return (
    <Link href={`/product/${product.slug || product.id}`} style={{ display: 'block' }} className="anim-fade-in">
      <div style={{ border: '1px solid var(--border-color)', borderRadius: '8px', overflow: 'hidden', background: 'var(--bg-main)', transition: 'box-shadow 0.2s', height: '100%', display: 'flex', flexDirection: 'column', boxShadow: '0 2px 8px rgba(74,64,53,0.05)' }}>
        <div style={{ aspectRatio: '1/1', background: 'var(--bg-alt)', position: 'relative' }}>
          ...
          {discount > 0 && <span style={{ position: 'absolute', top: 10, right: 10, background: 'var(--primary)', color: '#fff', fontSize: '10px', padding: '4px 8px', borderRadius: '50px' }}>-{discount}%</span>}
        </div>
    ...
    <div dir="rtl">
      ...
      <section style={{
        background: store.hero.imageUrl
          ? `linear-gradient(rgba(248, 244, 238, 0.5), rgba(248, 244, 238, 0.5)), url(${store.hero.imageUrl})`
          : '#F8F4EE',
        ...
        <div className="container anim-fade-in" style={{ width: '100%' }}>
          <h1 style={{
            ...
            {store.hero?.title?.replace(/<[^>]+>/g, '') || 'الراحة في كل لحظة'}
          </h1>
          ...
          <p style={{
            ...
            {store.hero?.subtitle || 'ملابس قطنية خالصة للأيام العادية التي تستحق الأناقة.'}
          </p>
            ...
            {Array.from({ length: countPage }).map((_, i) => {
              ...
              return (
                <Link key={pn} href={{ query: { page: pn } }} scroll={false} style={{
                  ...
                  borderRadius: '50px', border: `1px solid ${isActive ? 'var(--primary)' : 'var(--border-color)'}`,
      ...
      {/* ── FEATURES ── */}
      <section style={{ borderBottom: '1px solid var(--border-color)', background: 'var(--bg-alt)' }}>
        <div className="container grid-3" style={{ padding: '3rem 1.5rem' }}>
          {[
            { icon: <Truck size={20} />, t: 'توصيل سريع', d: 'لجميع الولايات' },
            { icon: <ShieldCheck size={20} />, t: 'قطن خالص ١٠٠٪', d: 'جودة مضمونة' },
            { icon: <CheckCircle2 size={20} />, t: 'تسوق آمن', d: 'دفع عند الاستلام' },
          ].map((item, idx) => (
            <div key={idx} style={{ textAlign: 'center' }}>
              <div style={{ display: 'inline-flex', padding: '1rem', background: 'var(--bg-main)', borderRadius: '50%', marginBottom: '1rem', color: 'var(--primary)' }}>{item.icon}</div>
      ...
      <div className="grid-2" style={{ gap: '3rem', alignItems: 'start' }}>
        ...
        <div>
          <div style={{ aspectRatio: '1/1', background: 'var(--bg-alt)', borderRadius: '8px', overflow: 'hidden', position: 'relative' }}>
            ...
            {discount > 0 && <span style={{ position: 'absolute', top: 12, right: 12, background: 'var(--primary)', color: '#fff', fontSize: '12px', padding: '4px 10px', borderRadius: '50px' }}>خصم {discount}%</span>}
          </div>
          {allImages.length > 1 && (
  ...
  return (
    <div>
      <form onSubmit={handleSubmit}>
        <div style={{ background: 'var(--bg-alt)', padding: '1.5rem', borderRadius: '12px', marginBottom: '1.5rem' }}>
          ...
          <FR label="الكمية">
            <div style={{ display: 'flex', alignItems: 'center', border: '1px solid var(--border-color)', borderRadius: '50px', width: 'fit-content', background: 'var(--bg-main)' }}>
  ...
  return (
    <div className="container" style={{ padding: '4rem 1.5rem', minHeight: '70vh' }}>
      ...
      <div className="grid-2" style={{ gap: '3rem', alignItems: 'start' }}>
        ...
        <div style={{ background: 'var(--bg-alt)', padding: '2rem', borderRadius: '12px' }}>
