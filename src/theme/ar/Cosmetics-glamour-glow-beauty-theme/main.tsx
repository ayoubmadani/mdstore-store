...
const THEME_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Buttons & Inputs ── */
  .btn-primary {
    background: var(--primary); color: #fff; padding: 0.8rem 1.5rem; border-radius: 50px;
    border: none; font-weight: 600; cursor: pointer; transition: background 0.2s, box-shadow 0.2s;
    display: inline-flex; align-items: center; justify-content: center; gap: 0.5rem;
    letter-spacing: 0.5px;
  }
  .btn-primary:hover { background: var(--primary-hover); box-shadow: 0 4px 15px rgba(212,70,123,0.35); }
  .btn-primary:disabled { opacity: 0.7; cursor: not-allowed; }

  .btn-outline {
    background: transparent; color: var(--text-main); padding: 0.8rem 1.5rem; border-radius: 50px;
  ...
  .input-base {
    ...
    background: #fff; font-family: inherit; color: var(--text-main); outline: none;
    transition: border-color 0.2s, box-shadow 0.2s;
  }
  .input-base:focus { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(212,70,123,0.1); }
...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  const SearchResults = () => (
    <div style={{
      ...
      maxHeight: '350px',
      overflowY: 'auto',
      boxShadow: '0 10px 30px rgba(212,70,123,0.12)',
      marginTop: '5px',
      borderRadius: '12px'
    }}>
      {loading ? (
        ...
        listSearch.map((p: any) => (
          ...
          >
            <img src={p.productImage || p.imagesProduct?.[0]?.imageUrl} style={{ width: 40, height: 40, objectFit: 'cover', borderRadius: '8px' }} alt="" />
            <div>
  ...
  return (
    <header dir="rtl" style={{ borderBottom: '1px solid var(--border-color)', background: '#fff', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 1px 8px rgba(212,70,123,0.06)' }}>
      <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '75px' }}>

        <div style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
          <Link href="/" style={{ fontSize: '1.3rem', fontWeight: 900, color: 'var(--primary)', letterSpacing: '-0.5px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.2rem' }}>
          <div style={{ position: 'relative', width: '250px' }} className="lg-block">
            <form onSubmit={handleSearchSubmit}>
              <input
                ...
                style={{ width: '100%', padding: '0.6rem 1rem 0.6rem 2.5rem', border: '1px solid var(--border-color)', background: 'var(--bg-main)', borderRadius: '50px', outline: 'none', fontSize: '0.8rem', color: 'var(--text-main)' }}
              />
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
            <Link href="/cart" style={{ position: 'relative', color: 'var(--text-main)' }}>
              <ShoppingBag size={22} />
              {itemsCartCount > 0 && (
                <span style={{ position: 'absolute', top: -5, right: -8, background: '#C9A227', color: '#fff', fontSize: 9, width: 17, height: 17, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%', fontWeight: 700 }}>
                  {itemsCartCount}
                </span>
              )}
            </Link>
      ...
      {open && (
        <div className="anim-fade-in" style={{ position: 'absolute', top: '75px', left: 0, right: 0, background: '#fff', borderBottom: '1px solid var(--border-color)', padding: '1.2rem', zIndex: 110 }}>
          <div style={{ position: 'relative', marginBottom: '1.5rem' }}>
            <form onSubmit={handleSearchSubmit}>
              <input
                ...
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                style={{ width: '100%', padding: '0.8rem 1rem', border: '1px solid var(--border-color)', borderRadius: '12px', background: 'var(--bg-main)', outline: 'none', color: 'var(--text-main)' }}
              />
            </form>
            {searchQuery.length >= 2 && <SearchResults />}
          </div>
...
export function Footer({ store }: any) {
  return (
    <footer dir="rtl" style={{ padding: '4rem 0 2rem', background: '#2D1B2E', color: '#F0D0DC' }}>
      <div className="container grid-3">
        <div>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '1rem', color: 'var(--primary)' }}>{store?.name}</h3>
          <p style={{ color: '#C8A0B0', fontSize: '0.9rem', lineHeight: 1.6 }}>جمال حقيقي، أناقة لا تُضاهى. اكتشفي مجموعتنا الفاخرة من مستحضرات التجميل.</p>
        </div>
        <div>
          <h4 style={{ fontWeight: 700, marginBottom: '1rem', color: '#F0D0DC', borderBottom: '1px solid #4D2B3E', paddingBottom: '0.5rem' }}>روابط</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.9rem' }}>
            <Link href="/" style={{ color: '#C8A0B0', transition: 'color 0.2s' }}>الرئيسية</Link>
            <Link href="/contact" style={{ color: '#C8A0B0', transition: 'color 0.2s' }}>تواصل معنا</Link>
            <Link href="/Privacy" style={{ color: '#C8A0B0', transition: 'color 0.2s' }}>سياسة الخصوصية</Link>
            <Link href="/Terms" style={{ color: '#C8A0B0', transition: 'color 0.2s' }}>الشروط والأحكام</Link>
          </div>
        </div>
        <div>
          <h4 style={{ fontWeight: 700, marginBottom: '1rem', color: '#F0D0DC', borderBottom: '1px solid #4D2B3E', paddingBottom: '0.5rem' }}>تواصلي معنا</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.9rem', color: '#C8A0B0' }}>
      ...
      <div className="container" style={{ textAlign: 'center', marginTop: '3rem', paddingTop: '1.5rem', borderTop: '1px solid #4D2B3E', color: '#8B6B7A', fontSize: '0.8rem' }}>
...
export function Card({ product, displayImage, discount, store, viewDetails }: any) {
  ...
  return (
    <Link href={`/product/${product.slug || product.id}`} style={{ display: 'block' }} className="anim-fade-in">
      <div style={{ border: `1px solid var(--border-color)`, borderRadius: '12px', overflow: 'hidden', background: '#fff', transition: 'box-shadow 0.3s, transform 0.3s', height: '100%', display: 'flex', flexDirection: 'column' }}
        onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.boxShadow = '0 8px 30px rgba(212,70,123,0.18)'; (e.currentTarget as HTMLDivElement).style.transform = 'translateY(-3px)'; }}
      ...
      >
        <div style={{ aspectRatio: '1/1', background: 'var(--bg-alt)', position: 'relative', overflow: 'hidden' }}>
          ...
          {discount > 0 && <span style={{ position: 'absolute', top: 10, right: 10, background: 'var(--primary)', color: '#fff', fontSize: '10px', padding: '4px 10px', borderRadius: '50px', fontWeight: 600 }}>-{discount}%</span>}
        </div>
    ...
    <div dir="rtl">
      ...
      <section style={{
        background: store.hero?.imageUrl
          ? `linear-gradient(rgba(45,27,46,0.55), rgba(45,27,46,0.55)), url(${store.hero.imageUrl})`
          : 'linear-gradient(135deg, #D4467B, #8B1A4A)',
        ...
        <div className="container anim-fade-in" style={{ width: '100%' }}>
          <p style={{ fontSize: '0.85rem', letterSpacing: '3px', color: '#F9C8DC', textTransform: 'uppercase', marginBottom: '1rem', fontWeight: 500 }}>✦ أناقة · جمال · فخامة ✦</p>
          <h1 style={{
            ...
            {store.hero?.title?.replace(/<[^>]+>/g, '') || 'اكتشفي جمالك الحقيقي'}
          </h1>
          ...
          <p style={{
            ...
            {store.hero?.subtitle || 'مستحضرات تجميل فاخرة تُبرز جمالك الطبيعي بلمسة من الأناقة والرقي.'}
          </p>
      ...
      <section id="products" className="container" style={{ padding: '4rem 1.5rem' }}>
        ...
        {products.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '4rem', background: 'var(--bg-alt)', borderRadius: '12px' }}>
            ...
            {Array.from({ length: countPage }).map((_, i) => {
              ...
              return (
                <Link key={pn} href={{ query: { page: pn } }} scroll={false} style={{
                  ...
                  borderRadius: '50%', border: `1px solid ${isActive ? 'var(--primary)' : 'var(--border-color)'}`,
      ...
      <section style={{ borderTop: '1px solid var(--border-color)', borderBottom: '1px solid var(--border-color)', background: 'var(--bg-alt)' }}>
        <div className="container grid-3" style={{ padding: '3rem 1.5rem' }}>
          {[
            { icon: <Truck size={20} color="var(--primary)" />, t: 'توصيل سريع', d: 'لجميع الولايات' },
            { icon: <ShieldCheck size={20} color="var(--primary)" />, t: 'منتجات أصيلة', d: 'جودة مضمونة 100%' },
            { icon: <CheckCircle2 size={20} color="var(--primary)" />, t: 'دفع آمن', d: 'دفع عند الاستلام' },
          ].map((item, idx) => (
            <div key={idx} style={{ textAlign: 'center' }}>
              <div style={{ display: 'inline-flex', padding: '1rem', background: '#fff', borderRadius: '50%', marginBottom: '1rem', boxShadow: '0 4px 15px rgba(212,70,123,0.1)' }}>{item.icon}</div>
  ...
  return (
    <div dir="rtl" className="container" style={{ padding: '3rem 1.5rem' }}>
      <div className="grid-2" style={{ gap: '3rem', alignItems: 'start' }}>
        ...
        {/* Gallery */}
        <div>
          <div style={{ aspectRatio: '1/1', background: 'var(--bg-alt)', borderRadius: '12px', overflow: 'hidden', position: 'relative' }}>
            {allImages[sel] ? <img src={allImages[sel]} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" /> : null}
            {discount > 0 && <span style={{ position: 'absolute', top: 12, right: 12, background: 'var(--primary)', color: '#fff', fontSize: '12px', padding: '4px 12px', borderRadius: '50px', fontWeight: 600 }}>خصم {discount}%</span>}
          </div>
          {allImages.length > 1 && (
            <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem', overflowX: 'auto' }}>
              {allImages.map((img: string, idx: number) => (
                <button key={idx} onClick={() => setSel(idx)} style={{ width: 60, height: 60, flexShrink: 0, border: `2px solid ${sel === idx ? 'var(--primary)' : 'var(--border-color)'}`, borderRadius: '8px', padding: 0, cursor: 'pointer', overflow: 'hidden' }}>
          ...
          {product.offers?.length > 0 && (
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              {product.offers.map((o: any) => (
                <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '1rem', border: `1px solid ${selectedOffer === o.id ? 'var(--primary)' : 'var(--border-color)'}`, borderRadius: '10px', marginBottom: '0.5rem', cursor: 'pointer', background: selectedOffer === o.id ? 'var(--bg-alt)' : 'transparent' }}>
...
const FR = ({ error, label, children }: { error?: string; label?: string; children: React.ReactNode }) => (
  <div style={{ marginBottom: '1rem' }}>
    {label && <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 500, marginBottom: '0.4rem', color: 'var(--text-main)' }}>{label}</label>}
    {children}
  ...
  return (
    <div>
      <form onSubmit={handleSubmit}>
        <div style={{ background: 'var(--bg-alt)', padding: '1.5rem', borderRadius: '12px', marginBottom: '1.5rem', border: '1px solid var(--border-color)' }}>
          ...
          <FR label="طريقة التوصيل">
            <div className="grid-2">
              {(['home', 'office'] as const).map(t => (
                <button key={t} type="button" onClick={() => setFd({ ...fd, typeLivraison: t })} className="btn-outline" style={{ borderColor: fd.typeLivraison === t ? 'var(--primary)' : 'var(--border-color)', background: fd.typeLivraison === t ? 'var(--bg-alt)' : 'transparent', color: fd.typeLivraison === t ? 'var(--primary)' : 'var(--text-main)' }}>
          ...
          <FR label="الكمية">
            <div style={{ display: 'flex', alignItems: 'center', border: '1px solid var(--border-color)', borderRadius: '50px', width: 'fit-content', background: '#fff' }}>
  ...
  return (
    <div className="container" style={{ padding: '4rem 1.5rem', minHeight: '70vh' }}>
      ...
      <div className="grid-2" style={{ gap: '3rem', alignItems: 'start' }}>
        <div>
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '1rem', padding: '1rem', border: '1px solid var(--border-color)', borderRadius: '12px', marginBottom: '1rem', background: '#fff' }}>
              <img src={item.product?.productImage} alt="" style={{ width: 80, height: 80, objectFit: 'cover', borderRadius: '8px' }} />
              <div style={{ flex: 1 }}>
        ...
        <div style={{ background: 'var(--bg-alt)', padding: '2rem', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
    ...
    <div className="container" style={{ padding: '5rem 1.5rem', maxWidth: '450px', margin: '0 auto' }} dir="rtl">
      {sent ? (
        <div style={{ textAlign: 'center', padding: '2rem 0' }} className="anim-fade-in">
          <div style={{ display: 'inline-flex', padding: '1rem', background: 'var(--bg-alt)', borderRadius: '50%', marginBottom: '1.5rem', boxShadow: '0 4px 15px rgba(212,70,123,0.15)' }}>
        ...
        <div>
          <div style={{ marginBottom: '2.5rem' }}>
            <h1 style={{ fontSize: '1.5rem', fontWeight: 900, marginBottom: '0.5rem', color: 'var(--text-main)' }}>تواصلي معنا</h1>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>يسعدنا الاستماع إليكِ. أرسلي رسالتك وسنرد عليكِ قريباً.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
            ...
            <textarea
              ...
              style={{ ...glamourInputStyle, resize: 'none' }}
              rows={5}
              placeholder="رسالتك..."
              required
            ></textarea>
            ...
            <button
              ...
              style={{
                ...
                boxShadow: '0 4px 15px rgba(212,70,123,0.3)'
              }}
            >
              {loading ? (
                <Loader2 size={16} className="animate-spin" />
              ) : (
                <>إرسالي رسالتك <Send size={14} style={{ transform: 'rotate(180deg)' }} /></>
              )}
            </button>
          </form>
        </div>
