            ...
            {store?.design?.logoUrl
              ...
              : (
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <div style={{ width: 28, height: 28, background: '#D4AF37', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ padding: '0.625rem 1.25rem', background: '#fff', borderTop: '1px solid #E8E8E8', position: 'relative' }} className="anim-slide-down">
            <form onSubmit={handleSearch} style={{ position: 'relative' }}>
              <input autoFocus type="text" placeholder="ابحث عن منتج..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                style={{ width: '100%', padding: '0.75rem 1rem 0.75rem 2.75rem', border: '1.5px solid #D4AF37', borderRadius: 8, background: '#F8F8F6', fontSize: '0.9rem', outline: 'none' }} />
      ...
      <div style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', flex: 1 }}>
        ...
        <div style={{ marginTop: 'auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.375rem', marginBottom: '1rem' }}>
            ...
            <span style={{ fontSize: '0.75rem', color: '#D4AF37', fontWeight: 700 }}>{store.currency || 'دج'}</span>
      ...
      <div className="details-inner" style={{ maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div style={{ position: 'sticky', top: 100 }}>
          <div style={{ position: 'relative', aspectRatio: '1/1', borderRadius: 14, overflow: 'hidden', background: '#F0F0EE', border: '1.5px solid #E8E8E8' }}>
            ...
            {discount > 0 && <div style={{ position: 'absolute', top: 12, right: 12, background: '#D4AF37', color: '#fff', padding: '3px 12px', borderRadius: 5, fontSize: 12, fontWeight: 800 }}>{discount}% خصم</div>}
            {allImages.length > 1 && (
              <>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
                <span className="price-mono" style={{
                  fontSize: '2.5rem',
                  fontWeight: 800,
                  color: '#fff', // السعر بالأبيض لتباين قوي على الأسود
            ...
            {product.offers?.length > 0 && (
              <div style={{ marginBottom: '1.25rem' }}>
                {product.offers.map((o: any) => (
                  <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.875rem 1rem', border: `1.5px solid ${selectedOffer === o.id ? '#D4AF37' : '#E8E8E8'}`, borderRadius: 10, cursor: 'pointer', marginBottom: '0.5rem', background: selectedOffer === o.id ? 'rgba(230,57,70,0.04)' : 'transparent', transition: 'all 0.18s' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    {attr.variants.map((v: any) => (
                      <button key={v.id} onClick={() => handleVariantSelection(attr.name, v.value)}
                        style={{ width: 30, height: 30, borderRadius: '50%', background: v.value, border: 'none', cursor: 'pointer', outline: `2.5px solid ${selectedVariants[attr.name] === v.value ? '#D4AF37' : 'transparent'}`, outlineOffset: 3, transition: 'outline 0.18s' }} />
                    ))}
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {attr.variants.map((v: any) => (
                      <button key={v.id} onClick={() => handleVariantSelection(attr.name, v.value)} style={{
                        padding: '0.4rem 1rem', border: `1.5px solid ${selectedVariants[attr.name] === v.value ? '#D4AF37' : '#E8E8E8'}`, borderRadius: 6,
                        fontSize: '0.85rem', fontWeight: 600,
                        color: selectedVariants[attr.name] === v.value ? '#D4AF37' : '#555',
          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.875rem', border: `1.5px solid ${fd.typeLivraison === t ? '#D4AF37' : '#E8E8E8'}`, borderRadius: 10, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? 'rgba(230,57,70,0.05)' : '#fff', transition: 'all 0.18s', fontFamily: 'inherit' }}>
                    <p style={{ fontWeight: 700, fontSize: '0.85rem', marginBottom: 4, color: fd.typeLivraison === t ? '#D4AF37' : '#555' }}>{t === 'home' ? 'للبيت' : 'للمكتب'}</p>
            ...
            <div style={{ background: '#F8F8F6', border: '1.5px solid #EBEBEB', borderRadius: 10, padding: '1rem 1.125rem', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '0.375rem' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.625rem', fontWeight: 800, color: '#D4AF37' }}>{total().toLocaleString()} <span style={{ fontSize: '0.8rem', fontFamily: 'inherit', fontWeight: 700, color: '#D4AF37' }}>دج</span></span>
              </div>
            </div>
  ...
  if (success) return (
    <div dir="rtl" style={{ minHeight: '70vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: '#F8F8F6' }}>
      <div style={{ textAlign: 'center', background: '#fff', padding: '3.5rem 2rem', borderRadius: 16, border: '1.5px solid #E8E8E8', maxWidth: 460, width: '100%' }}>
        ...
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#D4AF37', color: '#fff', padding: '0.8rem 2rem', borderRadius: 10, fontWeight: 700, fontSize: '0.9rem' }}>العودة للمتجر</Link>
      </div>
    </div>
  );
  ...
  if (!items.length) return (
    <div dir="rtl" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: '#F8F8F6' }}>
      <div style={{ textAlign: 'center', padding: '4rem 2rem', border: '2px dashed #E8E8E8', borderRadius: 16, maxWidth: 440, width: '100%', background: '#fff' }}>
        ...
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#D4AF37', color: '#fff', padding: '0.8rem 2rem', borderRadius: 10, fontWeight: 700, fontSize: '0.875rem' }}>تسوق الآن</Link>
      </div>
    </div>
  );
          ...
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '1rem', padding: '1rem', borderBottom: '1px solid #F0F0F0' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p className="price-mono" style={{ fontSize: '1.125rem', fontWeight: 800, color: '#D4AF37' }}>{item.finalPrice?.toLocaleString()} دج</p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ background: '#F8F8F6', border: '1.5px solid #EBEBEB', borderRadius: 10, padding: '1rem 1.125rem', margin: '1rem 0' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.75rem', fontWeight: 800, color: '#D4AF37' }}>{finalTotal.toLocaleString()} <span style={{ fontSize: '0.8rem', fontWeight: 700, color: '#D4AF37' }}>دج</span></span>
              </div>
            </div>
        ...
        <div style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E8E8E8', padding: '2rem' }}>
          {sent ? (
            <div style={{ textAlign: 'center', padding: '4rem 1rem' }}>
              ...
              <button onClick={() => setSent(false)} style={{ padding: '0.75rem 1.75rem', borderRadius: 10, border: '1.5px solid #D4AF37', background: 'transparent', color: '#D4AF37', fontWeight: 700, cursor: 'pointer', fontSize: '0.875rem', fontFamily: 'inherit' }}>إرسال رسالة أخرى</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
