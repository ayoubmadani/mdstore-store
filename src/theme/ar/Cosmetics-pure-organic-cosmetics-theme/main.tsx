...
/* ─── PURE & ORGANIC COSMETICS THEME CSS ─── */
const THEME_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Buttons & Inputs ── */
  .btn-primary {
    background: var(--primary); color: #fff; padding: 0.8rem 1.5rem; border-radius: 4px;
...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  const SearchResults = () => (
    <div style={{
      ...
      maxHeight: '350px',
      overflowY: 'auto',
      boxShadow: '0 8px 20px rgba(44,51,32,0.08)',
      marginTop: '5px',
      borderRadius: '6px'
    }}>
      {loading ? (
  ...
  return (
    <header dir="rtl" style={{ borderBottom: '1px solid var(--border-color)', background: '#FDFBF8', position: 'sticky', top: 0, zIndex: 100 }}>
          ...
          <div style={{ position: 'relative', width: '250px' }} className="lg-block">
            <form onSubmit={handleSearchSubmit}>
              <input
                ...
                style={{ width: '100%', padding: '0.6rem 1rem 0.6rem 2.5rem', border: '1px solid var(--border-color)', background: 'var(--bg-alt)', borderRadius: '4px', outline: 'none', fontSize: '0.8rem', color: 'var(--text-main)' }}
              />
      ...
      {open && (
        <div className="anim-fade-in" style={{ position: 'absolute', top: '75px', left: 0, right: 0, background: '#FDFBF8', borderBottom: '1px solid var(--border-color)', padding: '1.2rem', zIndex: 110 }}>
          <div style={{ position: 'relative', marginBottom: '1.5rem' }}>
            <form onSubmit={handleSearchSubmit}>
              <input
                ...
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                style={{ width: '100%', padding: '0.8rem 1rem', border: '1px solid var(--border-color)', borderRadius: '4px', background: 'var(--bg-alt)', outline: 'none', color: 'var(--text-main)' }}
              />
            </form>
            {searchQuery.length >= 2 && <SearchResults />}
          </div>
...
export function Footer({ store }: any) {
  return (
    <footer dir="rtl" style={{ padding: '4rem 0 2rem', background: '#2C3320', color: '#C8D4B8' }}>
      <div className="container grid-3">
        <div>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '1rem', color: '#E8F0DC' }}>{store?.name}</h3>
          <p style={{ color: '#9AAA88', fontSize: '0.9rem', lineHeight: 1.6 }}>منتجات طبيعية خالصة لبشرة صحية متوهجة. نؤمن بجمال الطبيعة ونقائها.</p>
        </div>
        <div>
          <h4 style={{ fontWeight: 700, marginBottom: '1rem', color: '#E8F0DC', borderBottom: '1px solid #3D5C3D', paddingBottom: '0.5rem' }}>روابط</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.9rem' }}>
            <Link href="/" style={{ color: '#9AAA88', transition: 'color 0.2s' }}>الرئيسية</Link>
            <Link href="/contact" style={{ color: '#9AAA88', transition: 'color 0.2s' }}>تواصل معنا</Link>
            <Link href="/Privacy" style={{ color: '#9AAA88', transition: 'color 0.2s' }}>سياسة الخصوصية</Link>
            <Link href="/Terms" style={{ color: '#9AAA88', transition: 'color 0.2s' }}>الشروط والأحكام</Link>
          </div>
        </div>
        <div>
          <h4 style={{ fontWeight: 700, marginBottom: '1rem', color: '#E8F0DC', borderBottom: '1px solid #3D5C3D', paddingBottom: '0.5rem' }}>تواصل</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.9rem', color: '#9AAA88' }}>
      ...
      <div className="container" style={{ textAlign: 'center', marginTop: '3rem', paddingTop: '1.5rem', borderTop: '1px solid #3D5C3D', color: '#7A8A70', fontSize: '0.8rem' }}>
...
export function Card({ product, displayImage, discount, store, viewDetails }: any) {
  ...
  return (
    <Link href={`/product/${product.slug || product.id}`} style={{ display: 'block' }} className="anim-fade-in">
      <div style={{ border: `1px solid var(--border-color)`, borderRadius: '4px', overflow: 'hidden', background: 'var(--bg-main)', transition: 'box-shadow 0.2s', height: '100%', display: 'flex', flexDirection: 'column' }}
        onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.boxShadow = '0 4px 16px rgba(44,51,32,0.10)'; }}
      ...
      >
        <div style={{ aspectRatio: '1/1', background: 'var(--bg-alt)', position: 'relative', overflow: 'hidden' }}>
          ...
          {discount > 0 && <span style={{ position: 'absolute', top: 10, right: 10, background: 'var(--primary)', color: '#fff', fontSize: '10px', padding: '3px 8px', borderRadius: '3px', fontWeight: 500 }}>-{discount}%</span>}
        </div>
    ...
    <div dir="rtl">
      ...
      <section style={{
        background: store.hero?.imageUrl
          ? `linear-gradient(rgba(44,51,32,0.4), rgba(44,51,32,0.4)), url(${store.hero.imageUrl})`
          : 'var(--bg-alt)',
        ...
        <div className="container anim-fade-in" style={{ width: '100%' }}>
          <p style={{ fontSize: '0.8rem', letterSpacing: '3px', color: store.hero?.imageUrl ? '#C8D4B8' : 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '1rem', fontWeight: 500 }}>طبيعي · عضوي · نقي</p>
          <h1 style={{
            fontSize: 'clamp(2rem, 5vw, 3.5rem)',
            color: store.hero?.imageUrl ? '#ffffff' : 'var(--primary)',
            fontWeight: 700,
            marginBottom: '1rem',
            textShadow: store.hero?.imageUrl ? '0 2px 4px rgba(0,0,0,0.2)' : 'none'
          }}>
            {store.hero?.title?.replace(/<[^>]+>/g, '') || 'الجمال الطبيعي يبدأ من هنا'}
          </h1>

          <p style={{
            fontSize: '1.1rem',
            color: store.hero?.imageUrl ? '#E8F0DC' : 'var(--text-muted)',
            maxWidth: '580px',
            margin: '0 auto 2rem',
            lineHeight: '1.7'
          }}>
            {store.hero?.subtitle || 'منتجات عضوية خالصة من الطبيعة، صُنعت لك لبشرة نضرة وصحية في كل يوم.'}
          </p>
      ...
      <section style={{ borderTop: '1px solid var(--border-color)', borderBottom: '1px solid var(--border-color)', background: 'var(--bg-alt)' }}>
        <div className="container grid-3" style={{ padding: '3rem 1.5rem' }}>
          {[
            ...
            { icon: <ShieldCheck size={20} color="var(--primary)" />, t: 'منتجات عضوية', d: '100% طبيعية ومعتمدة' },
            { icon: <CheckCircle2 size={20} color="var(--primary)" />, t: 'دفع عند الاستلام', d: 'تسوق بأمان تام' },
          ].map((item, idx) => (
            <div key={idx} style={{ textAlign: 'center' }}>
              <div style={{ display: 'inline-flex', padding: '1rem', background: 'var(--bg-main)', borderRadius: '50%', marginBottom: '1rem', border: '1px solid var(--border-color)' }}>{item.icon}</div>
      ...
      <div className="grid-2" style={{ gap: '3rem', alignItems: 'start' }}>
        ...
        <div>
          <div style={{ aspectRatio: '1/1', background: 'var(--bg-alt)', borderRadius: '4px', overflow: 'hidden', position: 'relative' }}>
            ...
            {discount > 0 && <span style={{ position: 'absolute', top: 12, right: 12, background: 'var(--primary)', color: '#fff', fontSize: '12px', padding: '4px 10px', borderRadius: '3px' }}>خصم {discount}%</span>}
          </div>
          {allImages.length > 1 && (
          ...
          {product.offers?.length > 0 && (
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              {product.offers.map((o: any) => (
                <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '1rem', border: `1px solid ${selectedOffer === o.id ? 'var(--primary)' : 'var(--border-color)'}`, borderRadius: '4px', marginBottom: '0.5rem', cursor: 'pointer', background: selectedOffer === o.id ? 'var(--bg-alt)' : 'transparent' }}>
    ...
    <div>
      <form onSubmit={handleSubmit}>
        <div style={{ background: 'var(--bg-alt)', padding: '1.5rem', borderRadius: '4px', marginBottom: '1.5rem', border: '1px solid var(--border-color)' }}>
          <h3 style={{ fontSize: '1.1rem', fontWeight: 600, marginBottom: '1rem', color: 'var(--text-main)' }}>الطلب السريع</h3>
          ...
          <FR label="طريقة التوصيل">
            <div className="grid-2">
              {(['home', 'office'] as const).map(t => (
                <button key={t} type="button" onClick={() => setFd({ ...fd, typeLivraison: t })} className="btn-outline" style={{ borderColor: fd.typeLivraison === t ? 'var(--primary)' : 'var(--border-color)', background: fd.typeLivraison === t ? 'var(--bg-main)' : 'transparent', color: fd.typeLivraison === t ? 'var(--primary)' : 'var(--text-main)' }}>
          ...
          <FR label="الكمية">
            <div style={{ display: 'flex', alignItems: 'center', border: '1px solid var(--border-color)', borderRadius: '4px', width: 'fit-content', background: 'var(--bg-main)' }}>
  ...
  return (
    <div className="container" style={{ padding: '4rem 1.5rem', minHeight: '70vh' }}>
      ...
      <div className="grid-2" style={{ gap: '3rem', alignItems: 'start' }}>
        <div>
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '1rem', padding: '1rem', border: '1px solid var(--border-color)', borderRadius: '4px', marginBottom: '1rem', background: 'var(--bg-main)' }}>
        ...
        <div style={{ background: 'var(--bg-alt)', padding: '2rem', borderRadius: '4px', border: '1px solid var(--border-color)' }}>
    ...
    <div className="container" style={{ padding: '5rem 1.5rem', maxWidth: '450px', margin: '0 auto' }} dir="rtl">
      {sent ? (
        <div style={{ textAlign: 'center', padding: '2rem 0' }} className="anim-fade-in">
          <div style={{ display: 'inline-flex', padding: '1rem', background: 'var(--bg-alt)', borderRadius: '50%', marginBottom: '1.5rem', border: '1px solid var(--border-color)' }}>
        ...
        <div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
            ...
            <textarea
              ...
              style={{ ...organicInputStyle, resize: 'none' }}
              rows={5}
              placeholder="رسالتك..."
              required
            ></textarea>
