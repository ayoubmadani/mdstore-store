            ...
            {store?.design?.logoUrl
              ...
              : (
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <div style={{ width: 28, height: 28, background: '#F472B6', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div className="details-inner" style={{ maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div style={{ position: 'sticky', top: 100 }}>
          <div style={{ position: 'relative', aspectRatio: '1/1', borderRadius: 14, overflow: 'hidden', background: '#F0F0EE', border: '1.5px solid #E8E8E8' }}>
            ...
            {discount > 0 && <div style={{ position: 'absolute', top: 12, right: 12, background: '#F472B6', color: '#fff', padding: '3px 12px', borderRadius: 5, fontSize: 12, fontWeight: 800 }}>{discount}% خصم</div>}
            {allImages.length > 1 && (
              <>
          ...
          {allImages.length > 1 && (
            <div className="thumb-row">
              {allImages.map((img: string, idx: number) => (
                <button key={idx} onClick={() => setSel(idx)} style={{ flexShrink: 0, width: 60, height: 60, borderRadius: 8, overflow: 'hidden', border: `2px solid ${sel === idx ? '#F472B6' : '#E8E8E8'}`, opacity: sel === idx ? 1 : 0.55, cursor: 'pointer', padding: 0, background: 'none', transition: 'all 0.18s' }}>
        ...
        <div>
          <div style={{ background: '#fff', borderRadius: 14, padding: '1.75rem', border: '1.5px solid #E8E8E8' }}>
            ...
            <div style={{ background: '#FFF5F5', border: '1.5px solid #FFCDD2', borderRadius: 10, padding: '1rem 1.25rem', marginBottom: '1.5rem' }}>
              <p style={{ fontSize: '0.75rem', color: '#F472B6', fontWeight: 700, marginBottom: '0.25rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}>السعر</p>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.375rem' }}>
                <span className="price-mono" style={{ fontSize: '2.25rem', fontWeight: 800, color: '#111' }}>{finalPrice.toLocaleString()}</span>
                <span style={{ fontWeight: 700, color: '#F472B6' }}>دج</span>
              </div>
            </div>
            ...
            {product.offers?.length > 0 && (
              <div style={{ marginBottom: '1.25rem' }}>
                {product.offers.map((o: any) => (
                  <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.875rem 1rem', border: `1.5px solid ${selectedOffer === o.id ? '#F472B6' : '#E8E8E8'}`, borderRadius: 10, cursor: 'pointer', marginBottom: '0.5rem', background: selectedOffer === o.id ? 'rgba(230,57,70,0.04)' : 'transparent', transition: 'all 0.18s' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    {attr.variants.map((v: any) => (
                      <button key={v.id} onClick={() => handleVariantSelection(attr.name, v.value)}
                        style={{ width: 30, height: 30, borderRadius: '50%', background: v.value, border: 'none', cursor: 'pointer', outline: `2.5px solid ${selectedVariants[attr.name] === v.value ? '#F472B6' : 'transparent'}`, outlineOffset: 3, transition: 'outline 0.18s' }} />
                    ))}
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {attr.variants.map((v: any) => (
                      <button key={v.id} onClick={() => handleVariantSelection(attr.name, v.value)} style={{
                        padding: '0.4rem 1rem', border: `1.5px solid ${selectedVariants[attr.name] === v.value ? '#F472B6' : '#E8E8E8'}`, borderRadius: 6,
                        fontSize: '0.85rem', fontWeight: 600,
                        color: selectedVariants[attr.name] === v.value ? '#F472B6' : '#555',
