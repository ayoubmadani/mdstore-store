...
export function Navbar({store,domain}:{store:any;domain:string}){
  ...
  return(
    <>
            ...
            {showS&&(
              <div style={{position:'absolute',top:'calc(100% + 4px)',right:0,width:'300px',zIndex:200,background:'var(--s1)',border:'1px solid var(--br2)'}} className="sd srch-drop">
                <form onSubmit={go} style={{display:'flex',alignItems:'center',borderBottom:'1px solid var(--br)'}}>
                  <input autoFocus type="text" value={sq} onChange={e=>setSq(e.target.value)} placeholder="ابحث عن منتج..." style={{flex:1,padding:'11px 14px',background:'transparent',border:'none',color:'var(--tx)',fontSize:'13px',fontFamily:"'Raleway',sans-serif",outline:'none'}}/>
                ...
                {sq.length>=2&&(
                  <div style={{maxHeight:'55vh',overflowY:'auto'}} className="fi">
                    {ld?<p style={{padding:'14px',fontSize:'11px',color:'var(--mu)',letterSpacing:'0.1em',textAlign:'center'}}>جاري البحث...</p>
                    :ls.length>0?(
                      <>
                        {ls.map((p:any)=>(
                          <Link href={`/product/${p.id}`} key={p.id} onClick={()=>setSq('')}
                            ...
                            <div>
                              ...
                              <p style={{fontSize:'12px',color:'var(--cp)',fontWeight:700}}>{p.price} دج</p>
                            </div>
                          </Link>
                        ))}
                        <button onClick={go} style={{width:'100%',padding:'11px',background:'none',border:'none',borderTop:'1px solid var(--br)',color:'var(--cp)',fontWeight:700,fontSize:'10px',letterSpacing:'0.18em',textTransform:'uppercase',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:6,fontFamily:"'Raleway',sans-serif"}}>
                          عرض جميع النتائج <ArrowLeft size={11}/>
                        </button>
                      </>
                    ):<p style={{padding:'14px',fontSize:'11px',color:'var(--mu)',textAlign:'center',letterSpacing:'0.1em'}}>لا توجد نتائج</p>}
                  </div>
                )}
              </div>
            )}
      ...
      {/* Mobile menu */}
      {mOpen&&(
        <div dir="rtl" style={{position:'fixed',inset:0,zIndex:200,background:'var(--bg)',paddingTop:'60px',display:'flex',flexDirection:'column'}} className="fi">
          ...
          <div style={{flex:1,display:'flex',flexDirection:'column',justifyContent:'center',padding:'0 32px'}}>
            {[{h:'/',l:'الرئيسية'},{h:'/cart',l:'السلة'},{h:'/contact',l:'تواصل'},{h:'/Privacy',l:'الخصوصية'},{h:'/Terms',l:'الشروط'}].filter(lnk => lnk.h !== '/cart' || store?.cart !== false).map(lnk=>(
    ...
    <footer dir="rtl" style={{background:'var(--s1)',borderTop:'1px solid var(--br)',padding:'48px 24px 24px',fontFamily:"'Raleway',sans-serif"}}>
      <div className="footer-grid" style={{maxWidth:1200,margin:'0 auto',paddingBottom:32,borderBottom:'1px solid var(--br)'}}>
        ...
        <div>
          ...
          {[{h:'/',l:'الرئيسية'},{h:'/cart',l:'السلة'},{h:'/contact',l:'تواصل معنا'},{h:'/Privacy',l:'الخصوصية'},{h:'/Terms',l:'الشروط'}].filter(lnk => lnk.h !== '/cart' || store?.cart !== false).map(lnk=>(
        ...
        <div>
          <p className="lbl" style={{marginBottom:14,color:'var(--cp)'}}>معلومات</p>
      ...
      <Link href={`/product/${product.slug||product.id}`} style={{display:'block'}}>
        ...
        <div style={{padding:'12px 2px 0'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            <span style={{fontSize:'14px',fontWeight:700,color:'var(--cp)'}}>{price.toLocaleString()} دج</span>
    ...
    <div dir="rtl">
          ...
          <div className="fu" style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',gap:24,flexWrap:'wrap'}}>
            <div>
              ...
              <p style={{fontSize:'13px',color:'rgba(245,241,235,0.55)',letterSpacing:'0.06em',maxWidth:340,lineHeight:1.8}}>
                {store.hero?.subtitle||'أزياء راقية. توصيل سريع لجميع الولايات.'}
              </p>
            </div>
            <a href="#products" className="btn-dark" style={{textDecoration:'none',flexShrink:0}}>تسوق الآن</a>
          </div>
        </div>
      </section>
      ...
      {cats.length>0&&(
        <div style={{padding:'20px 32px',borderBottom:'1px solid var(--br)',display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
          <Link href="/" style={{padding:'7px 18px',background:'var(--cp)',color:'var(--bg)',fontSize:'10px',fontWeight:700,letterSpacing:'0.14em',textTransform:'uppercase',fontFamily:"'Raleway',sans-serif"}}>الكل</Link>
          {cats.map((cat:any)=>(
      ...
      <section id="products" style={{padding:'32px 32px 64px'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:24}}>
          <h2 style={{fontSize:'14px',fontWeight:700,letterSpacing:'0.12em',textTransform:'uppercase',color:'var(--tx)'}}>المجموعة</h2>
          <span style={{fontSize:'11px',color:'var(--mu)',letterSpacing:'0.06em'}}>{products.length} منتج</span>
        </div>

        {products.length===0?(
          <div style={{minHeight:300,display:'flex',alignItems:'center',justifyContent:'center',background:'var(--s2)'}}>
            <span className="bn" style={{fontSize:'2.5rem',color:'var(--mu)',letterSpacing:'0.08em'}}>لا توجد منتجات</span>
          </div>
        ):(
...
export function Details({product,discount,allImages,allAttrs,finalPrice,inStock,autoGen,selectedVariants,setSelectedOffer,selectedOffer,handleVariantSelection,domain}:any){
  ...
  return(
    <div dir="rtl" style={{background:'var(--bg)',fontFamily:"'Raleway',sans-serif"}}>
      <div className="det-grid">
        ...
        <div style={{padding:'40px 36px',overflowY:'auto',borderRight:'1px solid var(--br)'}}>
          ...
          <div style={{display:'flex',alignItems:'baseline',gap:10,marginBottom:28,paddingBottom:24,borderBottom:'1px solid var(--br)'}}>
            <span style={{fontSize:'1.8rem',fontWeight:800,color:'var(--cp)'}}>{finalPrice.toLocaleString()} دج</span>
            {product.priceOriginal&&parseFloat(product.priceOriginal)>finalPrice&&<span style={{fontSize:'13px',textDecoration:'line-through',color:'var(--mu)'}}>{parseFloat(product.priceOriginal).toLocaleString()} دج</span>}
          </div>

          {product.offers?.length>0&&(
            <div style={{marginBottom:24,paddingBottom:24,borderBottom:'1px solid var(--br)'}}>
              <span className="lbl">اختر العرض</span>
              {product.offers.map((o:any)=>(
                <label key={o.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'11px 14px',border:`1px solid ${selectedOffer===o.id?'var(--cp)':'var(--br2)'}`,background:selectedOffer===o.id?'var(--cl)':'transparent',cursor:'pointer',marginBottom:6,transition:'all .18s'}}>
                  ...
                  <span style={{fontWeight:700,color:selectedOffer===o.id?'var(--cp)':'var(--tx)',fontSize:'14px'}}>{o.price.toLocaleString()} دج</span>
                </label>
              ))}
            </div>
          )}
          ...
          {product.desc&&(
            <div style={{marginTop:32,paddingTop:24,borderTop:'1px solid var(--br)'}}>
              <span className="lbl">وصف المنتج</span>
      ...
      {product.store?.cart&&(
        <div style={{display:'flex',gap:8,marginBottom:14}}>
          <button onClick={addCart} disabled={added} className="btn-ghost" style={{flex:1,padding:'12px',opacity:added?1:undefined,borderColor:added?'#22c55e':undefined,color:added?'#22c55e':undefined}}>
            {added?<><CheckCircle2 size={13}/>أُضيف للسلة</>:<><ShoppingCart size={13}/>أضف للسلة</>}
          </button>
          <button onClick={()=>setON(true)} className="btn-dark" style={{flex:1,padding:'12px'}}>اطلب الآن</button>
        </div>
      )}
        ...
        <div style={{borderTop:'1px solid var(--br)',paddingTop:20,marginTop:8}}>
          {product.store?.cart&&(
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
              ...
              <button onClick={()=>setON(false)} style={{background:'none',border:'none',cursor:'pointer',color:'var(--mu)',display:'flex',alignItems:'center',gap:4,fontSize:'10px',fontFamily:"'Raleway',sans-serif",letterSpacing:'0.14em'}}><X style={{width:10,height:10}}/>إلغاء</button>
            </div>
          )}
          <form onSubmit={submit}>
            <div className="f2">
              <FR label="الاسم" error={errors.name}><input type="text" value={fd.customerName} onChange={e=>setFd({...fd,customerName:e.target.value})} placeholder="الاسم الكامل" style={IS(!!errors.name)} onFocus={onF} onBlur={e=>onBl(e,!!errors.name)}/></FR>
              <FR label="الهاتف" error={errors.phone}><input type="tel" value={fd.customerPhone} onChange={e=>setFd({...fd,customerPhone:e.target.value})} placeholder="0X XX XX XX XX" style={IS(!!errors.phone)} onFocus={onF} onBlur={e=>onBl(e,!!errors.phone)}/></FR>
            </div>
            <div className="f2">
              ...
              <FR label="البلدية" error={errors.c}>
                <div style={{position:'relative'}}>
                  ...
                  <select value={fd.customerCommune} disabled={!fd.customerWelaya||lcC} onChange={e=>setFd({...fd,customerCommune:e.target.value})} style={{...IS(!!errors.c),paddingLeft:30,opacity:!fd.customerWelaya?0.5:1}} onFocus={onF} onBlur={e=>onBl(e,!!errors.c)}>
                    <option value="">{lcC?'...':'اختر البلدية'}</option>{communes.map(c=><option key={c.id} value={c.id}>{c.ar_name}</option>)}
                  </select>
                </div>
              </FR>
            </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                {(['home','office'] as const).map(type=>(
                  <button key={type} type="button" onClick={()=>setFd(p=>({...p,typeLivraison:type}))}
                    ...
                    <p style={{fontSize:'10px',color:fd.typeLivraison===type?'var(--cp)':'var(--mu)',letterSpacing:'0.12em',textTransform:'uppercase',marginBottom:4}}>{type==='home'?'للمنزل':'للمكتب'}</p>
                    {selW&&<p style={{fontSize:'13px',fontWeight:700,color:fd.typeLivraison===type?'var(--cp)':'var(--tx)',fontFamily:"'Raleway',sans-serif"}}>{(type==='home'?selW.livraisonHome:selW.livraisonOfice).toLocaleString()} دج</p>}
                  </button>
                ))}
              </div>
            </FR>
            ...
            <div style={{background:'var(--s1)',padding:'14px',marginBottom:14}}>
              {[{l:'السعر',v:`${fp.toLocaleString()} دج`},{l:'الكمية',v:`× ${fd.quantity}`},{l:'التوصيل',v:selW?`${getLiv().toLocaleString()} دج`:'—'}].map(r=>(
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',paddingTop:10}}>
                <span style={{fontSize:'11px',fontWeight:700,color:'var(--tx)',letterSpacing:'0.1em',textTransform:'uppercase'}}>الإجمالي</span>
                <span style={{fontSize:'1.4rem',fontWeight:800,color:'var(--cp)',fontFamily:"'Raleway',sans-serif"}}>{total().toLocaleString()} دج</span>
              </div>
            </div>

            <button type="submit" disabled={sub} className="btn-dark" style={{width:'100%',opacity:sub?0.6:1,cursor:sub?'not-allowed':'pointer'}}>
              {sub?<><Loader2 style={{width:13,height:13,animation:'sp 1s linear infinite'}}/>جاري...</>:<>تأكيد الطلب <ArrowLeft style={{width:13,height:13}}/></>}
            </button>
            <p style={{fontSize:'10px',color:'var(--mu)',textAlign:'center',marginTop:10,display:'flex',alignItems:'center',justifyContent:'center',gap:5,letterSpacing:'0.1em'}}>
              <Shield style={{width:10,height:10}}/> دفع عند الاستلام
            </p>
          </form>
        </div>
      )}
...
export function Cart({domain,store}:{domain:string;store:any}){
  ...
  const Center=({children}:{children:React.ReactNode})=>(
    <div dir="rtl" style={{minHeight:'60vh',display:'flex',alignItems:'center',justifyContent:'center',padding:'2rem',background:'var(--bg)',fontFamily:"'Raleway',sans-serif"}}>
      <div style={{textAlign:'center'}}>{children}</div>
    </div>
  );

  if(done)return<Center><CheckCircle2 style={{width:40,height:40,color:'var(--cp)',display:'block',margin:'0 auto 16px'}}/><h2 className="bn" style={{fontSize:'2.5rem',color:'var(--tx)',letterSpacing:'0.06em',marginBottom:8}}>تم تأكيد طلبك</h2><p style={{fontSize:'12px',color:'var(--mu)',marginBottom:20,letterSpacing:'0.08em'}}>سنتواصل معك قريباً.</p><Link href="/" className="btn-dark" style={{display:'inline-flex',textDecoration:'none'}}>متابعة التسوق</Link></Center>;
  if(!items.length)return<Center><ShoppingCart style={{width:32,height:32,color:'var(--mu)',display:'block',margin:'0 auto 14px'}}/><h2 className="bn" style={{fontSize:'2rem',color:'var(--mu)',letterSpacing:'0.06em',marginBottom:16}}>السلة فارغة</h2><Link href="/" className="btn-dark" style={{display:'inline-flex',textDecoration:'none'}}>تسوق الآن</Link></Center>;

  return(
    <div dir="rtl" style={{minHeight:'100vh',background:'var(--bg)',fontFamily:"'Raleway',sans-serif",padding:'32px'}}>
      <h1 style={{fontSize:'clamp(1.8rem,4vw,2.5rem)',fontWeight:800,color:'var(--tx)',letterSpacing:'0.04em',marginBottom:28}}>سلة التسوق</h1>
      <div className="cart-grid">
        ...
        <div>
          {items.map((item,i)=>(
            <div key={i} style={{display:'flex',gap:14,padding:'14px 0',borderBottom:'1px solid var(--br)'}}>
              ...
              <div style={{flex:1}}>
                ...
                <p style={{fontSize:'14px',fontWeight:800,color:'var(--cp)',marginBottom:10}}>{item.finalPrice?.toLocaleString()} دج</p>
          ...
          <div style={{display:'flex',justifyContent:'space-between',paddingTop:16}}>
            <span style={{fontSize:'11px',letterSpacing:'0.12em',textTransform:'uppercase',color:'var(--mu)'}}>المجموع الفرعي</span>
            <span style={{fontSize:'16px',fontWeight:800,color:'var(--tx)'}}>{sub2.toLocaleString()} دج</span>
          </div>
        </div>
        ...
        <div style={{background:'var(--s1)',padding:'24px'}}>
          ...
          <form onSubmit={submit}>
            <div className="f2">
              <FR label="الاسم" error={errors.name}><input type="text" value={fd.customerName} onChange={e=>setFd({...fd,customerName:e.target.value})} style={IS(!!errors.name)} onFocus={onF} onBlur={e=>onBl(e,!!errors.name)}/></FR>
              <FR label="الهاتف" error={errors.phone}><input type="tel" value={fd.customerPhone} onChange={e=>setFd({...fd,customerPhone:e.target.value})} style={IS(!!errors.phone)} onFocus={onF} onBlur={e=>onBl(e,!!errors.phone)}/></FR>
            </div>
            <div className="f2">
              <FR label="الولاية" error={errors.w}>
                <div style={{position:'relative'}}><ChevronDown style={{position:'absolute',left:10,top:'50%',transform:'translateY(-50%)',width:12,height:12,color:'var(--mu)',pointerEvents:'none'}}/><select value={fd.customerWelaya} onChange={e=>setFd({...fd,customerWelaya:e.target.value,customerCommune:''})} style={{...IS(!!errors.w),paddingLeft:30}} onFocus={onF} onBlur={e=>onBl(e,!!errors.w)}><option value="">اختر</option>{wilayas.map(w=><option key={w.id} value={w.id}>{w.id} - {w.ar_name}</option>)}</select></div>
              </FR>
              <FR label="البلدية" error={errors.c}>
                <div style={{position:'relative'}}><ChevronDown style={{position:'absolute',left:10,top:'50%',transform:'translateY(-50%)',width:12,height:12,color:'var(--mu)',pointerEvents:'none'}}/><select value={fd.customerCommune} disabled={lcC||!fd.customerWelaya} onChange={e=>setFd({...fd,customerCommune:e.target.value})} style={{...IS(!!errors.c),paddingLeft:30,opacity:!fd.customerWelaya?0.5:1}} onFocus={onF} onBlur={e=>onBl(e,!!errors.c)}><option value="">{lcC?'...':'اختر'}</option>{communes.map(c=><option key={c.id} value={c.id}>{c.ar_name}</option>)}</select></div>
              </FR>
            </div>
            {[{l:'المنتجات',v:`${sub2.toLocaleString()} دج`},{l:'التوصيل',v:getLiv()?`${getLiv().toLocaleString()} دج`:'—'}].map(r=>(
            ...
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 0 16px'}}>
              <span style={{fontSize:'11px',fontWeight:700,color:'var(--tx)',letterSpacing:'0.12em',textTransform:'uppercase'}}>الإجمالي</span>
              <span style={{fontSize:'1.4rem',fontWeight:800,color:'var(--cp)'}}>{grand.toLocaleString()} دج</span>
            </div>
            <button type="submit" disabled={sub} className="btn-dark" style={{width:'100%',opacity:sub?0.6:1,cursor:sub?'not-allowed':'pointer'}}>
              {sub?<><Loader2 style={{width:13,height:13,animation:'sp 1s linear infinite'}}/>جاري...</>:<>تأكيد الطلب <ArrowLeft style={{width:13,height:13}}/></>}
            </button>
...
export function Privacy(){return<PageWrap title="سياسة الخصوصية"><Row title="البيانات التي نجمعها" body="الاسم ورقم الهاتف وعنوان التوصيل فقط — الحد الأدنى لمعالجة طلبك."/><Row title="استخدام البيانات" body="تُستخدم حصرياً لمعالجة وتوصيل طلبك. لا تسويق ولا بيع لأطراف ثالثة."/><Row title="الأمان" body="نستخدم تشفيراً لحماية بياناتك في جميع الأوقات."/><Row title="مشاركة البيانات" body="لا نبيع بياناتك. تُشارك فقط مع شركاء التوصيل." tag="مضمون"/></PageWrap>}
export function Terms(){return<PageWrap title="شروط الاستخدام"><Row title="الطلبات والأسعار" body="لا رسوم خفية. السعر المعروض هو السعر النهائي."/><Row title="جودة المنتجات" body="نضمن جودة جميع منتجاتنا." tag="مضمون"/><Row title="التوصيل" body="التوصيل خلال 24-72 ساعة. الدفع عند الاستلام."/><Row title="القانون" body="تخضع هذه الشروط لقوانين الجمهورية الجزائرية."/></PageWrap>}
export function Cookies(){return<PageWrap title="سياسة الكوكيز"><Row title="ملفات ضرورية" body="مطلوبة لتشغيل الجلسات وسلة التسوق." tag="دائماً"/><Row title="ملفات التفضيلات" body="تحفظ إعداداتك لتجربة أفضل." tag="اختياري"/><Row title="ملفات التحليل" body="بيانات مجمعة لتحسين التجربة." tag="اختياري"/></PageWrap>}

export function Contact({store}:{store?:any}){
  const [form,setForm]=useState({name:'',email:'',phone:'',message:''});
  const [sent,setSent]=useState(false);
  const [loading,setL]=useState(false);
  const submit=async(e:React.FormEvent)=>{e.preventDefault();setL(true);try{await axios.post(`${API}/user/contact-user/message`,{...form,storeId:store?.id});setSent(true)}catch{showError('حدث خطأ')}finally{setL(false)}};
  return(
    <div dir="rtl" style={{background:'var(--bg)',fontFamily:"'Raleway',sans-serif",minHeight:'100vh',padding:'40px 32px 80px'}}>
      ...
      <div className="contact-layout">
        ...
        {sent?(<div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:240,textAlign:'center'}}><CheckCircle2 style={{width:36,height:36,color:'var(--cp)',marginBottom:12}}/><h2 style={{fontWeight:800,fontSize:'1.2rem',color:'var(--tx)',marginBottom:6}}>تم الإرسال</h2><p style={{fontSize:'12px',color:'var(--mu)'}}>سنتواصل معك قريباً</p></div>):(
          <form onSubmit={submit} style={{display:'flex',flexDirection:'column',gap:4}}>
            ...
            <FR label="البريد الإلكتروني"><input type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} required style={IS()} onFocus={onF} onBlur={e=>onBl(e)}/></FR>
            <FR label="رسالتك"><textarea value={form.message} onChange={e=>setForm({...form,message:e.target.value})} rows={5} required style={{...IS(),resize:'none'}} onFocus={onF} onBlur={e=>onBl(e)}/></FR>
            <button type="submit" disabled={loading} className="btn-dark" style={{justifyContent:'center',marginTop:8,opacity:loading?0.65:1,cursor:loading?'not-allowed':'pointer'}}>
              {loading?<><Loader2 style={{width:13,height:13,animation:'sp 1s linear infinite'}}/>جاري...</>:'إرسال الرسالة'}
            </button>
          </form>
        )}
      </div>
