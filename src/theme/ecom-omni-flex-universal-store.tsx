    ...
    <footer dir={isRTL ? 'rtl' : 'ltr'} style={{backgroundColor:'var(--navy)',fontFamily:"'Inter',sans-serif",borderTop:'1px solid var(--line-lt)'}}>
      <div style={{maxWidth:'1280px',margin:'0 auto',padding:'64px 24px 32px'}}>
        <div className="footer-g" style={{paddingBottom:'48px',borderBottom:'1px solid rgba(255,255,255,0.05)'}}>
          ...
          {/* قسم — قانوني */}
          <div>
            <p className="bc" style={{fontSize:'14px',fontWeight:700,letterSpacing:'0.1em',color:'var(--blue)',marginBottom:'24px'}}>{t.legalNav}</p>
            {legalLinks.map(lnk=>(
              <a key={lnk.h} href={lnk.h} style={{display:'block',fontSize:'13px',color:'rgba(255,255,255,0.5)',marginBottom:'12px',transition:'all 0.2s'}}
                onMouseEnter={e=>{const el=e.currentTarget as HTMLElement;el.style.color='var(--white)';el.style.paddingInlineStart='5px';}}
                onMouseLeave={e=>{const el=e.currentTarget as HTMLElement;el.style.color='rgba(255,255,255,0.5)';el.style.paddingInlineStart='0';}}>
      ...
      <div className="details-g" style={{maxWidth:'1280px',margin:'0 auto'}}>
        ...
        <div className="details-R">
          ...
          {(product.shippingFree || (store?.supportFreeShipping && store?.freeShippingMinAmount != null)) && (
            <p style={{fontSize:'13px',fontWeight:700,color:'var(--blue)',marginBottom:'18px'}}>
                ...
                <label key={o.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 14px',border:`1.5px solid ${selectedOffer===o.id?'var(--blue)':'var(--line)'}`,cursor:'pointer',marginBottom:'8px',transition:'all 0.2s',backgroundColor:selectedOffer===o.id?'rgba(58,134,255,0.05)':'var(--white)'}}>
                  <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
                    ...
                    <div>
                      ...
                      {o.subTitle && <p style={{fontSize:'11px',color:'var(--dim)',margin:'2px 0 0'}}>{o.subTitle}</p>}
                      <p style={{fontSize:'11px',color:'var(--dim)',margin:0}}>{t.qty}: {o.quantity}</p>
                      {o.shippingFree && <p style={{fontSize:'10px',fontWeight:700,color:'var(--blue)',margin:'2px 0 0'}}>{t.freeShippingBadge}</p>}
                    </div>
                  </div>
          ...
          {allAttrs.map((attr:any)=>(
            <div key={attr.id} style={{marginBottom:'18px',paddingBottom:'18px',borderBottom:'1px solid var(--line)'}}>
              ...
              {attr.displayMode==='color' ? (
                ...
                <div style={{display:'flex',flexWrap:'wrap',gap:'6px'}}>
                  {attr.variants.map((v:any)=>{const s=selectedVariants[attr.name]===v.value; const available=!product.variantDetails?.length||product.variantDetails.some((vd:any)=>Object.entries({...selectedVariants,[attr.name]:v.value}).every(([n,val])=>vd.name.some((e:any)=>e.attrName===n&&e.value===val))); return <button key={v.id} onClick={()=>available&&handleVariantSelection(attr.name,v.value)} className="bc" style={{padding:'8px 16px',border:`1.5px solid ${s?'var(--blue)':'var(--line)'}`,backgroundColor:s?'var(--blue)':'transparent',color:s?'white':(available?'var(--mid)':'#bbb'),fontSize:'13px',fontWeight:700,letterSpacing:'0.1em',cursor:available?'pointer':'not-allowed',transition:'all 0.2s',textDecoration:available?'none':'line-through'}}>{v.name}</button>;})}
                </div>
              )}
            </div>
          ))}
    ...
    <div dir={isRTL ? 'rtl' : 'ltr'} style={{minHeight:'100vh',background:'var(--off)',padding:'2.5rem 1.5rem 5rem'}}>
      <div style={{maxWidth:1280,margin:'0 auto'}}>
        ...
        {freeShippingMin != null && (
          <div style={{background: freeShippingReached ? 'rgba(58,134,255,0.06)' : 'var(--off)', border:`1px solid ${freeShippingReached ? 'var(--blue)' : 'var(--line)'}`, color: freeShippingReached ? 'var(--blue)' : 'var(--dim)', padding:'12px 16px', marginBottom:'1.5rem', fontWeight:700, fontSize:'13px', textAlign:'center'}}>
        ...
        <div className="cart-layout">
              ...
              <div key={i} style={{display:'flex',gap:'1rem',padding:'14px',borderBottom:'1px solid var(--line)'}}>
                ...
                <div style={{flex:1,display:'flex',flexDirection:'column',justifyContent:'space-between'}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:'8px'}}>
                    <span style={{fontSize:'0.9rem',fontWeight:700,color:'var(--dim)'}}>× {item.quantity}</span>
          ...
          <div style={{background:'var(--white)',border:'1px solid var(--line)',borderTop:'3px solid var(--blue)',padding:'22px',alignSelf:'start'}}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {/* Summary */}
              <div style={{border:'1px solid var(--line)',marginBottom:'14px',overflow:'hidden'}}>
                {[{l:t.subtotal,v:`${cartTotal.toLocaleString()} ${currency}`},{l:t.delivery,v: !selW ? '—' : freeShippingReached ? t.freeShippingBadge : `${getLiv().toLocaleString()} ${currency}`}].map(row=>(
    ...
    <div dir={t.dir} style={{ minHeight: '100vh', background: 'var(--off)', padding: '3rem 1.25rem' }}>
      <div style={{ maxWidth: 480, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', background: '#fff', padding: '3rem 2rem', borderRadius: 16, border: '1px solid var(--line)', marginBottom: '1.5rem' }}>
          <div style={{ width: 60, height: 60, borderRadius: '50%', background: 'rgba(58,134,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.25rem' }}>
            <CheckCircle2 size={30} style={{ color: 'var(--blue)' }} />
          </div>
          <h2 className="bc" style={{ fontSize: '1.6rem', fontWeight: 800, color: 'var(--navy)', marginBottom: '0.5rem', letterSpacing: '0.04em' }}>{t.successTitle}</h2>
          <p style={{ color: 'var(--mid)', lineHeight: 1.7, fontSize: 14 }}>{t.successDesc}</p>
        </div>
        ...
        {order && (order.productName || order.total != null) && (
          <div style={{ background: '#fff', borderRadius: 16, border: '1px solid var(--line)', padding: '1.25rem 1.5rem', marginBottom: '1.5rem' }}>
            <p style={{ fontWeight: 800, fontSize: '0.75rem', marginBottom: '0.75rem', color: 'var(--navy)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{t.orderInfo}</p>
            {order.productName && (
              <div style={{ paddingBottom: 10, marginBottom: 10, borderBottom: '1px solid var(--line)', fontSize: 14, fontWeight: 700, color: 'var(--navy)' }}>{order.productName}</div>
            )}
            {order.total != null && (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 13, color: 'var(--mid)' }}>{t.total}</span>
                <span className="bc" style={{ fontSize: '1.2rem', fontWeight: 800, color: 'var(--blue)' }}>{Number(order.total).toLocaleString()} {currency}</span>
              </div>
            )}
          </div>
        )}
        ...
        <div style={{ background: '#fff', borderRadius: 16, border: '1px solid var(--line)', overflow: 'hidden', marginBottom: '1.5rem' }}>
          {t.successSteps.map((step, i) => {
            ...
            const done = i === 0;
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '1rem 1.25rem', borderBottom: i < t.successSteps.length - 1 ? '1px solid var(--line)' : 'none', background: done ? 'rgba(58,134,255,0.06)' : 'transparent' }}>
                <div style={{ width: 36, height: 36, flexShrink: 0, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: done ? 'var(--blue)' : 'rgba(58,134,255,0.08)', color: done ? '#fff' : 'var(--mid)' }}>
                  <Icon size={16} />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: '0.85rem', fontWeight: 700, color: done ? 'var(--navy)' : 'var(--mid)', marginBottom: 2 }}>{step.title}</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.7rem' }}>
          <Link href="/" className="btn-blue" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '12px 32px', textDecoration: 'none' }}>{t.shopNow}</Link>
          <Link href="/" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0.8rem', borderRadius: 10, border: '1px solid var(--line)', color: 'var(--mid)', fontWeight: 700, fontSize: '0.85rem', textDecoration: 'none' }}>
            {t.backToShop}
          </Link>
        </div>
      </div>
