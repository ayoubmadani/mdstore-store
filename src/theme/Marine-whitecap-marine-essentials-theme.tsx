...
const IconMessageCircle = ({ size = 16, color = 'currentColor' }: { size?: number; color?: string }) => (
...
const IconDownload = ({ size = 16, color = 'currentColor' }: { size?: number; color?: string }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg>
);
  ...
  return (
    <div className="wc-card" style={{ display: 'flex', flexDirection: 'column', height: '100%', border: `1px solid ${BD}`, borderRadius: 8, overflow: 'hidden', background: CARD, transition: 'transform 0.28s cubic-bezier(.22,.68,0,1.2), box-shadow 0.28s ease' }}
      ...
      <div style={{ aspectRatio: '1/1', overflow: 'hidden', position: 'relative' }}>
        ...
        {product?.isDigital ? (
          <span style={{ position: 'absolute', top: 8, insetInlineStart: 8, background: A, color: '#fff', fontSize: '0.7rem', fontWeight: 800, padding: '3px 8px', borderRadius: 4, display: 'flex', alignItems: 'center' }}>
            <IconDownload size={12} />
          </span>
        ) : product?.shippingFree && (
          <span style={{ position: 'absolute', top: 8, insetInlineStart: 8, background: A, color: '#fff', fontSize: '0.7rem', fontWeight: 800, padding: '3px 8px', borderRadius: 4 }}>
            🚚
          </span>
        )}
      </div>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 1.5rem' }}>
        <div className="wc-details-grid" style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '2.5rem' }}>
          ...
          <div>
            ...
            {(product?.shippingFree || ((storeprop || product?.store)?.supportFreeShipping && (storeprop || product?.store)?.freeShippingMinAmount != null)) && (
              <div style={{ marginBottom: 20, padding: '10px 14px', border: `1px solid ${A}`, borderRadius: 6, background: AL, fontSize: '0.85rem', fontWeight: 700, color: AD }}>
                🚚 {product.shippingFree ? t.freeShippingBadge : t.freeShippingThreshold.replace('{{amount}}', `${Number((storeprop || product?.store).freeShippingMinAmount).toLocaleString()} ${product?.store?.currency}`)}
              </div>
            )}
                  ...
                  {product.offers.map((o: Offer) => (
                    <label key={o.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', border: `1px solid ${selectedOffer === o.id ? A : BD}`, borderRadius: 6, background: selectedOffer === o.id ? AL : '#fff', cursor: 'pointer', transition: 'all 0.2s' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        {o.subTitle && <div style={{ fontSize: '0.78rem', color: SUB }}>{o.subTitle}</div>}
                        {o.shippingFree && <div style={{ fontSize: '0.78rem', color: A, fontWeight: 700 }}>🚚 {t.freeShippingBadge}</div>}
                      </div>
  ...
  const validate = () => {
    ...
    if (product?.isDigital) {
      if (contactMethod === 'email') {
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test((fd.customerEmail || '').trim())) errs.email = t.errEmail;
      } else {
        if (!/^(0|\+213)[5-7]\d{8}$/.test(fd.customerWhatsapp || '')) errs.whatsapp = t.errWhatsapp;
      }
    } else {
        ...
        <div style={{ animation: 'fadeUp 0.4s ease' }}>
          ...
          {product?.isDigital ? (
            <div style={{ marginBottom: '0.875rem' }}>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, marginBottom: 6, color: TXT }}>{t.contactQuestion}</label>
              <div style={{ display: 'flex', borderRadius: 6, overflow: 'hidden', border: `1px solid ${BD}`, marginBottom: 10 }}>
                <button type="button" onClick={() => { setContactMethod('email'); setFd(p => ({ ...p, customerWhatsapp: '' })); }}
                  style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '10px 0', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.85rem', fontWeight: 700, background: contactMethod === 'email' ? A : '#fff', color: contactMethod === 'email' ? '#fff' : TXT }}>
                  <IconMail size={14} />{t.contactViaEmail}
                </button>
                <button type="button" onClick={() => { setContactMethod('whatsapp'); setFd(p => ({ ...p, customerEmail: '' })); }}
                  style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '10px 0', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.85rem', fontWeight: 700, background: contactMethod === 'whatsapp' ? A : '#fff', color: contactMethod === 'whatsapp' ? '#fff' : TXT }}>
                  <IconMessageCircle size={14} />{t.contactViaWhatsapp}
                </button>
              </div>
              {contactMethod === 'email' ? (
                <div>
                  <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, marginBottom: 6, color: TXT }}>{t.orderEmail}</label>
                  <input type="email" dir="ltr" value={fd.customerEmail} onChange={e => setFd(p => ({ ...p, customerEmail: e.target.value }))} placeholder={t.emailPh}
                    style={{ ...inputBase, borderColor: errors.email ? ERR : BD }} />
                  {errors.email && <p style={{ fontSize: '0.75rem', color: ERR, marginTop: '0.3rem', display: 'flex', alignItems: 'center', gap: 4 }}><IconAlert size={11} color={ERR} /> {errors.email}</p>}
                </div>
              ) : (
                <div>
                  <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, marginBottom: 6, color: TXT }}>{t.whatsapp}</label>
                  <input type="tel" dir="ltr" value={fd.customerWhatsapp} onChange={e => setFd(p => ({ ...p, customerWhatsapp: e.target.value }))} placeholder={t.whatsappPh}
                    style={{ ...inputBase, borderColor: errors.whatsapp ? ERR : BD }} />
                  {errors.whatsapp && <p style={{ fontSize: '0.75rem', color: ERR, marginTop: '0.3rem', display: 'flex', alignItems: 'center', gap: 4 }}><IconAlert size={11} color={ERR} /> {errors.whatsapp}</p>}
                </div>
              )}
            </div>
          ) : (
          ...
          <div style={{ background: BG, borderRadius: 8, padding: '1rem', marginBottom: 16 }}>
            {[
              { l: t.price, v: `${fp.toLocaleString()} ${store?.currency}` },
              { l: t.qty, v: `× ${qty}` },
              ...(product?.isDigital ? [] : [{ l: t.delivery, v: !selW ? '—' : orderFreeShipping ? t.freeShippingBadge : `${getLiv().toLocaleString()} ${store?.currency}` }]),
            ].map(row => (
    ...
    <div style={{ background: BG, padding: '2.5rem 0', minHeight: '60vh' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 1.5rem' }}>
        ...
        {freeShippingMin != null && (
          <div style={{
            border: `1px solid ${freeShippingReached ? A : BD}`, borderRadius: 8,
            background: freeShippingReached ? AL : '#fff', padding: '0.875rem 1.125rem', marginBottom: '1.5rem',
            color: freeShippingReached ? AD : SUB, fontSize: '0.85rem', fontWeight: 700,
          }}>
            ...
            <div style={{ background: '#fff', border: `1px solid ${BD}`, borderRadius: 10, padding: '1.25rem' }}>
              ...
              <div style={{ background: BG, borderRadius: 8, padding: '1rem', marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: `1px solid ${BD}` }}>
                  ...
                  <span style={{ fontSize: '0.9rem', fontWeight: 700, color: TXT, whiteSpace: 'nowrap' }}>{!selW ? '—' : freeShippingReached ? t.freeShippingBadge : `${getLiv().toLocaleString()} ${store?.currency}`}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 10, marginTop: 4 }}>
                  <span style={{ fontSize: '0.95rem', fontWeight: 800, color: TXT }}>{t.total}</span>
...
export function Success({ store, order }: { store: any; domain: string; order: any }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, padding: '3rem 1.25rem' }}>
      <div style={{ maxWidth: 480, margin: '0 auto' }}>
        <div style={{ background: AL, border: `1px solid ${A}`, borderRadius: 10, padding: '2.5rem 2rem', textAlign: 'center', marginBottom: '1.5rem' }}>
          <div style={{ width: 64, height: 64, borderRadius: '50%', background: A, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.25rem', fontSize: '1.75rem' }}>✓</div>
          <h2 style={{ margin: '0 0 0.5rem', color: TXT }}>{t.successTitle}</h2>
        ...
        {order && (order.productName || order.total != null) && (
          <div style={{ background: CARD, borderRadius: 10, border: `1px solid ${BD}`, padding: '1.25rem 1.5rem', marginBottom: '1.5rem' }}>
        ...
        <div style={{ background: CARD, borderRadius: 10, border: `1px solid ${BD}`, overflow: 'hidden', marginBottom: '1.5rem' }}>
          {t.successSteps.map((step, i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '1rem 1.25rem', borderBottom: i < t.successSteps.length - 1 ? `1px solid ${BD}` : 'none', background: done ? AL : 'transparent' }}>
                <div style={{ width: 32, height: 32, flexShrink: 0, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: done ? A : BG, color: done ? '#fff' : SUB, fontWeight: 700, fontSize: '0.85rem' }}>
                  {i + 1}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.7rem' }}>
          <Link href="/" style={{ background: A, color: '#fff', padding: '0.875rem 2rem', borderRadius: 6, textDecoration: 'none', fontWeight: 700, textAlign: 'center' }}>
            {t.shopNow}
          </Link>
          <Link href="/" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0.8rem', borderRadius: 6, border: `1px solid ${BD}`, color: SUB, fontWeight: 700, fontSize: '0.85rem', textDecoration: 'none' }}>
            {t.backToShop}
          </Link>
        </div>
      </div>
