...
import {
  ...
  Trash2, Loader2, MapPin, Leaf, Sparkles, Droplets, Heart, Flower2, Package, Truck,
  ...
  return (
    <div className="co-card">
      ...
      <div style={{ position:'relative', aspectRatio:'3/4', background: CR, overflow:'hidden' }}>
        ...
        {product.isDigital ? (
          <div style={{ position:'absolute', bottom:12, left:12, background: RO, color:'#fff', fontSize:'0.62rem', fontWeight:700, padding:'5px 10px', borderRadius:20, display:'flex', alignItems:'center' }}>
            <Download size={12} />
          </div>
        ) : product.shippingFree && (
          <div style={{ position:'absolute', bottom:12, left:12, background: INK, color:'#fff', fontSize:'0.62rem', fontWeight:700, padding:'5px 10px', borderRadius:20 }}>
            🚚
          </div>
        )}
        {/* Heart */}
        ...
        <div>
          <div className="info-panel">
            ...
            {(product.shippingFree || ((store || product?.store)?.supportFreeShipping && (store || product?.store)?.freeShippingMinAmount != null)) && (
              <p style={{ fontSize:'0.8rem', fontWeight:700, color: RO, marginBottom:'1.25rem' }}>
                  ...
                  <label key={o.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'0.9rem 1rem', border:`1.5px solid ${selectedOffer===o.id ? RO : BD}`, borderRadius:12, cursor:'pointer', marginBottom:'0.5rem', background: selectedOffer===o.id ? ROL : CARD, transition:'all 0.18s' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:'0.625rem' }}>
                      ...
                      <div>
                        ...
                        {o.subTitle && <p style={{ fontSize:'0.68rem', color: SUB, marginTop:2 }}>{o.subTitle}</p>}
                        <p style={{ fontSize:'0.68rem', color: SUB }}>{t.qty}: {o.quantity}</p>
                        {o.shippingFree && <p style={{ fontSize:'0.65rem', fontWeight:700, color: RO, marginTop:2 }}>{t.freeShippingBadge}</p>}
                      </div>
                    </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            {product.isDigital ? (
              <div style={{ marginBottom:'0.875rem' }}>
                <p style={{ fontSize:'0.62rem', fontWeight:700, color: SUB, marginBottom:'0.5rem', textTransform:'uppercase', letterSpacing:'0.1em' }}>{t.contactQuestion}</p>
                <div style={{ display:'flex', borderRadius:12, overflow:'hidden', border:`1.5px solid ${BD}`, marginBottom:'0.75rem' }}>
                  <button type="button" onClick={() => { setContactMethod('email'); setFd(p => ({ ...p, customerWhatsapp: '' })); }} style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', gap:6, padding:'0.7rem 0', border:'none', cursor:'pointer', fontFamily:'inherit', fontSize:'0.8rem', fontWeight:600, background: contactMethod==='email' ? RO : 'transparent', color: contactMethod==='email' ? '#fff' : SUB }}>
                    <Mail size={14}/>{t.contactViaEmail}
                  </button>
                  <button type="button" onClick={() => { setContactMethod('whatsapp'); setFd(p => ({ ...p, customerEmail: '' })); }} style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', gap:6, padding:'0.7rem 0', border:'none', cursor:'pointer', fontFamily:'inherit', fontSize:'0.8rem', fontWeight:600, background: contactMethod==='whatsapp' ? RO : 'transparent', color: contactMethod==='whatsapp' ? '#fff' : SUB }}>
                    <MessageCircle size={14}/>{t.contactViaWhatsapp}
                  </button>
                </div>
                {contactMethod === 'email' ? (
                  <FR error={errors.customerEmail} label={t.orderEmail}>
                    <input type="email" dir="ltr" value={fd.customerEmail} onChange={e => setFd({ ...fd, customerEmail: e.target.value })} placeholder={t.orderEmailPh} style={inp(!!errors.customerEmail)} />
                  </FR>
                ) : (
                  ...
                  <div className="delivery-grid">
                    {(['home','office'] as const).map(dtype => (
                      <button key={dtype} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: dtype }))} style={{ padding:'0.8rem', border:`1.5px solid ${fd.typeLivraison===dtype ? RO : BD}`, borderRadius:12, textAlign:'center', cursor:'pointer', background: fd.typeLivraison===dtype ? ROL : CARD, transition:'all 0.18s', fontFamily:'inherit' }}>
                        ...
                        {selW && <p className="price-mono" style={{ fontSize:'1rem', fontWeight:700, color: fd.typeLivraison===dtype ? ROD : SUB }}>{orderFreeShipping ? t.freeShippingBadge : `${(dtype==='home'?selW.livraisonHome:selW.livraisonOfice).toLocaleString()} ${currency}`}</p>}
                      </button>
                    ))}
                  </div>
                </div>
  ...
  return (
    <div dir={isRTL ? 'rtl' : 'ltr'} style={{ padding:'2.5rem 1.5rem', maxWidth:1320, margin:'0 auto', minHeight:'100vh', background: BG }}>
      ...
      {freeShippingMin != null && (
        <div style={{ background: freeShippingReached ? ROL : CR, border: `1px solid ${freeShippingReached ? RO : BD}`, color: freeShippingReached ? ROD : SUB, borderRadius: 14, padding: '0.75rem 1.125rem', marginBottom: '1.75rem', fontWeight: 600, fontSize: '0.85rem', textAlign: 'center' }}>
              ...
              <div className="delivery-grid">
                {(['home','office'] as const).map(dtype => (
                  <button key={dtype} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: dtype }))} style={{ padding:'0.85rem', border:`1.5px solid ${fd.typeLivraison===dtype ? RO : BD}`, borderRadius:12, textAlign:'center', cursor:'pointer', background: fd.typeLivraison===dtype ? ROL : CARD, fontFamily:'inherit', transition:'all 0.18s' }}>
                    ...
                    {selW && <p style={{ fontWeight:700, fontSize:'0.95rem', color: fd.typeLivraison===dtype ? ROD : SUB }}>{freeShippingReached ? t.freeShippingBadge : `${(dtype==='home'?selW.livraisonHome:selW.livraisonOfice).toLocaleString()} ${currency}`}</p>}
                  </button>
                ))}
              </div>
            </div>
...
export function Success({ store, order }: { store: any; domain: string; order: any }) {
  ...
  return (
    <div dir={isRTL ? 'rtl' : 'ltr'} style={{ minHeight: '100vh', background: BG, padding: '3rem 1.25rem' }}>
      <div style={{ maxWidth: 480, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', background: CARD, padding: '3rem 2rem', borderRadius: 24, border: `1px solid ${BD}`, boxShadow: '0 24px 64px rgba(193,123,142,0.1)', marginBottom: '1.5rem' }}>
          <div style={{ width: 60, height: 60, borderRadius: '50%', background: `linear-gradient(135deg,${RO},${ROD})`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.75rem', boxShadow: '0 8px 24px rgba(193,123,142,0.4)' }}>
            <CheckCircle2 size={28} color="#fff" />
          </div>
          <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '2rem', fontWeight: 600, color: INK, marginBottom: '0.625rem', letterSpacing: '-0.02em' }}>{t.successTitle}</h2>
          <p style={{ color: SUB, lineHeight: 1.9, fontSize: '0.9rem', fontWeight: 300 }}>{t.successDesc}</p>
        </div>

        {order && (order.productName || order.total != null) && (
          <div style={{ background: CARD, borderRadius: 24, border: `1px solid ${BD}`, padding: '1.25rem 1.5rem', marginBottom: '1.5rem' }}>
            ...
            {order.productName && (
              <div style={{ paddingBottom: 10, marginBottom: 10, borderBottom: `1px solid ${BD}`, fontSize: 14, fontWeight: 600, color: INK }}>{order.productName}</div>
            )}
            {order.total != null && (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 13, color: SUB }}>{t.total}</span>
                <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '1.3rem', fontWeight: 600, color: RO }}>{Number(order.total).toLocaleString()} {currency}</span>
              </div>
            )}
          </div>
        )}

        <div style={{ background: CARD, borderRadius: 24, border: `1px solid ${BD}`, overflow: 'hidden', marginBottom: '1.5rem' }}>
          {t.successSteps.map((step, i) => {
            const Icon = stepIcons[i] ?? CheckCircle2;
            const done = i === 0;
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '1rem 1.25rem', borderBottom: i < t.successSteps.length - 1 ? `1px solid ${BD}` : 'none', background: done ? 'rgba(193,123,142,0.06)' : 'transparent' }}>
                <div style={{ width: 36, height: 36, flexShrink: 0, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: done ? `linear-gradient(135deg,${RO},${ROD})` : '#F5EDEA', color: done ? '#fff' : SUB }}>
                  <Icon size={16} />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: '0.85rem', fontWeight: 600, color: done ? INK : SUB, marginBottom: 2 }}>{step.title}</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.7rem' }}>
          <Link href="/" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: `linear-gradient(135deg,${RO},${ROD})`, color: '#fff', padding: '0.9rem 2rem', borderRadius: 12, fontWeight: 600, fontSize: '0.875rem', textDecoration: 'none' }}>
            <ShoppingBag size={17} /> {t.shopNow}
          </Link>
          <Link href="/" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0.8rem', borderRadius: 12, border: `1px solid ${BD}`, color: SUB, fontWeight: 600, fontSize: '0.85rem', textDecoration: 'none' }}>
            {t.backToShop}
          </Link>
        </div>
      </div>
