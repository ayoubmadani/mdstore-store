          ...
          <div style={{ padding: '0.625rem 1.25rem', background: '#fff', borderTop: '1px solid #E8E8E8', position: 'relative' }} className="anim-slide-down">
            <form onSubmit={handleSearch} style={{ position: 'relative' }}>
              <input autoFocus type="text" placeholder={t.search} value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                style={{ width: '100%', padding: isRTL ? '0.75rem 1rem 0.75rem 2.75rem' : '0.75rem 2.75rem 0.75rem 1rem', border: '1.5px solid #16A34A', borderRadius: 8, background: '#F8F8F6', fontSize: '0.9rem', outline: 'none' }} />
              <Search size={15} style={{ position: 'absolute', ...(isRTL ? { left: 12 } : { right: 12 }), top: '50%', transform: 'translateY(-50%)', color: '#16A34A' }} />
            </form>
            {searchQuery.length >= 2 && <DropResults />}
          </div>
        )}
  ...
  return (
    <div className="card-wrap" style={{ background: '#fff', border: '1.5px solid #EBEBEB', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column', height: '100%', position: 'relative', transition: 'all 0.28s ease' }}
      ...
      <div style={{ position: 'relative', aspectRatio: '1/1', background: '#F4F4F2', overflow: 'hidden' }}>
        ...
        {product.isDigital ? (
          <div style={{ position: 'absolute', top: 10, left: 10, background: '#16A34A', color: '#fff', fontSize: 10, fontWeight: 800, padding: '3px 8px', borderRadius: 4, display: 'flex', alignItems: 'center' }}>
            <Download size={12} />
          </div>
        ...
        <div>
          <div className="info-container">
            ...
            {(product.shippingFree || ((store || product?.store)?.supportFreeShipping && (store || product?.store)?.freeShippingMinAmount != null)) && (
              <p style={{ fontSize: '0.8rem', fontWeight: 700, color: '#16A34A', marginBottom: '1.25rem' }}>
                  ...
                  <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.875rem 1rem', border: `1.5px solid ${selectedOffer === o.id ? '#16A34A' : '#E8E8E8'}`, borderRadius: 10, cursor: 'pointer', marginBottom: '0.5rem', background: selectedOffer === o.id ? 'rgba(22, 163, 74, 0.04)' : 'transparent', transition: 'all 0.18s' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                      ...
                      <div>
                        ...
                        {o.shippingFree && <p style={{ fontSize: '0.7rem', fontWeight: 700, color: '#16A34A', marginTop: 2 }}>{t.freeShippingBadge}</p>}
                      </div>
                    </div>
                      ...
                      <button
                        ...
                        style={
                          attr.displayMode === 'color' ? {
                            ...
                            color: isSelected ? '#16A34A' : (available ? '#555' : '#bbb'), cursor: available ? 'pointer' : 'not-allowed', transition: 'all 0.18s',
            ...
            {product.isDigital ? (
              <div style={{ marginBottom: '0.875rem' }}>
                ...
                <div style={{ display: 'flex', borderRadius: 10, overflow: 'hidden', border: '1.5px solid #E8E8E8', marginBottom: '0.75rem' }}>
                  <button type="button" onClick={() => { setContactMethod('email'); setFd(p => ({ ...p, customerWhatsapp: '' })); }} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '0.7rem 0', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.8rem', fontWeight: 700, background: contactMethod === 'email' ? '#16A34A' : 'transparent', color: contactMethod === 'email' ? '#fff' : '#555' }}>
                    <Mail size={14} />{t.contactViaEmail}
                  </button>
                  <button type="button" onClick={() => { setContactMethod('whatsapp'); setFd(p => ({ ...p, customerEmail: '' })); }} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '0.7rem 0', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.8rem', fontWeight: 700, background: contactMethod === 'whatsapp' ? '#16A34A' : 'transparent', color: contactMethod === 'whatsapp' ? '#fff' : '#555' }}>
  ...
  return (
    <div dir={isRTL ? 'rtl' : 'ltr'} style={{ padding: '2.5rem 1.5rem', maxWidth: 1280, margin: '0 auto', minHeight: '100vh' }}>
      ...
      {freeShippingMin != null && (
        <div style={{ background: freeShippingReached ? 'rgba(22,163,74,0.06)' : '#F8F8F6', border: `1.5px solid ${freeShippingReached ? '#16A34A' : '#E8E8E8'}`, color: freeShippingReached ? '#16A34A' : '#555', borderRadius: 10, padding: '0.75rem 1.125rem', marginBottom: '1.5rem', fontWeight: 700, fontSize: '0.85rem', textAlign: 'center' }}>
