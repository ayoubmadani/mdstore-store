...
const jsonEn = {
  ...
  qty: 'Quantity', price: 'Price', product: 'Product', delivery: 'Delivery', total: 'Total',
    ...
    return (
        <div className="soft-card" style={{ background: '#fff', border: `2px solid ${hov ? pal.accent : 'var(--border)'}`, borderRadius: 22, overflow: 'hidden', display: 'flex', flexDirection: 'column', height: '100%', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}
            ...
            <div style={{ position: 'relative', aspectRatio: '1/1', background: pal.bg, overflow: 'hidden' }}>
                ...
                {product.isDigital ? (
                    <div style={{ position: 'absolute', bottom: 10, right: 10, background: pal.accent, color: '#fff', fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 50, display: 'flex', alignItems: 'center' }}>
                        <Download size={12} />
                    </div>
                ) : product.shippingFree && (
                    <div style={{ position: 'absolute', bottom: 10, right: 10, background: pal.accent, color: '#fff', fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 50 }}>
                        🚚
                    </div>
                )}
                    ...
                    <div>
                        <div style={{ background: '#fff', borderRadius: 24, padding: '2rem', border: '2px solid var(--lavender-lt)' }}>
                            ...
                            {(product.shippingFree || (store?.supportFreeShipping && store?.freeShippingMinAmount != null)) && (
                                <div style={{ marginBottom: '1.5rem', padding: '0.75rem 1rem', border: '2px solid var(--lavender-dk)', borderRadius: 16, background: 'var(--lavender-lt)', fontSize: '0.85rem', fontWeight: 700, color: 'var(--lavender-dk)' }}>
                                        ...
                                        <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.875rem 1rem', border: `2px solid ${selectedOffer === o.id ? 'var(--lavender-dk)' : 'var(--border)'}`, borderRadius: 16, cursor: 'pointer', marginBottom: '0.5rem', background: selectedOffer === o.id ? 'var(--lavender-lt)' : 'transparent', transition: 'all 0.2s' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                                                ...
                                                <div>
                                                    ...
                                                    {o.subTitle && <p style={{ fontSize: '0.72rem', color: 'var(--text-soft)', fontWeight: 600 }}>{o.subTitle}</p>}
                                                    <p style={{ fontSize: '0.72rem', color: 'var(--text-soft)', fontWeight: 600 }}>{t.qty}: {o.quantity}</p>
                                                    {o.shippingFree && <p style={{ fontSize: '0.72rem', color: 'var(--lavender-dk)', fontWeight: 700 }}>🚚 {t.freeShippingBadge}</p>}
                                                </div>
                                            </div>
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        {product.isDigital ? (
                            <div style={{ marginBottom: '0.875rem' }}>
                                ...
                                <div style={{ display: 'flex', borderRadius: 16, overflow: 'hidden', border: '2px solid var(--border)', marginBottom: '0.75rem' }}>
                                    <button type="button" onClick={() => { setContactMethod('email'); setFd(p => ({ ...p, customerWhatsapp: '' })); }} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '0.7rem 0', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.8rem', fontWeight: 700, background: contactMethod === 'email' ? 'var(--lavender-dk)' : 'transparent', color: contactMethod === 'email' ? '#fff' : 'var(--text-soft)' }}>
                                        <Mail size={14} />{t.contactViaEmail}
                                    </button>
                                    <button type="button" onClick={() => { setContactMethod('whatsapp'); setFd(p => ({ ...p, customerEmail: '' })); }} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '0.7rem 0', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.8rem', fontWeight: 700, background: contactMethod === 'whatsapp' ? 'var(--lavender-dk)' : 'transparent', color: contactMethod === 'whatsapp' ? '#fff' : 'var(--text-soft)' }}>
                                    ...
                                    <div className="delivery-grid">
                                        {(['home', 'office'] as const).map(dlvType => (
                                            <button key={dlvType} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: dlvType }))} style={{ padding: '0.875rem', border: `2px solid ${fd.typeLivraison === dlvType ? 'var(--lavender-dk)' : 'var(--border)'}`, borderRadius: 16, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === dlvType ? 'var(--lavender-lt)' : '#fff', fontFamily: 'inherit', transition: 'all 0.2s' }}>
                                                ...
                                                {selW && <p className="font-serif" style={{ fontSize: '1rem', fontWeight: 700, color: fd.typeLivraison === dlvType ? 'var(--lavender-dk)' : 'var(--text-soft)' }}>{orderFreeShipping ? t.freeShippingBadge : `${Number(dlvType === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} ${store?.currency || 'DZD'}`}</p>}
                                            </button>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <div style={{ background: 'var(--lavender-lt)', borderRadius: 20, padding: '1.125rem', marginBottom: '1.25rem' }}>
                            ...
                            {[
                                ...
                                { l: t.price, v: `${fp.toLocaleString()} ${store?.currency || 'DZD'}` },
                                { l: t.qty, v: `× ${qty}` },
        ...
        <div dir={isRTL ? 'rtl' : 'ltr'} style={{ minHeight: '100vh', background: 'var(--bg)', padding: '2.5rem 1.5rem 5rem' }}>
            <div style={{ maxWidth: 1200, margin: '0 auto' }}>
                ...
                {freeShippingMin != null && (
                    <div style={{
                        border: `2px solid ${freeShippingReached ? 'var(--lavender-dk)' : 'var(--lavender-lt)'}`, borderRadius: 20,
                        background: freeShippingReached ? 'var(--lavender-lt)' : '#fff', padding: '0.875rem 1.125rem', marginBottom: '1.5rem',
                        color: freeShippingReached ? 'var(--lavender-dk)' : 'var(--text-soft)', fontSize: '0.85rem', fontWeight: 700,
                    }}>
                        ...
                        <form onSubmit={handleSubmit}>
                                ...
                                <div className="delivery-grid">
                                    {(['home', 'office'] as const).map(dlvType => (
                                        ...
                                        >
                                            ...
                                            {selW && <p className="font-serif" style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--lavender-dk)', marginTop: 3 }}>{freeShippingReached ? t.freeShippingBadge : `${Number(dlvType === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} ${store?.currency || 'DZD'}`}</p>}
                                        </button>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div style={{ background: 'var(--lavender-lt)', borderRadius: 18, padding: '1rem', marginBottom: '1rem' }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: '0.625rem', marginBottom: '0.625rem', borderBottom: '1px solid rgba(196,181,253,0.35)' }}>
                                    ...
                                    <span style={{ fontWeight: 700, fontSize: '0.9rem' }}>{!selW ? '—' : freeShippingReached ? t.freeShippingBadge : `${getLiv().toLocaleString()} ${store?.currency || 'DZD'}`}</span>
                                </div>
    ...
    return (
        <div dir={t.dir} style={{ minHeight: '100vh', background: 'var(--bg)', padding: '3rem 1.25rem' }}>
            <div style={{ maxWidth: 480, margin: '0 auto' }}>
                <div style={{ textAlign: 'center', background: '#fff', padding: '3rem 2rem', borderRadius: 28, border: '2px solid var(--lavender)', boxShadow: '0 12px 40px rgba(124,58,237,0.12)', marginBottom: '1.5rem' }}>
                    <div style={{ width: 68, height: 68, borderRadius: '50%', background: 'rgba(124,58,237,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.25rem' }}>
                        <CheckCircle2 size={34} style={{ color: 'var(--lavender-dk)' }} />
                    </div>
                    <h2 className="font-serif" style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--lavender-dk)', marginBottom: '0.625rem' }}>{t.successTitle}</h2>
                ...
                {order && (order.productName || order.total != null) && (
                    <div style={{ background: '#fff', borderRadius: 20, border: '1px solid var(--lavender)', padding: '1.25rem 1.5rem', marginBottom: '1.5rem' }}>
                        <p style={{ fontSize: '0.78rem', fontWeight: 700, color: 'var(--lavender-dk)', marginBottom: '0.75rem' }}>{t.orderInfo}</p>
                        {order.productName && (
                            <div style={{ paddingBottom: 10, marginBottom: 10, borderBottom: '1px solid var(--lavender)', fontSize: 14, fontWeight: 700, color: 'var(--text-mid)' }}>{order.productName}</div>
                        )}
                        {order.total != null && (
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <span style={{ fontSize: 13, color: 'var(--text-mid)' }}>{t.total}</span>
                                <span style={{ fontSize: '1.2rem', fontWeight: 800, color: 'var(--lavender-dk)' }}>{Number(order.total).toLocaleString()} {currency}</span>
                            </div>
                        )}
                    </div>
                )}

                <div style={{ background: '#fff', borderRadius: 20, border: '1px solid var(--lavender)', overflow: 'hidden', marginBottom: '1.5rem' }}>
                    {t.successSteps.map((step, i) => {
                        const Icon = stepIcons[i] ?? CheckCircle2;
                        const done = i === 0;
                        return (
                            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '1rem 1.25rem', borderBottom: i < t.successSteps.length - 1 ? '1px solid var(--lavender)' : 'none', background: done ? 'rgba(124,58,237,0.06)' : 'transparent' }}>
                                <div style={{ width: 36, height: 36, flexShrink: 0, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: done ? 'var(--lavender-dk)' : '#F5F3FF', color: done ? '#fff' : 'var(--lavender-dk)' }}>
                                    <Icon size={16} />
                                </div>
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    <p style={{ fontSize: '0.85rem', fontWeight: 700, color: done ? 'var(--text-mid)' : 'var(--lavender-dk)', marginBottom: 2 }}>{step.title}</p>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.7rem' }}>
                    <Link href="/" className="btn-soft" style={{ ...BTN_PRI, textDecoration: 'none', display: 'inline-flex', justifyContent: 'center' }}>{t.shopNow}</Link>
                    <Link href="/" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0.8rem', borderRadius: 14, border: '1px solid var(--lavender)', color: 'var(--lavender-dk)', fontWeight: 700, fontSize: '0.85rem', textDecoration: 'none' }}>
                        {t.backToShop}
                    </Link>
                </div>
            </div>
