          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(dt => (
                  <button key={dt} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: dt }))} style={{ padding: '0.875rem', border: `1.5px solid ${fd.typeLivraison === dt ? '#E63946' : '#E8E8E8'}`, borderRadius: 10, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === dt ? 'rgba(230,57,70,0.05)' : '#fff', transition: 'all 0.18s', fontFamily: 'inherit' }}>
                    ...
                    {selW && <p className="price-mono" style={{ fontSize: '1.125rem', fontWeight: 800, color: fd.typeLivraison === dt ? '#111' : '#ccc' }}>{(dt === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} {product?.store?.currency || 'DZD'}</p>}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div style={{ background: '#F8F8F6', border: '1.5px solid #EBEBEB', borderRadius: 10, padding: '1rem 1.125rem', marginBottom: '1rem' }}>
              ...
              {[
                ...
                { l: t.delivery, v: selW ? `${getLiv().toLocaleString()} ${product?.store?.currency || 'DZD'}` : '—' },
              ].map(r => (
