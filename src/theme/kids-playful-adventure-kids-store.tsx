            ...
            <div style={{ padding: '3.5rem 1.5rem 2rem', maxWidth: 1280, margin: '0 auto' }}>
                <div className="footer-cols">
                    ...
                    <div>
                        ...
                        {[{ h: '/', l: t.home, e: '🏠' }, { h: '/cart', l: t.cart, e: '🛒' }, { h: '/contact', l: t.contact, e: '📞' }].filter(lnk => lnk.h !== '/cart' || store?.cart !== false).map((lnk, i) => (
                    ...
                    {/* قسم 3 — قانوني */}
                    <div>
                        <h4 style={{ fontFamily: "'Boogaloo',cursive", fontSize: '1.1rem', color: 'var(--yellow)', marginBottom: '1.25rem' }}>⚖️ {t.legalNav}</h4>
                        {[{ h: '/Privacy', l: t.privacy, e: '🔒' }, { h: '/Terms', l: t.terms, e: '📋' }, { h: '/cookies', l: t.cookies, e: '🍪' }].map((lnk, i) => (
                            <a key={i} href={lnk.h} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.9rem', fontWeight: 700, color: 'rgba(255,255,255,0.5)', marginBottom: '0.625rem', transition: 'all 0.2s' }}
                                onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.color = 'var(--yellow)'; el.style.transform = 'translateX(-4px)'; }}
                                onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.color = 'rgba(255,255,255,0.5)'; el.style.transform = ''; }}>
                                <span>{lnk.e}</span>{lnk.l}
    ...
    return (
        <div className="adv-card" style={{ background: '#fff', border: `3px solid ${hov ? pal.border : 'var(--border)'}`, borderRadius: 18, overflow: 'hidden', display: 'flex', flexDirection: 'column', height: '100%', boxShadow: hov ? `0 16px 48px ${pal.border}22` : '0 2px 10px rgba(0,0,0,0.05)' }}
            ...
            <div style={{ position: 'relative', aspectRatio: '1/1', background: pal.bg, overflow: 'hidden' }}>
                ...
                {product.isDigital ? (
                    <div style={{ position: 'absolute', top: 10, left: 10, background: pal.accent, color: '#fff', fontWeight: 900, padding: '3px 10px', borderRadius: 6, display: 'flex', alignItems: 'center' }}>
                        <Download size={12} />
                    </div>
                ) : product.shippingFree && (
                    <div style={{ position: 'absolute', top: 10, left: 10, background: pal.accent, color: '#fff', fontWeight: 900, padding: '3px 10px', borderRadius: 6, fontFamily: "'Boogaloo',cursive", fontSize: '0.85rem' }}>
                        🚚
                    </div>
                )}
            </div>
                    ...
                    <div>
                        <div style={{ background: '#fff', borderRadius: 20, padding: '2rem', border: `3px solid ${pal.border}` }}>
                            ...
                            {(product.shippingFree || (store?.supportFreeShipping && store?.freeShippingMinAmount != null)) && (
                                <div style={{ marginBottom: '1.5rem', padding: '0.75rem 1rem', border: `3px solid ${pal.border}`, borderRadius: 14, background: pal.bg, fontSize: '0.85rem', fontWeight: 800, color: pal.accent }}>
                                        ...
                                        <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.875rem 1rem', border: `3px solid ${selectedOffer === o.id ? pal.border : 'var(--border)'}`, borderRadius: 14, cursor: 'pointer', marginBottom: '0.5rem', background: selectedOffer === o.id ? pal.bg : 'transparent', transition: 'all 0.2s' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                                                ...
                                                <div>
                                                    ...
                                                    {o.subTitle && <p style={{ fontSize: '0.72rem', color: 'var(--soft)', fontWeight: 700 }}>{o.subTitle}</p>}
                                                    <p style={{ fontSize: '0.72rem', color: 'var(--soft)', fontWeight: 700 }}>{t.qty}: {o.quantity}</p>
                                                    {o.shippingFree && <p style={{ fontSize: '0.72rem', color: pal.accent, fontWeight: 800 }}>🚚 {t.freeShippingBadge}</p>}
                                                </div>
                                            </div>
                                ...
                                <div key={attr.id} style={{ marginBottom: '1.25rem' }}>
                                    ...
                                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                                        {attr.variants.map((v: any) => {
                                            ...
                                            if (attr.displayMode === 'color') {
                                                return (
                                                    <button key={v.id} onClick={() => available && handleVariantSelection(attr.name, v.value)}
                                                        style={{ width: 30, height: 30, borderRadius: 8, background: v.value, border: 'none', cursor: available ? 'pointer' : 'not-allowed', outline: `3px solid ${isSelected ? pal.border : 'transparent'}`, outlineOffset: 3, transition: 'outline 0.2s', opacity: available ? 1 : 0.35 }}
                                                    />
                                                );
                                            }
                                            ...
                                            return (
                                                <button key={v.id} onClick={() => available && handleVariantSelection(attr.name, v.value)}
                                                    style={{ padding: '0.4rem 1rem', border: `2.5px solid ${isSelected ? pal.border : 'var(--border)'}`, borderRadius: 8, fontWeight: 800, fontSize: '0.85rem', background: isSelected ? pal.bg : '#fff', color: isSelected ? pal.accent : (available ? 'var(--mid)' : '#bbb'), cursor: available ? 'pointer' : 'not-allowed', fontFamily: 'inherit', transition: 'all 0.2s', textDecoration: available ? 'none' : 'line-through' }}>
                                                    {v.name}
                                                </button>
                                            );
                                        })}
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        {product.isDigital ? (
                            <div style={{ marginBottom: '0.875rem' }}>
                                <p style={{ fontSize: '0.78rem', fontWeight: 900, color: 'var(--mid)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.5rem' }}>{t.contactQuestion}</p>
                                <div style={{ display: 'flex', borderRadius: 12, overflow: 'hidden', border: '3px solid var(--border)', marginBottom: '0.75rem' }}>
                                    <button type="button" onClick={() => { setContactMethod('email'); setFd(p => ({ ...p, customerWhatsapp: '' })); }} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '0.7rem 0', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.8rem', fontWeight: 900, background: contactMethod === 'email' ? 'var(--blue)' : 'transparent', color: contactMethod === 'email' ? '#fff' : 'var(--soft)' }}>
                                        <Mail size={14} />{t.contactViaEmail}
                                    </button>
                                    <button type="button" onClick={() => { setContactMethod('whatsapp'); setFd(p => ({ ...p, customerEmail: '' })); }} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '0.7rem 0', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.8rem', fontWeight: 900, background: contactMethod === 'whatsapp' ? 'var(--blue)' : 'transparent', color: contactMethod === 'whatsapp' ? '#fff' : 'var(--soft)' }}>
                                    ...
                                    <div className="delivery-grid">
                                        {(['home', 'office'] as const).map(dtype => (
                                            <button key={dtype} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: dtype }))} style={{ padding: '0.875rem', border: `3px solid ${fd.typeLivraison === dtype ? 'var(--blue)' : 'var(--border)'}`, borderRadius: 12, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === dtype ? 'var(--blue-lt)' : '#fff', fontFamily: 'inherit', transition: 'all 0.2s' }}>
                                                ...
                                                {selW && <p className="font-boogaloo" style={{ fontSize: '1rem', color: fd.typeLivraison === dtype ? 'var(--blue)' : 'var(--soft)' }}>{orderFreeShipping ? t.freeShippingBadge : `${Number(dtype === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} ${store?.currency || 'DZD'}`}</p>}
                                            </button>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <div className="dot-bg" style={{ background: '#fff', border: '3px solid var(--border)', borderRadius: 16, padding: '1.125rem', marginBottom: '1.25rem' }}>
                            ...
                            {[
                                ...
                                { l: t.qty, v: `× ${qty}` },
                                ...(product.isDigital ? [] : [{ l: t.delivery, v: !selW ? '—' : orderFreeShipping ? t.freeShippingBadge : `${getLiv().toLocaleString()} ${store?.currency || 'DZD'}` }]),
                            ].map(r => (
        ...
        <div dir={isRTL ? 'rtl' : 'ltr'} style={{ minHeight: '100vh', background: 'var(--bg)', padding: '2.5rem 1.5rem 5rem' }}>
            <div style={{ maxWidth: 1200, margin: '0 auto' }}>
                ...
                {freeShippingMin != null && (
                    <div style={{
                        border: `3px solid ${freeShippingReached ? 'var(--green)' : 'var(--border)'}`, borderRadius: 16,
                        background: freeShippingReached ? 'var(--green-lt)' : '#fff', padding: '0.875rem 1.125rem', marginBottom: '1.5rem',
                        color: freeShippingReached ? 'var(--green-dk)' : 'var(--soft)', fontSize: '0.85rem', fontWeight: 800,
                    }}>
                            ...
                            <div key={i} style={{ display: 'flex', gap: '1rem', padding: '1rem 1.25rem', borderBottom: '2px solid var(--border)' }}>
                                ...
                                <div style={{ flex: 1 }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', marginTop: '0.375rem' }}>
                                        <span className="font-boogaloo" style={{ fontSize: '0.9rem', color: 'var(--text)' }}>× {item.quantity}</span>
                        ...
                        <form onSubmit={handleSubmit}>
                                ...
                                <div className="delivery-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                                    {(['home', 'office'] as const).map(dtype => (
                                        ...
                                        >
                                            ...
                                            {selW && <p className="font-boogaloo" style={{ fontWeight: 800, fontSize: '1rem', color: 'var(--blue)' }}>{freeShippingReached ? t.freeShippingBadge : `${Number(dtype === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} ${store?.currency || 'DZD'}`}</p>}
                                        </button>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="dot-bg" style={{ background: '#fff', border: '3px solid var(--border)', borderRadius: 14, padding: '1rem', marginBottom: '1rem' }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: '0.625rem', marginBottom: '0.625rem', borderBottom: '2px dashed var(--border)' }}>
                                    ...
                                    <span style={{ fontWeight: 900, fontSize: '0.9rem' }}>{!selW ? '—' : freeShippingReached ? t.freeShippingBadge : `${getLiv().toLocaleString()} ${store?.currency || 'DZD'}`}</span>
                                </div>
    ...
    return (
        <div dir={t.dir} style={{ minHeight: '100vh', background: 'var(--bg)', padding: '3rem 1.25rem' }}>
            <div style={{ maxWidth: 480, margin: '0 auto' }}>
                <div style={{ textAlign: 'center', background: 'var(--white)', padding: '3rem 2rem', borderRadius: 24, border: '3px solid var(--blue)', boxShadow: '0 12px 32px rgba(0,102,255,0.15)', marginBottom: '1.5rem' }}>
                    <div style={{ width: 68, height: 68, borderRadius: '50%', background: 'var(--blue-lt)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.25rem' }}>
                        <CheckCircle2 size={34} style={{ color: 'var(--blue)' }} />
                    </div>
                    <h2 className="font-boogaloo" style={{ fontSize: '2rem', color: 'var(--blue)', marginBottom: '0.625rem' }}>{t.successTitle}</h2>
                ...
                {order && (order.productName || order.total != null) && (
                    <div style={{ background: 'var(--white)', borderRadius: 20, border: '1px solid var(--border)', padding: '1.25rem 1.5rem', marginBottom: '1.5rem' }}>
                        <p style={{ fontSize: '0.72rem', fontWeight: 900, color: 'var(--blue)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.875rem' }}>📦 {t.orderInfo}</p>
                        {order.productName && (
                            <div style={{ paddingBottom: 10, marginBottom: 10, borderBottom: '1px solid var(--border)', fontSize: 14, fontWeight: 700, color: 'var(--text)' }}>{order.productName}</div>
                        )}
                        {order.total != null && (
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <span style={{ fontSize: 13, color: 'var(--mid)' }}>{t.total}</span>
                                <span className="font-boogaloo" style={{ fontSize: '1.3rem', color: 'var(--blue)' }}>{Number(order.total).toLocaleString()} {currency}</span>
                            </div>
                        )}
                    </div>
                )}

                <div style={{ background: 'var(--white)', borderRadius: 20, border: '1px solid var(--border)', overflow: 'hidden', marginBottom: '1.5rem' }}>
                    {t.successSteps.map((step, i) => {
                        const Icon = stepIcons[i] ?? CheckCircle2;
                        const done = i === 0;
                        return (
                            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '1rem 1.25rem', borderBottom: i < t.successSteps.length - 1 ? '1px solid var(--border)' : 'none', background: done ? 'var(--blue-lt)' : 'transparent' }}>
                                <div style={{ width: 36, height: 36, flexShrink: 0, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: done ? 'var(--blue)' : '#F1F3F8', color: done ? '#fff' : 'var(--soft)' }}>
                                    <Icon size={16} />
                                </div>
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    <p style={{ fontSize: '0.85rem', fontWeight: 800, color: done ? 'var(--text)' : 'var(--soft)', marginBottom: 2 }}>{step.title}</p>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.7rem' }}>
                    <Link href="/" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: 'var(--blue)', color: '#fff', padding: '0.9rem 2rem', borderRadius: 16, fontWeight: 800, fontSize: '0.9rem', textDecoration: 'none' }}>
                        <ShoppingBag size={17} /> {t.shopNow}
                    </Link>
                    <Link href="/" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0.8rem', borderRadius: 16, border: '2px solid var(--border)', color: 'var(--mid)', fontWeight: 800, fontSize: '0.85rem', textDecoration: 'none' }}>
                        🛍️ {t.backToShop}
                    </Link>
                </div>
            </div>
