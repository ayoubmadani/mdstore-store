  ...
  return (
    <div className="util-card" style={{ background: CARD, display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ position: 'relative', aspectRatio: '1/1', background: BG, overflow: 'hidden' }}>
        ...
        {product.isDigital ? (
          <div style={{ position: 'absolute', top: 8, left: 8, background: E, color: '#fff', fontSize: 10, fontWeight: 800, padding: '3px 7px', borderRadius: 2, display: 'flex', alignItems: 'center' }}>
            <Download size={12} />
          </div>
        ) : product.shippingFree && (
          <div style={{ position: 'absolute', top: 8, left: 8, background: INK, color: '#fff', fontSize: 10, fontWeight: 800, padding: '2px 7px', borderRadius: 2 }}>🚚</div>
        )}
      </div>
        ...
        <div>
          <div className="info-container">
            ...
            {(product.shippingFree || (store?.supportFreeShipping && store?.freeShippingMinAmount != null)) && (
              <p style={{ fontSize: '0.8rem', fontWeight: 700, color: ED, marginBottom: '1.25rem' }}>
                  ...
                  <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.7rem 0.875rem', border: `1px solid ${selectedOffer === o.id ? E : BD}`, cursor: 'pointer', marginBottom: '0.375rem', background: selectedOffer === o.id ? EL : CARD, transition: 'all 0.15s' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      ...
                      <div>
                        ...
                        {o.shippingFree && <p style={{ fontSize: '0.68rem', fontWeight: 700, color: ED, marginTop: 2 }}>{t.freeShippingBadge}</p>}
                      </div>
                    </div>
                  ...
                  {attr.variants.map((v: any) => {
                    ...
                    return (
                      <button key={v.id} onClick={() => available && handleVariantSelection(attr.name, v.value)} style={
                        attr.displayMode === 'color' ? { width: 26, height: 26, borderRadius: '50%', background: v.value, border: `1px solid ${BD}`, cursor: available ? 'pointer' : 'not-allowed', outline: `2px solid ${isSelected ? E : 'transparent'}`, outlineOffset: 2, opacity: available ? 1 : 0.35 }
                        : attr.displayMode === 'image' ? { width: 40, height: 40, backgroundImage: `url(${v.value})`, backgroundSize: 'cover', backgroundPosition: 'center', border: `2px solid ${isSelected ? E : BD}`, cursor: available ? 'pointer' : 'not-allowed', transition: 'all 0.15s', opacity: available ? 1 : 0.35 }
                        : { padding: '0.35rem 0.75rem', border: `1px solid ${isSelected ? E : BD}`, fontSize: '0.8rem', fontWeight: 600, background: isSelected ? EL : CARD, color: isSelected ? ED : (available ? SUB : '#bbb'), cursor: available ? 'pointer' : 'not-allowed', transition: 'all 0.15s', fontFamily: 'inherit', textDecoration: available ? 'none' : 'line-through' }
          ...
          <form onSubmit={handleSubmit}>
            ...
            {product.isDigital ? (
              <div style={{ marginBottom: '0.875rem' }}>
                <p style={{ fontSize: '0.7rem', fontWeight: 700, color: SUB, marginBottom: '0.5rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>{t.contactQuestion}</p>
                <div style={{ display: 'flex', border: `1px solid ${BD}`, overflow: 'hidden', marginBottom: '0.75rem' }}>
                  <button type="button" onClick={() => { setContactMethod('email'); setFd(p => ({ ...p, customerWhatsapp: '' })); }} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '0.7rem 0', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.8rem', fontWeight: 700, background: contactMethod === 'email' ? E : 'transparent', color: contactMethod === 'email' ? '#fff' : SUB }}>
                    <Mail size={14} />{t.contactViaEmail}
                  </button>
                  <button type="button" onClick={() => { setContactMethod('whatsapp'); setFd(p => ({ ...p, customerEmail: '' })); }} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '0.7rem 0', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.8rem', fontWeight: 700, background: contactMethod === 'whatsapp' ? E : 'transparent', color: contactMethod === 'whatsapp' ? '#fff' : SUB }}>
                  ...
                  <div className="delivery-grid">
                    {(['home', 'office'] as const).map(dtype => (
                      <button key={dtype} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: dtype }))} style={{ padding: '0.7rem', border: `1px solid ${fd.typeLivraison === dtype ? E : BD}`, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === dtype ? EL : CARD, transition: 'all 0.15s', fontFamily: 'inherit' }}>
                        ...
                        {selW && <p className="price-mono" style={{ fontSize: '0.95rem', fontWeight: 800, color: fd.typeLivraison === dtype ? ED : '#9CA3AF' }}>{orderFreeShipping ? t.freeShippingBadge : `${(dtype === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} ${store?.currency || 'DZD'}`}</p>}
                      </button>
                    ))}
                  </div>
                </div>
  ...
  return (
    <div dir={isRTL ? 'rtl' : 'ltr'} style={{ padding: '2rem 1.5rem', maxWidth: 1280, margin: '0 auto', minHeight: '100vh', background: BG }}>
      ...
      {freeShippingMin != null && (
        <div style={{ background: freeShippingReached ? EL : BG, border: `1px solid ${freeShippingReached ? 'rgba(16,185,129,0.3)' : BD}`, color: freeShippingReached ? ED : SUB, borderRadius: 4, padding: '0.75rem 1.125rem', marginBottom: '1.5rem', fontWeight: 700, fontSize: '0.85rem', textAlign: 'center' }}>
              ...
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(dtype => (
                  <button key={dtype} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: dtype }))} style={{ padding: '0.7rem', border: `1px solid ${fd.typeLivraison === dtype ? E : BD}`, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === dtype ? EL : CARD, fontFamily: 'inherit', transition: 'all 0.15s' }}>
                    ...
                    {selW && <p style={{ fontWeight: 800, fontSize: '0.9rem', color: fd.typeLivraison === dtype ? ED : '#9CA3AF' }}>{freeShippingReached ? t.freeShippingBadge : `${(dtype === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} ${store?.currency || 'DZD'}`}</p>}
                  </button>
                ))}
              </div>
            </div>
    ...
    <div dir={t.dir} style={{ minHeight: '100vh', background: BG, padding: '3rem 1.25rem' }}>
      <div style={{ maxWidth: 480, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', background: CARD, padding: '3rem 2rem', borderRadius: 4, border: `1px solid ${BD}`, marginBottom: '1.5rem' }}>
          <div style={{ width: 64, height: 64, background: '#F3F4F6', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.25rem' }}>
            <CheckCircle2 size={32} style={{ color: E }} />
          </div>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: INK, marginBottom: '0.5rem' }}>{t.successTitle}</h2>
          ...
          <div style={{ background: CARD, borderRadius: 4, border: `1px solid ${BD}`, padding: '1.25rem 1.5rem', marginBottom: '1.5rem' }}>
            ...
            {order.total != null && (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: '1.2rem', fontWeight: 800, color: E }}>{Number(order.total).toLocaleString()} {currency}</span>
              </div>
            )}
          </div>
        )}
        ...
        <div style={{ background: CARD, borderRadius: 4, border: `1px solid ${BD}`, overflow: 'hidden', marginBottom: '1.5rem' }}>
          {t.successSteps.map((step, i) => {
            ...
            const done = i === 0;
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '1rem 1.25rem', borderBottom: i < t.successSteps.length - 1 ? `1px solid ${BD}` : 'none', background: done ? '#F3F4F6' : 'transparent' }}>
                <div style={{ width: 36, height: 36, flexShrink: 0, borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', background: done ? E : '#F3F4F6', color: done ? '#fff' : SUB }}>
                  <Icon size={16} />
                </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.7rem' }}>
          <Link href="/" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, background: E, color: '#fff', padding: '0.875rem 2rem', borderRadius: 4, fontWeight: 700, fontSize: '0.875rem', textDecoration: 'none' }}>
