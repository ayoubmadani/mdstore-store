    ...
    <Link href={`/product/${product.slug || product.id}`} className="pbf-card" style={{ textDecoration: 'none', display: 'block' }}
      ...
      <div className="pbf-card-body">
        ...
        <div className="pbf-card-price-row">
          ...
          {product?.isDigital ? (
            <span className="pbf-card-badge" style={{ display: 'inline-flex', alignItems: 'center' }}><Download size={11} /></span>
          ) : product?.shippingFree && <span className="pbf-card-badge">🚚</span>}
      ...
      <div className="pbf-details-inner" style={{ display: 'grid', gap: '2.5rem' }}>
        ...
        <div>
          ...
          {(product?.shippingFree || (store?.supportFreeShipping && store?.freeShippingMinAmount != null)) && (
            <div style={{ marginBottom: 22, padding: '10px 14px', border: `1px solid ${SAGE_DARK}`, borderRadius: 4, background: BG_ALT, fontSize: '0.85rem', fontWeight: 700, color: SAGE_DARK }}>
                ...
                {product.offers.map((o: Offer) => (
                  <label key={o.id} style={{ display: 'flex', alignItems: 'center', gap: 10, border: `1px solid ${selectedOffer === o.id ? SAGE_DARK : BORDER}`, borderRadius: 4, padding: '10px 12px', cursor: 'pointer' }}>
                    ...
                    <span style={{ display: 'flex', flexDirection: 'column' }}>
                      ...
                      {o.shippingFree && <span style={{ color: SAGE_DARK, fontWeight: 700, fontSize: '0.76rem' }}>🚚 {t.freeShippingBadge}</span>}
                    </span>
    ...
    <div style={{ marginTop: 24, borderTop: `1px solid ${BORDER}`, paddingTop: 24 }}>
      {(!isOrderNow && !product?.isDigital) ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {product?.isDigital ? (
            <div>
              <label className="pbf-label">{t.contactQuestion}</label>
              <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
                <button type="button" onClick={() => { setContactMethod('email'); setFd((f) => ({ ...f, customerWhatsapp: '' })); }}
                  style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '10px', borderRadius: 4, border: `1px solid ${contactMethod === 'email' ? SAGE_DARK : BORDER}`, background: contactMethod === 'email' ? SAGE_DARK : 'none', color: contactMethod === 'email' ? '#fff' : MUTED, cursor: 'pointer', minHeight: 44 }}>
                  <Mail size={14} />{t.contactViaEmail}
                </button>
                <button type="button" onClick={() => { setContactMethod('whatsapp'); setFd((f) => ({ ...f, customerEmail: '' })); }}
                  style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '10px', borderRadius: 4, border: `1px solid ${contactMethod === 'whatsapp' ? SAGE_DARK : BORDER}`, background: contactMethod === 'whatsapp' ? SAGE_DARK : 'none', color: contactMethod === 'whatsapp' ? '#fff' : MUTED, cursor: 'pointer', minHeight: 44 }}>
                  <MessageCircle size={14} />{t.contactViaWhatsapp}
                </button>
              </div>
              {contactMethod === 'email' ? (
                <div>
                  <label className="pbf-label">{t.orderEmail}</label>
                  <input className="pbf-input-field" type="email" dir="ltr" placeholder={t.emailPh} value={fd.customerEmail}
                    onChange={(e) => setFd((f) => ({ ...f, customerEmail: e.target.value }))} />
                  {errors.customerEmail && <span style={{ color: '#b3453a', fontSize: '0.75rem' }}>{errors.customerEmail}</span>}
                </div>
              ) : (
                <div>
                  <label className="pbf-label">{t.whatsapp}</label>
                  <input className="pbf-input-field" type="tel" dir="ltr" placeholder={t.whatsappPh} value={fd.customerWhatsapp}
                    onChange={(e) => setFd((f) => ({ ...f, customerWhatsapp: e.target.value }))} />
                  {errors.customerWhatsapp && <span style={{ color: '#b3453a', fontSize: '0.75rem' }}>{errors.customerWhatsapp}</span>}
                </div>
              )}
            </div>
          ) : (
  ...
  return (
    <div className="pbf-container" style={{ padding: '2.5rem 1.5rem' }}>
      ...
      {freeShippingMin != null && (
        <div style={{
          border: `1px solid ${freeShippingReached ? SAGE_DARK : BORDER}`, borderRadius: 6,
          background: freeShippingReached ? BG_ALT : '#fff', padding: '12px 16px', marginBottom: '1.5rem',
          color: freeShippingReached ? SAGE_DARK : MUTED, fontSize: '0.85rem', fontWeight: 700,
        }}>
          ...
          <div style={{ border: `1px solid ${BORDER}`, borderRadius: 6, padding: 18, display: 'flex', flexDirection: 'column', gap: 14, background: '#fff' }}>
            ...
            <div style={{ borderTop: `1px solid ${BORDER}`, paddingTop: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                ...
                <span style={{ color: INK, whiteSpace: 'nowrap', flexShrink: 0 }}>{!selW ? '—' : freeShippingReached ? t.freeShippingBadge : `${getLiv().toLocaleString()} ${store?.currency}`}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
...
export function Success({ store, order }: { store: any; domain: string; order: any }) {
  ...
  return (
    <div className={`pbf-root lang-${getLang(store)}`} style={{ minHeight: '100vh', padding: '3rem 1.25rem' }}>
      <div style={{ maxWidth: 480, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', background: BG_ALT, border: `1px solid ${BORDER}`, borderRadius: 4, padding: '3rem 2rem', marginBottom: '1.5rem' }}>
          <div style={{ width: 60, height: 60, borderRadius: '50%', background: '#fff', border: `1px solid ${SAGE}`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.25rem' }}>
            <Check size={28} color={SAGE_DARK} />
          </div>
          <h2 className="pbf-display" style={{ color: INK, fontSize: '1.6rem', marginBottom: 10 }}>{t.successTitle}</h2>
        ...
        {order && (order.productName || order.total != null) && (
          <div style={{ background: '#fff', border: `1px solid ${BORDER}`, borderRadius: 4, padding: '1.25rem 1.5rem', marginBottom: '1.5rem' }}>
            <p style={{ fontWeight: 700, fontSize: '0.75rem', color: SAGE_DARK, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>{t.orderInfo}</p>
            {order.productName && (
              <div style={{ paddingBottom: 10, marginBottom: 10, borderBottom: `1px solid ${BORDER}`, fontSize: 14, fontWeight: 700, color: INK }}>{order.productName}</div>
            )}
            {order.total != null && (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 13, color: MUTED }}>{t.total}</span>
                <span className="pbf-display" style={{ fontSize: '1.2rem', color: SAGE_DARK }}>{Number(order.total).toLocaleString()} {currency}</span>
              </div>
            )}
          </div>
        )}

        <div style={{ background: '#fff', border: `1px solid ${BORDER}`, borderRadius: 4, overflow: 'hidden', marginBottom: '1.5rem' }}>
          {t.successSteps.map((step, i) => {
            const Icon = stepIcons[i] ?? Check;
            const done = i === 0;
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '1rem 1.25rem', borderBottom: i < t.successSteps.length - 1 ? `1px solid ${BORDER}` : 'none', background: done ? BG_ALT : 'transparent' }}>
                <div style={{ width: 34, height: 34, flexShrink: 0, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: done ? SAGE : '#fff', border: done ? 'none' : `1px solid ${BORDER}`, color: done ? '#fff' : MUTED }}>
                  <Icon size={15} />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: '0.85rem', fontWeight: 700, color: done ? INK : MUTED, marginBottom: 2 }}>{step.title}</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.7rem' }}>
          <Link href="/" className="pbf-btn-primary" style={{ textDecoration: 'none', display: 'flex', justifyContent: 'center' }}>{t.shopNow}</Link>
