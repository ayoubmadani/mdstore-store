...
import {
  Search, X, ShoppingCart, Menu, Phone, Mail, MessageCircle, MapPin, ChevronDown,
...
export function Card({ product, displayImage, discount, store, viewDetails }: any) {
  ...
  return (
    <Link href={`/product/${product.slug || product.id}`} className="pt-card"
      ...
      {product?.isDigital ? (
        <span className="pt-card-free" style={{ display: 'flex', alignItems: 'center' }}><Download size={12} /></span>
      ) : product?.shippingFree && <span className="pt-card-free">🚚</span>}
      {img && !imgErr ? (
      ...
      <div className="pt-details">
        ...
        <div>
          ...
          {(product?.shippingFree || (store?.supportFreeShipping && store?.freeShippingMinAmount != null)) && (
            <div style={{ marginBottom: 20, padding: '10px 14px', border: `1.5px solid ${A}`, borderRadius: 12, background: AL, fontSize: '0.85rem', fontWeight: 700, color: A }}>
              🚚 {product.shippingFree ? t.freeShippingBadge : t.freeShippingThreshold.replace('{{amount}}', fmt(Number(store.freeShippingMinAmount)))}
            </div>
          )}
                    ...
                    <button key={off.id} onClick={() => setSelectedOffer(on ? null : off.id)}
                      ...
                      <span style={{ display: 'flex', alignItems: 'center', gap: 10, fontWeight: 700, fontSize: '0.9rem' }}>
                        ...
                        <span style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                          <span>{off.name}</span>
                          {off.subTitle && <span style={{ fontSize: '0.74rem', color: MUT, fontWeight: 500 }}>{off.subTitle}</span>}
                          {off.shippingFree && <span style={{ fontSize: '0.74rem', color: A, fontWeight: 700 }}>🚚 {t.freeShippingBadge}</span>}
                        </span>
                      </span>
      ...
      {(isOrderNow || product?.isDigital) && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {product?.isDigital ? (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
                ...
                <button type="button" onClick={() => { setContactMethod('whatsapp'); set('customerEmail', ''); }}
                  style={{ padding: '0.75rem', borderRadius: 12, cursor: 'pointer', fontWeight: 700, fontSize: '0.85rem', minHeight: 46, border: `1.5px solid ${contactMethod === 'whatsapp' ? A : BD}`, background: contactMethod === 'whatsapp' ? AL : CARD, color: contactMethod === 'whatsapp' ? A : SUB, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                  <MessageCircle size={16} /> {t.contactViaWhatsapp}
                </button>
              </div>
              {contactMethod === 'email' ? (
                <div>
                  <label style={label}>{t.orderEmail}</label>
                  <input className="pt-input" dir="ltr" type="email" style={errors.customerEmail ? { borderColor: ERR } : undefined}
                    value={fd.customerEmail} onChange={(e) => set('customerEmail', e.target.value)} placeholder={t.emailPh} />
                  {errors.customerEmail && <p style={errStyle}><AlertCircle size={12} /> {errors.customerEmail}</p>}
                </div>
              ) : (
                <div>
                  <label style={label}>{t.whatsapp}</label>
                  <input className="pt-input pt-mono" dir="ltr" type="tel" style={errors.customerWhatsapp ? { borderColor: ERR } : undefined}
                    value={fd.customerWhatsapp} onChange={(e) => set('customerWhatsapp', e.target.value)} placeholder={t.whatsappPh} inputMode="tel" />
                  {errors.customerWhatsapp && <p style={errStyle}><AlertCircle size={12} /> {errors.customerWhatsapp}</p>}
                </div>
              )}
            </div>
          ) : (
          ...
          <div style={{ background: BG, border: `1px solid ${BD}`, borderRadius: 14, padding: '1rem 1.25rem' }}>
            ...
            {!product?.isDigital && (
              <SummaryRow l={t.delivery} v={!selW ? '—' : orderFreeShipping ? t.freeShippingBadge : fmt(getLiv())} />
            )}
    ...
    <div className="pt-container" style={{ padding: '2rem 1.5rem' }}>
      ...
      {freeShippingMin != null && (
        <div style={{
          ...
          color: freeShippingReached ? A : SUB, fontSize: '0.85rem', fontWeight: 700,
        }}>
          {freeShippingReached ? t.freeShippingReached : t.freeShippingRemaining.replace('{{amount}}', fmt(Number(freeShippingRemainingAmt)))}
        </div>
      )}
        ...
        <div style={{ background: CARD, border: `1px solid ${BD}`, borderRadius: 18, padding: '1.5rem', alignSelf: 'flex-start', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ background: BG, border: `1px solid ${BD}`, borderRadius: 14, padding: '1rem 1.25rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, padding: '5px 0' }}>
              ...
              <span className="pt-mono" style={{ whiteSpace: 'nowrap', fontWeight: 700 }}>{!selW ? '—' : freeShippingReached ? t.freeShippingBadge : fmt(getLiv())}</span>
            </div>
    ...
    <div dir={t.dir} style={{ minHeight: '100vh', background: BG, padding: '3rem 1.25rem' }}>
      <div style={{ maxWidth: 480, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.7rem' }}>
          <Link href="/"><span className="pt-btn" style={{ width: '100%', justifyContent: 'center', display: 'flex' }}>{t.shopNow}</span></Link>
