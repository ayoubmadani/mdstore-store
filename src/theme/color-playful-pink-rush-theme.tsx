...
/*
  ...
  ██╔══██╗██║   ██║██║     ██║  ██║    ██║   ██║██╔══██╗██╔══██╗██╔══██║██║╚████║
  ██████╦╝╚██████╔╝███████╗██████╔╝    ╚██████╔╝██║  ██║██████╔╝██║  ██║██║ ╚███║
  ═══════ THEME: PLAYFUL / COLOR: #EC4899 + #1D1D1D + #F8F8F6
*/
...
const THEME_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Ticker ── */
  .ticker-wrap { overflow: hidden; background: #EC4899; color: #fff; height: 36px; display: flex; align-items: center; }
...
const S = {
  ...
  inputErr: { borderColor: '#EC4899' } as React.CSSProperties,
  btnPrimary: {
    width: '100%', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8,
    background: '#EC4899', color: '#fff', fontWeight: 700, fontSize: '0.95rem',
...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  const DropResults = () => (
    <div style={{ paddingTop: 25, position: 'absolute', top: 'calc(100% + 6px)', right: 0, left: 0, background: '#fff', border: '1.5px solid #E0E0E0', borderRadius: 10, boxShadow: '0 12px 40px rgba(0,0,0,0.12)', zIndex: 60, maxHeight: '300px', overflowY: 'auto' }} className="anim-slide-down">
      ...
      {loading ? <div style={{ padding: '1.5rem', textAlign: 'center', color: '#EC4899', fontSize: '0.85rem', fontWeight: 600 }}>{t.searching}</div>
        : listSearch.length > 0 ? (
            ...
            {listSearch.map((p: any) => (
              <Link href={`/product/${p.id}`} key={p.id} onClick={() => setSearchQuery('')}
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '0.75rem', color: '#EC4899', fontWeight: 700 }}>{p.price} {store?.currency || 'DZD'}</div>
                </div>
              </Link>
            ))}
            <button
              ...
              style={{
                width: '100%', padding: '12px', background: 'rgba(236,72,153,0.07)', border: 'none',
                borderTop: '1px solid rgba(236,72,153,0.2)', color: '#EC4899', fontWeight: 800,
          ...
          <div className="nav-desktop">
            ...
            {store?.cart !== false && (
              <Link href="/cart" style={{ position: 'relative', background: '#111', color: '#fff', width: 40, height: 40, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.2s' }}
                onMouseEnter={e => ((e.currentTarget as HTMLAnchorElement).style.background = '#EC4899')}
                onMouseLeave={e => ((e.currentTarget as HTMLAnchorElement).style.background = '#111')}>
                <ShoppingCart size={17} />
                {count > 0 && <span style={{ position: 'absolute', top: -5, right: -5, background: '#EC4899', color: '#fff', fontSize: 10, fontWeight: 700, width: 17, height: 17, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid #F8F8F6' }}>{count}</span>}
              </Link>
            )}
          ...
          <div className="nav-mobile">
            ...
            {store?.cart !== false && (
              <Link href="/cart" style={{ position: 'relative', width: 38, height: 38, borderRadius: 8, border: '1.5px solid #E0E0E0', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#333' }}>
                ...
                {count > 0 && <span style={{ position: 'absolute', top: -4, right: -4, background: '#EC4899', color: '#fff', fontSize: 9, fontWeight: 800, width: 15, height: 15, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{count}</span>}
              </Link>
            )}
          ...
          <div style={{ padding: '0.625rem 1.25rem', background: '#fff', borderTop: '1px solid #E8E8E8', position: 'relative' }} className="anim-slide-down">
            <form onSubmit={handleSearch} style={{ position: 'relative' }}>
              <input autoFocus type="text" placeholder={t.search} value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                style={{ width: '100%', padding: isRTL ? '0.75rem 1rem 0.75rem 2.75rem' : '0.75rem 2.75rem 0.75rem 1rem', border: '1.5px solid #EC4899', borderRadius: 8, background: '#F8F8F6', fontSize: '0.9rem', outline: 'none' }} />
              <Search size={15} style={{ position: 'absolute', ...(isRTL ? { left: 12 } : { right: 12 }), top: '50%', transform: 'translateY(-50%)', color: '#EC4899' }} />
            </form>
            {searchQuery.length >= 2 && <DropResults />}
          </div>
        )}
          ...
          <div style={{ padding: '0.375rem 1.25rem 0.875rem' }}>
            {[{ h: '/', l: t.home }, { h: '/contact', l: t.contact }].map(i => (
              <Link key={i.h} href={i.h} onClick={() => setOpen(false)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 0', borderBottom: '1px solid #F0F0F0', fontSize: '0.9rem', fontWeight: 600, color: '#111' }}>
                {i.l} {isRTL ? <ArrowLeft size={14} style={{ color: '#EC4899' }} /> : <ChevronRight size={14} style={{ color: '#EC4899' }} />}
              </Link>
            ))}
        ...
        <div className="footer-inner">
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', marginBottom: '1rem' }}>
              <div style={{ width: 32, height: 32, background: '#EC4899', borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div>
            ...
            {[
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', marginBottom: '0.75rem', color: '#666', fontSize: '0.875rem' }}>
                <span style={{ color: '#EC4899' }}>{r.icon}</span>{r.val}
              </div>
            ))}
...
export function Card({ product, displayImage, discount, store, viewDetails }: any) {
  ...
  return (
    <div className="card-wrap" style={{ background: '#fff', border: '1.5px solid #EBEBEB', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column', height: '100%', position: 'relative', transition: 'all 0.28s ease' }}
      onMouseEnter={e => { const el = e.currentTarget as HTMLDivElement; el.style.borderColor = '#EC4899'; el.style.boxShadow = '0 8px 32px rgba(236,72,153,0.1)'; el.style.transform = 'translateY(-4px)'; }}
      ...
      <div style={{ position: 'relative', aspectRatio: '1/1', background: '#F4F4F2', overflow: 'hidden' }}>
        ...
        {discount > 0 && (
          <div style={{ position: 'absolute', top: 10, right: 10, background: '#EC4899', color: '#fff', fontSize: 11, fontWeight: 800, padding: '2px 9px', borderRadius: 4 }}>
            -{discount}%
          </div>
        )}
        {product.isDigital ? (
          <div style={{ position: 'absolute', top: 10, left: 10, background: '#EC4899', color: '#fff', fontSize: 10, fontWeight: 800, padding: '3px 8px', borderRadius: 4, display: 'flex', alignItems: 'center' }}>
            <Download size={12} />
          </div>
  ...
  return (
    <div dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <section style={{ position: 'relative', background: '#111', overflow: 'hidden' }}>
        ...
        {/* Geometric accent */}
        <div style={{ position: 'absolute', top: 0, left: 0, width: '40%', height: '100%', background: 'linear-gradient(135deg, #EC4899 0%, transparent 60%)', opacity: 0.12, pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', bottom: -60, right: -60, width: 300, height: 300, border: '60px solid rgba(236,72,153,0.07)', borderRadius: '50%', pointerEvents: 'none' }} />

        <div className="hero-inner" style={{ maxWidth: 1280, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          {/* Text */}
          <div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(236,72,153,0.15)', border: '1px solid rgba(236,72,153,0.3)', borderRadius: 6, padding: '0.375rem 0.875rem', marginBottom: '1.5rem' }}>
              <Zap size={13} color="#EC4899" fill="#EC4899" />
              <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#EC4899', letterSpacing: '0.08em', textTransform: 'uppercase' }}>{store.name}</span>
            </div>
            ...
            <div className="hero-actions">
              <a href="#products" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#EC4899', color: '#fff', fontWeight: 700, fontSize: '0.9rem', padding: '0.875rem 1.75rem', borderRadius: 10, transition: 'all 0.2s', boxShadow: '0 4px 20px rgba(236,72,153,0.35)' }}
                onMouseEnter={e => ((e.currentTarget as HTMLAnchorElement).style.background = '#DB2777')}
                onMouseLeave={e => ((e.currentTarget as HTMLAnchorElement).style.background = '#EC4899')}>
        ...
        <div className="trust-grid" style={{ maxWidth: 1280, margin: '0 auto' }}>
          {([<Truck size={18} />, <Shield size={18} />, <CheckCircle2 size={18} />, <Phone size={18} />] as const).map((icon, i) => {
            ...
            <div key={i} style={{ padding: '1.125rem 1rem', textAlign: 'center', borderLeft: i < 3 ? '1px solid #EBEBEB' : 'none' }}>
              <div style={{ color: '#EC4899', marginBottom: '0.375rem', display: 'flex', justifyContent: 'center' }}>{icon}</div>
      ...
      {cats.length > 0 && (
        <section style={{ padding: '4rem 1.5rem', maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.75rem' }}>
            <div style={{ width: 4, height: 28, background: '#EC4899', borderRadius: 2 }} />
            <h2 style={{ fontSize: 'clamp(1.375rem,3.5vw,2rem)', fontWeight: 800, color: '#111' }}>{t.categories}</h2>
          </div>
          <div className="cats-grid">
            <Link href="?" style={{ display:'inline-flex', alignItems:'center', padding:'0.5rem 1.25rem', borderRadius:999, border:`1.5px solid ${!activeCategory ? '#EC4899' : '#ccc'}`, background: !activeCategory ? '#EC4899' : 'transparent', color: !activeCategory ? '#fff' : 'inherit', fontSize:'0.82rem', fontWeight:600, cursor:'pointer' }}>
                {t.all}
              </Link>
              {cats.map((cat: any) => {
              ...
              <Link key={cat.id} href={`?category=${cat.id}`} style={{
                padding: '0.75rem 0.875rem', border: `1.5px solid ${isActive ? '#EC4899' : '#EBEBEB'}`, borderRadius: 8,
                textAlign: 'center', fontSize: '0.875rem', fontWeight: 600, color: isActive ? '#fff' : '#333',
                background: isActive ? '#EC4899' : '#fff', transition: 'all 0.18s'
              }}
                onMouseEnter={e => { const el = e.currentTarget; el.style.borderColor = '#EC4899'; el.style.color = '#EC4899'; el.style.background = 'rgba(236,72,153,0.04)'; }}
                onMouseLeave={e => { const el = e.currentTarget; el.style.borderColor = isActive ? '#EC4899' : '#EBEBEB'; el.style.color = isActive ? '#fff' : '#333'; el.style.background = isActive ? '#EC4899' : '#fff'; }}>
                {cat.name}
              </Link>
              );
            })}
        ...
        {countPage > 1 && (
          <div className="pagination" dir={isRTL ? 'rtl' : 'ltr'}>
            ...
            {Array.from({ length: countPage }).map((_, i) => {
              ...
              return (
                <Link key={pn} href={{ query: { page: pn } }} scroll={false} style={{ width: 36, height: 36, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.875rem', fontWeight: 700, border: `1.5px solid ${isA ? '#EC4899' : '#EBEBEB'}`, background: isA ? '#EC4899' : '#fff', color: isA ? '#fff' : '#111' }}>
                  {pn}
                </Link>
              );
            })}
      ...
      <div className="details-inner" style={{ maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div className="gallery-container">
          <div style={{ position: 'relative', aspectRatio: '1/1', borderRadius: 14, overflow: 'hidden', background: '#F0F0EE', border: '1.5px solid #E8E8E8' }}>
            ...
            {discount > 0 && (
              <div style={{ position: 'absolute', top: 12, right: 12, background: '#EC4899', color: '#fff', padding: '3px 12px', borderRadius: 5, fontSize: 12, fontWeight: 800 }}>
          ...
          {allImages.length > 1 && (
            <div className="thumb-row">
              {allImages.map((img: string, idx: number) => (
                <button key={idx} onClick={() => setSel(idx)} style={{ flexShrink: 0, width: 60, height: 60, borderRadius: 8, overflow: 'hidden', border: `2px solid ${sel === idx ? '#EC4899' : '#E8E8E8'}`, opacity: sel === idx ? 1 : 0.55, cursor: 'pointer', padding: 0, background: 'none', transition: 'all 0.18s' }}>
        ...
        <div>
          <div className="info-container">
            ...
            <div style={{
              background: '#FDF2F8', // خلفية حمراء باهتة جداً لراحة العين
              border: '1.5px solid #EC4899', // إطار أحمر صريح
              borderRadius: 12,
            ...
            {(product.shippingFree || ((store || product?.store)?.supportFreeShipping && (store || product?.store)?.freeShippingMinAmount != null)) && (
              <p style={{ fontSize: '0.8rem', fontWeight: 700, color: '#EC4899', marginBottom: '1.25rem' }}>
            ...
            {product.offers?.length > 0 && (
              <div style={{ marginBottom: '1.25rem' }}>
                {product.offers.map((o: any) => (
                  <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.875rem 1rem', border: `1.5px solid ${selectedOffer === o.id ? '#EC4899' : '#E8E8E8'}`, borderRadius: 10, cursor: 'pointer', marginBottom: '0.5rem', background: selectedOffer === o.id ? 'rgba(236, 72, 153, 0.04)' : 'transparent', transition: 'all 0.18s' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                      <div style={{ width: 18, height: 18, borderRadius: '50%', border: `2px solid ${selectedOffer === o.id ? '#EC4899' : '#ccc'}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        {selectedOffer === o.id && <div style={{ width: 9, height: 9, borderRadius: '50%', background: '#EC4899' }} />}
                      </div>
                      ...
                      <div>
                        ...
                        {o.shippingFree && <p style={{ fontSize: '0.7rem', fontWeight: 700, color: '#EC4899', marginTop: 2 }}>{t.freeShippingBadge}</p>}
                      </div>
                    </div>
                    <span className="price-mono" style={{ fontWeight: 800, color: '#EC4899', fontSize: '1.1rem' }}>{o.price.toLocaleString()} { 'DZD'}</span>
                  </label>
                ))}
              </div>
            )}
                      ...
                      <button
                        ...
                        style={
                          attr.displayMode === 'color' ? {
                            ...
                            outline: `2.5px solid ${isSelected ? '#EC4899' : 'transparent'}`, outlineOffset: 3
                          } : attr.displayMode === 'image' ? {
                            width: 44, height: 44, borderRadius: 10, backgroundImage: `url(${v.value})`, backgroundSize: 'cover',
                            backgroundPosition: 'center', opacity: available ? 1 : 0.35, border: `2px solid ${isSelected ? '#EC4899' : '#E8E8E8'}`,
                            cursor: available ? 'pointer' : 'not-allowed', transition: 'all 0.18s'
                          } : {
                            padding: '0.45rem 1.1rem', border: `1.5px solid ${isSelected ? '#EC4899' : '#E8E8E8'}`, borderRadius: 8,
                            fontSize: '0.85rem', fontWeight: 600, background: isSelected ? 'rgba(236, 72, 153, 0.05)' : '#fff',
                            color: isSelected ? '#EC4899' : (available ? '#555' : '#bbb'), cursor: available ? 'pointer' : 'not-allowed', transition: 'all 0.18s',
...
const FR = ({ error, label, children }: { error?: string; label?: string; children: React.ReactNode }) => (
  <div>
    ...
    {children}
    {error && <p style={{ fontSize: '0.75rem', color: '#EC4899', marginTop: '0.3rem', display: 'flex', alignItems: 'center', gap: 4 }}><AlertCircle size={11} />{error}</p>}
  </div>
);
          ...
          <form onSubmit={handleSubmit}>
            ...
            {product.isDigital ? (
              <div style={{ marginBottom: '0.875rem' }}>
                ...
                <div style={{ display: 'flex', borderRadius: 10, overflow: 'hidden', border: '1.5px solid #E8E8E8', marginBottom: '0.75rem' }}>
                  <button type="button" onClick={() => { setContactMethod('email'); setFd(p => ({ ...p, customerWhatsapp: '' })); }} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '0.7rem 0', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.8rem', fontWeight: 700, background: contactMethod === 'email' ? '#EC4899' : 'transparent', color: contactMethod === 'email' ? '#fff' : '#555' }}>
                    <Mail size={14} />{t.contactViaEmail}
                  </button>
                  <button type="button" onClick={() => { setContactMethod('whatsapp'); setFd(p => ({ ...p, customerEmail: '' })); }} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '0.7rem 0', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.8rem', fontWeight: 700, background: contactMethod === 'whatsapp' ? '#EC4899' : 'transparent', color: contactMethod === 'whatsapp' ? '#fff' : '#555' }}>
                ...
                <div style={{ marginBottom: '0.875rem' }}>
                  ...
                  <div className="delivery-grid">
                    {(['home', 'office'] as const).map(dt => (
                      <button key={dt} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: dt }))} style={{ padding: '0.875rem', border: `1.5px solid ${fd.typeLivraison === dt ? '#EC4899' : '#E8E8E8'}`, borderRadius: 10, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === dt ? 'rgba(236,72,153,0.05)' : '#fff', transition: 'all 0.18s', fontFamily: 'inherit' }}>
                        <p style={{ fontWeight: 700, fontSize: '0.85rem', marginBottom: 4, color: fd.typeLivraison === dt ? '#EC4899' : '#555' }}>{dt === 'home' ? t.deliveryHome : t.deliveryOffice}</p>
            ...
            <div style={{ background: '#F8F8F6', border: '1.5px solid #EBEBEB', borderRadius: 10, padding: '1rem 1.125rem', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '0.375rem' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.625rem', fontWeight: 800, color: '#EC4899' }}>{total().toLocaleString()} <span style={{ fontSize: '0.8rem', fontFamily: 'inherit', fontWeight: 700, color: '#EC4899' }}>{ 'DZD'}</span></span>
              </div>
            </div>

            {submitError && <p style={{ color: '#EC4899', fontSize: '0.85rem', textAlign: 'center', marginTop: 8, fontWeight: 600 }}>{submitError}</p>}
            <button type="submit" disabled={sub} style={{ ...S.btnPrimary, opacity: sub ? 0.7 : 1 }}
              onMouseEnter={e => !sub && ((e.currentTarget as HTMLButtonElement).style.background = '#DB2777')}
              onMouseLeave={e => ((e.currentTarget as HTMLButtonElement).style.background = '#EC4899')}>
  ...
  if (success) return (
    <div dir={isRTL ? 'rtl' : 'ltr'} style={{ minHeight: '70vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: '#F8F8F6' }}>
      <div style={{ textAlign: 'center', background: '#fff', padding: '3.5rem 2rem', borderRadius: 16, border: '1.5px solid #E8E8E8', maxWidth: 460, width: '100%' }}>
        ...
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#EC4899', color: '#fff', padding: '0.8rem 2rem', borderRadius: 10, fontWeight: 700, fontSize: '0.9rem' }}>{t.backToShop}</Link>
      </div>
    </div>
  );
  ...
  if (!items.length) return (
    <div dir={isRTL ? 'rtl' : 'ltr'} style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: '#F8F8F6' }}>
      <div style={{ textAlign: 'center', padding: '4rem 2rem', border: '2px dashed #E8E8E8', borderRadius: 16, maxWidth: 440, width: '100%', background: '#fff' }}>
        ...
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#EC4899', color: '#fff', padding: '0.8rem 2rem', borderRadius: 10, fontWeight: 700, fontSize: '0.875rem' }}>{t.shopNow}</Link>
      </div>
    </div>
  );
  ...
  return (
    <div dir={isRTL ? 'rtl' : 'ltr'} style={{ padding: '2.5rem 1.5rem', maxWidth: 1280, margin: '0 auto', minHeight: '100vh' }}>
      ...
      {freeShippingMin != null && (
        <div style={{ background: freeShippingReached ? 'rgba(236,72,153,0.06)' : '#F8F8F6', border: `1.5px solid ${freeShippingReached ? '#EC4899' : '#E8E8E8'}`, color: freeShippingReached ? '#EC4899' : '#555', borderRadius: 10, padding: '0.75rem 1.125rem', marginBottom: '1.5rem', fontWeight: 700, fontSize: '0.85rem', textAlign: 'center' }}>
          ...
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '1rem', padding: '1rem', borderBottom: '1px solid #F0F0F0' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p className="price-mono" style={{ fontSize: '1.125rem', fontWeight: 800, color: '#EC4899' }}>{item.finalPrice?.toLocaleString()} {store.currency || 'DZD'}</p>
          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(dt => (
                  <button
                    ...
                    style={{
                      padding: '0.75rem',
                      border: `2px solid ${fd.typeLivraison === dt ? '#EC4899' : '#E8E8E8'}`,
                      borderRadius: 10,
                      textAlign: 'center',
                      cursor: 'pointer',
                      background: fd.typeLivraison === dt ? 'rgba(236,72,153,0.04)' : '#fff',
                      fontFamily: 'inherit',
                      transition: 'all 0.2s'
                    }}
                  >
                    <span style={{ display: 'block', fontSize: '1.25rem', marginBottom: 3 }}>{dt === 'home' ? '🏠' : '🏢'}</span>
                    <p style={{ fontWeight: 700, fontSize: '0.78rem', color: fd.typeLivraison === dt ? '#EC4899' : '#888' }}>{dt === 'home' ? t.deliveryHome : t.deliveryOffice}</p>
            ...
            <div style={{ background: '#F8F8F6', border: '1.5px solid #EBEBEB', borderRadius: 10, padding: '1rem 1.125rem', margin: '1rem 0' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.75rem', fontWeight: 800, color: '#EC4899' }}>{finalTotal.toLocaleString()} <span style={{ fontSize: '0.8rem', fontWeight: 700, color: '#EC4899' }}>{store.currency || 'DZD'}</span></span>
              </div>
            </div>
...
const Shell = ({ children, title, dir }: { children: React.ReactNode; title: string; dir?: string }) => (
  <div dir={dir} style={{ minHeight: '100vh', background: '#F8F8F6' }}>
    <div style={{ background: '#111', paddingTop: 96, paddingBottom: 48, paddingLeft: 24, paddingRight: 24, textAlign: 'center' }}>
      <div style={{ display: 'inline-block', background: 'rgba(236,72,153,0.15)', border: '1px solid rgba(236,72,153,0.3)', padding: '0.375rem 1rem', borderRadius: 6, marginBottom: '1.25rem' }}>
        <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#EC4899', letterSpacing: '0.1em', textTransform: 'uppercase' }}>MdStore</span>
      </div>
...
const InfoBlock = ({ title, body }: { title: string; body: string }) => (
  <div style={{ padding: '1.375rem 0', borderBottom: '1px solid #EBEBEB', display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
    <div style={{ width: 4, height: 20, background: '#EC4899', borderRadius: 2, flexShrink: 0, marginTop: 4 }} />
    <div>
      ...
      <div className="contact-inner" style={{ maxWidth: 1100, margin: '0 auto', padding: '4rem 1.5rem 6rem' }}>
        ...
        <div>
          <div style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E8E8E8', padding: '1.75rem', marginBottom: '1rem' }}>
            <p style={{ fontSize: '0.7rem', fontWeight: 800, color: '#EC4899', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '1.25rem' }}>{t.contactInfoTitle}</p>
            {[
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.875rem', marginBottom: '1.125rem' }}>
                <div style={{ width: 44, height: 44, borderRadius: 10, background: 'rgba(236,72,153,0.08)', border: '1px solid rgba(236,72,153,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#EC4899', flexShrink: 0 }}>{r.icon}</div>
                <div>
        ...
        <div style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E8E8E8', padding: '2rem' }}>
          {sent ? (
            <div style={{ textAlign: 'center', padding: '4rem 1rem' }}>
              ...
              <p style={{ color: '#888', lineHeight: 1.7, marginBottom: '2rem', fontSize: '0.9375rem' }}>{t.successDesc}</p>
              <button onClick={() => setSent(false)} style={{ padding: '0.75rem 1.75rem', borderRadius: 10, border: '1.5px solid #EC4899', background: 'transparent', color: '#EC4899', fontWeight: 700, cursor: 'pointer', fontSize: '0.875rem', fontFamily: 'inherit' }}>{t.sendAnother}</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
              ...
              <button type="submit" disabled={loading} style={{ ...S.btnPrimary, opacity: loading ? 0.7 : 1 }}
                onMouseEnter={e => !loading && ((e.currentTarget as HTMLButtonElement).style.background = '#DB2777')}
                onMouseLeave={e => ((e.currentTarget as HTMLButtonElement).style.background = '#EC4899')}>
