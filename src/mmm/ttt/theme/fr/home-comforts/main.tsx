...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  return (
    <>
      ...
      <nav dir="ltr" style={{ position: 'sticky', top: 0, zIndex: 50, background: scrolled ? 'rgba(250,247,244,0.97)' : CARD, backdropFilter: 'blur(14px)', borderBottom: `1px solid ${BD}`, boxShadow: scrolled ? '0 4px 24px rgba(44,24,16,0.08)' : 'none', transition: 'all 0.25s' }}>
            ...
            {store?.design?.logoUrl && store.design.logoUrl !== '/default-logo.png' && !imgError ? (
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div>
                  ...
                  <span style={{ fontSize: '0.58rem', color: SUB }}>Mobilier et décoration</span>
                </div>
              </div>
            )}
          </Link>
        ...
        <div className="glb-search-ov" onClick={e => { if (e.target === e.currentTarget) setShowSearch(false); }}>
          <div className="glb-search-panel">
            ...
            <form className="glb-search-form" onSubmit={handleSearch}>
              ...
              <input ref={searchInputRef} className="glb-search-input" type="text" placeholder="Rechercher dans les meubles et décoration..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </form>
            {loading && <p style={{ textAlign: 'center', color: W, fontSize: '0.82rem', padding: '2rem' }}>Recherche en cours...</p>}
                ...
                {listSearch.map((p: any) => (
                  <Link key={p.id} href={`/product/${p.slug || p.id}`} className="glb-search-card" onClick={() => setShowSearch(false)}>
                    ...
                    <div className="glb-search-card-info">
                      ...
                      <p style={{ fontSize: '0.78rem', fontWeight: 700, color: W }}>{Number(p.price).toLocaleString()} DA</p>
                    </div>
                  </Link>
                ))}
              </div>
            ...
            {!loading && searchQuery.length >= 2 && listSearch.length === 0 && (
              <p style={{ textAlign: 'center', color: WB, fontSize: '0.875rem', padding: '3rem' }}>Aucun résultat</p>
            )}
          </div>
        </div>
      )}
        ...
        <div className='footer-inner'>
          <div>
            ...
            <p style={{ fontSize: '0.875rem', lineHeight: 1.85, maxWidth: 280, color: 'rgba(255,255,255,0.45)' }}>
              {store?.hero?.subtitle?.substring(0, 90) || 'Mobilier et décoration dans une ambiance chaleureuse et confortable.'}
            </p>
            <p style={{ marginTop: '2rem', fontSize: '0.68rem', color: 'rgba(255,255,255,0.18)' }}>© {new Date().getFullYear()} {store?.name}</p>
          </div>
          <div>
            <h4 style={{ fontSize: '0.72rem', fontWeight: 700, color: W, textTransform: 'uppercase' as const, letterSpacing: '0.1em', marginBottom: '1.125rem' }}>Pages</h4>
          ...
          <div>
            <h4 style={{ fontSize: '0.72rem', fontWeight: 700, color: W, textTransform: 'uppercase' as const, letterSpacing: '0.1em', marginBottom: '1.125rem' }}>Contact</h4>
            {[
    ...
    <div dir="ltr">
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 1280, margin: '0 auto', padding: 'clamp(4rem,10vw,7rem) 1.5rem clamp(3.5rem,8vw,6rem)' }}>
          <div style={{ maxWidth: 700 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(196,154,108,0.18)', border: '1px solid rgba(196,154,108,0.35)', borderRadius: 6, padding: '0.35rem 1rem', marginBottom: '1.75rem' }}>
              ...
              <span style={{ fontSize: '0.68rem', fontWeight: 700, color: WB, letterSpacing: '0.12em', textTransform: 'uppercase' as const }}>Mobilier et décoration</span>
            </div>
          ...
          <div style={{ display: 'flex', gap: 'clamp(1.5rem,4vw,4rem)', marginTop: 'clamp(2.5rem,5vw,4rem)', paddingTop: 'clamp(1.5rem,3vw,2.5rem)', borderTop: '1px solid rgba(196,154,108,0.2)' }}>
            {[{ n: '+200', l: 'Meuble' }, { n: '58', l: 'Wilaya' }, { n: '100%', l: 'Qualité Garantie' }].map((s, i) => (
              <div key={i}>
      ...
      <section style={{ background: CARD, borderBottom: `1px solid ${BD}` }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(2,1fr)' }} className="trust-row">
          {[
            { icon: <Truck size={16} />, t: 'Livraison pour 58 wilayas', s: 'Rapide et Sécurisé' },
            { icon: <Shield size={16} />, t: 'Paiement À la livraison', s: 'Sans risques' },
            { icon: <Sofa size={16} />, t: 'Mobilier de haute qualité', s: 'Robuste et confortable' },
            { icon: <HomeIcon size={16} />, t: 'Designs exclusifs', s: 'Pour chaque maison' },
          ].map((item, i) => (
      ...
      {cats.length > 0 && (
        <section style={{ padding: '2rem 1.5rem', maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
            <h2 style={{ fontSize: '1.125rem', fontWeight: 700, color: INK }}>Parcourir les Catégories</h2>
          </div>
      ...
      <section id="products" style={{ padding: '0 1.5rem 5rem', maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem', paddingTop: cats.length > 0 ? 0 : '2rem' }}>
          <div>
            <h2 style={{ fontSize: '1.25rem', fontWeight: 700, color: INK }}>Tous les Produits</h2>
            {store.count > 0 && <p style={{ fontSize: '0.72rem', color: SUB, marginTop: 3 }}>{store.count} Produit Disponible</p>}
          </div>
        </div>
        {products.length === 0 ? (
      ...
      <div className="details-inner" style={{ maxWidth: 1280, margin: '0 auto' }}>
        <div className="gallery-container">
          <div style={{ position: 'relative', aspectRatio: '4/3', overflow: 'hidden', background: WL, borderRadius: 20, border: `1px solid ${BD}` }}>
            ...
            {discount > 0 && <div style={{ position: 'absolute', top: 12, right: 12, background: W, color: '#fff', padding: '3px 10px', fontSize: 11, fontWeight: 700, borderRadius: 20 }}>{discount}% Réduction</div>}
            {allImages.length > 1 && (
              <>
        ...
        <div>
          <div className="info-container">
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.375rem', background: WL, border: `1px solid ${WB}`, borderRadius: 20, padding: '0.3rem 0.875rem', marginBottom: '0.75rem' }}>
              <HomeIcon size={11} color={W} /><span style={{ fontSize: '0.65rem', fontWeight: 600, color: WD }}>Mobilier et décoration</span>
            </div>
            ...
            <div style={{ padding: '1.25rem', background: WL, borderRadius: 14, border: `1px solid ${WB}`, marginBottom: '1.5rem' }}>
              <p style={{ fontSize: '0.65rem', fontWeight: 600, color: SUB, marginBottom: '0.25rem', textTransform: 'uppercase' as const, letterSpacing: '0.08em' }}>Prix</p>
              ...
              <div style={{ marginBottom: '1.25rem' }}>
                {product.offers.map((o: any) => (
                  <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.875rem', border: `1.5px solid ${selectedOffer === o.id ? W : BD}`, borderRadius: 12, cursor: 'pointer', marginBottom: '0.5rem', background: selectedOffer === o.id ? WL : CARD, transition: 'all 0.18s' }}>
                    ...
                    <span className="price-mono" style={{ fontWeight: 700, color: WD }}>{o.price.toLocaleString()} DA</span>
                  </label>
                ))}
              </div>
            )}
        ...
        <div className="anim-fade-up">
          {product.store.cart && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <p style={{ fontWeight: 600, fontSize: '0.7rem', color: SUB, textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Informations de livraison</p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: '0.875rem' }}>
              <p style={{ fontSize: '0.7rem', fontWeight: 600, color: SUB, marginBottom: '0.5rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Livraison</p>
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.75rem', border: `1.5px solid ${fd.typeLivraison === t ? W : BD}`, borderRadius: 12, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? WL : CARD, transition: 'all 0.18s', fontFamily: 'inherit' }}>
                    <p style={{ fontWeight: 600, fontSize: '0.8rem', marginBottom: 3, color: fd.typeLivraison === t ? WD : SUB }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
                    {selW && <p className="price-mono" style={{ fontSize: '1rem', fontWeight: 700, color: fd.typeLivraison === t ? WD : SUB }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} DA</p>}
                  </button>
                ))}
              </div>
            </div>
            <div style={{ marginBottom: '0.875rem' }}>
              <p style={{ fontSize: '0.7rem', fontWeight: 600, color: SUB, marginBottom: '0.5rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Quantité</p>
            ...
            <div style={{ background: WL, borderRadius: 14, border: `1px solid ${WB}`, padding: '1rem 1.125rem', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.75rem', fontWeight: 700, color: WD }}>{total().toLocaleString()} <span style={{ fontSize: '0.75rem' }}>DA</span></span>
              </div>
            </div>
  ...
  if (!items.length) return (
    <div dir="ltr" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: BG }}>
      <div style={{ textAlign: 'center', padding: '4rem 2rem', border: `1px solid ${BD}`, borderRadius: 20, maxWidth: 400, width: '100%', background: CARD }}>
        ...
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: W, color: '#fff', padding: '0.75rem 1.875rem', fontWeight: 700, fontSize: '0.875rem', borderRadius: 10 }}>Acheter maintenant</Link>
      </div>
    </div>
  );
      ...
      <div className="cart-inner">
        <div style={{ background: CARD, borderRadius: 16, border: `1px solid ${BD}`, alignSelf: 'start', overflow: 'hidden' }}>
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '0.875rem', padding: '0.875rem 1rem', borderBottom: `1px solid ${BD}` }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p className="price-mono" style={{ fontSize: '1.125rem', fontWeight: 700, color: WD }}>{item.finalPrice?.toLocaleString()} DA</p>
          ...
          <div style={{ padding: '1rem', background: WL, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span className="price-mono" style={{ fontSize: '1.25rem', fontWeight: 700, color: WD }}>{cartTotal.toLocaleString()} DA</span>
          </div>
        </div>
        <div style={{ background: CARD, borderRadius: 16, border: `1px solid ${BD}`, padding: '1.75rem', alignSelf: 'start' }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ margin: '1rem 0' }}>
              <p style={{ fontSize: '0.7rem', fontWeight: 600, color: SUB, textTransform: 'uppercase' as const, letterSpacing: '0.06em', marginBottom: '0.5rem' }}>Type de livraison</p>
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.75rem', border: `1.5px solid ${fd.typeLivraison === t ? W : BD}`, borderRadius: 12, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? WL : CARD, fontFamily: 'inherit', transition: 'all 0.18s' }}>
                    <p style={{ fontWeight: 600, fontSize: '0.8rem', color: fd.typeLivraison === t ? WD : SUB, marginBottom: 3 }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
                    {selW && <p style={{ fontWeight: 700, fontSize: '0.95rem', color: fd.typeLivraison === t ? WD : SUB }}>{(t === 'home' ? selW.livraisonHome : selW.livraisonOfice).toLocaleString()} DA</p>}
                  </button>
                ))}
              </div>
            </div>
            <div style={{ background: WL, borderRadius: 14, border: `1px solid ${WB}`, padding: '1rem', margin: '1rem 0' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.875rem', fontWeight: 700, color: WD }}>{finalTotal.toLocaleString()} <span style={{ fontSize: '0.75rem' }}>DA</span></span>
              </div>
            </div>
    ...
    <div dir="ltr" style={{ background: BG, minHeight: '100vh' }}>
      <div style={{ background: '#2C1810', paddingTop: 88, paddingBottom: 48, paddingLeft: 24, paddingRight: 24 }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <h1 style={{ fontSize: 'clamp(2rem,6vw,3.5rem)', fontWeight: 700, color: '#fff' }}>Contactez-nous</h1>
        </div>
      </div>
      <div className="contact-inner" style={{ maxWidth: 1100, margin: '0 auto', padding: '3rem 1.5rem 6rem' }}>
        <div>
          ...
          <div style={{ background: WL, borderRadius: 12, border: `1px solid ${WB}`, padding: '0.75rem 1rem', display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
            ...
            <span style={{ fontSize: '0.78rem', fontWeight: 600, color: WD }}>Réponse sous une heure</span>
          </div>
        </div>
        <div style={{ background: CARD, borderRadius: 16, border: `1px solid ${BD}`, padding: '2rem' }}>
          {sent ? (
            <div style={{ textAlign: 'center', padding: '3.5rem 1rem' }}>
              <CheckCircle2 size={48} style={{ color: W, display: 'block', margin: '0 auto 1.25rem' }} />
              <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: INK, marginBottom: '0.5rem' }}>Envoyé !</h2>
              <p style={{ color: SUB, lineHeight: 1.75, marginBottom: '2rem', fontSize: '0.9rem' }}>Nous vous répondrons dans les plus brefs délais.</p>
              <button onClick={() => setSent(false)} style={{ padding: '0.7rem 1.875rem', border: `1px solid ${W}`, borderRadius: 10, background: WL, color: WD, fontWeight: 600, cursor: 'pointer', fontSize: '0.875rem', fontFamily: 'inherit' }}>Envoyer un autre message</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
