...
/*
  ...
  ██╔══██╗██║   ██║██║     ██║  ██║    ██║   ██║██╔══██╗██╔══██╗██╔══██║██║╚████║
  ██████╦╝╚██████╔╝███████╗██████╔╝    ╚██████╔╝██║  ██║██████╔╝██║  ██║██║ ╚███║
  ═══════ THEME: BOLD / COLOR: #F472B6 + #1D1D1D + #F8F8F6
*/
...
const THEME_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Ticker ── */
  /* fond changé en rose */
  .ticker-wrap { overflow: hidden; background: #F472B6; color: #fff; height: 36px; display: flex; align-items: center; }
...
const S = {
  ...
  inputErr: { borderColor: '#F472B6' } as React.CSSProperties,
  btnPrimary: {
    width: '100%', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8,
    background: '#F472B6', color: '#fff', fontWeight: 700, fontSize: '0.95rem',
...
export function Navbar({ store, domain }: { store: any; domain: string }) {
  ...
  const DropResults = () => (
    <div style={{ paddingTop: 25, position: 'absolute', top: 'calc(100% + 6px)', right: 0, left: 0, background: '#fff', border: '1.5px solid #E0E0E0', borderRadius: 10, boxShadow: '0 12px 40px rgba(0,0,0,0.12)', zIndex: 60, overflow: 'hidden' }} className="anim-slide-down">
      ...
      {loading ? <div style={{ padding: '1rem', textAlign: 'center', color: '#F472B6', fontSize: '0.85rem' }}>Recherche en cours...</div>
        : listSearch.length > 0 ? (
            ...
            {listSearch.map((p: any) => (
              <Link href={`/product/${p.id}`} key={p.id} onClick={() => setSearchQuery('')}
                ...
                <div>
                  ...
                  <div style={{ fontSize: '0.75rem', color: '#F472B6', fontWeight: 700 }}>{p.price} DA</div>
                </div>
              </Link>
            ))}
            {/* Bouton Voir plus intégré dans la liste */}
        ...
        ) : searchQuery.length >= 2 && <div style={{ padding: '1rem', textAlign: 'center', color: '#aaa', fontSize: '0.85rem' }}>Aucun résultat</div>}
    </div>
  );
          ...
          <div className="nav-search-d" style={{ position: 'relative' }}>
            <form onSubmit={handleSearch}>
              <input type="text" placeholder="Rechercher ici..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                style={{ width: '100%', padding: '0.625rem 2.75rem 0.625rem 1rem', borderRadius: 8, border: '1.5px solid #E0E0E0', background: '#fff', fontSize: '0.875rem', outline: 'none' }}
          ...
          <div style={{ padding: '0.625rem 1.25rem', background: '#fff', borderTop: '1px solid #E8E8E8', position: 'relative' }} className="anim-slide-down">
            <form onSubmit={handleSearch} style={{ position: 'relative' }}>
              <input autoFocus type="text" placeholder="Rechercher un produit..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                style={{ width: '100%', padding: '0.75rem 2.75rem 0.75rem 1rem', border: '1.5px solid #F472B6', borderRadius: 8, background: '#F8F8F6', fontSize: '0.9rem', outline: 'none' }} />
              <Search size={15} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', color: '#F472B6' }} />
            </form>
            {searchQuery.length >= 2 && <DropResults />}
          </div>
        )}
        ...
        <div className="footer-inner">
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', marginBottom: '1rem' }}>
              <div style={{ width: 32, height: 32, background: '#F472B6', borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div>
            ...
            {[
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', marginBottom: '0.75rem', color: '#666', fontSize: '0.875rem' }}>
                <span style={{ color: '#F472B6' }}>{r.icon}</span>{r.val}
              </div>
            ))}
...
export function Card({ product, displayImage, discount, store, viewDetails }: any) {
  ...
  return (
    <div className="card-wrap" style={{ background: '#fff', border: '1.5px solid #EBEBEB', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column', height: '100%', position: 'relative', transition: 'all 0.28s ease' }}
      onMouseEnter={e => { const el = e.currentTarget as HTMLDivElement; el.style.borderColor = '#F472B6'; el.style.boxShadow = '0 8px 32px rgba(230,57,70,0.1)'; el.style.transform = 'translateY(-4px)'; }}
      ...
      <div style={{ position: 'relative', aspectRatio: '1/1', background: '#F4F4F2', overflow: 'hidden' }}>
        ...
        {discount > 0 && (
          <div style={{ position: 'absolute', top: 10, right: 10, background: '#F472B6', color: '#fff', fontSize: 11, fontWeight: 800, padding: '2px 9px', borderRadius: 4 }}>
            -{discount}%
          </div>
        )}
      </div>
      ...
      <div style={{ padding: '1rem', display: 'flex', flexDirection: 'column', flex: 1 }}>
        ...
        <div style={{ marginTop: 'auto' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.375rem', marginBottom: '0.75rem' }}>
            ...
            <span style={{ fontSize: '0.75rem', color: '#999', fontWeight: 500 }}>{store.currency || 'DA'}</span>
  ...
  return (
    <div dir="ltr">
      ...
      <section style={{ position: 'relative', background: '#111', overflow: 'hidden' }}>
        ...
        {/* Geometric accent - dégradé modifié en rose */}
        <div style={{ position: 'absolute', top: 0, left: 0, width: '40%', height: '100%', background: 'linear-gradient(135deg, #F472B6 0%, transparent 60%)', opacity: 0.12, pointerEvents: 'none' }} />

        {/* Circle accent - couleur du cercle transparent modifiée */}
        <div style={{ position: 'absolute', bottom: -60, right: -60, width: 300, height: 300, border: '60px solid rgba(244, 114, 182, 0.07)', borderRadius: '50%', pointerEvents: 'none' }} />

        <div className="hero-inner" style={{ maxWidth: 1280, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          {/* Text */}
          <div>
            {/* Badge - fond et bordures modifiés */}
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(244, 114, 182, 0.15)', border: '1px solid rgba(244, 114, 182, 0.3)', borderRadius: 6, padding: '0.375rem 0.875rem', marginBottom: '1.5rem' }}>
              <Zap size={13} color='#F472B6' fill="#F472B6" />
              <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#F472B6', letterSpacing: '0.08em', textTransform: 'uppercase' }}>{store.name}</span>
            </div>
            ...
            <h1 style={{ fontSize: 'clamp(2.25rem,6vw,4.5rem)', fontWeight: 800, color: '#fff', lineHeight: 1.1, letterSpacing: '-0.03em', marginBottom: '1.25rem' }}
              dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(store.hero?.title || 'Acheter<br/><span style="color:#F472B6">En confiance</span> et obtenez<br/>le meilleur') }} />
            ...
            <div className="hero-actions">
              <a href="#products" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#F472B6', color: '#fff', fontWeight: 700, fontSize: '0.9rem', padding: '0.875rem 1.75rem', borderRadius: 10, transition: 'all 0.2s', boxShadow: '0 4px 20px rgba(244, 114, 182, 0.35)' }}
        ...
        <div className="trust-grid" style={{ maxWidth: 1280, margin: '0 auto' }}>
          {[
            ...
            <div key={i} style={{ padding: '1.125rem 1rem', textAlign: 'center', borderLeft: i < 3 ? '1px solid #EBEBEB' : 'none' }}>
              <div style={{ color: '#F472B6', marginBottom: '0.375rem', display: 'flex', justifyContent: 'center' }}>{item.icon}</div>
      ...
      {cats.length > 0 && (
        <section style={{ padding: '4rem 1.5rem', maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.75rem' }}>
            <div style={{ width: 4, height: 28, background: '#F472B6', borderRadius: 2 }} />
            <h2 style={{ fontSize: 'clamp(1.375rem,3.5vw,2rem)', fontWeight: 800, color: '#111' }}>Catégories</h2>
          </div>
          <div className="cats-grid">
            <Link href="?" style={{ display:'inline-flex', alignItems:'center', padding:'0.5rem 1.25rem', borderRadius:999, border:`1.5px solid ${!activeCategory ? '#F472B6' : '#ccc'}`, background: !activeCategory ? '#F472B6' : 'transparent', color: !activeCategory ? '#fff' : 'inherit', fontSize:'0.82rem', fontWeight:600, cursor:'pointer' }}>
                Tout
              </Link>
              {cats.map((cat: any) => {
              ...
              <Link key={cat.id} href={`?category=${cat.id}`} style={{
                padding: '0.75rem 0.875rem', border: `1.5px solid ${isActive ? '#F472B6' : '#EBEBEB'}`, borderRadius: 8,
                textAlign: 'center', fontSize: '0.875rem', fontWeight: 600, color: isActive ? '#fff' : '#333',
                background: isActive ? '#F472B6' : '#fff', transition: 'all 0.18s'
              }}
                onMouseEnter={e => { const el = e.currentTarget; el.style.borderColor = '#F472B6'; el.style.color = '#F472B6'; el.style.background = 'rgba(230,57,70,0.04)'; }}
                onMouseLeave={e => { const el = e.currentTarget; el.style.borderColor = isActive ? '#F472B6' : '#EBEBEB'; el.style.color = isActive ? '#fff' : '#333'; el.style.background = isActive ? '#F472B6' : '#fff'; }}>
                {cat.name}
              </Link>
              );
            })}
        ...
        {countPage > 1 && (
          <div className="pagination" dir="ltr">
            ...
            {Array.from({ length: countPage }).map((_, i) => {
              ...
              return (
                <Link key={pn} href={{ query: { page: pn } }} scroll={false} style={{ width: 36, height: 36, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.875rem', fontWeight: 700, border: `1.5px solid ${isA ? '#F472B6' : '#EBEBEB'}`, background: isA ? '#F472B6' : '#fff', color: isA ? '#fff' : '#111' }}>
                  {pn}
                </Link>
              );
            })}
...
export function Details({ product, discount, allImages, allAttrs, finalPrice, selectedVariants, setSelectedOffer, selectedOffer, handleVariantSelection, domain }: any) {
  ...
  // Observer la taille de l'écran pour contrôler le sticky
  useEffect(() => {
      ...
      <div className="details-inner" style={{ maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div style={{ position: 'relative', aspectRatio: '1/1', borderRadius: 14, overflow: 'hidden', background: '#fff', border: '1.5px solid #E8E8E8' }}>
            ...
            {discount > 0 && (
              <div style={{ position: 'absolute', top: 12, right: 12, background: '#F472B6', color: '#fff', padding: '3px 12px', borderRadius: 5, fontSize: 12, fontWeight: 800 }}>
                {discount}% Réduction
              </div>
            )}
          ...
          {allImages.length > 1 && (
            <div className="thumb-row" style={{ display: 'flex', gap: '0.625rem', marginTop: '0.875rem', overflowX: 'auto', paddingBottom: '5px', scrollbarWidth: 'none' }}>
              {allImages.map((img: string, idx: number) => (
                <button key={idx} onClick={() => setSel(idx)} style={{ flexShrink: 0, width: 55, height: 55, borderRadius: 8, overflow: 'hidden', border: `2px solid ${sel === idx ? '#F472B6' : '#E8E8E8'}`, opacity: sel === idx ? 1 : 0.55, cursor: 'pointer', padding: 0, background: 'none' }}>
        ...
        <div>
          <div style={{ background: '#fff', borderRadius: 14, padding: isDesktop ? '1.75rem' : '1.25rem', border: '1.5px solid #E8E8E8' }}>
            ...
            {/* Bloc prix (rose) */}
            <div style={{ background: '#FFF5F7', border: '1.5px solid #FCE7F3', borderRadius: 12, padding: '1.25rem', marginBottom: '1.5rem' }}>
              <p style={{ fontSize: '0.75rem', color: '#F472B6', fontWeight: 700, marginBottom: '0.25rem', textTransform: 'uppercase' }}>Prix total</p>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.375rem' }}>
                <span className="price-mono" style={{ fontSize: '2.5rem', fontWeight: 800, color: '#111' }}>{finalPrice.toLocaleString()}</span>
                <span style={{ fontWeight: 700, color: '#F472B6', fontSize: '1.2rem' }}>DA</span>
              </div>
            </div>
            ...
            {product.offers?.length > 0 && (
              <div style={{ marginBottom: '1.25rem' }}>
                {product.offers.map((o: any) => (
                  <label key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', border: `1.5px solid ${selectedOffer === o.id ? '#F472B6' : '#E8E8E8'}`, borderRadius: 10, cursor: 'pointer', marginBottom: '0.625rem', background: selectedOffer === o.id ? '#FFF5F7' : 'transparent', transition: '0.2s' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                      <div style={{ width: 18, height: 18, borderRadius: '50%', border: `2px solid ${selectedOffer === o.id ? '#F472B6' : '#ccc'}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        {selectedOffer === o.id && <div style={{ width: 9, height: 9, borderRadius: '50%', background: '#F472B6' }} />}
                      </div>
                      <input type="radio" name="offer" checked={selectedOffer === o.id} onChange={() => setSelectedOffer(o.id)} style={{ display: 'none' }} />
                      <div>
                        <p style={{ fontWeight: 600, color: '#111', fontSize: '0.9rem' }}>{o.name}</p>
                        <p style={{ fontSize: '0.75rem', color: '#888' }}>Quantité: {o.quantity}</p>
                      </div>
                    </div>
                    <span className="price-mono" style={{ fontWeight: 800, color: '#F472B6', fontSize: '1.1rem' }}>{o.price.toLocaleString()} DA</span>
                  </label>
                ))}
              </div>
            )}

            {/* Options (couleur et taille) */}
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {attr.variants.map((v: any) => {
                    ...
                    return (
                      ...
                      >
                        {/* Texte affiché uniquement pour tailles ou options textuelles */}
...
const FR = ({ error, label, children }: { error?: string; label?: string; children: React.ReactNode }) => (
  <div>
    ...
    {children}
    {error && <p style={{ fontSize: '0.75rem', color: '#F472B6', marginTop: '0.3rem', display: 'flex', alignItems: 'center', gap: 4 }}><AlertCircle size={11} />{error}</p>}
  </div>
);
        ...
        <div className="cart-add-btns" style={{ marginBottom: '1.25rem' }}>
          ...
          <button
            ...
            style={{
              flex: 1,
              ...S.btnPrimary,
              background: '#F472B6', // couleur rose principale
              width: 'auto',
              borderRadius: 10
            }}
            onMouseEnter={e => ((e.currentTarget as HTMLButtonElement).style.background = '#DB2777')} // rose plus foncé au survol
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: '0.875rem' }}>
              ...
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setFd(p => ({ ...p, typeLivraison: t }))} style={{ padding: '0.875rem', border: `1.5px solid ${fd.typeLivraison === t ? '#F472B6' : '#E8E8E8'}`, borderRadius: 10, textAlign: 'center', cursor: 'pointer', background: fd.typeLivraison === t ? 'rgba(230,57,70,0.05)' : '#fff', transition: 'all 0.18s', fontFamily: 'inherit' }}>
                    <p style={{ fontWeight: 700, fontSize: '0.85rem', marginBottom: 4, color: fd.typeLivraison === t ? '#F472B6' : '#555' }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
            ...
            <div style={{ background: '#F8F8F6', border: '1.5px solid #EBEBEB', borderRadius: 10, padding: '1rem 1.125rem', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '0.375rem' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.625rem', fontWeight: 800, color: '#F472B6' }}>{total().toLocaleString()} <span style={{ fontSize: '0.8rem', fontFamily: 'inherit', fontWeight: 700, color: '#F472B6' }}>DA</span></span>
              </div>
            </div>
  ...
  if (success) return (
    <div dir="ltr" style={{ minHeight: '70vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: '#F8F8F6' }}>
      <div style={{ textAlign: 'center', background: '#fff', padding: '3.5rem 2rem', borderRadius: 16, border: '1.5px solid #E8E8E8', maxWidth: 460, width: '100%' }}>
        ...
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#F472B6', color: '#fff', padding: '0.8rem 2rem', borderRadius: 10, fontWeight: 700, fontSize: '0.9rem' }}>Retour à la boutique</Link>
      </div>
    </div>
  );
  ...
  if (!items.length) return (
    <div dir="ltr" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: '#F8F8F6' }}>
      <div style={{ textAlign: 'center', padding: '4rem 2rem', border: '2px dashed #E8E8E8', borderRadius: 16, maxWidth: 440, width: '100%', background: '#fff' }}>
        ...
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#F472B6', color: '#fff', padding: '0.8rem 2rem', borderRadius: 10, fontWeight: 700, fontSize: '0.875rem' }}>Acheter maintenant</Link>
      </div>
    </div>
  );
          ...
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '1rem', padding: '1rem', borderBottom: '1px solid #F0F0F0' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p className="price-mono" style={{ fontSize: '1.125rem', fontWeight: 800, color: '#F472B6' }}>{item.finalPrice?.toLocaleString()} DA</p>
              ...
              <button onClick={() => update(items.filter((_, idx) => idx !== i))} style={{ color: '#ccc', padding: '0.375rem', borderRadius: 6, background: 'none', border: 'none', cursor: 'pointer', alignSelf: 'center', transition: 'color 0.18s' }}
                onMouseEnter={e => ((e.currentTarget as HTMLButtonElement).style.color = '#F472B6')}
          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="delivery-grid">
                {(['home', 'office'] as const).map(t => (
                  ...
                  >
                    ...
                    <p style={{ fontWeight: 700, fontSize: '0.78rem', color: fd.typeLivraison === t ? '#F472B6' : '#888' }}>{t === 'home' ? 'À domicile' : 'Au bureau'}</p>
            ...
            <div style={{ background: '#F8F8F6', border: '1.5px solid #EBEBEB', borderRadius: 10, padding: '1rem 1.125rem', margin: '1rem 0' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                ...
                <span className="price-mono" style={{ fontSize: '1.75rem', fontWeight: 800, color: '#F472B6' }}>{finalTotal.toLocaleString()} <span style={{ fontSize: '0.8rem', fontWeight: 700, color: '#F472B6' }}>DA</span></span>
              </div>
            </div>
            ...
            >
              {submitting ? (
                <>
                  <Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> Traitement en cours...
                </>
              ) : (
  ...
  <div dir="ltr" style={{ minHeight: '100vh', background: '#F8F8F6' }}>
    <div style={{ background: '#111', paddingTop: 96, paddingBottom: 48, paddingLeft: 24, paddingRight: 24, textAlign: 'center' }}>
      <div style={{ display: 'inline-block', background: 'rgba(230,57,70,0.15)', border: '1px solid rgba(230,57,70,0.3)', padding: '0.375rem 1rem', borderRadius: 6, marginBottom: '1.25rem' }}>
        <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#F472B6', letterSpacing: '0.1em', textTransform: 'uppercase' }}>MdStore</span>
      </div>
...
const InfoBlock = ({ title, body }: { title: string; body: string }) => (
  <div style={{ padding: '1.375rem 0', borderBottom: '1px solid #EBEBEB', display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
    <div style={{ width: 4, height: 20, background: '#F472B6', borderRadius: 2, flexShrink: 0, marginTop: 4 }} />
    <div>
      ...
      <div className="contact-inner" style={{ maxWidth: 1100, margin: '0 auto', padding: '4rem 1.5rem 6rem' }}>
        ...
        <div>
          <div style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E8E8E8', padding: '1.75rem', marginBottom: '1rem' }}>
            <p style={{ fontSize: '0.7rem', fontWeight: 800, color: '#F472B6', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '1.25rem' }}>Informations de contact</p>
            {[
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.875rem', marginBottom: '1.125rem' }}>
                <div style={{ width: 44, height: 44, borderRadius: 10, background: 'rgba(230,57,70,0.08)', border: '1px solid rgba(230,57,70,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#F472B6', flexShrink: 0 }}>{r.icon}</div>
                <div>
        ...
        <div style={{ background: '#fff', borderRadius: 14, border: '1.5px solid #E8E8E8', padding: '2rem' }}>
          {sent ? (
            <div style={{ textAlign: 'center', padding: '4rem 1rem' }}>
              ...
              <button onClick={() => setSent(false)} style={{ padding: '0.75rem 1.75rem', borderRadius: 10, border: '1.5px solid #F472B6', background: 'transparent', color: '#F472B6', fontWeight: 700, cursor: 'pointer', fontSize: '0.875rem', fontFamily: 'inherit' }}>Envoyer un autre message</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
              ...
              <button
                ...
                style={{
                  ...S.btnPrimary,
                  background: '#F472B6', // couleur rose principale (Pink 400)
                  color: '#fff',
                ...
                onMouseEnter={e => !loading && ((e.currentTarget as HTMLButtonElement).style.background = '#DB2777')} // rose foncé au survol
                onMouseLeave={e => ((e.currentTarget as HTMLButtonElement).style.background = '#F472B6')}
              >
                {loading ? (
                  <>
